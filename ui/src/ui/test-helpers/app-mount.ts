...
import "../app.ts";
import type { AlienApp } from "../app.ts";
...
const mountedApps = new Set<AlienApp>();

function collectMountedApps() {
  return new Set<AlienApp>([
    ...mountedApps,
    ...document.querySelectorAll<AlienApp>("alien-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<AlienApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<AlienApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("alien-app") as AlienApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window.__ALIEN_CONTROL_UI_BASE_PATH__ = undefined;
