...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://alien.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://alien.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://alien.ai/abc-123", baseHref)).toBe(
      "blob:https://alien.ai/abc-123",
    );
  });
