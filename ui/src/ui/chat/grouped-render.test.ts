...
vi.mock("../views/agents-utils.ts", () => {
  ...
  return {
    assistantAvatarFallbackUrl: () => "/alien-molty.png",
    agentLogoUrl: () => "/alien-logo.svg",
...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__alien__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  localStorageValues.delete("alien:skipDeleteConfirm");
}
...
describe("grouped chat rendering", () => {
  ...
  it("renders allowed transcript and content image variants", async () => {
    ...
    const renderUserMedia = (message: unknown) => {
      ...
      const renderMessage = () =>
        renderGroupedMessage(container, message, "user", {
          ...
          localMediaPreviewRoots: ["/tmp/alien"],
    ...
    let container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/alien/user-upload.png",
      timestamp: Date.now(),
    });
    ...
    expect(
      ...
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Fuser-upload.png&mediaTicket=ticket-user",
    );
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPaths: ["/tmp/alien/first.png", "/tmp/alien/second.jpg"],
    ...
    expect(
      ...
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ffirst.png&mediaTicket=ticket-user",
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Fsecond.jpg&mediaTicket=ticket-user",
    ]);
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/__alien__/media/user-upload.pdf",
    ...
    expect(documentLink?.getAttribute("href")).toBe("/__alien__/media/user-upload.pdf");
    vi.unstubAllGlobals();
  });
  ...
  it("fetches managed chat images with auth and renders blob previews", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      const headers = init?.headers as Headers;
      expect(headers.get("Authorization")).toBe("Bearer session-token");
      expect(headers.get("x-alien-requester-session-key")).toBe("agent:main:main");
      return {
        ok: true,
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/alien/test image.png\nMEDIA:/tmp/alien/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ftest+image.png&meta=1",
    ...
    expect(image?.getAttribute("src")).toBe(
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ftest+image.png&mediaTicket=ticket-local",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ftest-doc.pdf&mediaTicket=ticket-local",
    );
  ...
  it("refreshes cached local assistant media tickets before they expire without another render", async () => {
    ...
    expect(
      ...
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ftest+image.png&mediaTicket=ticket-old",
    );
    ...
    expect(
      ...
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ftest+image.png&mediaTicket=ticket-new",
    );
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(
      ...
      "/alien/__alien__/assistant-media?source=%2Ftmp%2Falien%2Ftest+image.png&mediaTicket=ticket-fresh",
    );
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__alien__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__alien__/media/test-doc.pdf");
    expect(container.textContent).not.toContain("Unavailable");
  });
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/alien/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/alien/__alien__/assistant-media?source=%2FC%3A%2Ftmp%2Falien%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/alien/__alien__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/alien/__alien__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__alien__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasHostUrl: "http://127.0.0.1:19003/__alien__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__alien__/cap/cap_123/__alien__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__alien__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "/__alien__/canvas/documents/cv_inline_default/index.html",
    );
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__alien__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
