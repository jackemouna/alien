...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[alien] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
...
export function inferBasePath() {
  ...
  const configured = window.__ALIEN_CONTROL_UI_BASE_PATH__;
