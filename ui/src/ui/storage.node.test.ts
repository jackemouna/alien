  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __ALIEN_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__ALIEN_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__ALIEN_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /alien/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/alien"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/alien/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/alien"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("alien.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "alien.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/alien",
    ...
    const scopedKey = "alien.control.settings.v1:wss://gateway.example:8443/alien";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      borderRadius: 50,
      sessionsByGateway: {
        "wss://gateway.example:8443/alien": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", async () => {
    ...
    const scopedKey = `alien.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("falls back to claw when persisted custom theme data is invalid", async () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `alien.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        theme: "custom",
        themeMode: "dark",
  ...
  it("caps persisted session scopes to the most recent gateways", async () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `alien.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("persists local user identity separately from gateway settings", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("alien.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "👾",
    });
  });
  ...
  it("removes the persisted local user identity when cleared", async () => {
    ...
    expect(localStorage.getItem("alien.control.user.v1")).toBeNull();
  });
});
