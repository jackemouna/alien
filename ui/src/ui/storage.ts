const SETTINGS_KEY_PREFIX = "alien.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "alien.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "alien.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "alien.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "alien.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "alien.control.token.v1:";
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("alien.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
