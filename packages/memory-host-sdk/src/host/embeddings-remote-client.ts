...
import { requireApiKey, resolveApiKeyForProvider } from "./alien-runtime-auth.js";
...
function resolveAlienAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.ALIEN_VERSION?.trim() : undefined;
  return {
    originator: "alien",
    ...(version ? { version } : {}),
    "User-Agent": version ? `alien/${version}` : "alien",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveAlienAttributionHeaders());
  }
