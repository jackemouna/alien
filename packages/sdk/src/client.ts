...
export type AlienOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: AlienTransport;
};

function resolveGatewayUrl(options: AlienOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Alien Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Alien Gateway yet`);
}
...
function readChatProjection(event: AlienEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: AlienEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: AlienEvent): boolean {
  return (
...
export class Alien {
  ...
  private readonly transport: AlienTransport;
  private readonly normalizedEvents = new EventHub<AlienEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, AlienEvent[]>();
  ...
  events(filter?: (event: AlienEvent) => boolean): AsyncIterable<AlienEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: AlienEvent) => boolean,
  ): AsyncIterable<AlienEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: AlienEvent): AlienEvent | undefined => {
    ...
    const matches = (event: AlienEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: AlienEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): AlienEvent[] {
...
export class AgentsNamespace {
  constructor(private readonly client: Alien) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<AlienEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Alien,
