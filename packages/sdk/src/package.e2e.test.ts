...
describe("Alien SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-sdk-consumer-"));
    tempDirs.push(tempDir);

    await runCommand("pnpm", ["--filter", "@alien/sdk", "build"], {
      cwd: repoRoot,
      timeoutMs: 180_000,
    });
    ...
    const importScript = `
      import { GatewayClientTransport, Alien, normalizeGatewayEvent } from "@alien/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof Alien !== "function") throw new Error("missing client export");
