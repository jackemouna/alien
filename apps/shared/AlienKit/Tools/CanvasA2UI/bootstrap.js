...
import "@a2ui/lit/ui";
import { themeContext } from "@alien/a2ui-theme-context";
...
class AlienA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.alienA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["alien:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.alienCanvasA2UIAction ??
      globalThis.alienCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.alienCanvasA2UIAction) {
...
if (!customElements.get("alien-a2ui-host")) {
  customElements.define("alien-a2ui-host", AlienA2UIHost);
}
