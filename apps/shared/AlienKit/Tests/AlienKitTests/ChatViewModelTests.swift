...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> AlienChatHistoryPayload
{
    AlienChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> AlienChatSessionEntry {
    AlienChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> AlienChatSessionEntry
{
    AlienChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> AlienChatModelChoice {
    AlienChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [AlienChatHistoryPayload],
    sessionsResponses: [AlienChatSessionsListResponse] = [],
    modelResponses: [[AlienChatModelChoice]] = [],
    ...
    -> (TestChatTransport, AlienChatViewModel)
{
...
private func sendUserMessage(_ vm: AlienChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, AlienChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AlienChatHistoryPayload]
    private let sessionsResponses: [AlienChatSessionsListResponse]
    private let modelResponses: [[AlienChatModelChoice]]
    ...
    private let stream: AsyncStream<AlienChatTransportEvent>
    private let continuation: AsyncStream<AlienChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<AlienChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<AlienChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> AlienChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AlienChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [AlienChatAttachmentPayload]) async throws -> AlienChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return AlienChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AlienChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AlienChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [AlienChatModelChoice] {
    ...
    func emit(_ evt: AlienChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func appendsExternalSessionUserMessageForActiveSession() async throws {
        ...
        transport.emit(
            .sessionMessage(
                AlienSessionMessageEventPayload(
                    sessionKey: "agent:main:main",
                    message: AlienChatMessage(
                        role: "user",
                        content: [
                            AlienChatMessageContent(
                                type: "text",
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = AlienChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = AlienChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: AlienChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = AlienChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: AlienChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(AlienChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == AlienChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = AlienChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: AlienChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = AlienChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = AlienChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = AlienChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { AlienChatViewModel(sessionKey: "main", transport: transport) }
