...
struct GatewayNodeSessionTests {
    ...
    func scannedSetupCodePrefersBootstrapAuthOverStoredDeviceToken() async throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["ALIEN_STATE_DIR"]
        setenv("ALIEN_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("ALIEN_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("ALIEN_STATE_DIR")
            }
    ...
    func normalizeCanvasHostUrlPreservesExplicitSecureCanvasPort() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "https://canvas.example.com:9443/__alien__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com")!)

        #expect(normalized == "https://canvas.example.com:9443/__alien__/cap/token")
    }

    @Test
    func normalizeCanvasHostUrlBackfillsGatewayHostForLoopbackCanvas() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "http://127.0.0.1:18789/__alien__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com:7443")!)

        #expect(normalized == "https://gateway.example.com:7443/__alien__/cap/token")
    }
