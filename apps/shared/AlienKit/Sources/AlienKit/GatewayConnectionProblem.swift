...
public enum GatewayConnectionProblemMapper {
    ...
    private static func map(_ authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
        ...
        case .authTokenMissing:
            return self.problem(
                ...
                docsURL: self.docsURL(
                    ...
                    fallback: "https://docs.alien.ai/gateway/authentication"),
        ...
        case .authTokenNotConfigured:
            return self.problem(
                ...
                actionCommand: authError.actionCommand ?? "alien config set gateway.auth.token <new-token>",
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.alien.ai/gateway/authentication"),
                requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            return self.problem(
                ...
                actionLabel: authError.actionLabel ?? "Fix on gateway",
                actionCommand: authError.actionCommand ?? "alien config set gateway.auth.password <new-password>",
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.alien.ai/gateway/authentication"),
                requestId: authError.requestId,
        ...
        case .authBootstrapTokenInvalid:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.alien.ai/platforms/ios"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        case .authDeviceTokenMismatch:
            return self.problem(
                ...
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.alien.ai/gateway/pairing"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        ...
        case .deviceAuthSignatureExpired:
            return self.problem(
                ...
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.alien.ai/gateway/troubleshooting"),
                requestId: authError.requestId,
                retryable: true,
                pauseReconnect: true,
                authError: authError)
        ...
        case .authTailscaleIdentityMissing:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.alien.ai/gateway/tailscale"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .pinMismatch:
            ...
            return GatewayConnectionProblem(
                ...
                actionCommand: nil,
                docsURL: URL(string: "https://docs.alien.ai/gateway/troubleshooting"),
                retryable: false,
                pauseReconnect: true,
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "Alien could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                actionCommand: nil,
                docsURL: URL(string: "https://docs.alien.ai/gateway/troubleshooting"),
                retryable: true,
        ...
        if nsError.domain == URLError.errorDomain {
            ...
            case .cannotFindHost, .dnsLookupFailed, .notConnectedToInternet, .networkConnectionLost,
                ...
                return GatewayConnectionProblem(
                    ...
                    message: "Alien could not reach the gateway over the current network.",
    ...
    private static func approvalCommand(requestId: String?) -> String {
        if let requestId = self.nonEmpty(requestId) {
            return "alien devices approve \(requestId)"
        }
        return "alien devices list"
    }
