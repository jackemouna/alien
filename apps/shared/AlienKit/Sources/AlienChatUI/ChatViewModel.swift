...
private let chatUILogger = Logger(subsystem: "ai.alien", category: "AlienChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class AlienChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [AlienChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [AlienChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [AlienPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AlienChatPendingToolCall] = []
    public private(set) var sessions: [AlienChatSessionEntry] = []
    private let transport: any AlienChatTransport
    private var sessionDefaults: AlienChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: AlienChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any AlienChatTransport,
    ...
    public var sessionChoices: [AlienChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [AlienChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AlienPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AlienChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AlienChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AlienChatMessage) -> AlienChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AlienChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AlienChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: AlienChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: AlienChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: AlienChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [AlienChatMessage],
        incoming: [AlienChatMessage]) -> [AlienChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AlienChatMessage]) -> [AlienChatMessage] {
        var result: [AlienChatMessage] = []
    ...
    private static func dedupeKey(for message: AlienChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AlienChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> AlienChatAttachmentPayload in
            AlienChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AlienChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = AlienChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: AlienChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: AlienSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: AlienChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AlienAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AlienChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AlienPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
