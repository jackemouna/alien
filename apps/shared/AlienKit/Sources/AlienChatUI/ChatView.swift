...
@MainActor
public struct AlienChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: AlienChatViewModel
    ...
    private var visibleMessages: [AlienChatMessage] {
        let base: [AlienChatMessage]
    ...
    private func mergeToolResults(in messages: [AlienChatMessage]) -> [AlienChatMessage] {
    ...
    private func isToolResultMessage(_ message: AlienChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: AlienChatMessage) -> Bool {
    ...
    private func primaryText(in message: AlienChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: AlienChatMessage) -> Bool {
    ...
    private func toolCalls(in message: AlienChatMessage) -> [AlienChatMessageContent] {
    ...
    private func inlineToolResults(in message: AlienChatMessage) -> [AlienChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: AlienChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AlienChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(AlienChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
