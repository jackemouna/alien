...
struct ChatMessageBubble: View {
    let message: AlienChatMessage
    let style: AlienChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AlienChatTheme.userText : AlienChatTheme.assistantText
    ...
    private var inlineAttachments: [AlienChatMessageContent] {
    ...
    private var toolCalls: [AlienChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AlienChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AlienChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AlienChatTheme.onboardingAssistantBubble
        }
        return AlienChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AlienChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AlienChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AlienChatTheme.userText : AlienChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AlienChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AlienChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AlienChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AlienChatTheme.assistantText)
            }
        }
    }
}
