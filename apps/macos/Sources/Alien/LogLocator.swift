...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ALIEN_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/alien")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("alien-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("alien-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/alien/ (rolling or stdout), or nil if none exist.
