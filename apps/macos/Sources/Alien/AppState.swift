...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = AlienConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            AlienConfigFile.canonicalHostForComparison(host) ==
                AlienConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = AlienConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: AlienConfigFile.loadDict(),
            draft: .init(
        ...
        AlienConfigFile.saveDict(synced.root)
    }
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/alien"
        state.remoteCliPath = ""
        return state
    }
}
