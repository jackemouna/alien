...
enum DebugActions {
    private static let verboseDefaultsKey = "alien.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "alien.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AlienPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Alien", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AlienPaths.configURL
        guard
