...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.alien", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AlienSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AlienSessionPreviewEntry,
    ...
    private static func cache(payload: AlienSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AlienChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AlienKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AlienKit.AnyCodable]) -> [AlienChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AlienChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AlienChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AlienChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AlienChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AlienChatMessage) -> String? {
