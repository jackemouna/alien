...
private let webChatSwiftLogger = Logger(subsystem: "ai.alien", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "alien.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: AlienChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> AlienChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(AlienChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            AlienChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return AlienChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [AlienChatAttachmentPayload]) async throws -> AlienChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AlienChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AlienGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    AlienSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: AlienProtocol.ModelChoice) -> AlienChatModelChoice {
        AlienChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AlienChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AlienChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AlienChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: AlienChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AlienChatView>) -> NSViewController
    {
