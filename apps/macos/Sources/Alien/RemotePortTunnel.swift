...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.alien", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        guard let sshKey = AlienConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = AlienConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.alien.remote.tunnel.port", qos: .utility)
            do {
