...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.alien.mac`) even if the shipped
    ...
    {
        if environment["ALIEN_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "alien.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "alien.nixMode") { return true }
