...
final class TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.alien", category: "talk.mlx")
    ...
    func synthesize(
        ...
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("alien-mlx-tts-\(token.uuidString)", isDirectory: true)
    ...
    private static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["ALIEN_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("alien-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["alien-mlx-tts"],
            displayName: "alien-mlx-tts")
    }
