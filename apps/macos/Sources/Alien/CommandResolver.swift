...
enum CommandResolver {
    private static let projectRootDefaultsKey = "alien.gatewayProjectRootPath"
    private static let helperName = "alien"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let alienEntry = root.appendingPathComponent("alien.mjs").path
        if FileManager().isReadableFile(atPath: alienEntry) { return alienEntry }
        let binEntry = root.appendingPathComponent("bin/alien.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/alien")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let alienPaths = self.alienManagedPaths(home: home)
        if !alienPaths.isEmpty {
            extras.insert(contentsOf: alienPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + alienPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func alienManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".alien"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func alienExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAlienExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("alien.mjs").path,
            root.appendingPathComponent("bin/alien.js").path,
        ]
    ...
    static func hasAnyAlienInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.alienExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let alienPath = self.projectAlienExecutable(projectRoot: root) {
            return [alienPath, subcommand] + extraArgs
        }
        if let alienPath = self.alienExecutable(searchPaths: searchPaths) {
            return [alienPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "alien", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            alien entrypoint missing (looked for dist/index.js or alien.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real alien CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/alien"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/alien.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/alien.mjs"
            node "$PRJ/alien.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/alien.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/alien.js"
            node "$PRJ/bin/alien.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent alien"
          pnpm --silent alien \(quotedArgs);
        else
          echo "alien CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AlienConfigFile.loadDict()
