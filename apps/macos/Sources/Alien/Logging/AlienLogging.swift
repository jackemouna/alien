...
enum AlienLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AlienOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AlienFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AlienLogging.bootstrapIfNeeded()
        let label = AlienLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AlienOSLogHandler: AppLogLevelBackedHandler {
...
struct AlienFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AlienLogging.parseLabel(self.label)
