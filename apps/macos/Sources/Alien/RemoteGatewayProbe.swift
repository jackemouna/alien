...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        ...
        case .tokenRequired:
            "Paste the token configured on the gateway host. "
                + "On the gateway host, run `alien config get gateway.auth.token`. "
                + "If the gateway uses an environment variable instead, use `ALIEN_GATEWAY_TOKEN`."
        case .tokenMismatch:
            "Check `gateway.auth.token` or `ALIEN_GATEWAY_TOKEN` on the gateway host and try again."
        case .gatewayTokenNotConfigured:
            "This gateway is set to token auth, but no `gateway.auth.token` is configured on the gateway host. "
                ...
                + "set `ALIEN_GATEWAY_TOKEN` before starting the gateway."
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired Alien client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. "
                + "Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired Alien client. "
                + "In your Alien chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .tokenRequired, .gatewayTokenNotConfigured:
            "No token yet? Generate one on the gateway host with "
                + "`alien doctor --generate-gateway-token`, then set it as `gateway.auth.token`."
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired Alien client yet, "
                + "approve the pending request on the gateway host with `alien devices approve`."
    ...
    var statusMessage: String {
        ...
        case .tokenMismatch:
            "Gateway token mismatch. Check gateway.auth.token or ALIEN_GATEWAY_TOKEN on the gateway host."
        case .gatewayTokenNotConfigured:
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired Alien client, "
