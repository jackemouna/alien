...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AlienProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.alien", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AlienKit.AnyCodable]()) {
                $0[$1.key] = AlienKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AlienProtocol.AnyCodable?) -> [String: AlienProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AlienProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AlienKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AlienProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("alien.control.heartbeat")
    static let controlAgentEvent = Notification.Name("alien.control.agent")
}
