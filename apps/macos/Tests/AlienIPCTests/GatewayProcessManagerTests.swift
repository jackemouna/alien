...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["ALIEN_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "alien-gateway",
                executablePath: "/tmp/alien-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 alien-gateway @ /tmp/alien-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
