...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: AlienProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AlienProtocol.AnyCodable] = [
            "presence": .init([AlienProtocol.AnyCodable(entry)]),
        ]
        let payload = AlienProtocol.AnyCodable(payloadMap)
