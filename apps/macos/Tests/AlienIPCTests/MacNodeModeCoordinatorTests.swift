...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(AlienCapability.browser.rawValue))
        #expect(!commands.contains(AlienBrowserCommand.proxy.rawValue))
        #expect(commands.contains(AlienCanvasCommand.present.rawValue))
