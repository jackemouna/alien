...
import Foundation
import Testing
@testable import AlienDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("ALIEN_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("ALIEN_WIDE_AREA_DOMAIN", "alien.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("ALIEN_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("ALIEN_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._alien-gw._tcp.alien.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.alien.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Alien)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/alien/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Alien)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/alien/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._alien-gw._tcp.alien.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
