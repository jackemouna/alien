...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["alien"]
        let text = "uh alien"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "alien")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["alien", "hey alien"]
        let text = "hey alien"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey alien")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey alien do thing"
        let config = WakeWordGateConfig(triggers: ["alien"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["alien"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "alien")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "alien tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["alien", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["alien", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "alien")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["alien"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "alien", start: 0.2, duration: 0.1, range: transcript.range(of: "alien")),
