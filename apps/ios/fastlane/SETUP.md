...
ASC_KEYCHAIN_SERVICE=alien-asc-key
...
```bash
ASC_APP_IDENTIFIER=ai.alien.client
# or
...
Tip: run `scripts/ios-team-id.sh` from repo root to print a Team ID for `.env`. The helper prefers the canonical Alien team (`Y5PE65HELJ`) when present locally; otherwise it prefers the first non-personal team from your Xcode account (then personal team if needed). Fastlane uses this helper automatically if `IOS_DEVELOPMENT_TEAM` is missing.

For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `ALIEN_APNS_TEAM_ID`, `ALIEN_APNS_KEY_ID`, and either `ALIEN_APNS_PRIVATE_KEY_P8` or `ALIEN_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/beta/Alien-<version>.ipa` exists
...
- The beta flow regenerates `apps/ios/Alien.xcodeproj` from `apps/ios/project.yml` before archiving
