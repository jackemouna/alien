...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [AlienCapability.canvas.rawValue, AlienCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(AlienCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(AlienCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = AlienLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(AlienCapability.location.rawValue) }

        caps.append(AlienCapability.device.rawValue)
        caps.append(AlienCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(AlienCapability.watch.rawValue)
        }
        caps.append(AlienCapability.photos.rawValue)
        caps.append(AlienCapability.contacts.rawValue)
        caps.append(AlienCapability.calendar.rawValue)
        caps.append(AlienCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(AlienCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            AlienScreenCommand.record.rawValue,
            AlienSystemCommand.notify.rawValue,
            AlienChatCommand.push.rawValue,
        ...
        if caps.contains(AlienCapability.camera.rawValue) {
        ...
        if caps.contains(AlienCapability.location.rawValue) {
            commands.append(AlienLocationCommand.get.rawValue)
        }
        if caps.contains(AlienCapability.device.rawValue) {
            commands.append(AlienDeviceCommand.status.rawValue)
            commands.append(AlienDeviceCommand.info.rawValue)
        }
        if caps.contains(AlienCapability.watch.rawValue) {
            commands.append(AlienWatchCommand.status.rawValue)
            commands.append(AlienWatchCommand.notify.rawValue)
        }
        if caps.contains(AlienCapability.photos.rawValue) {
            commands.append(AlienPhotosCommand.latest.rawValue)
        }
        if caps.contains(AlienCapability.contacts.rawValue) {
            commands.append(AlienContactsCommand.search.rawValue)
            commands.append(AlienContactsCommand.add.rawValue)
        }
        if caps.contains(AlienCapability.calendar.rawValue) {
            commands.append(AlienCalendarCommand.events.rawValue)
            commands.append(AlienCalendarCommand.add.rawValue)
        }
        if caps.contains(AlienCapability.reminders.rawValue) {
            commands.append(AlienRemindersCommand.list.rawValue)
            commands.append(AlienRemindersCommand.add.rawValue)
        }
        if caps.contains(AlienCapability.motion.rawValue) {
            commands.append(AlienMotionCommand.activity.rawValue)
            commands.append(AlienMotionCommand.pedometer.rawValue)
        }
