...
struct OnboardingWizardView: View {
    ...
    @State private var statusLine: String = "In your Alien chat, run /pair qr, then scan the code here."
    ...
    private var introStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Welcome to Alien")
                .font(.largeTitle.weight(.bold))
                .multilineTextAlignment(.center)
                .padding(.bottom, 10)

            Text("Turn this iPhone into a secure Alien node for chat, voice, camera, and device tools.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Label("Use Alien from your phone", systemImage: "message.fill")
            }
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        .font(.headline)
                    Text(
                        "The connected Alien agent can use device capabilities you enable, "
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Scan a QR code from your Alien gateway or continue with manual setup.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 32)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `alien devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "alien devices approve <requestId>"
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `\(commandLine)`\n"
                            + "2) `/pair approve` in your Alien chat\n"
                            + "\(requestLine)\n"
                            + "Alien will also retry automatically when you return to this app.")
                }
            }
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your Alien chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "alien.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your Alien chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "alien.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "alien.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "alien.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
