...
final class MotionService: MotionServicing {
    func activities(params: AlienMotionActivityParams) async throws -> AlienMotionActivityPayload {
        ...
        let mapped: [AlienMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AlienMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = AlienPedometerPayload(
