...
@MainActor
enum AlienAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class AlienAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.alien.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.alien.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.alien.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? AlienAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "alien.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "alien.watch.promptId"
    static let sessionKeyKey = "alien.watch.sessionKey"
    static let actionPrimaryIDKey = "alien.watch.action.primary.id"
    static let actionPrimaryLabelKey = "alien.watch.action.primary.label"
    static let actionSecondaryIDKey = "alien.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "alien.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "alien.watch.action.primary"
    static let actionSecondaryIdentifier = "alien.watch.action.secondary"
    static let actionIdentifierPrefix = "alien.watch.action."
    static let actionIDKeyPrefix = "alien.watch.action.id."
    static let actionLabelKeyPrefix = "alien.watch.action.label."
    static let categoryPrefix = "alien.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AlienWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AlienWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AlienWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Alien" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AlienWatchAction]) -> [UNNotificationAction] {
...
struct AlienApp: App {
    ...
    @UIApplicationDelegateAdaptor(AlienAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        AlienAppModelRegistry.appModel = appModel
...
extension AlienApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Alien: installing uncaught exception handler")
