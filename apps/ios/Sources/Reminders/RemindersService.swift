...
final class RemindersService: RemindersServicing {
    func list(params: AlienRemindersListParams) async throws -> AlienRemindersListPayload {
        ...
        let payload: [AlienReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return AlienRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AlienRemindersAddParams) async throws -> AlienRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AlienReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AlienRemindersAddPayload(reminder: payload)
    }
