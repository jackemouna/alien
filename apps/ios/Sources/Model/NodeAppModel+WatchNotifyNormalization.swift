...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AlienWatchNotifyParams) -> AlienWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AlienWatchAction]?,
        kind: String?,
        promptId: String?) -> [AlienWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AlienWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AlienWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AlienWatchAction(id: "approve", label: "Approve"),
                AlienWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AlienWatchAction(id: "open_phone", label: "Open iPhone"),
                AlienWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AlienWatchAction(id: "done", label: "Done"),
            AlienWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AlienWatchAction(id: "open_phone", label: "Open iPhone"),
            AlienWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AlienWatchRisk?,
        priority: AlienNotificationPriority?) -> AlienWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AlienNotificationPriority?,
        risk: AlienWatchRisk?) -> AlienNotificationPriority?
    {
