...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> AlienDeviceStatusPayload {
        ...
        return AlienDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> AlienBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AlienBatteryState = switch device.batteryState {
        ...
        return AlienBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AlienThermalStatusPayload {
        let state: AlienThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AlienThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AlienStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AlienStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
