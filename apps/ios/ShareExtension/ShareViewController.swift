...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.alien.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Alien", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Alien gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AlienShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Alien is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("alien-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
