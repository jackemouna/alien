# Alien iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Alien Gateway as a `role: node`.
...
- Beta release uses canonical `ai.alien.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `AlienPushTransport=relay`, `AlienPushDistribution=official`, and `AlienPushAPNsEnvironment=production`.
...
- `ALIEN_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export ALIEN_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Alien-<version>.ipa`
   - `apps/ios/build/beta/Alien-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Alien.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `AlienPushTransport=direct` and `AlienPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `ALIEN_APNS_TEAM_ID`, `ALIEN_APNS_KEY_ID`, and either `ALIEN_APNS_PRIVATE_KEY_P8` or `ALIEN_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.alien/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `ALIEN_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `AlienPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `alien.json`. `ALIEN_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Alien official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Alien node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
