...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_alien-gw._tcp", domain: "local.", interface: nil)
    ...
    @Test @MainActor func manualConnectionsAllowPrivateLanPlaintext() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "alien.local", useTLS: false) == false)
