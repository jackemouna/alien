...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Alien Android gateway connection failure.
    ...
    - tell me whether `alien devices list` should show a pending pairing request
    - if more signal is needed, ask for `alien qr --json`, `alien devices list`, and `alien nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Alien gateway diagnostics", report))
