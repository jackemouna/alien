...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    return when (command) {
      ...
      AlienCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      AlienCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      AlienCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      AlienCanvasA2UICommand.Push.rawValue, AlienCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      AlienCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      AlienCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      AlienCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      AlienLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      AlienDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      AlienDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      AlienDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      AlienDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      AlienNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      AlienNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      AlienSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      AlienTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      AlienTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      AlienTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      AlienTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.alien.app.protocol.AlienPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      AlienContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      AlienContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      AlienCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      AlienCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      AlienMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      AlienMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      AlienSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      AlienSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      AlienCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
