...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "Alien Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode -> "Alien Node · Talk"
              state.connected -> "Alien Node · Connected"
              else -> "Alien Node"
            }
          val text =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "Alien node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.alien.app.action.STOP"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.alien.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.alien.app.extra.VOICE_CAPTURE_MODE"
