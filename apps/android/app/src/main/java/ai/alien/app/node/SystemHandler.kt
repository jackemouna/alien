...
private const val NOTIFICATION_CHANNEL_BASE_ID = "alien.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Alien Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Alien Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Alien Active")
      }
