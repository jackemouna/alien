    ...
    runBlocking {
      ...
      val server =
        startGatewayServer(json) { webSocket, id, method, frame ->
          when (method) {
            "connect" -> {
              webSocket.send(connectResponseFrame(id, canvasHostUrl = "http://127.0.0.1/__alien__/cap/old-cap"))
            }
      ...
      try {
        ...
        assertEquals(
          "http://127.0.0.1:${server.port}/__alien__/cap/new-cap",
