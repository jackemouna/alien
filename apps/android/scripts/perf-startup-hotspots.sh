...
Default package/activity target Alien Android startup.
EOF
}
...
if [[ -z "$OUTPUT_PERF_DATA" ]]; then
  OUTPUT_PERF_DATA="/tmp/alien-startup-$(date +%Y%m%d-%H%M%S).perf.data"
fi
...
tmp_dir="$(mktemp -d -t alien-android-hotspots.XXXXXX)"
