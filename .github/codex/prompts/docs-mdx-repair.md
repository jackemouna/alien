# Alien Docs MDX Repair Agent

You are repairing generated Alien documentation after a fast MDX validation failure.
...
1. Read `.alien-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".alien-sync/mdx/${LOCALE}.json"`.
