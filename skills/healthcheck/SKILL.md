---
name: healthcheck
description: Audit and harden hosts running Alien for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# Alien Host Hardening

## Overview

Assess and harden the host running Alien, then align it to a user-defined risk tolerance without breaking access. Use Alien security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Alien changes the host firewall, SSH, or OS updates; it does not.
...
5. Alien gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Alien can access sensitive data.
...
### 2) Run Alien security audits (read-only)

As part of the default read-only checks, run `alien security audit --deep`. Only offer alternatives if the user requests them:

1. `alien security audit` (faster, non-probing)
2. `alien security audit --json` (structured output)

Offer to apply Alien safe defaults (numbered):

1. `alien security audit --fix`

Be explicit that `--fix` only tightens Alien defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Alien version/update status (read-only)

As part of the default read-only checks, run `alien update status`.
...
- Credential hygiene notes (location of Alien creds, prefer disk encryption)
...
After Alien install or first hardening pass, run at least one baseline audit and version check:

- `alien security audit`
- `alien security audit --deep`
- `alien update status`

Ongoing monitoring is recommended. Use the Alien cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `alien cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `alien update status`
...
Before creating, `alien cron list` and match on exact `name`. If found, `alien cron edit <id> ...`.
If not found, `alien cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `alien update status` (preferred for source checkouts and channels)
2. `npm view alien version` (published npm version)

## Alien command accuracy

Use only supported commands and flags:

- `alien security audit [--deep] [--fix] [--json]`
- `alien status` / `alien status --deep`
- `alien health --json`
- `alien update status`
- `alien cron add|list|runs|run`

Do not invent CLI flags or imply Alien enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Alien compaction:
