---
name: node-connect
description: Diagnose Alien Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Alien is advertising that route, then fix pairing/auth.
...
- whether `alien devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `alien qr --json`. It uses the same setup-code payload Android scans.

```bash
alien config get gateway.mode
alien config get gateway.bind
alien config get gateway.tailscale.mode
alien config get gateway.remote.url
alien config get gateway.auth.mode
alien config get gateway.auth.allowTailscale
alien config get plugins.entries.device-pair.config.publicUrl
alien qr --json
alien devices list
alien nodes status
```

If this Alien instance is pointed at a remote gateway, also run:

```bash
alien qr --remote --json
```
...
If `alien qr --json` says `Gateway is only bound to loopback`:
...
- `alien devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run alien qr again, rescan, then approve the pending device pairing.`
