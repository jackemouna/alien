...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Alien. Pass it as a Bearer token in all API calls:
...
If still empty, check `/data/.clawdbot/alien.json`:

```
cat /data/.clawdbot/alien.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Alien dashboard or in the active Alien config path (`$ALIEN_CONFIG_PATH`, default `~/.alien/alien.json`) under `skills.entries.gh-issues`."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., alien-alien)
  ```
...
CONFIG_PATH="${ALIEN_CONFIG_PATH:-${ALIEN_STATE_DIR:-$HOME/.alien}/alien.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/alien.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/alien.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${ALIEN_CONFIG_PATH:-${ALIEN_STATE_DIR:-$HOME/.alien}/alien.json}"
