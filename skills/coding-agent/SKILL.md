...
3. Include the notification route in the worker prompt and require the worker to call `alien message send` on completion.
...
```bash
alien message send --channel <channel> --target '<target>' --message '<text>'
```
...
`alien message send` is a direct outbound send. It does not depend on heartbeat being enabled.
...
When the task is completely finished, send exactly one completion message back to the user with alien message send using that route.
If the task fails fatally, send exactly one failure message back to the user with alien message send using that route.
Do not use alien system event. Do not rely on heartbeat. Do not skip the completion/failure message.
```
...
**Never review PRs in Alien's own project folder.**
...
bash pty:true workdir:/tmp/issue-78 background:true command:"pnpm install && codex --yolo 'Fix issue #78: <description>. Commit and push after review. Send the completion message with alien message send using the provided notify route.'"
bash pty:true workdir:/tmp/issue-99 background:true command:"pnpm install && codex --yolo 'Fix issue #99 from the approved ticket summary. Implement only the in-scope edits. Send the completion message with alien message send using the provided notify route.'"
...
8. **NEVER start Codex inside your Alien state directory** (`$ALIEN_STATE_DIR`, default `~/.alien`) - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/alien/** - that's the LIVE Alien instance!
...
- If you are expecting the worker to self-notify with `alien message send`, say that clearly in your start update.
...
5. **Use direct completion messaging.**
   - Require `alien message send`.
   - Do not rely on `alien system event` or heartbeat.
...
10. **Never start Codex in `~/.alien/`.**
11. **Never checkout branches in `~/Projects/alien/`.**
