...
1. Download the runtime for your OS (extracts into `$ALIEN_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.alien/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$ALIEN_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.alien/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Alien config file (`$ALIEN_CONFIG_PATH`, default `~/.alien/alien.json`):
