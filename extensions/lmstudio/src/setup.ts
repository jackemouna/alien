...
import { withAgentModelAliases } from "alien/plugin-sdk/provider-onboard";
import {
...
import { WizardCancelledError, type WizardPrompter } from "alien/plugin-sdk/setup";
import {
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.ALIEN_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: AlienConfig): AlienConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<AlienConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
