import { CUSTOM_LOCAL_AUTH_MARKER } from "alien/plugin-sdk/provider-auth";
...
import { resolveAgentModelPrimaryValue } from "alien/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "alien/plugin-sdk/provider-setup";
import type { WizardPrompter } from "alien/plugin-sdk/setup";
...
vi.mock("alien/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("alien/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("alien/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("alien/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): AlienConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as AlienConfig),
    env: params?.env ?? {},
...
describe("lmstudio setup", () => {
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("ALIEN_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
