...
import type { EmbeddedRunAttemptParams } from "alien/plugin-sdk/agent-harness";
import { classifyEmbeddedPiRunResultForModelFallback } from "alien/plugin-sdk/agent-harness-runtime";
import {
  createContractRunResult,
  OUTCOME_FALLBACK_RUNTIME_CONTRACT,
} from "alien/plugin-sdk/agent-runtime-test-contracts";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-codex-outcome-contract-"));
...
describe("Outcome/fallback runtime contract - Codex app-server adapter", () => {
  it("preserves an empty terminal turn for Alien-owned fallback classification", async () => {
  ...
  it("preserves reasoning-only terminal turns for Alien-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
  ...
  it("preserves planning-only terminal turns for Alien-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
