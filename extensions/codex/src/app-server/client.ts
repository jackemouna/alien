...
import { embeddedAgentLog, ALIEN_VERSION } from "alien/plugin-sdk/agent-harness-runtime";
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "Alien did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
...
function timeoutServerRequestResponse(
  ...
  return {
    contentItems: [
      {
        type: "inputText",
        text: `Alien dynamic tool call timed out after ${CODEX_DYNAMIC_TOOL_SERVER_REQUEST_TIMEOUT_MS}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): void {
  ...
  if (!detectedVersion) {
    throw new Error(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but Alien could not determine the running Codex version. Update the configured Codex app-server binary, or remove custom command overrides to use the managed binary.`,
    );
  }
...
export function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Alien, Codex Desktop, or an env override, so only the slash-delimited
