...
describe("managed Codex app-server binary", () => {
  ...
  it("resolves the plugin-local bundled Codex binary", async () => {
    const pluginRoot = path.join("/tmp", "alien", "extensions", "codex");
  ...
  it("resolves Windows Codex command shims", () => {
    const pluginRoot = path.win32.join("C:\\", "Alien", "dist", "extensions", "codex");
  ...
  it("uses the package root when the resolver is bundled into a dist chunk", () => {
    expect(__testing.resolveDefaultCodexPluginRoot("/repo/alien/dist")).toBe("/repo/alien");
    expect(__testing.resolveDefaultCodexPluginRoot("/repo/alien/dist-runtime")).toBe(
      "/repo/alien",
    );
    expect(
      __testing.resolveDefaultCodexPluginRoot("/repo/alien/extensions/codex/src/app-server"),
    ).toBe("/repo/alien/extensions/codex");
  });

  it("finds Codex in the package install root used by packaged plugins", async () => {
    const installRoot = path.join("/tmp", "alien-plugin-package", "codex");
  ...
  it("falls back to the resolved Codex package bin when no command shim exists", async () => {
    const installRoot = await mkdtemp(path.join(os.tmpdir(), "alien-codex-package-"));
    const pluginRoot = path.join(installRoot, "dist", "extensions", "codex");
  ...
  it("fails clearly when the managed Codex binary is missing", async () => {
    await expect(
      resolveManagedCodexAppServerStartOptions(startOptions("managed"), {
        platform: "darwin",
        pluginRoot: path.join("/tmp", "alien", "extensions", "codex"),
