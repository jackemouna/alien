...
async function findManagedCodexAppServerCommandPath(params: {
  ...
  throw new Error(
    [
      ...
      "Reinstall or update Alien, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or ALIEN_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}
