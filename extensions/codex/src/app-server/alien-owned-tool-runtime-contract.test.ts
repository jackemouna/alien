import type { AnyAgentTool } from "alien/plugin-sdk/agent-harness";
import { wrapToolWithBeforeToolCallHook } from "alien/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installAlienOwnedToolHooks,
  mediaToolResult,
  resetAlienOwnedToolHooks,
  textToolResult,
...
describe("Alien-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetAlienOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installAlienOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installAlienOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installAlienOwnedToolHooks();
