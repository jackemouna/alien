...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`Alien session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`Alien session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-codex-command-"));
  });
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this Alien session to Codex thread thread-123.",
    });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this Alien session yet.",
    });
  });
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("Alien session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("Alien session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("Alien session key: `agent:main:discord:two`");
    expect(request.text).toContain("Alien session id: `session-two`");
