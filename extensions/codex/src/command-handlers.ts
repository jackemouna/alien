...
import type { PluginCommandContext, PluginCommandResult } from "alien/plugin-sdk/plugin-entry";
...
const CODEX_DIAGNOSTICS_SOURCE = "alien-diagnostics";
...
async function bindConversation(
  ...
  if (!ctx.sessionFile) {
    return {
      text: "Cannot bind Codex because this command did not include an Alien session file.",
    };
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Alien session file.";
  }
  ...
  return `Attached this Alien session to Codex thread ${formatCodexDisplayText(
    effectiveThreadId,
  )}.`;
}
...
async function stopConversationTurn(
  ...
  if (!sessionFile) {
    return "Cannot stop Codex because this command did not include an Alien session file.";
  }
...
async function steerConversationTurn(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot steer Codex because this command did not include an Alien session file.";
  }
  return (
...
async function setConversationModel(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex model because this command did not include an Alien session file.";
  }
  const [model = ""] = args;
...
async function setConversationFastMode(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex fast mode because this command did not include an Alien session file.";
  }
  const value = args[0];
...
async function setConversationPermissions(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex permissions because this command did not include an Alien session file.";
  }
  const value = args[0];
...
async function requestCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return {
      text: "Cannot send Codex diagnostics because this command did not include an Alien session file.",
    };
  }
  ...
  if (targets.length === 0) {
    return {
      text: [
        "No Codex thread is attached to this Alien session yet.",
...
async function previewCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return "Cannot send Codex diagnostics because this command did not include an Alien session file.";
  }
  const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
  if (targets.length === 0) {
    return [
  ...
  return [
    ...
    "The completed diagnostics reply will list the Alien session ids and Codex thread ids that were sent.",
    ...(displayReason ? [`Note: ${displayReason}`] : []),
    "Included: Codex logs and spawned Codex subthreads when available.",
  ].join("\n");
}
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`Alien session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`Alien session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`Alien session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
...
async function startThreadAction(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an Alien session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Alien session yet.`;
  }
