...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("alien.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this Alien session yet.");
  }
  return binding;
}
