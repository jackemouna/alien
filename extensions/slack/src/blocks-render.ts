...
function readSlackAlienBlockIndex(blockId: string, prefix: string): number | undefined {
...
export function resolveSlackInteractiveBlockOffsets(
  ...
  for (const block of blocks ?? []) {
    ...
    buttonIndexOffset = Math.max(
      buttonIndexOffset,
      readSlackAlienBlockIndex(blockId, "alien_reply_buttons_") ?? 0,
    );
    selectIndexOffset = Math.max(
      selectIndexOffset,
      readSlackAlienBlockIndex(blockId, "alien_reply_select_") ?? 0,
    );
  }
...
export function buildSlackInteractiveBlocks(
  ...
  return reduceInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `alien_reply_buttons_${++state.buttonIndex}`,
        elements,
      });
      return state;
    }
    ...
    state.blocks.push({
      type: "actions",
      block_id: `alien_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
