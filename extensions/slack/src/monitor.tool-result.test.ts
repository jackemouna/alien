import { CURRENT_MESSAGE_MARKER } from "alien/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "alien/plugin-sdk/channel-test-helpers";
import { resetInboundDedupe } from "alien/plugin-sdk/reply-dedupe";
import { HISTORY_CONTEXT_MARKER } from "alien/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\balien\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("alien: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("alien: hello <@U2>");
  });
