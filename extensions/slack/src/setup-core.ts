import { hasConfiguredSecretInput } from "alien/plugin-sdk/secret-input";
import {
...
import { formatDocsLink } from "alien/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: AlienConfig, accountId: string): AlienConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: AlienConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: AlienConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: AlienConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: AlienConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: AlienConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: AlienConfig; accountId: string }) =>
    ...
    disable: (cfg: AlienConfig) => setSetupChannelEnabled(cfg, channel, false),
