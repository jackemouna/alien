...
type SlackRouteBinding = NonNullable<AlienConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  AlienConfig,
  { bindingsRef: AlienConfig["bindings"]; normalizedCfg: AlienConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: AlienConfig): AlienConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as AlienConfig)
    : cfg;
