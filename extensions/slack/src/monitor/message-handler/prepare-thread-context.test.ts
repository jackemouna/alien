...
import { resolveEnvelopeFormatOptions } from "alien/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  const storeFixture = createSlackSessionStoreFixture("alien-slack-thread-context-");
  ...
  async function resolveAllowlistedThreadContext(params: {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as AlienConfig),
