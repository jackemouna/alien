...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, /^alien_cmdarg/, "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "alien_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^alien_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("alien_cmdarg")).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("alien_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("alien_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("alien_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("alien_cmdarg");
    expect(blockId).toContain("alien_cmdarg_ext:");
    const token = (blockId ?? "").slice("alien_cmdarg_ext:".length);
