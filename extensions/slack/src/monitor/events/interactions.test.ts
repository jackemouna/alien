...
vi.mock("alien/plugin-sdk/system-event-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("alien/plugin-sdk/system-event-runtime")>();
  return {
    ...actual,
...
vi.mock("alien/plugin-sdk/approval-gateway-runtime", () => ({
...
describe("registerSlackInteractionEvents", () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "alien:verify" }],
            },
          ],
        },
      },
  ...
  it("registers a matcher that accepts plugin action ids beyond the Alien prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("alien:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "alien:reply_button" }],
            },
          ],
        },
      },
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"alien:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "alien:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "alien:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "alien:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "alien:datetime" }],
            },
          ],
        },
      },
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:alien:deploy_form",
      callbackId: "alien:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("keeps no-channel modal events open when allowFrom is unset", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V444",
          callback_id: "alien:routing_form",
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@alien.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.alien.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "alien:long_richtext",
