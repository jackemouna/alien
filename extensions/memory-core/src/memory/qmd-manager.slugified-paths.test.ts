...
import { resolveMemoryBackendConfig } from "alien/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager slugified path resolution", () => {
  ...
  async function createManager(params?: { cfg?: AlienConfig }) {
  ...
  beforeEach(async () => {
    ...
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-qmd-slugified-"));
