...
import type { MemoryEmbeddingProbeResult } from "alien/plugin-sdk/memory-core-host-engine-storage";
import { resolveMemoryRemDreamingConfig } from "alien/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: AlienConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredAlienTmpDir(), "alien-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: AlienConfig): string {
...
function resolveAgent(cfg: AlienConfig, agent?: string) {
...
function resolveAgentIds(cfg: AlienConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: alien memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredAlienTmpDir(), "alien-rem-backfill-"),
      );
      try {
