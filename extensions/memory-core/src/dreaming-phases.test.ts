...
import { RequestScopedSubagentRuntimeError } from "alien/plugin-sdk/error-runtime";
import { resolveSessionTranscriptsDirForAgent } from "alien/plugin-sdk/memory-core-host-runtime-core";
import {
...
const LIGHT_DREAMING_TEST_CONFIG: AlienConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("alien-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__alien_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __alien_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__alien_memory_core_light_sleep__",
            "",
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    ...
    vi.stubEnv("ALIEN_STATE_DIR", path.join(workspaceDir, ".state"));
  ...
  it("skips dreaming-generated narrative transcripts during session ingestion", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          type: "custom",
          customType: "alien:bootstrap-context:full",
          data: {
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__alien_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: AlienConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
