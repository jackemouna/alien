...
import { resolveMemoryDreamingConfig } from "alien/plugin-sdk/memory-core-host-status";
import type { AlienPluginApi, PluginCommandContext } from "alien/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "alien/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: AlienConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: AlienConfig, enabled: boolean): AlienConfig {
...
function formatStatus(cfg: AlienConfig): string {
...
export async function handleDreamingCommand(api: AlienPluginApi, ctx: PluginCommandContext) {
  const args = ctx.args?.trim() ?? "";
  const [firstToken = ""] = args
    .split(/\s+/)
    .filter(Boolean)
    .map((token) => normalizeLowercaseStringOrEmpty(token));
  const currentConfig = api.runtime.config.current() as AlienConfig;
...
export function registerDreamingCommand(api: AlienPluginApi): void {
  api.registerCommand({
    name: "dreaming",
