...
import { resolveAccountEntry } from "alien/plugin-sdk/account-resolution";
import { tryReadSecretFileSync } from "alien/plugin-sdk/core";
import type {
  LineAccountConfig,
  LineConfig,
  LineTokenSource,
...
export function listLineAccountIds(cfg: AlienConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: AlienConfig): string {
