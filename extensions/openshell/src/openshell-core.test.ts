...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from Alien config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "alien-session-1234",
        backendId: "openshell",
        runtimeLabel: "alien-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "alien-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "alien-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "alien-session-5678",
        backendId: "openshell",
        runtimeLabel: "alien-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "alien",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "alien-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "alien-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("alien-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("alien-openshell-fs-");
    const outsideDir = await makeTempDir("alien-openshell-outside-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("alien-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("alien-openshell-agent-");
