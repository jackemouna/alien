...
import { createSandboxTestContext } from "alien/plugin-sdk/test-fixtures";
import {
...
const ALIEN_OPENSHELL_E2E = process.env.ALIEN_E2E_OPENSHELL === "1";
const ALIEN_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const ALIEN_OPENSHELL_COMMAND =
  process.env.ALIEN_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "alien-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && ALIEN_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: ALIEN_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(ALIEN_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(ALIEN_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-openshell-e2e-"));
      ...
      const gatewayName = `alien-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `alien-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/alien-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: ALIEN_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("alien-openshell-e2e");
