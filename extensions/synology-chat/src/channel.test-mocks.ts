...
vi.mock("alien/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("alien/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("alien/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("alien/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("alien/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("alien/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/alien/synology-chat-sessions.json"),
