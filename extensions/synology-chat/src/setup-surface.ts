...
function getChannelConfig(cfg: AlienConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: AlienConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: AlienConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: AlienConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Alien uses to send replies back to Chat.",
      ],
