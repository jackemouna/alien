...
describe("listMicrosoftVoices", () => {
  ...
  it("records voice discovery exchanges in debug proxy capture mode", async () => {
    ...
    process.env.ALIEN_DEBUG_PROXY_SESSION_ID = "ms-voices-session";
  ...
  it("does not double-capture voice discovery when the global fetch patch is installed", async () => {
    ...
    process.env.ALIEN_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.ALIEN_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.ALIEN_DEBUG_PROXY_SESSION_ID = "ms-voices-global-session";
