...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<AlienPluginApi["registerService"]>[0];
};
...
function setup(config: Record<string, unknown>): Registered {
  ...
  const api = createTestPluginApi({
    ...
    config: {},
    pluginConfig: config,
    runtime: { tts: { textToSpeechTelephony: vi.fn() } } as unknown as AlienPluginApi["runtime"],
    logger: noopLogger,
...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("alien.voice-call.runtime")];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("alien.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("alien.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("ALIEN_CLI", "1");
    process.argv = ["node", "alien", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager.initiateCall).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "Alien voice call smoke test.",
        mode: "notify",
      });
