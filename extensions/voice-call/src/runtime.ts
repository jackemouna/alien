...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Alien agent receiving delegated requests from a live phone voice bridge.",
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as AlienConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as AlienConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
