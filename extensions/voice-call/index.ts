...
import { ErrorCodes, errorShape } from "alien/plugin-sdk/gateway-runtime";
...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "realtime.toolPolicy": {
      ...
      help: "Controls the shared alien_agent_consult tool.",
      advanced: true,
    },
    "realtime.consultPolicy": {
      label: "Realtime Consult Policy",
      help: "Guides when the realtime voice model should call alien_agent_consult.",
      advanced: true,
    },
...
const VoiceCallToolSchema = Type.Union([
  Type.Object({
    ...
    sessionKey: Type.Optional(Type.String({ description: "Alien session key for the call" })),
    requesterSessionKey: Type.Optional(
      Type.String({ description: "Alien session key that initiated the call" }),
    ),
...
function isCliOnlyProcess(): boolean {
  return process.env.ALIEN_CLI === "1" && !process.argv.slice(2).includes("gateway");
}

const VOICE_CALL_RUNTIME_KEY = Symbol.for("alien.voice-call.runtime");
const VOICE_CALL_RUNTIME_PROMISE_KEY = Symbol.for("alien.voice-call.runtimePromise");
const VOICE_CALL_RUNTIME_STOP_PROMISE_KEY = Symbol.for("alien.voice-call.runtimeStopPromise");
