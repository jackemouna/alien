...
import { type JsonSchemaObject, validateJsonSchemaValue } from "alien/plugin-sdk/config-schema";
...
describe("qqbot config", () => {
  ...
  it("accepts SecretRef-backed credentials in the runtime schema", () => {
    const parsed = QQBotConfigSchema.safeParse({
      ...
      upgradeUrl: "https://docs.alien.ai/channels/qqbot",
      upgradeMode: "doc",
      accounts: {
        bot2: {
          appId: "654321",
  ...
  it("preserves top-level media and upgrade config on the default account", () => {
    ...
    expect(resolved.config.upgradeUrl).toBe("https://docs.alien.ai/channels/qqbot");
