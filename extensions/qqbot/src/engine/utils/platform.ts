 ...
 * silk-wasm availability checks — all without importing `alien/plugin-sdk`.
...
/** Return a path under `~/.alien/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".alien", "qqbot", ...subPaths);
}

/** Return a path under `~/.alien/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.alien/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Alien's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".alien", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.alien/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.alien/media`, Alien's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `alien/src/media/local-roots.ts`). Using it as a
 ...
 */
function getAlienMediaDir(): string {
  return path.join(getHomeDir(), ".alien", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".alien", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Alien's shared `~/.alien/media`
  ...
  const allowedRoots = [getAlienMediaDir(), getQQBotMediaPath()];
