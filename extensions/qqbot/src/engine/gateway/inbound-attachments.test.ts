...
vi.mock("../utils/platform.js", () => ({
  getQQBotMediaDir: () => "/tmp/alien-qqbot-downloads",
}));
...
describe("engine/gateway/inbound-attachments", () => {
  ...
  it("prefers voice_wav_url for voice downloads and transcribes with configured STT", async () => {
    downloadFileMock.mockResolvedValue("/tmp/alien-qqbot-downloads/voice.wav");
    ...
    expect(transcribeAudioMock).toHaveBeenCalledWith("/tmp/alien-qqbot-downloads/voice.wav", {
      channels: { qqbot: { stt: {} } },
    });
    expect(result.voiceAttachmentPaths).toEqual(["/tmp/alien-qqbot-downloads/voice.wav"]);
