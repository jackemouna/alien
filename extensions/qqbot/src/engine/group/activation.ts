 ...
 *   2. `$ALIEN_STATE_DIR` / `$CLAWDBOT_STATE_DIR`
 *   3. `~/.alien/agents/{agentId}/sessions/sessions.json`
 */
function resolveSessionStorePath(
  ...
  const stateDir =
    process.env.ALIEN_STATE_DIR?.trim() ||
    process.env.CLAWDBOT_STATE_DIR?.trim() ||
    path.join(process.env.HOME || process.env.USERPROFILE || "", ".alien");
