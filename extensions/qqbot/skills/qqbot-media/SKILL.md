...
3. **待发送的本地文件须落在 Alien 媒体目录下**：生成、下载或复制出的文件应写入 **`~/.alien/media/qqbot/`**（或其子目录），再写进 `<qqmedia>`。不要只放在 `~/.alien/workspace/` 等工作区根目录——平台安全策略只允许从 `~/.alien/media/`（含 `media/qqbot`）等受信根路径上传，否则会拦截、发不出去。
