...
let runtimeEnvModule: typeof import("alien/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): AlienConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("alien/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("alien/plugin-sdk/runtime-env");
  ...
  it("passes Alien event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
