...
let listNativeCommandSpecs: typeof import("alien/plugin-sdk/command-auth").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  const baseCfg: AlienConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<AlienConfig["channels"]>["discord"] =
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("alien/plugin-sdk/command-auth"));
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<AlienConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions to Discord's limit", () => {
    ...
    const discordConfig = {} as NonNullable<AlienConfig["channels"]>["discord"];
