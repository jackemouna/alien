...
describe("createDiscordGatewayPlugin", () => {
  ...
  it("resolves gateway metadata timeout from config, env, then default", () => {
    ...
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { ALIEN_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "25000" },
      }),
    ).toBe(25_000);
  ...
  it("leaves autoInteractions disabled so Alien owns interaction handoff", () => {
  ...
  it("keeps Alien metadata timeout out of gateway options", () => {
