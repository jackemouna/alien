...
import type { ChatCommandDefinition, CommandArgsParsing } from "alien/plugin-sdk/command-auth";
import type { ModelsProviderData } from "alien/plugin-sdk/command-auth";
import type { AlienConfig } from "alien/plugin-sdk/config-types";
import * as globalsModule from "alien/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "alien/plugin-sdk/session-store-runtime";
import * as commandTextModule from "alien/plugin-sdk/text-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "alien-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
