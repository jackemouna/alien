import { typedCases } from "alien/plugin-sdk/test-fixtures";
...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("alien/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Alien")).toBe("friends-of-alien");
  ...
  it("matches ids by default and names only when enabled", () => {
    ...
    expect(allowListMatches(allow, { name: "friends-of-alien" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(
      allowListMatches(allow, { name: "friends-of-alien" }, { allowNameMatching: true }),
    ).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-alien" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Alien"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-alien");
  });
  ...
  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-alien": { slug: "friends-of-alien" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Alien"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["alien-dm"],
        channelId: "1",
        channelName: "Alien DM",
        channelSlug: "alien-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
...
const channelRuntimeModule = await import("alien/plugin-sdk/system-event-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("alien/plugin-sdk/routing");
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    cfg: {} as import("alien/plugin-sdk/config-types").AlienConfig,
    accountId: "acc-1",
