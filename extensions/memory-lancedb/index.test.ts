...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "alien-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("alien/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      const mockApi = {
        ...
        runtime: {
          ...
          agent: {
            resolveAgentDir: vi.fn(() => "/tmp/alien-agent"),
          },
        },
        logger: {
          info: vi.fn(),
      ...
      vi.doUnmock("alien/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
  ...
  test("runs auto-recall through the registered before_prompt_build hook", async () => {
    ...
    vi.resetModules();
    vi.doMock("alien/plugin-sdk/runtime-env", () => ({
    ...
    try {
      ...
      vi.doUnmock("alien/plugin-sdk/runtime-env");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
