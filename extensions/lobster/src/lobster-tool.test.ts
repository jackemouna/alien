...
import type { AlienPluginApi, AlienPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<AlienPluginApi> = {}): AlienPluginApi {
...
function fakeCtx(overrides: Partial<AlienPluginToolContext> = {}): AlienPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AlienPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
