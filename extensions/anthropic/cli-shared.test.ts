...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it("maps Alien thinking levels to Claude effort args", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from Alien YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent Alien exec policy", () => {
    expect(
      resolveClaudePermissionMode({
        backendId: "claude-cli",
        agentId: "safe-agent",
