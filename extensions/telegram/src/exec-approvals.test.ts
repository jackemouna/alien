...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<AlienConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<AlienConfig["channels"]>["telegram"]>>,
): AlienConfig {
  return {
    channels: {
      telegram: {
