...
import { loadSessionStore } from "alien/plugin-sdk/session-store-runtime";
...
async function loadEnvelopeTimestampHelpers() {
  return await import("alien/plugin-sdk/channel-test-helpers");
}
...
describe("createTelegramBot", () => {
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/alien-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("blocks group model-selection callbacks for senders who are not authorized for /models", async () => {
    ...
    const storePath = `/tmp/alien-telegram-group-model-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("recomputes group model-selection callback auth from runtime command config", async () => {
    ...
    const storePath = `/tmp/alien-telegram-group-model-authz-runtime-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/alien-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/alien-telegram-model-default-${process.pid}-${Date.now()}.json`;
    const config: AlienConfig = {
      agents: {
        defaults: {
  ...
  it("formats non-default model selection confirmations with Telegram HTML parse mode", async () => {
    ...
    const storePath = `/tmp/alien-telegram-model-html-${process.pid}-${Date.now()}.json`;
    ...
    try {
      ...
      expect(editMessageTextSpy).toHaveBeenCalledWith(
        1234,
        17,
        `${CHECK_MARK_EMOJI} Model changed to <b>openai/gpt-5.4</b>\n\nSession-only model selection. Runtime unchanged. Use /model openai/gpt-5.4 --runtime &lt;runtime&gt; to switch harnesses. The agent default in alien.json is unchanged; /reset or a new session may return to that default.`,
  ...
  it("persists non-default model override using fresh config, not stale startup snapshot", async () => {
    ...
    const storePath = `/tmp/alien-telegram-model-fresh-cfg-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Alien" },
        },
      },
      me: { id: 999, username: "alien_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
