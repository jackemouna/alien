...
import * as runtimeEnvModule from "alien/plugin-sdk/runtime-env";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", ALIEN_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): AlienConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): AlienConfig => ({
    channels: {
      telegram: {
