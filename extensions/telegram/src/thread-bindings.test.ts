...
vi.mock("alien/plugin-sdk/json-store", async () => {
  const actual = await vi.importActual<typeof import("alien/plugin-sdk/json-store")>(
    "alien/plugin-sdk/json-store",
  );
...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    const acpRuntime = await vi.importActual<typeof import("alien/plugin-sdk/acp-runtime")>(
      "alien/plugin-sdk/acp-runtime",
    );
  ...
  it("does not persist lifecycle updates when manager persistence is disabled", async () => {
    stateDirOverride = fs.mkdtempSync(path.join(os.tmpdir(), "alien-telegram-bindings-"));
    process.env.ALIEN_STATE_DIR = stateDirOverride;
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-03-06T10:00:00.000Z"));
  ...
  it("persists unbinds before restart so removed bindings do not come back", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:alien-codex-app-server:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:alien-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:alien-codex-app-server:still-valid",
    );
