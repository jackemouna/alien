...
const { recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => ({
  ...
  resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/alien-session-store.json"),
}));
...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("alien/plugin-sdk/runtime-config-snapshot");
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/alien-session-store.json");
});
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("does not add a topic-cache store lookup for non-forum group reply threads", async () => {
    const resolveStorePath = vi.fn(() => "/tmp/alien/session-store.json");
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-telegram-topic-name-"));
