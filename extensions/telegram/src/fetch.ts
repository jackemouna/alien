...
// fix for the leak observed in alien#68128 is the transport lifecycle that
...
function resolveAlienProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.ALIEN_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveAlienProxyUrlForTelegram() : undefined;
