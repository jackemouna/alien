...
import { withStateDirEnv } from "alien/plugin-sdk/test-env";
...
describe("resolveTelegramToken", () => {
  ...
  function createTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-telegram-token-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  function createUnknownAccountConfig(): AlienConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: AlienConfig) {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked tokenFile paths", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as AlienConfig;
  ...
  // Regression: https://github.com/alien/alien/issues/53876
...
describe("telegram update offset store", () => {
  it("persists and reloads the last update id", async () => {
    await withStateDirEnv("alien-telegram-", async () => {
