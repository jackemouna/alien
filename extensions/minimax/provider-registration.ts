...
import { buildOauthProviderAuthResult } from "alien/plugin-sdk/provider-auth";
...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "alien/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "alien/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: AlienConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: AlienConfig): string | undefined {
...
export function registerMinimaxProviders(api: AlienPluginApi) {
