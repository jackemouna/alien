...
describe("buildMinimaxSpeechProvider", () => {
  ...
  describe("isConfigured", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "alien-minimax-tts-auth-"));
  ...
  describe("synthesize", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "alien-minimax-tts-synth-"));
      tempAgentDir = path.join(tempStateDir, "agents", "main", "agent");
      await mkdir(tempAgentDir, { recursive: true });
      process.env = {
        ...savedEnv,
        ALIEN_AGENT_DIR: tempAgentDir,
        ALIEN_STATE_DIR: tempStateDir,
      };
      clearMinimaxAuthEnv();
