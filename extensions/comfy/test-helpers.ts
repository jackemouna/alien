...
export function buildComfyConfig(config: Record<string, unknown>): AlienConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): AlienConfig {
  return {
    models: {
      providers: {
        comfy: config,
