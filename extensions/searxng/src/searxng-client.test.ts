import type { LookupFn } from "alien/plugin-sdk/ssrf-runtime";
...
vi.mock("alien/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("alien/plugin-sdk/provider-web-search")>();
...
describe("searxng client", () => {
  ...
  it("preserves a configured base-path prefix when building the search URL", () => {
    expect(
      ...
      "https://search.example.com/searxng/search?q=alien&format=json&categories=general%2Cnews&language=en",
    );
  });
