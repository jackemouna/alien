...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Alien attribution headers to native OpenAI speech requests", async () => {
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.ALIEN_DEBUG_PROXY_ENABLED = "1";
      ...
      process.env.ALIEN_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.ALIEN_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.ALIEN_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.ALIEN_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
