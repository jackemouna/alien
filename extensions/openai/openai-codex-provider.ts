...
import { loginOpenAICodexOAuth } from "alien/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "alien/plugin-sdk/provider-usage";
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.alien.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `alien models auth login --provider openai-codex` or `alien configure`.";
}
