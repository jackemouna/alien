...
function collectChromeMcpProfiles(cfg: AlienConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: AlienConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "alien";
    if (driver !== "existing-session") {
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Alien-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for Alien-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Alien-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Alien connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Alien does not bundle Chrome.`,
