import type { AlienPluginSecurityAuditContext } from "alien/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "alien/plugin-sdk/secret-input";
import { formatCliCommand } from "alien/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "alien/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: AlienPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("alien security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.ALIEN_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.ALIEN_GATEWAY_PASSWORD) ||
