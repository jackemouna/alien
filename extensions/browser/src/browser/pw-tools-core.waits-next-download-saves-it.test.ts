...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAlienTmpDir: vi.fn(() => "/tmp/alien"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-alien-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-alien-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredAlienTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredAlienTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAlienTmpDir.mockReturnValue("/tmp/alien");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".alien-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAlienTmpDir.mockReturnValue("/tmp/alien-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "alien-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "alien-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAlienTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "alien-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "alien-preferred", "downloads")}${path.sep}`),
    );
  });
