...
import { clearRuntimeConfigSnapshot } from "alien/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("alien-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".alien", "alien.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to Alien");
      expect(message).toContain("Restarting the Alien gateway will not launch it");
      expect(message).not.toContain("Restart the Alien gateway");
