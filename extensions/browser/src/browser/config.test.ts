...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("alien");
    expect(profile?.driver).toBe("alien");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const alien = resolveProfile(resolved, "alien");
    expect(alien?.driver).toBe("alien");
    expect(alien?.cdpPort).toBe(18800);
    expect(alien?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from ALIEN_GATEWAY_PORT when unset", () => {
    withEnv({ ALIEN_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const alien = resolveProfile(resolved, "alien");
      expect(alien?.cdpPort).toBe(19012);
      expect(alien?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ALIEN_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const alien = resolveProfile(resolved, "alien");
      expect(alien?.cdpPort).toBe(19022);
      expect(alien?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(alien?.cdpPort).toBe(19000);
    expect(alien?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "alien");
  ...
  describe("managed browser headless mode", () => {
    const noDisplayEnv = {
      ...
      [ALIEN_BROWSER_HEADLESS_ENV]: undefined,
    };
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          alien: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "alien")!;
    ...
    it("lets ALIEN_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          alien: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          platform: "linux",
          env: { ...noDisplayEnv, [ALIEN_BROWSER_HEADLESS_ENV]: "1" },
        }),
    ...
    it("lets request-local headless override beat env and profile/global config", () => {
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          ...
          platform: "linux",
          env: { ...noDisplayEnv, [ALIEN_BROWSER_HEADLESS_ENV]: "0" },
        }),
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "alien")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "alien")!;
      expect(
        getManagedBrowserMissingDisplayError(profileResolved, profile, {
          platform: "linux",
          env: noDisplayEnv,
        }),
      ).toContain("browser.profiles.alien.headless=false");
    });
  });
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "alien")!;
  ...
  describe("default profile preference", () => {
    it("defaults to alien profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("alien");
    });

    it("keeps alien default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("alien");
    ...
    it("keeps alien default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("alien");
    ...
    it("keeps alien default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
