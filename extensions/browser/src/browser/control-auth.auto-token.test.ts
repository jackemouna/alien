...
const mocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => AlienConfig>(),
  writeConfigFile: vi.fn<(cfg: AlienConfig) => Promise<void>>(async (_cfg) => {}),
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<AlienConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: AlienConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): AlienConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: AlienConfig) {
