...
function setupEnsureBrowserAvailableHarness() {
  ...
  const launchAlienChrome = vi.mocked(chromeModule.launchAlienChrome);
  const stopAlienChrome = vi.mocked(chromeModule.stopAlienChrome);
  ...
  const profile = ctx.forProfile("alien");

  return { launchAlienChrome, stopAlienChrome, isChromeCdpReady, profile, state };
}
...
describe("browser server-context ensureBrowserAvailable", () => {
  it("waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValueOnce(false).mockResolvedValue(true);
    mockLaunchedChrome(launchAlienChrome, 123);

    const promise = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(promise).resolves.toBeUndefined();

    expect(launchAlienChrome).toHaveBeenCalledTimes(1);
    expect(isChromeCdpReady).toHaveBeenCalled();
    expect(stopAlienChrome).not.toHaveBeenCalled();
  });
  ...
  it("stops launched chrome when CDP readiness never arrives", async () => {
    ...
    mockLaunchedChrome(launchAlienChrome, 321);
    ...
    expect(launchAlienChrome).toHaveBeenCalledTimes(1);
    expect(stopAlienChrome).toHaveBeenCalledTimes(1);
  });

  it("uses configured local CDP readiness timeout after launching", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady, profile, state } =
      setupEnsureBrowserAvailableHarness();
    state.resolved.localCdpReadyTimeoutMs = 250;
    isChromeCdpReady.mockResolvedValue(false);
    mockLaunchedChrome(launchAlienChrome, 322);
  ...
  it("deduplicates concurrent lazy-start calls to prevent PortInUseError", async () => {
    ...
    mockLaunchedChrome(launchAlienChrome, 456);
  ...
  it("deduplicates concurrent lazy-start calls across fresh profile contexts", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchAlienChrome, 457);

    const firstCtx = createBrowserRouteContext({ getState: () => state });
    const secondCtx = createBrowserRouteContext({ getState: () => state });
    const first = firstCtx.forProfile("alien").ensureBrowserAvailable();
    const second = secondCtx.forProfile("alien").ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(Promise.all([first, second])).resolves.toEqual([undefined, undefined]);
  ...
  it("passes request-local headless override to initial launch", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchAlienChrome, 654);
    ...
    expect(launchAlienChrome).toHaveBeenCalledTimes(1);
    expect(launchAlienChrome.mock.calls[0]?.[2]).toEqual({ headlessOverride: true });
    expect(stopAlienChrome).not.toHaveBeenCalled();
  });
  ...
  it("passes request-local headless override to the owned restart path", async () => {
    ...
    state.profiles.set("alien", {
      profile: profile.profile,
      running: {
        pid: 111,
        exe: { kind: "chromium", path: "/usr/bin/chromium" },
        userDataDir: "/tmp/alien-test",
        cdpPort: 18800,
        startedAt: Date.now(),
        proc: existingProc,
      },
    ...
    mockLaunchedChrome(launchAlienChrome, 987);
  ...
  it("does not share inflight lazy-start promises across different headless overrides", async () => {
    const { launchAlienChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("clears the concurrent lazy-start guard after launch failure", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAlienChrome.mockRejectedValueOnce(
    ...
    mockLaunchedChrome(launchAlienChrome, 789);
    const retry = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(retry).resolves.toBeUndefined();

    expect(launchAlienChrome).toHaveBeenCalledTimes(2);
    expect(stopAlienChrome).not.toHaveBeenCalled();
  });
  ...
  it("cools down repeated managed Chrome launch failures across route contexts", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAlienChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));

    for (let attempt = 0; attempt < 3; attempt += 1) {
      const ctx = createBrowserRouteContext({ getState: () => state });
      await expect(ctx.forProfile("alien").ensureBrowserAvailable()).rejects.toThrow(
        "Failed to start Chrome CDP",
      );
    }

    const cooledDownCtx = createBrowserRouteContext({ getState: () => state });
    await expect(cooledDownCtx.forProfile("alien").ensureBrowserAvailable()).rejects.toThrow(
      'Browser launch for profile "alien" is cooling down after 3 consecutive managed Chrome launch failures.',
    );
    await expect(cooledDownCtx.forProfile("alien").ensureBrowserAvailable()).rejects.toThrow(
      "set browser.enabled=false if the browser tool is not needed",
    );

    expect(launchAlienChrome).toHaveBeenCalledTimes(3);
    expect(stopAlienChrome).not.toHaveBeenCalled();
  });

  it("allows one managed Chrome launch attempt after the cooldown expires", async () => {
    const { launchAlienChrome, isChromeCdpReady, state } = setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAlienChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(retryCtx.forProfile("alien").ensureBrowserAvailable()).rejects.toThrow(
      "Failed to start Chrome CDP",
    );

    expect(launchAlienChrome).toHaveBeenCalledTimes(4);
  });
  ...
  it("reuses a pre-existing loopback browser after an initial short probe miss", async () => {
    ...
    expect(launchAlienChrome).not.toHaveBeenCalled();
    expect(stopAlienChrome).not.toHaveBeenCalled();
  });
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      'Port 18800 is in use for profile "alien" but not by alien.',
    );
    await expect(promise).rejects.toThrow(
      "set browser.profiles.alien.attachOnly=true so Alien attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint Alien can reach via browser.profiles.<name>.cdpUrl.",
    );
  ...
  it("retries remote CDP websocket reachability once before failing", async () => {
    const { launchAlienChrome, stopAlienChrome, isChromeCdpReady } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);

    const state = makeBrowserServerState();
    state.resolved.profiles.alien = {
  ...
  it("treats attachOnly loopback CDP as local control with remote-class probe timeouts", async () => {
    const { launchAlienChrome, stopAlienChrome } = setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
    const isChromeCdpReady = vi.mocked(chromeModule.isChromeCdpReady);
