...
import { prepareOomScoreAdjustedSpawn } from "alien/plugin-sdk/process-runtime";
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set ALIEN_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.alien/browser/${params.profile.name}/user-data.`,
    );
  }
...
export function resolveAlienUserDataDir(profileName = DEFAULT_ALIEN_BROWSER_PROFILE_NAME) {
...
export function buildAlienChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
export async function launchAlienChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ALIEN_BROWSER_COLOR).toUpperCase(),
  ...
  if (needsDecorate) {
    try {
      decorateAlienProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
      });
      log.info(`👾 alien browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`alien browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`alien browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `👾 alien browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
