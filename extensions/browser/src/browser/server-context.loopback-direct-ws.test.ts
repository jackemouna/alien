...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const alien = ctx.forProfile("alien");

    const opened = await alien.openTab("about:blank");
  ...
  it("uses an HTTP /json base for focus and close under strict SSRF", async () => {
    ...
    await alien.focusTab("T1");
    await alien.closeTab("T1");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await alien.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await alien.focusTab("T2");
    await alien.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(alien.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(alien.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(alien.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
