...
export const ALIEN_BROWSER_HEADLESS_ENV = "ALIEN_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_ALIEN_BROWSER_COLOR;
  }
...
function isLocalManagedProfile(profile: ResolvedBrowserProfile): boolean {
  return profile.driver === "alien" && profile.cdpIsLoopback && !profile.attachOnly;
}
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ALIEN_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ALIEN_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_ALIEN_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ALIEN_BROWSER_PROFILE_NAME]
        ? DEFAULT_ALIEN_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "alien";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[ALIEN_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  return (
    ...
    `Set ${ALIEN_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
