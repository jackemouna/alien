...
function localAlienProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "alien",
...
function createLocalAlienResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localAlienProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveAlienUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-reset-"));
    const profileDir = path.join(tempRoot, "alien");
    ...
    const ops = createLocalAlienResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveAlienUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-reset-no-own-"));
    const profileDir = path.join(tempRoot, "alien");
    fs.mkdirSync(profileDir, { recursive: true });
