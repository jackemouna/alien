...
import {
  buildAlienChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchAlienChrome,
  resolveAlienUserDataDir,
  stopAlienChrome,
} from "./chrome.js";
 ...
 * not exercise: launchAlienChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopAlienChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildAlienChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveAlienUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveAlienUserDataDir();
      expect(dir.endsWith(path.join("alien", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveAlienUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildAlienChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildAlienChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchAlienChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchAlienChrome(resolved, profile);
  ...
  describe("launchAlienChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "alien-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchAlienChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchAlienChrome(makeResolved(), profile);
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      ...
      const userDataDir = resolveAlienUserDataDir(profile.name);
      ...
      try {
        const running = await launchAlienChrome(
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchAlienChrome(resolved, profile)).rejects.toThrow(
    ...
    it("uses the configured local launch timeout while waiting for CDP discovery", async () => {
      ...
      try {
        ...
        const rejection = expect(launchAlienChrome(resolved, profile)).rejects.toThrow(
          /Failed to start Chrome CDP/,
        );
  ...
  describe("stopAlienChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopAlienChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopAlienChrome>[0],
        1,
      );
  ...
  describe("launchAlienChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "alien-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveAlienUserDataDir(profileName));
        ...
        const staged = resolveAlienUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_ALIEN_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchAlienChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchAlienChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateAlienProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateAlienProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateAlienProfile }, "decorateAlienProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".alien-profile-decorated") || s.endsWith("Preferences")) {
