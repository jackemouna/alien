...
const { stopAlienChromeMock } = vi.hoisted(() => ({
  stopAlienChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopAlienChrome: stopAlienChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["alien", "user"]);
    ...
    expect(stopMap.alien).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopAlienChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("alien browser stop failed: Error: oops");
  });
});
