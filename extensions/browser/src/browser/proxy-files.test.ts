...
import { MEDIA_MAX_BYTES } from "alien/plugin-sdk/media-runtime";
...
describe("persistBrowserProxyFiles", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("alien-browser-proxy-files-");
  });
  ...
  it("persists browser proxy files under the shared media store", async () => {
    ...
    expect(path.normalize(savedPath ?? "")).toContain(
      `${path.sep}.alien${path.sep}media${path.sep}browser${path.sep}`,
    );
  ...
  it("rejects browser proxy files that exceed the shared media size limit", async () => {
    ...
    await expect(
      fs.stat(path.join(tempHome.home, ".alien", "media", "browser")),
    ).rejects.toThrow();
  });
});
