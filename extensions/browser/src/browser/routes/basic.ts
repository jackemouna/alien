...
async function runBrowserLiveProbe(req: BrowserRequest, ctx: BrowserRouteContext) {
  ...
  try {
    ...
    return {
      ...
      status: "fail" as const,
      summary: String(err),
      fixHint: "Run alien browser start, then retry with alien browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "alien" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched alien profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post(
    ...
    asyncBrowserRoute(async (req, res) => {
      ...
      if (driver && driver !== "alien" && driver !== "clawd" && driver !== "existing-session") {
        return jsonError(
          res,
          400,
          `unsupported profile driver "${driver}"; use "alien", "clawd", or "existing-session"`,
        );
      }
