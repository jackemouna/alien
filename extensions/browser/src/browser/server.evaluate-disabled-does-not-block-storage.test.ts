...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        alien: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ALIEN_GATEWAY_PORT;
    process.env.ALIEN_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ALIEN_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ALIEN_GATEWAY_PASSWORD;
