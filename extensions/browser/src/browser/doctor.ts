...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "alien"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set ALIEN_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ? {}
        : {
            fixHint: "Run alien browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });
