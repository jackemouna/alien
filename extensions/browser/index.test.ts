...
type BrowserAutoEnableProbe = Parameters<AlienPluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "alien.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("lazy-loads the browser service on start", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/alien", logger: { warn: vi.fn() } });
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "alien" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "alien", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
