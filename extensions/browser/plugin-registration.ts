...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Alien's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Alien's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated Alien-managed browser (`alien`).",
...
export const browserPluginNodeHostCommands: AlienPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: AlienPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): AlienPluginService {
  let service: AlienPluginService | null = null;
...
export function registerBrowserPlugin(api: AlienPluginApi) {
  api.registerTool(((ctx: AlienPluginToolContext) =>
    createLazyBrowserTool({
      sandboxBridgeUrl: ctx.browser?.sandboxBridgeUrl,
      allowHostControl: ctx.browser?.allowHostControl,
      agentSessionKey: ctx.sessionKey,
    })) as AlienPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
