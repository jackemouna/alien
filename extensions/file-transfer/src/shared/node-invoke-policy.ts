...
}): Promise<AlienPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: AlienPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: AlienPluginNodeInvokePolicyContext,
): Promise<AlienPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): AlienPluginNodeInvokePolicy {
  return {
    commands: COMMANDS,
