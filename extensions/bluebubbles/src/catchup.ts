...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit ALIEN_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.ALIEN_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.alien.
  // Use resolvePreferredAlienTmpDir + string concat (mirrors
  ...
  // Canonical Alien state dir: honors `~` expansion + legacy/new
