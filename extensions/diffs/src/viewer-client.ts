...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-alien-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-alien-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.alienDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.alienDiffsError = "true";
