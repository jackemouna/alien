...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("alien-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/alien/plugins/diffs/view/",
    );
    expect((result?.details as Record<string, unknown>).viewerUrl).toEqual(
      expect.stringContaining("https://example.com/alien/plugins/diffs/view/"),
    );
  });
...
function createApi(pluginConfig?: Record<string, unknown>): AlienPluginApi {
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: AlienPluginToolContext = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
