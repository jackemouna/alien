...
import type { JsonSchemaObject } from "alien/plugin-sdk/config-schema";
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/alien/",
      }),
    ).toBe("https://example.com/alien");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/alien?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/alien#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/alien/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/alien",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/alien/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("alienDiffsReady");
