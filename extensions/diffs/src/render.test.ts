...
describe("renderDiffDocument", () => {
  ...
  it("renders before/after input into a complete viewer document", async () => {
    ...
    expect(rendered.html).toContain("data-alien-diff-root");
  ...
  it("resolves viewer assets under an optional base path", async () => {
    ...
    expect(
      new URL(loaderSrc ?? "", "https://example.com/alien/plugins/diffs/view/id/token").pathname,
    ).toBe("/alien/plugins/diffs/assets/viewer.js");
  });
  ...
  it("downgrades invalid language hints to plain text", async () => {
    ...
    const payloads = [...html.matchAll(/data-alien-diff-payload>(.*?)<\/script>/g)].map(
