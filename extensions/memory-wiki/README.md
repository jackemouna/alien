...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Alien**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.alien-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.alien-wiki/cache/agent-digest.json` and `.alien-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
