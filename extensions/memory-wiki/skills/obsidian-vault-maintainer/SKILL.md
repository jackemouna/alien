...
- Start from `alien wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `alien wiki obsidian status` before shelling out, then prefer the dedicated helpers like `alien wiki obsidian search`, `alien wiki obsidian open`, `alien wiki obsidian command`, and `alien wiki obsidian daily`.
