...
describe("buildWikiPromptSection", () => {
  ...
  it("can append a compact compiled digest snapshot when enabled", async () => {
    ...
    await fs.mkdir(path.join(rootDir, ".alien-wiki", "cache"), { recursive: true });
    await fs.writeFile(
      path.join(rootDir, ".alien-wiki", "cache", "agent-digest.json"),
      JSON.stringify(
        {
          claimCount: 8,
  ...
  it("stabilizes digest prompt ordering for prompt-cache-friendly output", async () => {
    ...
    const digestPath = path.join(rootDir, ".alien-wiki", "cache", "agent-digest.json");
