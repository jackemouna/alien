...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- alien:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- alien:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- alien:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- alien:wiki:${group.dir}:index:end -->`,
