...
describe("matrix qa cli registration", () => {
  const originalDisableForceExit = process.env.ALIEN_QA_MATRIX_DISABLE_FORCE_EXIT;
  ...
  afterEach(() => {
    if (originalDisableForceExit === undefined) {
      delete process.env.ALIEN_QA_MATRIX_DISABLE_FORCE_EXIT;
    } else {
      process.env.ALIEN_QA_MATRIX_DISABLE_FORCE_EXIT = originalDisableForceExit;
    }
    exitSpy.mockRestore();
  ...
  it("exits with failure after Matrix artifacts are written for a failed run", async () => {
    ...
    await expect(qa.parseAsync(["node", "alien", "matrix"])).rejects.toThrow("process.exit(1)");
  ...
  it("can disable the forced exit for direct test harnesses", async () => {
    process.env.ALIEN_QA_MATRIX_DISABLE_FORCE_EXIT = "1";
    const qa = new Command();
    matrixQaCliRegistration.register(qa);
    runQaMatrixCommand.mockRejectedValue(new Error("scenario failed"));

    await expect(qa.parseAsync(["node", "alien", "matrix"])).rejects.toThrow("scenario failed");
