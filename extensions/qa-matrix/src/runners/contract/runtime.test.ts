...
describe("matrix live qa runtime", () => {
  it("prints Matrix QA progress by default for non-interactive runs", () => {
    const previous = process.env.ALIEN_QA_MATRIX_PROGRESS;
    delete process.env.ALIEN_QA_MATRIX_PROGRESS;
    try {
      expect(liveTesting.shouldWriteMatrixQaProgress()).toBe(true);
      process.env.ALIEN_QA_MATRIX_PROGRESS = "0";
      expect(liveTesting.shouldWriteMatrixQaProgress()).toBe(false);
    } finally {
      if (previous === undefined) {
        delete process.env.ALIEN_QA_MATRIX_PROGRESS;
      } else {
        process.env.ALIEN_QA_MATRIX_PROGRESS = previous;
      }
    }
  });

  it("normalizes the Matrix QA hard timeout env", () => {
    const previous = process.env.ALIEN_QA_MATRIX_TIMEOUT_MS;
    try {
      process.env.ALIEN_QA_MATRIX_TIMEOUT_MS = "12345";
      expect(liveTesting.createMatrixQaRunDeadline().timeoutMs).toBe(12345);
      process.env.ALIEN_QA_MATRIX_TIMEOUT_MS = "nope";
      expect(liveTesting.createMatrixQaRunDeadline().timeoutMs).toBe(30 * 60_000);
    } finally {
      if (previous === undefined) {
        delete process.env.ALIEN_QA_MATRIX_TIMEOUT_MS;
      } else {
        process.env.ALIEN_QA_MATRIX_TIMEOUT_MS = previous;
      }
    }
  });

  it("normalizes the Matrix QA canary timeout env", () => {
    const previous = process.env.ALIEN_QA_MATRIX_CANARY_TIMEOUT_MS;
    try {
      delete process.env.ALIEN_QA_MATRIX_CANARY_TIMEOUT_MS;
      expect(liveTesting.resolveMatrixQaCanaryTimeoutMs()).toBe(45_000);
      process.env.ALIEN_QA_MATRIX_CANARY_TIMEOUT_MS = "90000";
      expect(liveTesting.resolveMatrixQaCanaryTimeoutMs()).toBe(90_000);
      process.env.ALIEN_QA_MATRIX_CANARY_TIMEOUT_MS = "nope";
      expect(liveTesting.resolveMatrixQaCanaryTimeoutMs()).toBe(45_000);
    } finally {
      if (previous === undefined) {
        delete process.env.ALIEN_QA_MATRIX_CANARY_TIMEOUT_MS;
      } else {
        process.env.ALIEN_QA_MATRIX_CANARY_TIMEOUT_MS = previous;
      }
    }
  });
