import { spawn as startAlienCliProcess } from "node:child_process";
...
import { redactSensitiveText } from "alien/plugin-sdk/logging-core";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `alien ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaAlienCliEntryPath(cwd: string): string {
...
export function startMatrixQaAlienCli(params: {
  ...
  const distEntryPath = resolveMatrixQaAlienCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startAlienCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
    env: params.env,
...
export async function runMatrixQaAlienCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaAlienCli(params).wait();
}
...
export async function createMatrixQaAlienCliRuntime(params: {
  accountId: string;
  accessToken: string;
