    ...
    await withStartedMatrixHarness(
      ...
      async ({ outputDir, result }) => {
        expect(calls).toEqual([
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml down --remove-orphans @/repo/alien`,
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml up -d @/repo/alien`,
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps --format json matrix-qa-homeserver @/repo/alien`,
        ]);
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml restart matrix-qa-homeserver @/repo/alien`,
        );
      },
    );
  });
  ...
  it("falls back to the container IP when the host port is unreachable", async () => {
    ...
    await withStartedMatrixHarness(
      ...
      ({ outputDir, result }) => {
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps -q matrix-qa-homeserver @/repo/alien`,
        );
        expect(calls).toContain(
          "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} container-123 @/repo/alien",
        );
      },
    );
  });
