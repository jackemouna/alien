...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredAlienTmpDir: vi.fn(() => path.join(os.tmpdir(), "alien-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredAlienTmpDir: pluginApiMocks.resolvePreferredAlienTmpDir,
...
function createApi(params?: {
  ...
  runtime?: AlienPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as AlienPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: AlienPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredAlienTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredAlienTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Alien iOS app:");
    ...
    expect(text).not.toContain("![Alien pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Alien iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://alien.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://alien.local:18789");
  });
