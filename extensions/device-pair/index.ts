...
import { definePluginEntry, type AlienPluginApi } from "alien/plugin-sdk/plugin-entry";
import {
...
function resolveAuthLabel(cfg: AlienPluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.ALIEN_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.ALIEN_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: AlienPluginApi): Promise<ResolveUrlResult> {
...
async function resolveMobilePairingGatewayUrl(api: AlienPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Alien devices",
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (target && canSendQrPngToChannel(channel)) {
            ...
            try {
              const { resolvePreferredAlienTmpDir, writeQrPngTempFile } =
                await loadDevicePairApiModule();
              qrFilePath = (
                await writeQrPngTempFile(setupCode, {
                  tmpRoot: resolvePreferredAlienTmpDir(),
              ...
              const sent = await sendQrPngToSupportedChannel({
                ...
                ctx,
                target,
                caption: ["Scan this QR code with the Alien iOS app:", "", ...infoLines].join(
                  "\n",
                ),
                qrFilePath,
              });
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                "Scan this QR code with the Alien iOS app:",
                "",
