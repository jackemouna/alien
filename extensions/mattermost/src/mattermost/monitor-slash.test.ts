...
describe("mattermost monitor slash", () => {
  ...
  it("registers deduped default and native skill commands across teams", async () => {
    vi.stubEnv("ALIEN_GATEWAY_PORT", "18888");
    ...
    resolveCallbackUrl.mockReturnValue("https://alien.test/slash");
    ...
    expect(registerSlashCommands.mock.calls[0]?.[0]).toMatchObject({
      ...
      callbackUrl: "https://alien.test/slash",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "mattermost: slash commands registered (2 commands across 2 teams, callback=https://alien.test/slash)",
    );
  });
