...
import { isTrustedProxyAddress, resolveClientIp, type AlienConfig } from "./runtime-api.js";

const INTERACTION_MAX_BODY_BYTES = 64 * 1024;
const INTERACTION_BODY_TIMEOUT_MS = 10_000;
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__alien_channel_id";
...
type InteractionCallbackConfig = Pick<AlienConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "alien-mattermost-interactions").update(botToken).digest("hex");
}
