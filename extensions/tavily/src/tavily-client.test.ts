...
vi.mock("alien/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: alien", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "alien" });
  });

  it("runTavilyExtract sends X-Client-Source: alien", async () => {
