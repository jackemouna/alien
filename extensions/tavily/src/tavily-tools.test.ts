...
import type { AlienPluginApi } from "alien/plugin-sdk/plugin-runtime";
...
function fakeApi(): AlienPluginApi {
  return {
    config: {},
  } as AlienPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.alien.ai", "", "alien.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.alien.ai", "alien.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as AlienConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
