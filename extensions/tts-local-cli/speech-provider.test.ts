...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "alien/plugin-sdk/speech-core";
...
vi.mock("alien/plugin-sdk/media-runtime", () => ({
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "alien-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.ALIEN_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("alien/plugin-sdk/media-runtime")>(
