...
import { SILENT_REPLY_TOKEN } from "alien/plugin-sdk/reply-chunking";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredAlienTmpDir(), "msteams-mention-"));
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredAlienTmpDir(), "msteams-retry-"));
