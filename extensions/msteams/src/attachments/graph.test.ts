...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the Alien User-Agent to guarded Graph attachment fetches", async () => {
  ...
  it("adds the Alien User-Agent to Graph shares downloads for reference attachments", async () => {
    mockGraphMediaFetch({
      messageId: "msg-share",
      messageResponse: {
        body: {},
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ Alien\/.+$/);
  });
});
