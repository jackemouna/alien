/**
 * Twitch channel plugin for Alien.
 *
...
} from "alien/plugin-sdk/channel-pairing";
 ...
 * for Alien. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: AlienConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: AlienConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: AlienConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: AlienConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("alien/plugin-sdk/runtime-env").RuntimeEnv;
