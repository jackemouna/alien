...
import { normalizeOptionalAccountId } from "alien/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "alien/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: AlienConfig, requestedAccountId?: string): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: ALIEN_TWITCH_ACCESS_TOKEN",
...
export async function configureWithEnvToken(
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var ALIEN_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ALIEN_TWITCH_ACCESS_TOKEN?.trim();
