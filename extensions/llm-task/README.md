...
`alien.invoke --each`) without adding new Alien code per workflow.
...
This extension depends on Alien internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Alien extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.alien/extensions` as a standalone plugin directory.
