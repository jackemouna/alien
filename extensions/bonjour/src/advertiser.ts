...
import type { PluginLogger } from "alien/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "alien/plugin-sdk/runtime-env";
...
// See https://github.com/alien/alien/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.ALIEN_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Alien"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Alien";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Alien\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.ALIEN_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "alien";
    ...
    const recreateAdvertiser = async (reason: string, opts?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or ALIEN_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
