...
import { VERSION } from "alien/plugin-sdk/cli-runtime";
import {
  resolveEnvHttpProxyUrl,
  shouldUseEnvHttpProxyForUrl,
} from "alien/plugin-sdk/fetch-runtime";
import { danger, success } from "alien/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "alien/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "alien/plugin-sdk/text-runtime";
import {
  readCredsJsonRaw,
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("alien channels login")}`,
              ),
            );
          }
        }
