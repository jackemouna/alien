import { resolveAccountEntry } from "alien/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "alien/plugin-sdk/channel-inbound-debounce";
import { formatCliCommand } from "alien/plugin-sdk/cli-runtime";
import { hasControlCommand } from "alien/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "alien/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "alien/plugin-sdk/reply-history";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("alien channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("alien channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("alien channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("alien channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
