...
const hoisted = vi.hoisted(() => ({
  listWhatsAppAccountIds: vi.fn((cfg: AlienConfig) => {
  ...
  resolveWhatsAppAccount: vi.fn(
    ({ cfg, accountId }: { cfg: AlienConfig; accountId?: string | null }) => ({
      enabled:
...
describe("whatsapp channel action helpers", () => {
  ...
  it("returns null when WhatsApp is not configured", () => {
    expect(
      describeWhatsAppMessageActions({ cfg: {} as AlienConfig, accountId: "default" }),
    ).toBeNull();
  });
