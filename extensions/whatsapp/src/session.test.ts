...
import { resetLogger, setLoggerOverride } from "alien/plugin-sdk/runtime-env";
...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join("/tmp", "alien-oauth", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("alien-wa-creds-test");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("alien-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("alien-wa-a");
    const authDirB = createTempAuthDir("alien-wa-b");
    const onError = vi.fn();
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    try {
      await writeCredsJsonAtomically("/tmp/alien-oauth/whatsapp/default", {
        me: { id: "123@s.whatsapp.net" },
      });

      expect(openMock.writeFileSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          path.join("/tmp", "alien-oauth", "whatsapp", "default", ".creds."),
        ),
        expect.any(String),
        expect.objectContaining({ mode: 0o600, flag: "wx" }),
      );
      ...
      expect(chmodSpy).toHaveBeenCalledWith(
        path.join("/tmp", "alien-oauth", "whatsapp", "default", "creds.json"),
        0o600,
      );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("alien-wa-creds-atomic");
