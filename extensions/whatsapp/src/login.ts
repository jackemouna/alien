...
import { defaultRuntime, type RuntimeEnv } from "alien/plugin-sdk/runtime-env";
import { logInfo } from "alien/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("alien channels login")} and scan the QR again.`,
        ),
      );
