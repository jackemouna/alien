...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "alien-test-web-logout-"));
    previousOAuthDir = process.env.ALIEN_OAUTH_DIR;
    process.env.ALIEN_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.ALIEN_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the Alien auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Alien auth root", async () => {
