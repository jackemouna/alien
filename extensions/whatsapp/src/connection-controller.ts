...
import { info } from "alien/plugin-sdk/runtime-env";
...
const WHATSAPP_LOGGED_OUT_RELINK_MESSAGE =
  "WhatsApp reported the session is logged out. Cleared cached web session; please rerun alien channels login and scan the QR again.";
...
export function closeWaSocket(
  ...
  try {
    if (typeof sock?.end === "function") {
      sock.end(new Error("Alien WhatsApp socket close"));
      return;
    }
    sock?.ws?.close?.();
  } catch {
