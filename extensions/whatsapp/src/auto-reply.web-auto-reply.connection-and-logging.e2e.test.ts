...
import { setLoggerOverride } from "alien/plugin-sdk/runtime-env";
import { withEnvAsync } from "alien/plugin-sdk/test-env";
...
describe("web auto-reply connection", () => {
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\[alien\\] first`),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\[alien\\] second`),
        );
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/alien-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/alien-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
