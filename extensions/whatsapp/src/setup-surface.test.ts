...
import { DEFAULT_ACCOUNT_ID, type AlienConfig } from "alien/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: AlienConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  ...
  resolveWhatsAppAuthDir: vi.fn<
    (params: { cfg: AlienConfig; accountId: string }) => { authDir: string }
  >(() => ({
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: AlienConfig };
}
  ...
  it("writes named-account DM policy and allowFrom instead of the channel root", async () => {
    ...
    const named = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "work",
        cfg: createWhatsAppWorkAccountConfig() as AlienConfig,
      }),
    );
  ...
  it("uses configured defaultAccount for omitted-account finalize writes", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "",
        cfg: createWhatsAppWorkAccountConfig({ defaultAccount: "work" }) as AlienConfig,
      }),
    );
