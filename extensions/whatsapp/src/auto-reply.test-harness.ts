...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("alien:webAutoReplySessionSockets");
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-session-"));
