...
import type { ChannelSetupWizard } from "alien/plugin-sdk/setup";
...
type WhatsAppConfig = NonNullable<NonNullable<AlienConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: AlienConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: AlienConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: AlienConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: AlienConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Alien can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Alien" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("alien channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
