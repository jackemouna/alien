...
import { info, success } from "alien/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("alien channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
