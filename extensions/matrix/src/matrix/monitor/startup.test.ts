...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleAlienDevices: [] as MatrixManagedDeviceInfo[],
      currentAlienDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      currentDeviceId: null,
      staleAlienDevices: [
        { deviceId: "DEV123", displayName: "Alien Device", current: false },
      ],
      currentAlienDevices: [],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Alien devices detected for @bot:example.org: DEV123. Run 'alien matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'alien matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
