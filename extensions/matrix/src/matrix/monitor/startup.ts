...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleAlienDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Alien devices detected for ${params.auth.userId}: ${deviceHealth.staleAlienDevices.map((device) => device.deviceId).join(", ")}. Run 'alien matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
