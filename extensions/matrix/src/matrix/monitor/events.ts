...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'alien matrix verify status --verbose --account ${accountId}' and 'alien matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Alien runtime is using the same account. " +
    `Check 'alien matrix verify status --verbose --account ${accountId}' and 'alien matrix devices list --account ${accountId}'.`
  );
}
