...
function hasTrustedAlienRootIndicator(packageRoot, packageJson) {
  ...
  const hasAlienBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("alien")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.alien === "string");
  const hasAlienEntrypoint = fs.existsSync(path.join(packageRoot, "alien.mjs"));
  return hasCliEntryExport || hasAlienBin || hasAlienEntrypoint;
}

function findAlienPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "alien" && hasTrustedAlienRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findAlienPackageRoot(moduleDir);
  if (location) {
