import { type ChannelDoctorAdapter } from "alien/plugin-sdk/channel-contract";
import type { AlienConfig } from "alien/plugin-sdk/config-types";
import {
  detectPluginInstallPathIssue,
  formatPluginInstallPathIssue,
  removePluginFromConfig,
} from "alien/plugin-sdk/runtime-doctor";
import {
...
function hasConfiguredMatrixChannel(cfg: AlienConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: AlienConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: AlienConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "alien doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "alien doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: AlienConfig): Promise<string[]> {
  ...
  return formatPluginInstallPathIssue({
    issue,
    pluginLabel: "Matrix",
    defaultInstallCommand: "alien plugins install @alien/matrix",
  }).map((entry) => `- ${entry}`);
}

export async function cleanStaleMatrixPluginConfig(cfg: AlienConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
