...
import type { ChannelSetupInput } from "alien/plugin-sdk/setup";
...
import { isAlienManagedMatrixDevice } from "./matrix/device-health.js";
...
function formatMatrixCliCommandParts(parts: string[], accountId?: string): string {
  const normalizedAccountId = normalizeAccountId(accountId);
  const command = ["alien", "matrix", ...parts];
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleAlienDeviceIds: addedDevices
        .filter((device) => !device.current && isAlienManagedMatrixDevice(device.displayName))
...
function printMatrixVerificationSummary(summary: MatrixCliVerificationSummary): void {
  ...
  console.log(`Initiated by Alien: ${summary.initiatedByMe ? "yes" : "no"}`);
...
function buildVerificationGuidance(
  ...
  if (status.serverDeviceKnown === false) {
    nextSteps.add(
      `This Matrix device is no longer listed on the homeserver. Create a new Alien Matrix device with ${formatMatrixCliCommand("account add --homeserver <url> --user-id <@user:server> --password <password> --device-name Alien-Gateway", accountId)}. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run ${formatMatrixCliCommand("account add --homeserver <url> --access-token <token>", accountId)}.`,
    );
  }
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.alien.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleAlienDeviceIds.length > 0) {
              const staleDeviceIds = result.deviceHealth.staleAlienDeviceIds
                .map((deviceId) => formatMatrixCliText(deviceId))
                .join(", ");
              console.log(
                `Matrix device hygiene warning: stale Alien devices detected (${staleDeviceIds}). Run ${formatMatrixCliCommand("devices prune-stale", result.accountId)}.`,
              );
            }
            ...
            const bindHint = `alien agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale Alien-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
