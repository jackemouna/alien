...
import { writeJsonFileAtomically } from "alien/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "alien/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "alien-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("alien/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("alien/plugin-sdk/runtime")).createBackupArchive;
