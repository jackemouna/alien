...
- Extension production code should import from `alien/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `alien.plugin.json` and the package
  `alien` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `alien/plugin-sdk/*`.
