  ...
  if (params.hasMemoryFiles || memoryProvider) {
    items.push(
      createMigrationConfigPatchItem({
        ...
        message: "Use Alien built-in file memory for imported Hermes memory files.",
        conflict:
    ...
    items.push(
      createMigrationConfigPatchItem({
        ...
        message: "Select the default Alien memory plugin for imported file memory.",
        conflict:
          !params.ctx.overwrite &&
  ...
  if (memoryProvider === "honcho") {
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes used Honcho memory. Alien keeps built-in memory selected until the matching plugin is installed and reviewed.",
        recommendation:
    ...
    items.push(
      createMigrationManualItem({
        ...
        source: "config.yaml:memory.provider",
        message: `Hermes memory provider "${memoryProvider}" does not have a known Alien mapping.`,
        recommendation: "Install or configure an equivalent Alien memory plugin manually.",
      }),
    );
  }
