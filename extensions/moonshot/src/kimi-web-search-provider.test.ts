...
describe("kimi web search provider", () => {
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "Alien is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result).toMatchObject({
        provider: "kimi",
        content: expect.stringContaining("Alien is available on GitHub."),
        citations: [],
      });
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "Alien", url: "https://github.com/alien/alien" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "Alien is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result).toMatchObject({
        provider: "kimi",
        content: expect.stringContaining("Alien is on GitHub."),
        citations: ["https://github.com/alien/alien"],
      });
      expect(result).not.toHaveProperty("error");
    });
  });
