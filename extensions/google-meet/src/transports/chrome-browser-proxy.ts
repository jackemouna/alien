...
export async function resolveChromeNodeInfo(params: {
  ...
  if (requested) {
    ...
    if (matches.length === 1) {
      ...
      throw new Error(
        `Configured Google Meet node ${requested} is not usable (${formatNodeLabel(node)}): ${describeNodeUsabilityIssues(node).join("; ")}. Start or reinstall \`alien node run\` on that Chrome host, approve pairing, and allow googlemeet.chrome plus browser.proxy.`,
      );
    }
    if (matches.length > 1) {
      throw new Error(
    ...
    throw new Error(
      `Configured Google Meet node ${requested} was not found. Run \`alien nodes status\` and start or approve the Chrome node.`,
    );
  }
  ...
  if (nodes.length === 0) {
    throw new Error(
      "No connected Google Meet-capable node with browser proxy. Run `alien node run` on the Chrome host with browser proxy enabled, approve pairing, and allow googlemeet.chrome plus browser.proxy.",
    );
  }
  if (nodes.length === 1) {
    return nodes[0];
