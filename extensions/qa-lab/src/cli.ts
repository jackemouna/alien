...
export function registerQaLabCli(program: Command) {
  ...
  credentials
    ...
    .option("--site-url <url>", "Override ALIEN_QA_CONVEX_SITE_URL")
    .option("--endpoint-prefix <path>", "Override ALIEN_QA_CONVEX_ENDPOINT_PREFIX")
  ...
  qa.command("docker-scaffold")
    ...
    .option("--image <name>", "Prebaked image name", "alien:qa-local-prebaked")
  ...
  qa.command("docker-build-image")
    ...
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    .option("--image <name>", "Image tag", "alien:qa-local-prebaked")
