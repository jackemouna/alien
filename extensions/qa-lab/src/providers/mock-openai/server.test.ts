  ...
  it("supports advanced QA memory and subagent recovery prompts", async () => {
    ...
    const threadMemorySearch = await fetch(`${server.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        stream: true,
        instructions:
          "@alien Thread memory check: what is the hidden thread codename stored only in memory? Use memory tools first and reply only in this thread.",
        input: [
          {
            role: "user",
            content: [
    ...
    const response = await fetch(`${server.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        ...
        input: [
          ...
          {
            type: "function_call_output",
            output: "QA mission: Understand this Alien repo from source + docs before acting.",
          },
        ],
      }),
    });
