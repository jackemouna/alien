import { listQaRunnerCliContributions } from "alien/plugin-sdk/qa-runner-runtime";
  ...
  return {
    ...
    register(qa) {
      qa.command(params.commandName)
        ...
        .action(() => {
          throw new Error(
            `QA runner "${params.commandName}" is installed but not active. Enable or allow plugin "${params.pluginId}" in your Alien config, then try again.`,
          );
        });
    },
  };
