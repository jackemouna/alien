...
describe("credential lease runtime", () => {
  ...
  it("acquires, heartbeats, and releases convex credentials", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      role: "maintainer",
      env: {
        ALIEN_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        ALIEN_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
      },
      fetchImpl,
  ...
  it("defaults convex credential role to ci when CI=true", async () => {
    ...
    await acquireQaCredentialLease({
      ...
      env: {
        CI: "true",
        ALIEN_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        ALIEN_QA_CONVEX_SECRET_CI: "ci-secret",
      },
      fetchImpl,
      resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
      parsePayload: (payload) =>
  ...
  it("retries convex acquire while the pool is exhausted", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      env: {
        ALIEN_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        ALIEN_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        ALIEN_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "90000",
      },
      fetchImpl,
      randomImpl: () => 0,
      timeImpl: () => nowMs,
  ...
  it("rejects non-https convex site URLs unless local insecure opt-in is enabled", async () => {
    await expect(
      acquireQaCredentialLease({
        ...
        source: "convex",
        env: {
          ALIEN_QA_CONVEX_SITE_URL: "http://qa-cred.example.convex.site",
          ALIEN_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        },
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
  ...
  it("allows loopback http URLs when ALIEN_QA_ALLOW_INSECURE_HTTP is enabled", async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(
      jsonResponse({
        status: "ok",
    ...
    await acquireQaCredentialLease({
      ...
      role: "maintainer",
      env: {
        ALIEN_QA_CONVEX_SITE_URL: "http://127.0.0.1:3210",
        ALIEN_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        ALIEN_QA_ALLOW_INSECURE_HTTP: "1",
      },
      fetchImpl,
      resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
      parsePayload: (payload) =>
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          ALIEN_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          ALIEN_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
          ALIEN_QA_CONVEX_ENDPOINT_PREFIX: "//evil.example",
        },
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
          payload as { groupId: string; driverToken: string; sutToken: string },
      }),
    ).rejects.toThrow("ALIEN_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path");
  });
  ...
  it("fails convex mode when auth secret is missing", async () => {
    ...
    ).rejects.toThrow("ALIEN_QA_CONVEX_SECRET_MAINTAINER");
  });
