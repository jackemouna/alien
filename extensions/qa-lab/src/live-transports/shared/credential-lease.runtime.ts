...
function normalizeEndpointPrefix(value: string | undefined): string {
  return normalizeQaCredentialEndpointPrefix({
    ...
    invalidAbsoluteMessage:
      "ALIEN_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path like /qa-credentials/v1.",
    invalidSegmentsMessage:
      "ALIEN_QA_CONVEX_ENDPOINT_PREFIX must not contain backslashes or .. path segments.",
  });
}
...
function resolveConvexAuthToken(env: NodeJS.ProcessEnv, role: QaCredentialRole): string {
  const roleToken =
    role === "ci"
      ? env.ALIEN_QA_CONVEX_SECRET_CI?.trim()
      : env.ALIEN_QA_CONVEX_SECRET_MAINTAINER?.trim();
  const token = roleToken;
  if (token) {
    return token;
  }
  if (role === "ci") {
    throw new Error("Missing ALIEN_QA_CONVEX_SECRET_CI for CI credential access.");
  }
  throw new Error("Missing ALIEN_QA_CONVEX_SECRET_MAINTAINER for maintainer credential access.");
}
...
function resolveConvexCredentialBrokerConfig(params: {
  ...
  const siteUrl = params.env.ALIEN_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new Error("Missing ALIEN_QA_CONVEX_SITE_URL for --credential-source convex.");
  }
  const baseUrl = normalizeConvexSiteUrl(siteUrl, params.env);
  const endpointPrefix = normalizeEndpointPrefix(params.env.ALIEN_QA_CONVEX_ENDPOINT_PREFIX);
  const ownerId =
    params.ownerId?.trim() ||
    params.env.ALIEN_QA_CREDENTIAL_OWNER_ID?.trim() ||
...
export async function acquireQaCredentialLease<TPayload>(
  ...
  const source = normalizeQaCredentialSource(opts.source ?? env.ALIEN_QA_CREDENTIAL_SOURCE);
  if (source === "env") {
    return {
      source: "env",
      kind: opts.kind,
  ...
  const role = normalizeQaCredentialRole(opts.role ?? env.ALIEN_QA_CREDENTIAL_ROLE, env);
