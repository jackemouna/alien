...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      __testing.resolveTelegramQaRuntimeEnv({
        ...
        ALIEN_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        ALIEN_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("ALIEN_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      __testing.resolveTelegramQaRuntimeEnv({
        ALIEN_QA_TELEGRAM_GROUP_ID: "qa-group",
        ALIEN_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        ALIEN_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("ALIEN_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
  ...
  it("applies ALIEN_QA_SUITE_PROGRESS override to Telegram live logging", () => {
    expect(
      __testing.shouldLogTelegramQaLiveProgress({
        CI: "false",
        ALIEN_QA_SUITE_PROGRESS: "true",
      }),
    ).toBe(true);
    expect(
      __testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        ALIEN_QA_SUITE_PROGRESS: "false",
      }),
    ).toBe(false);
    expect(
      __testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        ALIEN_QA_SUITE_PROGRESS: "definitely",
      }),
    ).toBe(true);
  });
  ...
  it("normalizes the Telegram QA canary timeout env", () => {
    ...
    expect(
      __testing.resolveTelegramQaCanaryTimeoutMs({
        ALIEN_QA_TELEGRAM_CANARY_TIMEOUT_MS: "90000",
      }),
    ).toBe(90_000);
    expect(
      __testing.resolveTelegramQaCanaryTimeoutMs({
        ALIEN_QA_TELEGRAM_CANARY_TIMEOUT_MS: "nope",
      }),
    ).toBe(30_000);
  });
  ...
  it("normalizes the Telegram QA scenario timeout env", () => {
    ...
    expect(
      __testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        ALIEN_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      __testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        ALIEN_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "nope",
      }),
    ).toBe(45_000);
  });
