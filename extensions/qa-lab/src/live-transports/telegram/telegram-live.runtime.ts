...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "ALIEN_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "ALIEN_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "ALIEN_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "ALIEN_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "ALIEN_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("ALIEN_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "ALIEN_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "ALIEN_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledAlienTelegramOnboardingPreflight(params: {
  openClawCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredAlienTmpDir(), "alien-npm-telegram-"),
  );
  const homeDir = path.join(tempRoot, "home");
  const stateDir = path.join(homeDir, ".alien");
  ...
  const env = {
    ...
    ALIEN_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-alien-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutAlienCommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledAlienTelegramOnboardingPreflight({
        openClawCommand: params.sutAlienCommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutAlienCommand
        ? {
            executablePath: params.sutAlienCommand,
