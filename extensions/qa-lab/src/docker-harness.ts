...
const QA_LAB_UI_OVERLAY_DIR = "/opt/alien-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        ALIEN_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      ALIEN_CONFIG_PATH: /tmp/alien/alien.json
      ...
      ALIEN_PROFILE: ""
    volumes:
      - ./state:/opt/alien-scaffold:ro
      - ${repoMount}:/opt/alien-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/alien/workspace /tmp/alien/state && cp /opt/alien-scaffold/alien.json /tmp/alien/alien.json && cp -R /opt/alien-scaffold/seed-workspace/. /tmp/alien/workspace/ && ln -snf /opt/alien-repo /tmp/alien/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t alien:qa-local-prebaked --build-arg ALIEN_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "alien:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "alien.json"),
  ];
