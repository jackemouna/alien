...
vi.mock("@alien/qa-channel/api.js", async () => await import("../../qa-channel/api.js"));
...
vi.mock("alien/plugin-sdk/proxy-capture", () => ({
  ...
  resolveDebugProxySettings: () => ({
    dbPath: process.env.ALIEN_DEBUG_PROXY_DB_PATH ?? "",
    blobDir: process.env.ALIEN_DEBUG_PROXY_BLOB_DIR ?? "",
    proxyUrl: process.env.ALIEN_DEBUG_PROXY_URL ?? "",
...
describe("qa-lab server", () => {
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "  process.exit(0);",
        "});",
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.ALIEN_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "setInterval(() => {}, 1000);",
      ].join("\n"),
      "utf8",
    );
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    cleanups.push(async () => {
      delete process.env.ALIEN_DEBUG_PROXY_DB_PATH;
      delete process.env.ALIEN_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
