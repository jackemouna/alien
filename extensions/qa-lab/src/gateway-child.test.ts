...
vi.mock("alien/plugin-sdk/temp-path", () => ({
  resolvePreferredAlienTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/alien-qa/alien.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/alien-qa/home",
    stateDir: "/tmp/alien-qa/state",
    xdgConfigHome: "/tmp/alien-qa/xdg-config",
    xdgDataHome: "/tmp/alien-qa/xdg-data",
    xdgCacheHome: "/tmp/alien-qa/xdg-cache",
    bundledPluginsDir: "/tmp/alien-qa/bundled-plugins",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.ALIEN_TEST_FAST).toBe("1");
    expect(env.ALIEN_QA_PARENT_PID).toBe(String(process.pid));
    expect(env.ALIEN_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.ALIEN_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.ALIEN_SKIP_STARTUP_MODEL_PREWARM).toBe("1");
    expect(env.ALIEN_BUNDLED_PLUGINS_DIR).toBe("/tmp/alien-qa/bundled-plugins");
    expect(env.ALIEN_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ALIEN_LIVE_OPENAI_KEY: "openai-live",
        ALIEN_LIVE_ANTHROPIC_KEY: "anthropic-live",
        ALIEN_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing Alien home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/alien-qa/home");
    expect(env.ALIEN_HOME).toBe("/tmp/alien-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping Alien home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.ALIEN_HOME).toBe("/tmp/alien-qa/home");
    expect(env.ALIEN_STATE_DIR).toBe("/tmp/alien-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping Alien home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        ALIEN_LIVE_ANTHROPIC_KEY: "anthropic-live",
        ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.ALIEN_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.ALIEN_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ALIEN_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        ALIEN_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.ALIEN_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.ALIEN_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ALIEN_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        ALIEN_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.ALIEN_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.ALIEN_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          ALIEN_LIVE_ANTHROPIC_KEY: "anthropic-live",
          ALIEN_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          ALIEN_LIVE_GEMINI_KEY: "gemini-live",
          ALIEN_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.ALIEN_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.ALIEN_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.ALIEN_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.ALIEN_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await __testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        ALIEN_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "ALIEN_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "ALIEN_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "ALIEN_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "ALIEN_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "ALIEN_QA_CONVEX_SECRET_CI=<redacted>",
        "ALIEN_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "botToken=<redacted>",
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      __testing.assertQaArtifactDirWithinRepo("/tmp/alien-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "alien.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("resolves bundled plugins by manifest id when the directory name differs", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "kimi-coding", "alien.plugin.json"),
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "alien.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "alien.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@alien/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "package.json"),
      JSON.stringify({ name: "alien", type: "module" }, null, 2),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@alien/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "alien.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    await expect(
      __testing.readQaLiveProviderConfigOverrides({
        providerIds: ["custom-openai"],
        env: { ALIEN_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
      }),
    ).resolves.toEqual({
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ alien: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ alien: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ alien: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
