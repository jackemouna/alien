...
describe("qa suite planning helpers", () => {
  it("normalizes suite concurrency to a bounded integer", () => {
    const previous = process.env.ALIEN_QA_SUITE_CONCURRENCY;
    delete process.env.ALIEN_QA_SUITE_CONCURRENCY;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ALIEN_QA_SUITE_CONCURRENCY = previous;
      }
    }
  });
  ...
  it("resolves a default worker startup stagger for concurrent suite workers", () => {
    ...
    expect(
      resolveQaSuiteWorkerStartStaggerMs(4, {
        ALIEN_QA_SUITE_WORKER_START_STAGGER_MS: "0",
      }),
    ).toBe(0);
  });
