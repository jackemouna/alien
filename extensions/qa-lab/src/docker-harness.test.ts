...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "alien.json"),
    ...
    expect(compose).toContain('      - "http://alien-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/alien-qa-lab-ui"');
    expect(compose).toContain(":/opt/alien-repo:ro");
    expect(compose).toContain("./state:/opt/alien-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/alien-scaffold/seed-workspace/. /tmp/alien/workspace/",
    );
    expect(compose).toContain("ALIEN_CONFIG_PATH: /tmp/alien/alien.json");
    expect(compose).toContain("ALIEN_STATE_DIR: /tmp/alien/state");
    expect(compose).toContain('ALIEN_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("ALIEN_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "alien.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/alien/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/alien",
        imageName: "alien:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("alien:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t alien:qa-local-prebaked --build-arg ALIEN_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/alien",
      ),
    ]);
  });
});
