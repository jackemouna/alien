...
import { disposeRegisteredAgentHarnesses } from "alien/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.ALIEN_QA_SUITE_PROGRESS);
...
function resolveQaSuiteTransportReadyTimeoutMs(
  ...
  const raw = env.ALIEN_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {
    return 120_000;
  }
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.ALIEN_QA_DEBUG === "1") {
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as AlienConfig
      : undefined,
  });
  writeQaSuiteProgress(
    progressEnabled,
  ...
  try {
    ...
    const keepTemp = process.env.ALIEN_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
