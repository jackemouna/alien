...
import { resolveSecretInputString, normalizeSecretInput } from "alien/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<AlienConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AlienConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: AlienConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: AlienConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: AlienConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: AlienConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: AlienConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: AlienConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: AlienConfig, override?: number): number {
