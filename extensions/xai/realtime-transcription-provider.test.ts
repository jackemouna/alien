    ...
    wss.handleUpgrade(request, socket, head, (ws) => {
      ...
      ws.on("message", (data, isBinary) => {
        ...
        if (event.type === "audio.done") {
          ws.send(JSON.stringify({ type: "transcript.done", text: "hello alien final" }));
          done();
        }
      });
    });
...
describe("xai realtime transcription provider", () => {
  ...
  it("streams raw binary audio and maps partial and final transcript events", async () => {
    ...
    await waitFor(() => expect(onTranscript).toHaveBeenCalledWith("hello alien final"));
    session.close();
    ...
    expect(onPartial).toHaveBeenCalledWith("hello alien");
  });
