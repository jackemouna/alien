...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-acpx-codex-auth-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("prepareAcpxCodexAuthConfig", () => {
  it("installs an isolated Codex ACP wrapper without synthesizing auth from canonical Alien OAuth", async () => {
