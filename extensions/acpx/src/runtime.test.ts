...
const DOCUMENTED_ALIEN_BRIDGE_COMMAND =
  "env ALIEN_HIDE_BANNER=1 ALIEN_SUPPRESS_NOTES=1 alien acp --url ws://127.0.0.1:18789 --token-file ~/.alien/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/alien/acpx/codex-acp-wrapper.mjs"`;
...
function makeRuntime(
  ...
  const runtime = new AcpxRuntime({
    ...
    agentRegistry: {
      resolve: (agentName: string) => (agentName === "alien" ? "alien acp" : agentName),
      list: () => ["codex", "alien"],
    },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes Alien Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(__testing.isCodexAcpCommand("alien acp")).toBe(false);
  });
  ...
  it("routes alien ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:alien:acp:test",
      backend: "acpx",
  ...
  it("routes non-alien sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for alien sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:alien:acp:test",
      backend: "acpx",
      runtimeSessionName: "alien-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the alien agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "alien" ? "codex" : agentName),
        list: () => ["codex", "alien"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the alien bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "alien acp" : agentName),
        list: () => ["codex", "alien"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped alien bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "alien" ? DOCUMENTED_ALIEN_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "alien"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node alien entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "alien" ? "env ALIEN_HIDE_BANNER=1 node alien.mjs acp" : agentName,
        list: () => ["codex", "alien"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:alien:acp:test",
        agentCommand: DOCUMENTED_ALIEN_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:alien:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:alien:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to alien bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
