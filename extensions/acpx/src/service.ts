...
const ENABLE_STARTUP_PROBE_ENV = "ALIEN_ACPX_RUNTIME_STARTUP_PROBE";
...
function resolveAllowedAgentsProbeAgent(ctx: AlienPluginServiceContext): string | undefined {
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: AlienPluginServiceContext): Promise<void> {
      if (process.env.ALIEN_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (ALIEN_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (!shouldRunStartupProbe() || process.env.ALIEN_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: AlienPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
