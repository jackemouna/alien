...
import { readJsonFileWithFallback } from "alien/plugin-sdk/json-store";
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--alien-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @alien/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Alien's isolated CODEX_HOME wrapper, but launch the plugin-local
...
if (!command) {
  console.error("[alien] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[alien] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Alien; fallback must not float to an unpatched newer release.
...
async function prepareIsolatedCodexHome(baseDir: string): Promise<string> {
  ...
  await fs.writeFile(
    ...
    "# Generated by Alien for Codex ACP sessions.\n",
    "utf8",
  );
  return codexHome;
}
