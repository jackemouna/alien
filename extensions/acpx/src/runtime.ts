...
const ALIEN_BRIDGE_EXECUTABLE = "alien";
const ALIEN_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_ALIEN_PREFIX = "openai-codex/";
...
function isAlienBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === ALIEN_BRIDGE_EXECUTABLE) {
    return parts[1] === ALIEN_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^alien(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === ALIEN_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_ALIEN_PREFIX)) {
    value = value.slice(CODEX_ACP_ALIEN_PREFIX.length);
  }
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isAlienBridgeCommand(command);
}
