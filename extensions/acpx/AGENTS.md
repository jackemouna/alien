...
The ACPX extension is a thin Alien wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `alien/acpx`, not in this extension.
...
Use this flow when Alien needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `alien/acpx` repo first.
2. In Alien, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Alien and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Alien ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Alien-specific glue, move it to `alien/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
