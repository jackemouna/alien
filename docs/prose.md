---
summary: "OpenProse: .prose workflows, slash commands, and state in Alien"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Alien it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
alien plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `alien plugins install ./path/to/local/open-prose-plugin`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Alien tools under the hood.
...
OpenProse programs map to Alien primitives:

| OpenProse concept         | Alien tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use Alien tool allowlists and approval gates to control side effects.
