...
read_when:
  ...
  - You encounter Task Flow or alien tasks flow in release notes or docs
...
- Browser availability and profile choice, for example `alien` for managed state or `user` when a signed-in Chrome session is required. See [Browser](/tools/browser).
...
`alien tasks flow cancel` sets a sticky cancel intent on the flow. Active tasks within the flow are cancelled, and no new steps are started. The cancel intent persists across restarts, so a cancelled flow stays cancelled even if the gateway restarts before all child tasks have terminated.
...
| Command                           | Description                                   |
| --------------------------------- | --------------------------------------------- |
| `alien tasks flow list`        | Shows tracked flows with status and sync mode |
| `alien tasks flow show <id>`   | Inspect one flow by flow id or lookup key     |
| `alien tasks flow cancel <id>` | Cancel a running flow and its active tasks    |

## How flows relate to tasks

Flows coordinate tasks, not replace them. A single flow may drive multiple background tasks over its lifetime. Use `alien tasks` to inspect individual task records and `alien tasks flow` to inspect the orchestrating flow.

## Related

- [Background Tasks](/automation/tasks) — the detached work ledger that flows coordinate
- [CLI: tasks](/cli/tasks) — CLI command reference for `alien tasks flow`
