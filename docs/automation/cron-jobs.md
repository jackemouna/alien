...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into Alien
...
<Steps>
  ...
  <Step title="Check your jobs">
    ```bash
    alien cron list
    alien cron show <job-id>
    ```
  </Step>
  <Step title="See run history">
    ```bash
    alien cron runs --id <job-id>
    ```
  </Step>
</Steps>
...
- Job definitions persist at `~/.alien/cron/jobs.json` so restarts do not lose schedules.
- Runtime execution state persists next to it in `~/.alien/cron/jobs-state.json`. If you track cron definitions in git, track `jobs.json` and gitignore `jobs-state.json`.
- After the split, older Alien versions can read `jobs.json` but may treat jobs as fresh because runtime fields now live in `jobs-state.json`.
- When `jobs.json` is edited while the Gateway is running or stopped, Alien compares the changed schedule fields with pending runtime slot metadata and clears stale `nextRunAtMs` values. Pure formatting or key-order-only rewrites preserve the pending slot.
...
- Isolated cron runs also guard against stale acknowledgement replies. If the first result is just an interim status update (`on it`, `pulling everything together`, and similar hints) and no descendant subagent run is still responsible for the final answer, Alien re-prompts once for the actual result before delivery.
...
This fires ~5–6 times per month instead of 0–1 times per month. Alien uses Croner's default OR behavior here. To require both conditions, use Croner's `+` day-of-week modifier (`0 9 15 * +1`) or schedule on one field and guard the other in your job's prompt or command.
...
<AccordionGroup>
  ...
  <Accordion title="Subagent and Discord delivery">
    When isolated cron runs orchestrate subagents, delivery also prefers the final descendant output over stale parent interim text. If descendants are still running, Alien suppresses that partial parent update instead of announcing it.

    For text-only Discord announce targets, Alien sends the canonical final assistant text once instead of replaying both streamed/intermediate text payloads and the final answer. Media and structured Discord payloads are still delivered as separate payloads so attachments and components are not dropped.
...
For isolated jobs, chat delivery is shared. If a chat route is available, the agent can use the `message` tool even when the job uses `--no-deliver`. If the agent sends to the configured/current target, Alien skips the fallback announce. Otherwise `announce`, `webhook`, and `none` only control what the runner does with the final reply after the agent turn.

When an agent creates an isolated reminder from an active chat, Alien stores the preserved live delivery target for the fallback announce route. Internal session keys may be lowercase; provider delivery targets are not reconstructed from those keys when current chat context is available.
...
Wire Gmail inbox triggers to Alien via Google PubSub.

<Note>
**Prerequisites:** `gcloud` CLI, `gog` (gogcli), Alien hooks enabled, Tailscale for the public HTTPS endpoint.
</Note>

### Wizard setup (recommended)

```bash
alien webhooks gmail setup --account alien@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `ALIEN_SKIP_GMAIL_WATCHER=1` to opt out.
...
alien cron show <jobId>

# Edit a job
alien cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
alien cron run <jobId>

# Run only if due
alien cron run <jobId> --due

# View run history
alien cron runs --id <jobId> --limit 50

# Delete a job
alien cron remove <jobId>

# Agent selection (multi-agent setups)
alien cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
alien cron edit <jobId> --clear-agent
```

<Note>
Model override note:

- `alien cron add|edit --model ...` changes the job's selected model.
...
If you hand-edit `jobs.json`, leave `jobs-state.json` out of source control. Alien uses that sidecar for pending slots, active markers, last-run metadata, and the schedule identity that tells the scheduler when an externally edited job needs a fresh `nextRunAtMs`.

Disable cron: `cron.enabled: false` or `ALIEN_SKIP_CRON=1`.
...
<AccordionGroup>
  <Accordion title="Cron not firing">
    - Check `cron.enabled` and `ALIEN_SKIP_CRON` env var.
    - Confirm the Gateway is running continuously.
    - For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
    - `reason: not-due` in run output means manual run was checked with `alien cron run <jobId> --due` and the job was not due yet.
  ...
  <Accordion title="Cron fired but no delivery">
    ...
    - If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`), Alien suppresses direct outbound delivery and also suppresses the fallback queued summary path, so nothing is posted back to chat.
  ...
  <Accordion title="Cron or heartbeat appears to prevent /new-style rollover">
    ...
    - For legacy rows created before those fields existed, Alien can recover `sessionStartedAt` from the transcript JSONL session header when the file is still available. Legacy idle rows without `lastInteractionAt` use that recovered start time as their idle baseline.
