---
summary: "Use StepFun models with Alien"
read_when:
  - You want StepFun models in Alien
  - You need StepFun setup guidance
title: "StepFun"
---

Alien includes a bundled StepFun provider plugin with two provider ids:
    ...
    **Best for:** general-purpose use via the standard StepFun endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        alien onboard --auth-choice stepfun-standard-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        alien onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        alien models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    **Best for:** Step Plan reasoning endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        alien onboard --auth-choice stepfun-plan-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        alien onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        alien models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - The provider is bundled with Alien, so there is no separate plugin install step.
    ...
    - Use `alien models list` and `alien models set <provider/model>` to inspect or switch models.
