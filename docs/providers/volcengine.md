...
read_when:
  - You want to use Volcano Engine or Doubao models with Alien
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    alien onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    alien models list --provider volcengine
    alien models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, Alien asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `alien onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Alien falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.alien/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Alien as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Alien setup.
  </Card>
</CardGroup>
