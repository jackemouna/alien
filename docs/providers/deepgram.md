...
Deepgram is a speech-to-text API. In Alien it is used for inbound
...
For batch transcription, Alien uploads the complete audio file to Deepgram
and injects the transcript into the reply pipeline (`{{Transcript}}` +
`[Audio]` block). For Voice Call streaming, Alien forwards live G.711
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Alien transcribes it
