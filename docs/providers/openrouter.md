---
summary: "Use OpenRouter's unified API to access many models in Alien"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Alien
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    alien onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    alien models set openrouter/<provider>/<model>
    ```
...
Alien submits text-to-video and image-to-video jobs to OpenRouter, polls
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), Alien also adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Alien
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Alien sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Alien uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Alien maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. Alien sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Alien keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, Alien forwards
