---
summary: "Use OpenCode Zen and Go catalogs with Alien"
read_when:
...
OpenCode exposes two hosted catalogs in Alien:
...
Both catalogs use the same OpenCode API key. Alien keeps the runtime provider ids
    ...
    **Best for:** the curated OpenCode multi-model proxy (Claude, GPT, Gemini).
    ...
    <Steps>
      <Step title="Run onboarding">
        ...
        ```bash
        alien onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Zen model as the default">
        ```bash
        alien config set agents.defaults.model.primary "opencode/claude-opus-4-6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        alien models list --provider opencode
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Go catalog">
    **Best for:** the OpenCode-hosted Kimi, GLM, and MiniMax lineup.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        alien onboard --auth-choice opencode-go
        ```

        Or pass the key directly:

        ```bash
        alien onboard --opencode-go-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Go model as the default">
        ```bash
        alien config set agents.defaults.model.primary "opencode-go/kimi-k2.6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        alien models list --provider opencode-go
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Alien keeps
