...
title: "Groq"
read_when:
  - You want to use Groq with Alien
  - You need the API key env var or CLI auth choice
  - You are configuring Whisper audio transcription on Groq
---

[Groq](https://groq.com) provides ultra-fast inference on open-weight models (Llama, Gemma, Kimi, Qwen, GPT OSS, and more) using custom LPU hardware. Alien includes a bundled Groq plugin that registers both an OpenAI-compatible chat provider and an audio media-understanding provider.
...
```bash Onboarding
alien onboard --auth-choice groq-api-key
```
...
```
  ...
  <Step title="Verify the catalog is reachable">
    ```bash
    alien models list --provider groq
    ```
  </Step>
</Steps>
...
Alien ships a manifest-backed Groq catalog with both reasoning and non-reasoning entries. Run `alien models list --provider groq` to see the bundled rows for your installed version, or check [console.groq.com/docs/models](https://console.groq.com/docs/models) for Groq's authoritative list.
...
<Tip>
  The catalog evolves with each Alien release. `alien models list --provider groq` shows the rows known to your installed version; cross-check with [console.groq.com/docs/models](https://console.groq.com/docs/models) for newly-added or deprecated models.
</Tip>

## Reasoning models

Alien maps its shared `/think` levels to Groq's model-specific `reasoning_effort` values:

- For `qwen/qwen3-32b`, disabled thinking sends `none` and enabled thinking sends `default`.
- For Groq GPT OSS reasoning models (`openai/gpt-oss-*`), Alien sends `low`, `medium`, or `high` based on `/think` level. Disabled thinking omits `reasoning_effort` because those models do not support a disabled value.
...
See [Thinking modes](/tools/thinking) for the shared `/think` levels and how Alien translates them per provider.
...
<AccordionGroup>
  <Accordion title="Environment availability for the daemon">
    ...
    <Warning>
      A key sitting only in `~/.profile` will not help a launchd or systemd daemon unless that environment is imported there too. Set the key in `~/.alien/.env` or via `env.shellEnv` to make it readable from the gateway process.
    </Warning>

  </Accordion>

  <Accordion title="Custom Groq model ids">
    Alien accepts any Groq model id at runtime. Use the exact id shown by Groq and prefix it with `groq/`. The bundled catalog covers the common cases; uncatalogued ids fall through to the default OpenAI-compatible template.
