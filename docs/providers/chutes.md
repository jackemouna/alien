...
title: "Chutes"
read_when:
  - You want to use Chutes with Alien
...
OpenAI-compatible API. Alien supports both browser OAuth and direct API-key
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ```bash
        alien onboard --auth-choice chutes
        ```
        Alien launches the browser flow locally, or shows a URL + redirect-paste
        flow on remote/headless hosts. OAuth tokens auto-refresh through Alien auth
        profiles.
      </Step>
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run the API key onboarding flow">
        ```bash
        alien onboard --auth-choice chutes-api-key
        ```
      </Step>
      <Step title="Verify the default model">
        After onboarding, the default model is set to
...
When Chutes auth is available, Alien queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, Alien falls
back to a bundled static catalog so onboarding and startup still work.

## Default aliases

Alien registers three convenience aliases for the bundled Chutes catalog:
