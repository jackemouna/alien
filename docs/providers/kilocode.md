---
summary: "Use Kilo Gateway's unified API to access many models in Alien"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Alien
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    alien onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    alien models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Alien treats `kilocode/kilo/auto` as the stable default ref, but does not
...
Alien dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Alien queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Alien keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Alien falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.alien/.env` or via `env.shellEnv`).
