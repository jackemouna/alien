...
read_when:
  - You want to use Vercel AI Gateway with Alien
...
<Tip>
Alien auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    alien onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    alien models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Alien accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Alien resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Alien Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.alien/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow trusted upstream model prefixes when Alien knows
