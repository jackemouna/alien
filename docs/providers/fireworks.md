...
title: "Fireworks"
read_when:
  - You want to use Fireworks with Alien
  - You need the Fireworks API key env var or default model id
  - You are debugging Kimi thinking-off behavior on Fireworks
---

[Fireworks](https://fireworks.ai) exposes open-weight and routed models through an OpenAI-compatible API. Alien includes a bundled Fireworks provider plugin that ships with two pre-cataloged Kimi models and accepts any Fireworks model or router id at runtime.
...
```bash Onboarding
alien onboard --auth-choice fireworks-api-key
```
...
```
  ...
  <Step title="Verify the model is available">
    ```bash
    alien models list --provider fireworks
    ```

    The list should include `Kimi K2.6` and `Kimi K2.5 Turbo (Fire Pass)`. If `FIREWORKS_API_KEY` is unresolved, `alien models status --json` reports the missing credential under `auth.unusableProfiles`.
...
<Note>
  Alien pins all Fireworks Kimi models to `thinking: off` because Fireworks rejects Kimi thinking parameters in production. Routing the same model through [Moonshot](/providers/moonshot) directly preserves Kimi reasoning output. See [thinking modes](/tools/thinking) for switching between providers.
</Note>

## Custom Fireworks model ids

Alien accepts any Fireworks model or router id at runtime. Use the exact id shown by Fireworks and prefix it with `fireworks/`. Dynamic resolution clones the Fire Pass template (text + image input, OpenAI-compatible API, default cost zero) and disables thinking automatically when the id matches the Kimi pattern.
...
<AccordionGroup>
  <Accordion title="How model id prefixing works">
    Every Fireworks model ref in Alien starts with `fireworks/` followed by the exact id or router path from the Fireworks platform. For example:

    - Router model: `fireworks/accounts/fireworks/routers/kimi-k2p5-turbo`
    - Direct model: `fireworks/accounts/fireworks/models/<model-name>`

    Alien strips the `fireworks/` prefix when constructing the API request and sends the remaining path to the Fireworks endpoint as the OpenAI-compatible `model` field.
  ...
  <Accordion title="Environment availability for the daemon">
    ...
    On macOS, `alien gateway install` already wires `~/.alien/.env` into the LaunchAgent environment file. Re-run install (or `alien doctor --fix`) after rotating the key.
