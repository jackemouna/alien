---
summary: "Use Venice AI privacy-focused models in Alien"
read_when:
  - You want privacy-focused inference in Alien
...
<Steps>
  ...
  <Step title="Configure Alien">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        alien onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    alien agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Alien shows all available Venice models. Pick based on your needs:
...
```bash
alien models set venice/kimi-k2-5
alien models set venice/claude-opus-4-6
```

List all available models:

```bash
alien models list --all --provider venice
```

You can also run `alien configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Alien fills the required DeepSeek V4
...
Alien keeps that provider-specific replay fix separate from the native
...
Alien ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
alien agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
alien agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
alien agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
alien agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
alien agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `alien models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
