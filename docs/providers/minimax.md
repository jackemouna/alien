---
summary: "Use MiniMax models in Alien"
read_when:
  - You want MiniMax models in Alien
  - You need MiniMax setup guidance
title: "MiniMax"
---

Alien's MiniMax provider defaults to **MiniMax M2.7**.
...
<Tabs>
    ...
    <Tabs>
      <Tab title="International">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            alien onboard --auth-choice minimax-global-oauth
            ```
          ...
          <Step title="Verify the model is available">
            ```bash
            alien models list --provider minimax-portal
            ```
          </Step>
        </Steps>
      </Tab>
      <Tab title="China">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            alien onboard --auth-choice minimax-cn-oauth
            ```
  ...
  <Tab title="API key">
    **Best for:** hosted MiniMax with Anthropic-compatible API.
    ...
    <Tabs>
      <Tab title="International">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            alien onboard --auth-choice minimax-global-api
            ```
          ...
          <Step title="Verify the model is available">
            ```bash
            alien models list --provider minimax
            ```
          </Step>
        </Steps>
      </Tab>
      <Tab title="China">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            alien onboard --auth-choice minimax-cn-api
            ```
    ...
    <Warning>
    On the Anthropic-compatible streaming path, Alien disables MiniMax thinking by default unless you explicitly set `thinking` yourself. MiniMax's streaming endpoint emits `reasoning_content` in OpenAI-style delta chunks instead of native Anthropic thinking blocks, which can leak internal reasoning into visible output if left enabled implicitly.
    </Warning>
...
entries, Alien materializes `MiniMax-M2.7` and
...
- If no TTS host is configured, Alien reuses the configured
...
- MiniMax T2A accepts fractional `speed` and `vol`, but `pitch` is sent as an
  integer; Alien truncates fractional `pitch` values before the API request.
...
<AccordionGroup>
  ...
  <Accordion title="Thinking defaults">
    On `api: "anthropic-messages"`, Alien injects `thinking: { type: "disabled" }` unless thinking is already explicitly set in params/config.
  ...
  <Accordion title="Coding Plan usage details">
    ...
    - Alien normalizes MiniMax coding-plan usage to the same `% left` display used by other providers. MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota, not consumed quota, so Alien inverts them. Count-based fields win when present.
    - When the API returns `model_remains`, Alien prefers the chat-model entry, derives the window label from `start_time` / `end_time` when needed, and includes the selected model name in the plan label so coding-plan windows are easier to distinguish.
...
- Use `alien models list` to confirm the current provider id, then switch with `alien models set minimax/MiniMax-M2.7` or `alien models set minimax-portal/MiniMax-M2.7`
...
<AccordionGroup>
  <Accordion title='"Unknown model: minimax/MiniMax-M2.7"'>
    ...
    - Running `alien configure` and selecting a **MiniMax** auth option, or
