---
summary: "Use Synthetic's Anthropic-compatible API in Alien"
read_when:
...
Alien registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    alien onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Alien's Anthropic client appends `/v1` to the base URL automatically, so use
...
`alien models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Alien appends `/v1` automatically.
