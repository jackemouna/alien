...
read_when:
  - You want to use Tencent Hy3 preview with Alien
  - You need the TokenHub API key setup
---

Tencent Cloud ships as a bundled provider plugin in Alien. It gives access to Tencent Hy3 preview through the TokenHub endpoint (`tencent-tokenhub`) using an OpenAI-compatible API.
...
```bash Onboarding
alien onboard --auth-choice tokenhub-api-key
```
...
```
  ...
  <Step title="Verify the model">
    ```bash
    alien models list --provider tencent-tokenhub
    ```
  </Step>
</Steps>
...
<Tip>
  The model id is `hy3-preview`. Do not confuse it with Tencent's `HY-3D-*` models, which are 3D generation APIs and are not the Alien chat model configured by this provider.
</Tip>
...
<AccordionGroup>
  <Accordion title="Endpoint override">
    Alien defaults to Tencent Cloud's `https://tokenhub.tencentmaas.com/v1` endpoint. Tencent also documents an international TokenHub endpoint:

    ```bash
    alien config set models.providers.tencent-tokenhub.baseUrl "https://tokenhub-intl.tencentmaas.com/v1"
    ```
  ...
  <Accordion title="Environment availability for the daemon">
    If the Gateway runs as a managed service (launchd, systemd, Docker), `TOKENHUB_API_KEY` must be visible to that process. Set it in `~/.alien/.env` or via `env.shellEnv` so launchd, systemd, or Docker exec environments can read it.
