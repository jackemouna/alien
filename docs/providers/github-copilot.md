---
summary: "Sign in to GitHub Copilot from Alien using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `alien models auth login-github-copilot` flow
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. Alien can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Alien

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Alien runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        alien models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        alien models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Alien talks to
...
```bash
# Skip confirmation
alien models auth login-github-copilot --yes

# Login and set the default model in one step
alien models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

If you already have a GitHub OAuth access token for Copilot, import it during
headless setup with `alien onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Alien
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Alien sends Copilot IDE-style request headers on Copilot transports,
  ...
  <Accordion title="Environment variable resolution order">
    Alien resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Alien uses the highest-priority one.
    The device-login flow (`alien models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Alien runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Alien can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Alien discovers available
...
1. Alien resolves your GitHub token (from env vars or auth profile).
...
available, Alien skips Copilot and tries the next provider.
