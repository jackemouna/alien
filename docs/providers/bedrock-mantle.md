---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with Alien"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with Alien
  - You need the Mantle OpenAI-compatible endpoint for GPT-OSS, Qwen, Kimi, or GLM
title: "Amazon Bedrock Mantle"
---

Alien includes a bundled **Amazon Bedrock Mantle** provider that connects to
    ...
    **Best for:** using AWS SDK-compatible credentials (shared config, SSO, web identity, instance or task roles).
    ...
    <Steps>
      ...
      <Step title="Verify models are discovered">
        ...
        Alien generates a Mantle bearer token from the credential chain automatically.
      </Step>
    </Steps>

    <Tip>
    When `AWS_BEARER_TOKEN_BEDROCK` is not set, Alien mints the bearer token for you from the AWS default credential chain, including shared credentials/config profiles, SSO, web identity, and instance or task roles.
    </Tip>
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, Alien uses it directly. Otherwise,
Alien attempts to generate a Mantle bearer token from the AWS default
...
<AccordionGroup>
  <Accordion title="Reasoning support">
    ...
    `thinking`, `reasoner`, or `gpt-oss-120b`. Alien sets `reasoning: true`
  ...
  <Accordion title="Endpoint unavailability">
    ...
    silently skipped. Alien does not error; other configured providers
  ...
  <Accordion title="Claude Opus 4.7 via the Anthropic Messages route">
    ...
    When you pin an Anthropic Messages model on the Mantle provider, Alien uses the `anthropic-messages` API surface instead of `openai-completions` for that model. Auth still comes from `AWS_BEARER_TOKEN_BEDROCK` (or the minted IAM bearer token).
