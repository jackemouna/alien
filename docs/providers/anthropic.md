---
summary: "Use Anthropic Claude via API keys or Claude CLI in Alien"
read_when:
  - You want to use Anthropic models in Alien
title: "Anthropic"
---

Anthropic builds the **Claude** model family. Alien supports two auth routes:
...
Alien treats Claude CLI reuse and `claude -p` usage as sanctioned unless
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        alien onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        alien models list --provider anthropic
        ```
      </Step>
    </Steps>
    ...
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        Alien detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        alien models list --provider anthropic
    ...
    <Tip>
    If you want the clearest billing path, use an Anthropic API key instead. Alien also supports subscription-style options from [OpenAI Codex](/providers/openai), [Qwen Cloud](/providers/qwen), [MiniMax](/providers/minimax), and [Z.AI / GLM](/providers/glm).
    </Tip>
...
Claude 4.6 models default to `adaptive` thinking in Alien when no explicit thinking level is set.
...
Alien supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    Alien's shared `/fast` toggle supports direct Anthropic traffic (API-key and OAuth to `api.anthropic.com`).
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Alien
    ...
    When an image or PDF is attached to a conversation, Alien automatically
  ...
  <Accordion title="1M context window (beta)">
    ...
    Alien maps this to `anthropic-beta: context-1m-2025-08-07` on requests.
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. Legacy token auth (`sk-ant-oat-*`) is rejected for 1M context requests — Alien logs a warning and falls back to the standard context window.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent** — new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `alien models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `alien models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `alien models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
