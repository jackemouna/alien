...
Alien sources most Go catalog rows from the bundled pi model registry and
supplements current upstream rows while the registry catches up. Run
`alien models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    Alien handles per-model routing automatically when the model ref uses
