---
summary: "Run Alien with LM Studio"
read_when:
  - You want to run Alien with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive Alien setup.
...
```bash
alien models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). Alien
...
For unauthenticated LM Studio servers, omit the key; Alien stores a local non-secret marker.
...
`usage` object, Alien recovers token counts from llama.cpp-style
...
options, Alien exposes the matching OpenAI-compatible `reasoning_effort`
...
on `/v1/chat/completions`; Alien normalizes that binary discovery shape to
...
LM Studio supports just-in-time (JIT) model loading, where models are loaded on first request. Alien preloads models through LM Studio's native load endpoint by default, which helps when JIT is disabled. To let LM Studio's JIT, idle TTL, and auto-evict behavior own model lifecycle, disable Alien's preload step:
