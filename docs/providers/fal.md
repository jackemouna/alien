---
summary: "fal image and video generation setup in Alien"
title: "Fal"
read_when:
  - You want to use fal image generation in Alien
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate or video_generate
---

Alien ships a bundled `fal` provider for hosted image and video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    alien onboard --auth-choice fal-api-key
    ```
  </Step>
...
explicit transparent-background control in Alien, so `background:
...
<Tip>
Use `alien models list --provider fal` to see the full list of available fal
