---
summary: "Use OpenAI via API keys or Codex subscription in Alien"
read_when:
  - You want to use OpenAI models in Alien
...
ChatGPT-plan coding agent through OpenAI's Codex clients. Alien keeps those
surfaces separate so config stays predictable.

Alien supports three OpenAI-family routes. Most ChatGPT/Codex subscribers
...
- **Codex subscription through PI** - ChatGPT/Codex sign-in with the normal Alien PI runner (`openai-codex/*` models)

OpenAI explicitly supports subscription OAuth usage in external tools and workflows like Alien.
...
| `openai-codex`                     | Provider prefix   | OpenAI Codex OAuth/subscription route through the normal Alien PI runner.                      |
| `codex` plugin                     | Plugin            | Bundled Alien plugin that provides native Codex app-server runtime and `/codex` chat controls. |
...
native `/codex` chat controls available. `alien doctor` warns about that
...
enable the bundled Codex app-server plugin. Alien enables that plugin only
...
through PI, `alien doctor` warns and leaves the route unchanged.
</Note>

## Alien feature coverage

| OpenAI capability         | Alien surface                                           | Status                                                 |
| ------------------------- | ---------------------------------------------------------- | ------------------------------------------------------ |
...
Alien can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. Alien forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        alien onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        alien onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        alien models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    <Warning>
    Alien does **not** expose `openai/gpt-5.3-codex-spark`. Live OpenAI API requests reject that model, and the current Codex catalog does not expose it either.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex app-server execution instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        alien onboard --auth-choice openai-codex
        ```
        ...
        ```bash
        alien models auth login --provider openai-codex --device-code
        ```
      </Step>
      <Step title="Use the native Codex runtime">
        ```bash
        alien config set plugins.entries.codex '{"enabled":true}' --strict-json --merge
        alien config set agents.defaults.model.primary openai/gpt-5.5
        alien config set agents.defaults.agentRuntime '{"id":"codex"}' --strict-json
        ```
      </Step>
      <Step title="Verify Codex auth is available">
        ```bash
        alien models list --provider openai-codex
        ```
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — Alien manages the resulting credentials in its own agent auth store.
    </Note>
    ...
    ```bash
    alien models status
    alien models auth list --provider openai-codex
    alien config get agents.defaults.model --json
    alien config get agents.defaults.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    alien models status --agent <id>
    alien models auth list --agent <id> --provider openai-codex
    ```
    ...
    ```bash
    alien models set openai-codex/gpt-5.5
    ...
    ```bash
    alien models auth login --provider openai-codex
    alien models status --probe --probe-provider openai-codex
    ```
    ...
    The default PI harness appears as `Runtime: Alien Pi Default`. When the
    ...
    selected, `alien doctor` warns that the model still resolves through PI.
    ...
    Alien treats model metadata and the runtime context cap as separate values.
    ...
    Alien uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `openai-codex/gpt-5.5` row while
    the account is authenticated, Alien synthesizes that OAuth model row so
...
`agentRuntime.id: "codex"`, but its auth is still account-based. Alien
selects auth in this order:

1. An explicit Alien `openai-codex` auth profile bound to the agent.
...
profile is selected, Alien also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
still accepted. Alien also protects the public OpenAI and
...
`alien infer image edit` when starting from an input file.
...
`openai-codex` OAuth profile is configured, Alien resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; Alien keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for Alien on macOS" size=3840x2160 count=1
```
...
Alien adds a shared GPT-5 prompt contribution for GPT-5-family runs across providers. It applies by model id, so `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.
...
The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification. Channel-specific reply and silent-message behavior stays in the shared Alien system prompt and outbound delivery policy. The GPT-5 guidance is always enabled for matching models. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    alien config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after Alien's generated fields, so use it for OpenAI-compatible endpoints that require additional keys such as `lang`. Prototype keys are ignored.
  ...
  <Accordion title="Speech-to-text">
    ...
    Alien's media-understanding transcription surface.
...
generation by overriding the base URL. On the image-generation path, Alien
...
Alien recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, Alien:
...
Alien 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in Alien
...
Alien. If an Azure request fails with a validation error, check the
...
Alien's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    Alien uses WebSocket-first with SSE fallback (`"auto"`) for both `openai/*` and `openai-codex/*`.
  ...
  <Accordion title="WebSocket warm-up">
    Alien enables WebSocket warm-up by default for `openai/*` and `openai-codex/*` to reduce first-turn latency.
  ...
  <Accordion title="Fast mode">
    Alien exposes a shared fast-mode toggle for `openai/*` and `openai-codex/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, Alien maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in Alien:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, Alien leaves `service_tier` untouched.
    </Warning>
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, Alien can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    Alien treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
