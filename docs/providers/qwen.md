---
summary: "Use Qwen Cloud via Alien's bundled qwen provider"
read_when:
  - You want to use Qwen with Alien
...
See [Issue #49557](https://github.com/alien/alien/issues/49557) for
background.

</Warning>

Alien now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        alien onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        alien onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        alien models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        alien onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        alien onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Alien currently ships this bundled Qwen catalog. The configured catalog is
...
For reasoning-enabled Qwen Cloud models, the bundled provider maps Alien
...
<AccordionGroup>
  ...
  <Accordion title="Qwen 3.6 Plus availability">
    ...
    Alien's bundled Qwen catalog does not advertise `qwen3.6-plus` on Coding
  ...
  <Accordion title="Video generation details">
    For video generation, Alien maps the configured Qwen region to the matching
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. Alien keys that off endpoint
  ...
  <Accordion title="Environment and daemon setup">
    ...
    available to that process (for example, in `~/.alien/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
