---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in Alien"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in Alien
title: "DeepInfra"
---
...
```bash
alien onboard --deepinfra-api-key <key>
```
...
## Supported Alien surfaces

The bundled plugin registers all DeepInfra surfaces that match current
Alien provider contracts:

| Surface                  | Default model                      | Alien config/tool                                     |
| ------------------------ | ---------------------------------- | -------------------------------------------------------- |
...
native model types. Alien does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

Alien dynamically discovers available DeepInfra models at startup. Use
