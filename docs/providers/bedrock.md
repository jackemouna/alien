---
summary: "Use Amazon Bedrock (Converse API) models with Alien"
read_when:
  - You want to use Amazon Bedrock models with Alien
  - You need AWS credential/region setup for model calls
title: "Amazon Bedrock"
---

Alien can use **Amazon Bedrock** models via pi-ai's **Bedrock Converse**
...
<Tabs>
  <Tab title="Access keys / env vars">
    **Best for:** developer machines, CI, or hosts where you manage AWS credentials directly.
    ...
    <Tip>
    With env-marker auth (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, or `AWS_BEARER_TOKEN_BEDROCK`), Alien auto-enables the implicit Bedrock provider for model discovery without extra config.
    </Tip>
    ...
    **Best for:** EC2 instances with an IAM role attached, using the instance metadata service for authentication.
    ...
    <Steps>
      <Step title="Enable discovery explicitly">
        When using IMDS, Alien cannot detect AWS auth from env markers alone, so you must opt in:

        ```bash
        alien config set plugins.entries.amazon-bedrock.config.discovery.enabled true
        alien config set plugins.entries.amazon-bedrock.config.discovery.region us-east-1
        ```
      </Step>
      <Step title="Optionally add an env marker for auto mode">
        If you also want the env-marker auto-detection path to work (for example, for `alien status` surfaces):
...
Alien can automatically discover Bedrock models that support **streaming**
...
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is `true`,
  Alien will try discovery even when no AWS env marker is present.
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is unset,
  Alien only auto-adds the
...
<Note>
For explicit `models.providers["amazon-bedrock"]` entries, Alien can still resolve Bedrock env-marker auth early from AWS env markers such as `AWS_BEARER_TOKEN_BEDROCK` without forcing full runtime auth loading. The actual model-call auth path still uses the AWS SDK default chain.
</Note>
...
<AccordionGroup>
  <Accordion title="Discovery config options">
    ...
    | ------ | ------- | ----------- |
    | `enabled` | auto | In auto mode, Alien only enables the implicit Bedrock provider when it sees a supported AWS env marker. Set `true` to force discovery. |
...
the instance profile, and enables Alien discovery on the EC2 host.
...
<AccordionGroup>
  <Accordion title="Inference profiles">
    Alien discovers **regional and global inference profiles** alongside
    ...
    foundation models in `alien models list`.

  </Accordion>

  <Accordion title="Claude Opus 4.7 temperature">
    Bedrock rejects the `temperature` parameter for Claude Opus 4.7. Alien
  ...
  <Accordion title="Notes and caveats">
    ...
    - Alien surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
