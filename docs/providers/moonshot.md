    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        alien onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        alien onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        alien models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        ALIEN_CONFIG_PATH=/tmp/alien-kimi/alien.json \
        ALIEN_STATE_DIR=/tmp/alien-kimi \
        alien agent --local \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        alien onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        alien models list --provider kimi
        ```
      </Step>
    </Steps>
...
Alien also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    Alien also maps runtime `/think` levels for Moonshot:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Alien normalizes incompatible `tool_choice` values to `auto` for compatibility.
    </Warning>
    ...
    default strategy. Alien only forwards `thinking.keep` for
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves tool_call ids shaped like `functions.<name>:<index>`. Alien preserves them unchanged so multi-turn tool calls keep working.
