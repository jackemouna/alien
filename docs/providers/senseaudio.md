...
SenseAudio can transcribe inbound audio and voice-note attachments through Alien's shared `tools.media.audio` pipeline. Alien posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Alien uploads the
...
<Note>
SenseAudio is batch STT only in Alien. Voice Call realtime transcription
