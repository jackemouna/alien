---
summary: "Run Alien with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run Alien against a local vLLM server
  - You want OpenAI-compatible /v1 endpoints with your own models
title: "vLLM"
---

vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. Alien connects to vLLM using the `openai-completions` API.

Alien can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth) and you do not define an explicit `models.providers.vllm` entry.

Alien treats `vllm` as a local OpenAI-compatible provider that supports
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    alien models list --provider vllm
    ```
  </Step>
</Steps>
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, Alien queries:
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden Alien attribution headers | Not injected on custom base URLs |
  ...
  <Accordion title="Qwen thinking controls">
    ...
    server expects Qwen chat-template kwargs. Alien maps `/think off` to:
  ...
  <Accordion title="Nemotron 3 thinking controls">
    ...
    returned as hidden reasoning or visible answer text. When an Alien session
  ...
  <Accordion title="Qwen tool calls appear as text">
    ...
    - vLLM returns an empty `tool_calls` array when Alien sends
      `tool_choice: "auto"`
    ...
    ```bash
    alien config set agents.defaults.models '{"vllm/Qwen-Qwen2.5-Coder-32B-Instruct":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Auth errors on requests">
    ...
    <Tip>
    If your vLLM server does not enforce auth, any non-empty value for `VLLM_API_KEY` works as an opt-in signal for Alien.
    </Tip>

  </Accordion>

  <Accordion title="No models discovered">
    Auto-discovery requires `VLLM_API_KEY` to be set **and** no explicit `models.providers.vllm` config entry. If you have defined the provider manually, Alien skips discovery and uses only your declared models.
  </Accordion>
  ...
  <Accordion title="Tools render as raw text">
    ...
    - confirm the exact model id with `alien models list --provider vllm`
