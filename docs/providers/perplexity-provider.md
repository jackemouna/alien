...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    alien config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
...
<Tabs>
  <Tab title="Native Perplexity API (pplx-)">
    When your key starts with `pplx-`, Alien uses the native Perplexity Search
  ...
  <Tab title="OpenRouter / Sonar (sk-or-)">
    When your key starts with `sk-or-`, Alien routes through OpenRouter using
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    Alien will detect the prefix and switch to the Sonar transport
    automatically.
