...
read_when:
  - You want to use Cloudflare AI Gateway with Alien
  - You need the account ID, gateway ID, or API key env var
---

Cloudflare AI Gateway sits in front of provider APIs and lets you add analytics, caching, and controls. For Anthropic, Alien uses the Anthropic Messages API through your Gateway endpoint.
...
When thinking is enabled for Anthropic Messages models, Alien strips trailing
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    alien onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your Alien config:
  ...
  <Step title="Verify the model is available">
    ```bash
    alien models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key sitting only in `~/.profile` will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.alien/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
