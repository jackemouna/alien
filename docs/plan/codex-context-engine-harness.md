...
summary: "Specification for making the bundled Codex app-server harness honor Alien context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Alien context-engine
...
Alien-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Alien-controlled code. The external
boundary is the Codex app-server protocol itself: Alien can control what it
...
For Codex harness turns, Alien should preserve this lifecycle:

1. Read the mirrored Alien session transcript.
...
8. Mirror the Codex result back into the Alien transcript.
...
11. Preserve Codex native compaction signals and Alien compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Alien should
not try to mutate the app-server's internal history except through supported
protocol calls.

Alien's transcript mirror remains the source for Alien features:
...
The context-engine interface returns Alien `AgentMessage[]`, not a Codex
...
- Label it clearly as Alien assembled context.
...
```text
Alien assembled context for this turn:
...
inside Alien and preserves context-engine semantics.
...
1. Alien context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Alien compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Alien compaction should prefer the context engine's `compact()` result for
the Alien transcript mirror and plugin state.
...
the Alien session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Alien session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Alien compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Alien core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Alien/plugin state.
