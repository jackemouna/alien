---
summary: "Get Alien installed and run your first chat in minutes."
read_when:
...
Install Alien, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.alien/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.alien/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Alien as a service account or want custom paths:

- `ALIEN_HOME` — home directory for internal path resolution
- `ALIEN_STATE_DIR` — override the state directory
- `ALIEN_CONFIG_PATH` — override the config file path
