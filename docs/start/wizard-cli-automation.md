---
summary: "Scripted onboarding and agent setup for the Alien CLI"
read_when:
...
Use `--non-interactive` to automate `alien onboard`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    Alien marks common vision model IDs as image-capable automatically. Add `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
Anthropic setup-token remains available as a supported onboarding token path, but Alien now prefers Claude CLI reuse when available.
For production, prefer an Anthropic API key.

## Add another agent

Use `alien agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.alien/workspace-<agentId>`.
