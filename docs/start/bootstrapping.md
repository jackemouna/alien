...
On the first agent run, Alien bootstraps the workspace (default
`~/.alien/workspace`):
...
For embedded/local model runs, Alien keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `alien onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.alien/workspace`).
</Note>
