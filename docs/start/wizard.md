...
CLI onboarding is the **recommended** way to set up Alien on macOS,
...
`alien dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`alien configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.alien/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `alien doctor` first.
</Note>
...
- CLI command reference: [`alien onboard`](/cli/onboard)
