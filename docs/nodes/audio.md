...
- **Media understanding (audio)**: If audio understanding is enabled (or auto-detected), Alien:
...
Alien auto-detects in this order and stops at the first working option:
...
- CLI `args` should use `{{MediaPath}}` for the local audio file path. Run `alien doctor --fix` to migrate deprecated `{input}` placeholders from older `audio.transcription.command` configs.
...
If no proxy env vars are set, direct egress is used. If proxy config is malformed, Alien logs a warning and falls back to direct fetch.

## Mention detection in groups

When `requireMention: true` is set for a group chat, Alien now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, Alien performs a "preflight" transcription.
...
- For `parakeet-mlx`, if you pass `--output-dir`, Alien reads `<output-dir>/<media-basename>.txt` when `--output-format` is `txt` (or omitted); non-`txt` output formats fall back to stdout parsing.
