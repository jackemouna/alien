...
```bash
alien nodes describe --node <idOrNameOrIp>
alien nodes canvas snapshot --node <idOrNameOrIp>
alien logs --follow
```
...
alien nodes status
alien approvals get --node <idOrNameOrIp>
alien approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`alien approvals get --node ...`), not in the gateway pairing record.
