...
Alien supports **camera capture** for agent workflows:
...
```bash
alien nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
alien nodes camera snap --node <id> --facing front
alien nodes camera clip --node <id> --duration 3000
alien nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`alien.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `alien` CLI to invoke camera commands on the macOS node.

Examples:

```bash
alien nodes camera list --node <id>            # list camera ids
alien nodes camera snap --node <id>            # prints MEDIA:<path>
alien nodes camera snap --node <id> --max-width 1280
alien nodes camera snap --node <id> --delay-ms 2000
alien nodes camera snap --node <id> --device-id <id>
alien nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
alien nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
alien nodes camera clip --node <id> --device-id <id>
alien nodes camera clip --node <id> --no-audio
```

Notes:

- `alien nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
alien nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
