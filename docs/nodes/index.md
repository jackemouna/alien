...
`alien nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`alien node run` or the
...
`alien devices list` before approving.
...
- `node.pair.*` (CLI: `alien nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `alien nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.alien/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, Alien also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If Alien cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
alien node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export ALIEN_GATEWAY_TOKEN="<gateway-token>"
alien node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `alien node run` supports token or password auth.
- Env vars are preferred: `ALIEN_GATEWAY_TOKEN` / `ALIEN_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `ALIEN_GATEWAY_*` env vars.

### Start a node host (service)

```bash
alien node install --host <gateway-host> --port 18789 --display-name "Build Node"
alien node start
alien node restart
```
...
If the node retries with changed auth details, re-run `alien devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `alien node run` / `alien node install` (persists in `~/.alien/node.json` on the node).
- `alien nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
alien approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
alien approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.alien/exec-approvals.json`.
...
```bash
alien config set tools.exec.host node
alien config set tools.exec.security allowlist
alien config set tools.exec.node "<id-or-name>"
```
...
```bash
alien nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
alien nodes canvas snapshot --node <idOrNameOrIp> --format png
alien nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
alien nodes canvas present --node <idOrNameOrIp> --target https://example.com
alien nodes canvas hide --node <idOrNameOrIp>
alien nodes canvas navigate https://example.com --node <idOrNameOrIp>
alien nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
alien nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
alien nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
alien nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
alien nodes camera list --node <idOrNameOrIp>
alien nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
alien nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
alien nodes camera clip --node <idOrNameOrIp> --duration 10s
alien nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
alien nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
alien nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
alien nodes location get --node <idOrNameOrIp>
alien nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
alien nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from Alien"}'
```
...
```bash
alien nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
alien nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
alien nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
alien nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
alien nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.alien/exec-approvals.json`).
...
```bash
alien config set tools.exec.node "node-id-or-name"
```
...
```bash
alien config unset tools.exec.node
alien config unset agents.list[0].tools.exec.node
```
...
Alien can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
alien node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.alien/node.json`.
- Exec approvals are enforced locally via `~/.alien/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `ALIEN_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `ALIEN_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `alien nodes …` works against this Mac).
