...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/alien`)
...
If the page fails to load, start the Gateway first: `alien gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `alien devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. Alien keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `alien devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.alien/alien.json` (`config.get`, `config.set`).
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop alien`, `please stop`) to abort out-of-band.
...
| `push/vapid-keys.json` (under the Alien state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `ALIEN_VAPID_PUBLIC_KEY`
- `ALIEN_VAPID_PRIVATE_KEY`
- `ALIEN_VAPID_SUBJECT` (defaults to `mailto:alien@localhost`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    alien gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, Alien **blocks** Control UI connections without device identity.
...
- `GET /__alien__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
```bash
ALIEN_CONTROL_UI_BASE_PATH=/alien/ pnpm ui:build
```
