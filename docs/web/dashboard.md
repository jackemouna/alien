...
- Re-open anytime: `alien dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `alien dashboard` still prints the
  clean URL and tells you to use the token from `ALIEN_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `ALIEN_GATEWAY_TOKEN`); `alien dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `alien dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `alien dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `ALIEN_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `alien status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `alien config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `ALIEN_GATEWAY_TOKEN` in this shell, then rerun `alien dashboard`
  - No shared secret configured: `alien doctor --generate-gateway-token`
