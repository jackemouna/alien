---
summary: "Architecture of Alien's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in Alien
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

Alien integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Alien uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Alien directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Alien's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAlienCodingTools()
...
├── alien-tools.ts              # Alien-specific tools
...
Image injection is prompt-local: Alien loads image refs from the current prompt and
...
2. **Custom Replacements**: Alien replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Alien Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Alien's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System prompt construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Alien CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Alien wraps this with `guardSessionManager()` for tool result safety.
...
Alien maintains an auth profile store with multiple API keys per provider:
...
Alien loads custom pi extensions for specialized behavior:
...
Alien also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Alien Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Alien tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.alien/agents/<agentId>/sessions/` (or `$ALIEN_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `ALIEN_LIVE_TEST=1`)
