...
- repo: `alien/docs`
- local checkout: `~/Projects/alien-docs`

## Source of truth

- English docs are authored in `alien/alien`.
...
1. Edit English docs in `alien/alien`.
2. Push to `main`.
3. `alien/alien/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `alien/docs`.
...
5. `alien/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `alien/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `alien/docs` instead.
...
- Sync metadata is written to `.alien-sync/source.json` in the publish repo.
- Source repo secret: `ALIEN_DOCS_SYNC_TOKEN`
- Publish repo secret: `ALIEN_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `alien/docs` first.
