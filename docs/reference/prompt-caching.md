...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. Alien normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- With Anthropic API-key auth profiles, Alien seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so Alien can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. Alien does not need to inject block-level cache markers.
- Alien uses `prompt_cache_key` to keep cache routing stable across turns and uses `prompt_cache_retention: "24h"` only when `cacheRetention: "long"` is selected on direct OpenAI hosts.
...
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). Alien maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, Alien injects Anthropic
...
handles provider-side prompt caching automatically. Alien does not inject
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, Alien
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; Alien maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, Alien automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  Alien manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  Alien maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, Alien derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean Alien is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

Alien splits the system prompt into a **stable prefix** and a **volatile
...
## Alien cache-stability guards

Alien also keeps several cache-sensitive payload shapes deterministic before
...
Alien exposes dedicated cache-trace diagnostics for embedded agent runs.
...
Alien keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.alien/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ALIEN_STATE_DIR/logs/cache-trace.jsonl`
...
- `ALIEN_CACHE_TRACE=1` enables cache tracing.
- `ALIEN_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ALIEN_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ALIEN_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ALIEN_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. Alien's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
