...
This page lists every configuration knob for Alien memory search. For conceptual overviews, see:
...
All memory search settings live under `agents.defaults.memorySearch` in `alien.json` unless noted otherwise.
...
When `provider` is not set, Alien selects the first available:
...
`memorySearch.provider` can point at a custom `models.providers.<id>` entry. Alien resolves that provider's `api` owner for the embedding adapter while preserving the custom provider id for endpoint, auth, and model-prefix handling. This lets multi-GPU or multi-host setups dedicate memory embeddings to a specific local endpoint:
...
<AccordionGroup>
  ...
  <Accordion title="Bedrock">
    ...
    Bedrock uses the AWS SDK default credential chain — no API keys needed. If Alien runs on EC2 with a Bedrock-enabled instance role, just set the provider and model:
  ...
  <Accordion title="Local (GGUF + node-llama-cpp)">
    ...
    ```bash
    alien memory status --deep --agent main
    alien memory index --force --agent main
    ```
...
When sqlite-vec is unavailable, Alien falls back to in-process cosine similarity automatically.
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.alien/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
`searchMode: "search"` is lexical/BM25-only. Alien does not run semantic vector readiness probes or QMD embedding maintenance for that mode, including during `memory status --deep`; `vsearch` and `query` continue to require QMD vector readiness and embeddings.

Alien prefers current QMD collection and MCP query shapes, but keeps older QMD releases working by trying compatible collection pattern flags and older MCP tool names when needed. When QMD advertises support for multiple collection filters, same-source collections are searched with one QMD process; older QMD builds keep the per-collection compatibility path. Same-source means durable memory collections are grouped together, while session transcript collections remain a separate group so source diversification still has both inputs.

<Note>
QMD model overrides stay on the QMD side, not Alien config. If you need to override QMD's models globally, set environment variables such as `QMD_EMBED_MODEL`, `QMD_RERANK_MODEL`, and `QMD_GENERATE_MODEL` in the gateway runtime environment.
</Note>
