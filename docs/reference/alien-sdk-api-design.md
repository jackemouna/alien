---
summary: "Reference design for the public Alien App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "Alien App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public Alien app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level Alien App SDK wrapper
---

This page is the detailed API reference design for the public
[Alien App SDK](/concepts/alien-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@alien/sdk` is the external app/client package for talking to the
  Gateway. `alien/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `Alien`, `Agent`, `Session`, `Run`,
...
- source: Alien, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@alien/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@alien/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@alien/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `alien/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `Alien`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [Alien App SDK](/concepts/alien-sdk)
