...
Alien integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Alien owns lifecycle when `channels.signal.autoStart=true`.
...
- Alien spawns `imsg rpc` as a child process (legacy iMessage integration).
