...
- In scope: strictly user-supplied credentials that Alien does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `alien.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `alien.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `alien doctor --fix` to migrate valid markers.
...
- Marker persistence is source-authoritative: Alien writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
- For web search:
