...
Alien has three public release lanes:
...
- Every stable Alien release ships the npm package and macOS app together;
...
Alien does not yet have an LTS or monthly support channel. Maintainers are
...
5. Bump every required version location for the intended tag, run
   ...
   `pnpm release:check`.
6. Run `Alien NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For beta, tag `vYYYY.M.D-beta.N`, then run `Alien Release Publish` from
   ...
   ClawHub in parallel, and then promotes the prepared Alien npm preflight
   artifact with the matching dist-tag as soon as plugin npm publish succeeds.
   ClawHub publishing may still be running while Alien npm publishes, but the
   release publish workflow does not finish until both plugin publish paths and
   the Alien npm publish path have completed successfully. After publish, run
   the post-publish package
   acceptance against the published `alien@YYYY.M.D-beta.N` or
   `alien@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `Alien Release Publish`, reusing the successful preflight artifact via
...
- Run `pnpm plugins:sync` after the root version bump and before tagging. It
  updates publishable plugin package versions, Alien peer/API compatibility
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `Alien Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `alien@beta`, `alien@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=alien@beta -f suite_profile=product -f published_upgrade_survivor_baseline=alien@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- Run `Alien Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.D` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful Alien npm
  ...
  workflow serializes plugin npm publish, plugin ClawHub publish, and Alien
  npm publish so the core package is not published before its externalized
  plugins.
- Release checks now run in a separate manual workflow:
  `Alien Release Checks`
- `Alien Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `Alien Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/alien-cross-os-release-checks-reusable.yml` directly
...
- `Alien Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an Alien branch or release tag
- `Alien NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `ALIEN_LIVE_TEST=1 ALIEN_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/alien-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/alien-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `ALIEN_NPM_TELEGRAM_PACKAGE_SPEC=alien@YYYY.M.D-beta.N ALIEN_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex ALIEN_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `ALIEN_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `alien/releases-private/.github/workflows/alien-npm-dist-tags.yml`
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=alien@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `Alien Release Checks`, prepares a
...
`rerun_group=all` or when `npm_telegram_package_spec` is set. `Alien Release
...
`Alien Release Checks` uses the trusted workflow ref to resolve the target
...
The cross-OS OpenAI install smoke uses `ALIEN_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=alien@YYYY.M.D-beta.N \
  -f npm_telegram_package_spec=alien@YYYY.M.D-beta.N \
...
The Docker box lives in `Alien Release Checks` through
`alien-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `Alien Release Checks`. It is the agentic
...
`scripts/resolve-alien-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `alien@beta`, `alien@latest`, or an exact Alien release
  version
...
`Alien Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=alien@2026.4.26
```
...
`Alien Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `Alien NPM Release` with the release tag, npm dist-tag, and
...
`plugin_publish_scope=selected` and `plugins=@alien/name` to
`Alien Release Publish`, or dispatch the child workflow directly when the
Alien package must not be published.

## NPM workflow inputs

`Alien NPM Release` accepts these operator-controlled inputs:
...
`Alien Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Alien NPM Release` preflight run id;
  required when `publish_alien_npm=true`
- `npm_dist_tag`: npm target tag for the Alien package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused repair work
- `plugins`: comma-separated `@alien/*` package names when
  `plugin_publish_scope=selected`
- `publish_alien_npm`: defaults to `true`; set `false` only when using the
  workflow as a plugin-only repair orchestrator

`Alien Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an Alien branch or
  release tag.
...
- For `Alien NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `Alien Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `Alien NPM Release` with `preflight_only=true`
...
6. Run `Alien Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ClawHub before promoting the Alien npm package
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/alien/alien/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/alien/alien/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/alien-npm-release.yml`](https://github.com/alien/alien/blob/main/.github/workflows/alien-npm-release.yml)
- [`.github/workflows/alien-release-checks.yml`](https://github.com/alien/alien/blob/main/.github/workflows/alien-release-checks.yml)
- [`.github/workflows/alien-cross-os-release-checks-reusable.yml`](https://github.com/alien/alien/blob/main/.github/workflows/alien-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-alien-package-candidate.mjs`](https://github.com/alien/alien/blob/main/scripts/resolve-alien-package-candidate.mjs)
- [`scripts/alien-npm-release-check.ts`](https://github.com/alien/alien/blob/main/scripts/alien-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/alien/alien/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/alien/alien/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`alien/maintainers/release/README.md`](https://github.com/alien/maintainers/blob/main/release/README.md)
