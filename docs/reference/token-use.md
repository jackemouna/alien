---
summary: "How Alien builds prompt context and reports token usage + costs"
read_when:
...
Alien tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Alien assembles its own system prompt on every run. It includes:
...
For images, Alien downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Alien session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `alien status --usage` and `alien channels list` show
...
available; otherwise Alien falls back to matching OAuth/API-key credentials
...
Alien keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Alien shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Alien starts an
...
Provider prompt caching only applies within the cache TTL window. Alien can
...
Anthropic's 1M context window is currently beta-gated. Alien can inject the
...
Alien skips the `context-1m-*` beta header because Anthropic currently
