...
Alien features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Alien reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us Alien-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that Alien can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `alien status --usage` and `alien channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Alien inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Alien prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Alien falls back to matching OAuth/API-key
...
Alien can pick up credentials from:
...
Alien's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Alien Claude-login path with **Extra Usage** enabled.
...
`alien models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
