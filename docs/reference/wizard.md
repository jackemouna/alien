...
This is the full reference for `alien onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.alien/alien.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `alien doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Alien now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.alien/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.alien/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.alien/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$ALIEN_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.alien/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `alien pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `alien configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `alien health`.
    - Tip: `alien status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
- Stores it under `~/.alien/tools/signal-cli/<version>/`.
...
Typical fields in `~/.alien/alien.json`:
...
`alien agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.alien/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.alien/agents/<agentId>/sessions/`.
