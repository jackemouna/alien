---
summary: "macOS app flow for controlling a remote Alien gateway over SSH"
read_when:
  - Setting up or debugging remote mac control
title: "Remote control"
---

This flow lets the macOS app act as a full remote control for an Alien gateway running on another host (desktop/server). It's the app's **Remote over SSH** (remote run) feature. All features-health checks, Voice Wake forwarding, and Web Chat-reuse the same remote SSH configuration from _Settings → General_.

## Modes

- **Local (this Mac)**: Everything runs on the laptop. No SSH involved.
- **Remote over SSH (default)**: Alien commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
`alien node install ...` and `alien node start` (or run
`alien node run ...` in the foreground), then target that browser-capable
node.

## Prereqs on the remote host

1. Install Node + pnpm and build/install the Alien CLI (`pnpm install && pnpm build && pnpm link --global`).
2. Ensure `alien` is on PATH for non-interactive shells (symlink into `/usr/local/bin` or `/opt/homebrew/bin` if needed).
3. Open SSH with key auth. We recommend **Tailscale** IPs for stable reachability off-LAN.

## macOS app setup

1. Open _Settings → General_.
2. Under **Alien runs**, pick **Remote over SSH** and set:
   ...
   - **CLI path** (advanced): optional path to a runnable `alien` entrypoint/binary (auto-filled when advertised).
3. Hit **Test remote**. Success indicates the remote `alien status --json` runs correctly. Failures usually mean PATH/CLI issues; exit 127 means the CLI isn't found remotely.
...
- Run `alien channels login --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
- Re-run login on that host if auth expires. Health check will surface link problems.

## Troubleshooting

- **exit 127 / not found**: `alien` isn't on PATH for non-login shells. Add it to `/etc/paths`, your shell rc, or symlink into `/usr/local/bin`/`/opt/homebrew/bin`.
- **Health probe failed**: check SSH reachability, PATH, and that Baileys is logged in (`alien status --json`).
...
Pick sounds per notification from scripts with `alien` and `node.invoke`, e.g.:

```bash
alien nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
