...
- Same path: run the app from a fixed location (for Alien, `dist/Alien.app`).
...
```bash
sudo tccutil reset Accessibility ai.alien.mac
sudo tccutil reset ScreenCapture ai.alien.mac
...
Workaround: move files into the Alien workspace (`~/.alien/workspace`) if you want to avoid per-folder grants.
