...
- App runs `alien health --json` via `ShellExecutor` every ~60s and on demand. The probe loads creds and reports status without sending messages.
- Cache the last good snapshot and the last error separately to avoid flicker; show the timestamp of each.

## When in doubt

- You can still use the CLI flow in [Gateway health](/gateway/health) (`alien status`, `alien status --deep`, `alien health --json`) and tail `/tmp/alien/alien-*.log` for `web-heartbeat` / `web-reconnect`.
