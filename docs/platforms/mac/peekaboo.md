...
read_when:
  - Hosting PeekabooBridge in Alien.app
...
Alien can host **PeekabooBridge** as a local, permission-aware UI automation
broker. This lets the `peekaboo` CLI drive UI automation while reusing the
macOS app's TCC permissions.

## What this is (and is not)

- **Host**: Alien.app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate `alien ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Alien is a thin broker host.

## Relationship to Computer Use

Alien has three desktop-control paths, and they intentionally stay separate:

- **PeekabooBridge host**: Alien.app can host the local PeekabooBridge socket.
  The `peekaboo` CLI remains the client and uses Alien.app's macOS
...
- **Codex Computer Use**: the bundled `codex` plugin prepares Codex app-server,
  ...
  Codex own native desktop-control tool calls during Codex-mode turns. Alien
  does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Alien can register TryCua's upstream
...
Use Peekaboo when you want the broad macOS automation surface and Alien.app's
...
when you want the CUA driver exposed to any Alien-managed runtime as a normal
MCP server.
...
When enabled, Alien starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Alien app TeamID).
...
- If `peekaboo` reports "bridge client is not authorized", ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or Alien.app)
