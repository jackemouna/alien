...
service via the external `alien` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.alien.gateway`
  (or `ai.alien.<profile>` when using `--profile`/`ALIEN_PROFILE`; legacy `com.alien.*` is supported).
