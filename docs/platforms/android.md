...
<Note>
The Android app has not been publicly released yet. The source code is available in the [Alien repository](https://github.com/alien/alien) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/alien/alien/blob/main/apps/android/README.md) for build instructions.
</Note>
...
- You can run the CLI (`alien`) on the gateway machine (or via SSH).
...
1. Set up a DNS-SD zone (example `alien.internal.`) on the gateway host and publish `_alien-gw._tcp` records.
...
1. Create `~/.alien/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
alien nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__alien__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__alien__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__alien__/a2ui/`.
...
Android supports launching Alien from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Alien..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Alien as the default assistant app.
</Note>
