...
read_when:
  - Installing Alien on Windows
  - Choosing between native Windows and WSL2
  - Looking for Windows companion app status
title: "Windows"
---

Alien supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `alien --version`, `alien doctor`, and `alien plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
alien agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `alien onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `alien onboard --non-interactive --install-daemon` and `alien gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, Alien falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, Alien now aborts that path quickly and falls back instead of hanging forever
- Scheduled Tasks are still preferred when available because they provide better supervisor status

If you want the native CLI only, without gateway service install, use one of these:

```powershell
alien onboard --non-interactive --skip-health
alien gateway run
```
...
### 2) Install the Alien gateway user service
...
```bash
systemctl --user is-enabled alien-gateway.service
systemctl --user status alien-gateway.service --no-pager
```
...
pnpm build
pnpm ui:build
pnpm alien onboard --install-daemon
```
