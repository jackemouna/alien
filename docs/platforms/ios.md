...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `alien.internal.`), **or**
...
Run `alien devices list` again before approval.
...
```bash
alien nodes status
alien gateway call node.list --params "{}"
```
...
- `ALIEN_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Alien iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Alien builds.
...
```bash
export ALIEN_APNS_TEAM_ID="TEAMID"
export ALIEN_APNS_KEY_ID="KEYID"
export ALIEN_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.alien/credentials/apns
chmod 700 ~/.alien/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.alien/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.alien/credentials/apns/AuthKey_KEYID.p8
export ALIEN_APNS_PRIVATE_KEY_PATH="$HOME/.alien/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_alien-gw._tcp` on `local.` and, when configured, the same
...
`alien.internal.`) and Tailscale split DNS.
...
```bash
alien nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__alien__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__alien__/canvas/` and `/__alien__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Alien node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Alien by invoking node
...
```bash
alien nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__alien; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
alien nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `alien devices list` and approve manually.
