...
2. `npm i -g alien@latest`
3. `alien onboard --install-daemon`
...
Alien installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `alien gateway install` and
`alien onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/alien-gateway[-<profile>].service`:

```
[Unit]
Description=Alien Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Alien therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Alien starts the child through a short
...
- Alien-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `ALIEN_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
