...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Alien
...
Running Alien via Node with `tsx` fails at startup with:

```
[alien] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `alien status` and `gateway:watch` worked.
