...
- Alien installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Alien"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Alien and restart the gateway.
5. Send a first DM and approve pairing (`alien pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Alien"` then scan the QR in Signal.
...
4. Configure Alien, restart gateway, verify channel:

```bash
# If you run the gateway as a user systemd service:
systemctl --user restart alien-gateway.service
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `alien pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Alien at it:
...
This skips auto-spawn and the startup wait inside Alien. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `alien pairing list signal`
  - `alien pairing approve signal <CODE>`
...
- **Typing indicators**: Alien sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Alien forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `alien doctor --fix`.
...
alien pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/alien/alien-$(date +%Y-%m-%d).log" | tail -20
```
