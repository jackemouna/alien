...
read_when:
  - Setting up Zalo Personal for Alien
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Alien.
...
Zalo Personal ships as a bundled plugin in current Alien releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `alien plugins install @alien/zalouser`
- Pinned version: `alien plugins install @alien/zalouser@2026.5.2`
- Or from a source checkout: `alien plugins install ./path/to/local/zalouser-plugin`
...
1. Ensure the Zalo Personal plugin is available.
   - Current packaged Alien releases already bundle it.
   - Older/custom installs can add it manually with the commands above.
2. Login (QR, on the Gateway machine):
   - `alien channels login --channel zalouser`
...
```bash
alien directory self --channel zalouser
alien directory peers list --channel zalouser --query "name"
alien directory groups list --channel zalouser --query "work"
```
...
- `alien pairing list zalouser`
- `alien pairing approve zalouser <code>`
...
- On startup, Alien resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, Alien stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Alien state. Example:
...
- Alien sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Alien sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `alien channels logout --channel zalouser && alien channels login --channel zalouser`
...
- The channel now runs fully in Alien without external CLI binaries.
