...
Nextcloud Talk ships as a bundled plugin in current Alien releases, so
...
```bash
alien plugins install @alien/nextcloud-talk
```
...
```bash
alien plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Alien" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
4. Configure Alien:
   ...
   ```bash
   alien channels add --channel nextcloud-talk \
...
- Approve via:
  - `alien pairing list nextcloud-talk`
  - `alien pairing approve nextcloud-talk <CODE>`
