...
read_when:
  - You want Alien to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Alien to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Alien releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`alien onboard`) and `alien channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
alien plugins install @alien/nostr
```
...
```bash
alien plugins install --link <path-to-local-nostr-plugin>
```
...
alien channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "alien@example.com",
        lud16: "alien@example.com",
      },
    },
  },
}
