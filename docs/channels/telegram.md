...
```
    ...
    Telegram does **not** use `alien channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `alien doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `alien doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `alien logs --follow`
...
- DM messages can carry `message_thread_id`; Alien preserves the thread ID for replies but keeps DMs on the flat session by default. Configure `channels.telegram.dm.threadReplies: "inbound"`, `channels.telegram.direct.<chatId>.threadReplies: "inbound"`, `requireTopic: true`, or a matching topic config when you intentionally want DM topic session isolation.
- Long polling uses grammY runner with per-chat/per-thread sequencing. Overall runner sink concurrency uses `agents.defaults.maxConcurrent`.
- Long polling is guarded inside each gateway process so only one active poller can use a bot token at a time. If you still see `getUpdates` 409 conflicts, another Alien gateway, script, or external poller is likely using the same token.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Alien can stream partial replies in real time:
    ...
    - legacy `channels.telegram.streamMode` and boolean `streaming` values are detected; run `alien doctor --fix` to migrate them to `channels.telegram.streaming.mode`

    Tool-progress preview updates are the short status lines shown while tools run, for example command execution, file reads, planning updates, or patch summaries. Telegram keeps these enabled by default to match released Alien behavior from `v2026.4.22` and later. To keep the edited preview for answer text but hide tool-progress lines, set:
    ...
    <Note>
      Telegram selected quote replies are the exception. When `replyToMode` is `"first"`, `"all"`, or `"batched"` and the inbound message includes selected quote text, Alien sends the final answer through Telegram's native quote-reply path instead of editing the answer preview, so `streaming.preview.toolProgress` cannot show the short status lines for that turn. Current-message replies without selected quote text still keep preview streaming. Set `replyToMode: "off"` when tool-progress visibility matters more than native quote replies, or set `streaming.preview.toolProgress: false` to acknowledge the trade-off.
    </Note>

    For text-only replies:

    - short DM/group/topic previews: Alien keeps the same preview message and performs the final edit in place
    ...
    - if the final edit fails before the completed text is confirmed, Alien uses normal final delivery and cleans up the stale preview

    For complex replies (for example media payloads), Alien falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Alien skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Alien retries as plain text.
...
```
    ...
    - `deleteWebhook`, `deleteMyCommands`, or `setMyCommands` failing with `404: Not Found` while direct Bot API curl commands work can mean `channels.telegram.apiRoot` was set to the full `/bot<TOKEN>` endpoint. `apiRoot` must be only the Bot API root, and `alien doctor --fix` removes an accidental trailing `/bot<TOKEN>`.
    - `getMe returned 401` means Telegram rejected the configured bot token. Update `botToken`, `tokenFile`, or `TELEGRAM_BOT_TOKEN` with the current BotFather token; Alien stops before polling so this is not reported as a webhook cleanup failure.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly. Alien pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnSessions` to remain enabled (default: `true`).
...
```
    ...
    - `~/.alien/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Alien enqueues system events like:
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    In long-polling mode Alien persists its restart watermark only after an update dispatches successfully. If a handler fails, that update remains retryable in the same process and is not written as completed for restart dedupe.
    ...
    Alien then processes the update asynchronously through the same per-chat/per-topic bot lanes used by long polling, so slow agent turns do not hold Telegram's delivery ACK.
  ...
  <Accordion title="Limits, retry, and CLI targets">
    ...
    - `channels.telegram.mediaGroupFlushMs` (default 500) controls how long Telegram albums/media groups are buffered before Alien dispatches them as one inbound message. Increase it if album parts arrive late; decrease it to reduce album reply latency.
...
```bash
alien message send --channel telegram --target 123456789 --message "hi"
alien message send --channel telegram --target @name --message "hi"
alien message send --channel telegram --target -1001234567890:topic:42 --message "hi topic"
```

    Telegram polls use `alien message poll` and support forum topics:

```bash
alien message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
alien message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Alien preserves the topic for the approval prompt and the follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `alien channels status` warns when config expects unmentioned group messages.
    - `alien channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `alien logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Alien now retries these as recoverable network errors.
    - During polling startup, Alien reuses the successful startup `getMe` probe for grammY so the runner does not need a second `getMe` before the first `getUpdates`.
    - If `deleteWebhook` fails with a transient network error during polling startup, Alien continues into long polling instead of making another pre-poll control-plane call. A still-active webhook surfaces as a `getUpdates` conflict; Alien then rebuilds the Telegram transport and retries webhook cleanup.
    ...
    - If logs include `Polling stall detected`, Alien restarts polling and rebuilds the Telegram transport after 120 seconds without completed long-poll liveness by default.
    - `alien channels status --probe` and `alien doctor` warn when a running polling account has not completed `getUpdates` after startup grace, when a running webhook account has not completed `setWebhook` after startup grace, or when the last successful polling transport activity is stale.
    ...
    - If the Alien managed proxy is configured through `ALIEN_PROXY_URL` for a service environment and no standard proxy env is present, Telegram uses that URL for Bot API transport too.
...
```
    ...
    - Node 22+ defaults to `autoSelectFamily=true` (except WSL2). Telegram DNS result order honors `ALIEN_TELEGRAM_DNS_RESULT_ORDER`, then `channels.telegram.network.dnsResultOrder`, then the process default such as `NODE_OPTIONS=--dns-result-order=ipv4first`; if none applies, Node 22+ falls back to `ipv4first`.
...
```
    ...
    - Environment overrides (temporary):
      - `ALIEN_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `ALIEN_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `ALIEN_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
<Note>
Multi-account precedence: when two or more account IDs are configured, set `channels.telegram.defaultAccount` (or include `channels.telegram.accounts.default`) to make default routing explicit. Otherwise Alien falls back to the first normalized account ID and `alien doctor` warns. Named accounts inherit `channels.telegram.allowFrom` / `groupAllowFrom`, but not `accounts.default.*` values.
</Note>
