---
summary: "Mattermost bot setup and Alien config"
read_when:
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    alien plugins install @alien/mattermost
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    alien plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Current packaged Alien releases already bundle it. Older/custom installs can add it manually with the commands above.
  </Step>
  ...
  <Step title="Configure Alien and start the gateway">
    Minimal config:
...
Native slash commands are opt-in. When enabled, Alien registers `oc_*` slash commands via the Mattermost API and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, Alien derives one from gateway host/port + `callbackPath`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when Alien registers `oc_*` commands.
    - Alien refreshes current Mattermost command registration before accepting each callback so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Alien.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Alien.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Alien, not `404`.
...
- Approve via:
  - `alien pairing list mattermost`
  - `alien pairing approve mattermost <CODE>`
...
Use these target formats with `alien message send` or cron/webhooks:
...
Alien resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Alien sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Alien sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), Alien falls back to sending a fresh final post so the reply is never lost.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, Alien derives the callback URL from `gateway.customBindHost` + `gateway.port`, then falls back to `http://localhost:<port>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and Alien run on the same host/network namespace.
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="alien-mattermost-interactions", data=botToken)`
  </Step>
...
secret = hmac.new(
    b"alien-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `alien message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: Alien did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to `http://127.0.0.1:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as Alien. Set an explicit externally reachable `commands.callbackUrl` instead.
