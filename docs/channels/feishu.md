...
<Note>
Requires Alien 2026.4.25 or above. Run `alien --version` to check. Upgrade with `alien update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  alien channels login --channel feishu
  ```
...
```bash
alien pairing list feishu
alien pairing approve feishu <CODE>
```
...
4. Check logs: `alien logs --follow`
...
5. Ensure the gateway is running: `alien gateway status`
6. Check logs: `alien logs --follow`

### App Secret leaked

1. Reset the App Secret in Feishu Open Platform / Lark Developer
2. Update the value in your config
3. Restart the gateway: `alien gateway restart`
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Alien
...
conversion fails, Alien falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, Alien
hydrates it from Feishu before routing the turn. Normal group replies that
Alien turns into threads keep using the reply root message ID (`om_*`) so the
