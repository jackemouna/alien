...
read_when:
  - You want to connect Alien to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Alien via the LINE Messaging API. The plugin runs as a webhook
...
```bash
alien plugins install @alien/line
```

Local checkout (when running from a git repo):

```bash
alien plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Alien applies strict pre-auth body limits and timeout before verification.
- Alien processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
alien pairing list line
alien pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.alien/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Alien validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
