...
- bot user = the Alien Matrix account that sends the reply
...
If no pushers come back, fix normal Matrix push delivery for this account before continuing.
  ...
  <Step title="Install the override push rule">
    Alien marks finalized text-only preview edits with `content["com.alien.finalized_preview"] = true`. Install a rule that matches that marker plus the bot MXID as sender:

```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/alien-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.alien\\.finalized_preview",
        "value": true
      },
...
```
    ...
    - `alien-finalized-preview-botname`: a rule ID unique per bot per recipient (pattern: `alien-finalized-preview-<botname>`)
    - `@bot:example.org`: your Alien bot MXID, not the recipient's
...
Push rules are keyed by `ruleId`: re-running `PUT` against the same ID updates a single rule. For multiple Alien bots notifying the same recipient, create one rule per bot with a distinct sender match.
