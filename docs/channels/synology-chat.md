---
summary: "Synology Chat webhook setup and Alien config"
read_when:
  - Setting up Synology Chat with Alien
...
Synology Chat ships as a bundled plugin in current Alien releases, so normal
...
```bash
alien plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `alien onboard` now shows Synology Chat in the same channel setup list as `alien channels add`.
   - Non-interactive setup: `alien channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Alien gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Alien.
   - Guided: `alien onboard`
   - Direct: `alien channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Alien accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `alien pairing list synology-chat`
  - `alien pairing approve synology-chat <CODE>`
...
```bash
alien message send --channel synology-chat --target 123456 --text "Hello from Alien"
alien message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
alien message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before Alien forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. Alien now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Alien
- `Rate limit exceeded`:
