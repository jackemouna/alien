...
Microsoft Teams ships as a bundled plugin in current Alien releases, so no
...
```bash
alien plugins install @alien/msteams
```
...
```bash
alien plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-alien-bot --allow-anonymous
devtunnel port create my-alien-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-alien-bot
...
**4. Configure Alien** using the credentials from the output:
...
- Talk to Alien via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists - they can change. Alien disables direct name matching by default; opt in explicitly with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Alien resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.alien/alien.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `alien-msteams` (must be unique) |
...
For production deployments, Alien supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, Alien uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, Alien defaults to client secret authentication. Existing configurations continue to work without changes.
...
Alien exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Alien in Teams", full: "Alien in Teams" },
...
Alien handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Alien only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AlienShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Alien sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `alien message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.alien/msteams-polls.json`.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries yet (inspect the store file if needed).

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool or CLI. Alien renders them as Teams Adaptive Cards from the generic presentation contract.
...
alien message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
alien message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
alien message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
alien message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- **Ignore** the `groupId` query parameter for Alien routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
