...
```bash
alien plugins install @alien/googlechat
```

Local checkout (when running from a git repo):

```bash
alien plugins install ./path/to/local/googlechat-plugin
```
...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `alien-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.alien/googlechat-service-account.json`).
5. Create a Google Chat app in the [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat):
   - Fill in the **Application info**:
     - **App name**: (e.g. `Alien`)
     - **Avatar URL**: (e.g. `https://alien.ai/logo.png`)
     - **Description**: (e.g. `Personal AI Assistant`)
   - Enable **Interactive features**.
   - Under **Functionality**, check **Join spaces and group conversations**.
   - Under **Connection settings**, select **HTTP endpoint URL**.
   - Under **Triggers**, select **Use a common HTTP endpoint URL for all triggers** and set it to your gateway's public URL followed by `/googlechat`.
     - _Tip: Run `alien status` to find your gateway's public URL._
...
6. **Enable the app status**:
   ...
   - Click **Save** again.
7. Configure Alien with the service account path + webhook audience:
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the Alien dashboard and other sensitive endpoints on your private network.
...
With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to Alien.
...
1. Google Chat sends webhook POSTs to the gateway. Each request includes an `Authorization: Bearer <token>` header.
   - Alien verifies bearer auth before reading/parsing full webhook bodies when the header is present.
...
2. Alien verifies the token against the configured `audienceType` + `audience`:
...
4. DM access is pairing by default. Unknown senders receive a pairing code; approve with:
   - `alien pairing approve googlechat <code>`
...
1. **Channel not configured**: The `channels.googlechat` section is missing from your config. Verify with:
   ...
   ```bash
   alien config get channels.googlechat
   ```
...
2. **Plugin not enabled**: Check plugin status:
   ...
   ```bash
   alien plugins list | grep googlechat
   ```
...
- Check `alien channels status --probe` for auth errors or missing audience config.
- If no messages arrive, confirm the Chat app's webhook URL + event subscriptions.
- If mention gating blocks replies, set `botUser` to the app's user resource name and verify `requireMention`.
- Use `alien logs --follow` while sending a test message to see if requests reach the gateway.
