...
read_when:
  - Setting up Twitch chat integration for Alien
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support via IRC connection. Alien connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

<Note>
Twitch ships as a bundled plugin in current Alien releases, so normal packaged builds do not need a separate install.
</Note>
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    alien plugins install @alien/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    alien plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `ALIEN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "alien", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ALIEN_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    ALIEN_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
