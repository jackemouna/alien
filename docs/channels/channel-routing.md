...
Alien routes replies **back to the channel where a message came from**. The
...
Alien infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Alien still records inbound session metadata, but it
...
when a guarded path must not create a new Alien session. In that mode,
Alien may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Alien would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.alien/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.alien`):

- `~/.alien/agents/<agentId>/sessions/sessions.json`
