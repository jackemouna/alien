...
Tlon is a decentralized messenger built on Urbit. Alien connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Alien releases, so normal packaged
...
```bash
alien plugins install @alien/tlon
```
...
```bash
alien plugins install ./path/to/local/tlon-plugin
```
...
By default, Alien blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `alien message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Alien replies in-thread.
