...
Dreaming is the background memory consolidation system in `memory-core`. It helps Alien move strong short-term signals into durable memory while keeping the process explainable and reviewable.
...
<Tabs>
  <Tab title="Promotion preview / apply">
    ```bash
    alien memory promote
    alien memory promote --apply
    alien memory promote --limit 5
...
If `alien memory status` reports `Dreaming status: blocked`, the managed cron exists but the default agent heartbeat is not firing. Check that heartbeat is enabled for the default agent and that its target is not `none`, then run `alien memory status --deep` again after the next heartbeat interval.
