---
summary: "Public Alien App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "Alien App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Alien
...
The **Alien App SDK** is the public client API for apps outside the
Alien process. Use `@alien/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@alien/sdk` talks to the Gateway from outside Alien.
  `alien/plugin-sdk/*` is only for plugins that run inside Alien and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@alien/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `Alien`                | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `AlienEvent`,
`AlienEventType`, `GatewayEvent`, `AlienTransport`,
...
`new Alien({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `AlienTransport`:
...
The SDK normalizes raw Gateway events into a stable `AlienEvent` envelope:
...
Use the App SDK when code lives outside Alien:
...
Use the Plugin SDK when code runs inside Alien:
...
App SDK code should import from `@alien/sdk`. Plugin code should import from
documented `alien/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [Alien App SDK API design](/reference/alien-sdk-api-design)
