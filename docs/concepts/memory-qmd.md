...
alongside Alien. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Alien falls back to the
...
Alien creates a self-contained QMD home under
`~/.alien/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Alien creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so Alien skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, Alien retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, Alien groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, Alien falls back to the builtin SQLite engine.
  ...
  `alien memory status` and one-shot CLI probes still recheck QMD directly.
...
Alien keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Alien checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Alien searches all
...
Older QMD builds only accept one collection filter. When Alien detects one
...
process, so you can tune QMD globally without adding new Alien config:
...
collection under `~/.alien/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Alien logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Alien
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Alien still reports
...
with `alien memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Alien uses.

**Many QMD subprocesses during search?** Update QMD if possible. Alien uses
...
`memory.qmd.searchMode = "search"`. Alien treats that mode as lexical-only,
...
the next startup reconciliation. Alien recreates stale managed collections
...
Alien's builtin symlink rules. Keep temporary monorepo checkouts under
