...
[Honcho](https://honcho.dev) adds AI-native memory to Alien. It persists
...
```bash
alien plugins install @honcho-ai/alien-honcho
alien honcho setup
alien gateway --force
```
...
Settings live under `plugins.entries["alien-honcho"].config`:
    ...
    entries: {
      "alien-honcho": {
        config: {
          ...
          workspaceId: "alien", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `alien honcho setup` detects and
...
```bash
alien honcho setup                        # Configure API key and migrate files
alien honcho status                       # Check connection status
alien honcho ask <question>               # Query Honcho about the user
alien honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/alien-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Alien integration guide](https://docs.honcho.dev/v3/guides/integrations/alien)
- [Memory](/concepts/memory) -- Alien memory overview
