...
- Alien lets provider SDKs handle normal short retries.
- For Stainless-based SDKs such as Anthropic and OpenAI, retryable responses
  ...
  `retry-after`. When that wait is longer than 60 seconds, Alien injects
...
- Override the cap with `ALIEN_SDK_RETRY_MAX_WAIT_SECONDS=<seconds>`.
...
Set retry policy per provider in `~/.alien/alien.json`:
