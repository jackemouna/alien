---
summary: "OAuth in Alien: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand Alien OAuth end-to-end
...
Alien also supports **provider plugins** that ship their own OAuth or API-key
flows. Run them via:

```bash
alien models auth login --provider <id>
```
...
- you log in via Alien _and_ via Claude Code / Codex CLI → one of them randomly gets "logged out" later

To reduce that, Alien treats `auth-profiles.json` as a **token sink**:

- the runtime reads credentials from **one place**
- we can keep multiple profiles and route them deterministically
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai-codex:default` profile, but once Alien has a local OAuth profile,
...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.alien/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.alien/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.alien/credentials/oauth.json` (imported into `auth-profiles.json` on first use)

All of the above also respect `$ALIEN_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, Alien uses read-through
...
Claude subscription limits, and Anthropic staff told us Alien-style Claude
CLI usage is allowed again. Alien therefore treats Claude CLI reuse and
...
If you want other subscription-style options in Alien, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/glm).
</Warning>

Alien also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

Alien supports Anthropic Claude CLI reuse again. If you already have a local
Claude login on the host, onboarding/configure can reuse it directly.

## OAuth exchange (how login works)

Alien's interactive login flows are implemented in `@mariozechner/pi-ai` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from Alien
2. Alien stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including Alien workflows.
...
Wizard path is `alien onboard` → auth choice `openai-codex`.
...
- exception: some external CLI credentials stay externally managed; Alien
  ...
  `openai-codex:default` profile, then Alien-owned refreshes keep the local
  profile canonical.
...
- `alien channels list --json` (shows `auth[]`)
