---
summary: "Move one Alien session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Alien session.
...
Alice can message Alien on Telegram and Discord:
...
Alien keeps the current session context and changes the reply route:
...
4. Keep the same Alien session, but receive future replies in Discord.
