---
summary: "How Alien separates model providers, models, channels, and agent runtimes"
...
| ------------- | ------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How Alien authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                              |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave Alien.                            |
...
implements the `codex` runtime. Public config uses `agentRuntime.id`; `alien
doctor --fix` rewrites older runtime-policy keys to that shape.

There are two runtime families:

- **Embedded harnesses** run inside Alien's prepared agent loop. Today this
...
| Surface                                              | Alien name/config                       | What it does                                                                                               |
| ---------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------------- |
...
| Codex OAuth provider route                           | `openai-codex/*` model refs                | Uses ChatGPT/Codex subscription OAuth through the normal Alien PI runner.                               |
...
That means Alien selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Alien session store
becomes Codex.
...
3. If the user asks for **Codex OAuth/subscription auth on the normal Alien
...
| Surface                     | Alien PI embedded                    | Codex app-server                                                            |
| --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Alien through the PI embedded runner | Codex app-server                                                            |
| Canonical thread state      | Alien transcript                     | Codex thread, plus Alien transcript mirror                               |
| Alien dynamic tools      | Native Alien tool loop               | Bridged through the Codex adapter                                           |
| Native shell and file tools | PI/Alien path                        | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Alien context assembly        | Alien projects assembled context into the Codex turn                     |
| Compaction                  | Alien or selected context engine     | Codex-native compaction, with Alien notifications and mirror maintenance |
| Channel delivery            | Alien                                | Alien                                                                    |

This ownership split is the main design rule:

- If Alien owns the surface, Alien can provide normal plugin hook behavior.
- If the native runtime owns the surface, Alien needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Alien should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Alien chooses an embedded runtime after provider and model resolution:
...
2. `ALIEN_AGENT_RUNTIME=<id>` forces that runtime for new or reset sessions.
...
4. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
5. If no runtime claims a turn in `auto` mode, Alien uses PI as the
...
If `alien doctor` warns that the `codex` plugin is enabled while
...
When a runtime is not PI, it should document what Alien surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Alien can edit history or only mirror it.                                   |
| Do Alien dynamic tools work?        | Messaging, sessions, cron, and Alien-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Alien-owned tools. |
