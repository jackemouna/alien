...
Alien gives agents tools to work across sessions, inspect status, and
...
After the target responds, Alien can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `alien-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Alien
...
the completion origin only identifies a channel Alien can still reuse the
