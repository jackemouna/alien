...
`node-llama-cpp` runtime package next to Alien, then point `local.modelPath`
at a GGUF file:
...
Alien indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.alien/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `alien memory index --force`
...
**Memory search disabled?** Check `alien memory status`. If no provider is
...
**Stale results?** Run `alien memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Alien falls back to in-process cosine similarity
automatically. `alien memory status --deep` reports the local vector store
