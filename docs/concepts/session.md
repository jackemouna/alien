---
summary: "How Alien manages conversation sessions"
read_when:
...
Alien organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `alien security audit`.
...
Alien automatically bounds session storage over time. By default, it runs
...
Preview with `alien sessions cleanup --dry-run`.

## Inspecting sessions

- `alien status` -- session store path and recent activity.
- `alien sessions --json` -- all sessions (filter with `--active <minutes>`).
