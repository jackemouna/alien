---
summary: "Delegate architecture: running Alien as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Alien as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Alien agent that:
...
Alien's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Alien's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.alien/workspace-delegate",
  tools: {
...
- Cron run history: `~/.alien/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.alien/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Alien's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
alien agents add delegate
```

This creates:

- Workspace: `~/.alien/workspace-delegate`
- State: `~/.alien/agents/delegate/agent`
- Sessions: `~/.alien/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.alien/workspace" },
      {
        id: "delegate",
        workspace: "~/.alien/workspace-delegate",
        tools: {
...
~/.alien/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.alien/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.alien/workspace-org",
        agentDir: "~/.alien/agents/org-assistant/agent",
...
recall view. Alien redacts credential/token-like text, truncates long
