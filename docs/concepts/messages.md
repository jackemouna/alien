...
Alien handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. Alien keeps a
...
Alien keeps that boundary explicit:
...
Alien separates the **prompt body** from the **command body**:
...
Alien can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, Alien
strips the silent text but still delivers the media attachment.
Alien resolves that behavior by conversation type:
...
Alien also uses silent replies for internal runner failures that happen
