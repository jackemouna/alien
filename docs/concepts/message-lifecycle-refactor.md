...
That shape fixes local bugs, but it leaves Alien with too many public
...
That gives Alien at-least-once recovery. Exactly-once behavior exists only
...
- Structured Alien-origin metadata for operational/system output so visible
...
Alien should borrow the vocabulary, not copy the surface.

What Alien needs beyond that model:
...
`thread.post()` style promises are not enough for Alien. They hide the
...
Origin describes who produced a message and how Alien should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be Alien-originated operational output.
...
Core owns the meaning of Alien-originated output. Channels own how that
...
Alien operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that Alien accepted
...
- **Inbound record ack:** confirms Alien persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared Alien echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropAlienEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
...
same visible gateway-failure text but without Alien origin metadata still
...
middleware chain, but Alien persists only the safe completed update id after
...
source if we need platform-level redelivery beyond Alien's restart
...
process dies there, Alien cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "alien/plugin-sdk/channel-message";
```
...
Before preflight authorization, core must run the shared Alien echo predicate
whenever `origin.decode` returns Alien-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  Alien can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- Alien gateway failure output must stay visible to humans, but tagged
...
| Slack                    | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps Alien gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix                   | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode Alien gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat              | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged Alien gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve Alien operational provenance.
...
- Add `alien/plugin-sdk/channel-message`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropAlienEcho` predicate in the channel-message SDK surface.
...
This is where the model stops being "agent replies" and becomes "Alien sends
messages".
...
- Gateway-failure origin factory and `shouldDropAlienEcho` predicate.
...
- Slack tagged Alien gateway failure messages stay visible outbound, tagged
...
- Matrix tagged Alien gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only Alien's persisted restart watermark.
...
- Alien-originated gateway failure output is visible to humans but tagged
