...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.alien/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.alien/agents/<agentId>/sessions`.
...
a local profile, but Alien does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.alien/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.alien` (or `ALIEN_STATE_DIR`)
- Workspace: `~/.alien/workspace` (or `~/.alien/workspace-<agentId>`)
- Agent dir: `~/.alien/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.alien/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Alien runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.alien/workspace` (or `~/.alien/workspace-<profile>` when `ALIEN_PROFILE` is set).
- State defaults to `~/.alien/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    alien agents add coding
    alien agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.alien/agents/<agentId>`.
  ...
  <Step title="Create channel accounts">
    ...
    ```bash
    alien channels login --channel whatsapp --account work
    ```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.alien/workspace-alex" },
      { id: "mia", workspace: "~/.alien/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Alien upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Alien falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.alien/workspace-main" },
          { id: "coding", workspace: "~/.alien/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.alien/workspace-main" },
          { id: "alerts", workspace: "~/.alien/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    alien channels login --channel whatsapp --account personal
    alien channels login --channel whatsapp --account biz
    ```

    `~/.alien/alien.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.alien/workspace-home",
            agentDir: "~/.alien/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.alien/workspace-work",
            agentDir: "~/.alien/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.alien/credentials/whatsapp/personal
              // authDir: "~/.alien/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.alien/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.alien/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.alien/workspace-opus",
