...
Alien has two separate streaming layers:
...
media block early, Alien remembers that delivery for the turn. If the final
...
distinct text around media that was already streamed, Alien still sends the
...
When block streaming is enabled, Alien can **merge consecutive block chunks**
...
- Discord: `streamMode` + boolean `streaming` remain runtime aliases for the `streaming` enum; run `alien doctor --fix` to rewrite persisted config.
- Slack: `streamMode` remains a runtime alias for `streaming.mode`; boolean `streaming` remains a runtime alias for `streaming.mode` plus `streaming.nativeTransport`; legacy `nativeStreaming` remains a runtime alias for `streaming.nativeTransport`. Run `alien doctor --fix` to rewrite persisted config.
