...
Paste this into `alien.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Alien can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Alien cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. Alien
...
5. Verify memory search itself works with `alien memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Alien auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `alien memory status --deep` to inspect the memory-search backend
      and index health.
