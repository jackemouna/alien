---
summary: "What experimental flags mean in Alien and which ones are currently documented"
...
Experimental features in Alien are **opt-in preview surfaces**. They are
...
- If you are rolling Alien out broadly, test experimental flags in a smaller
...
| ------------------------ | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Local model runtime      | `agents.defaults.experimental.localModelLean`             | A smaller or stricter local backend chokes on Alien's full default tool surface                             | [Local Models](/gateway/local-models)                                                         |
...
`agents.defaults.experimental.localModelLean: true` is a pressure-release valve for weaker local-model setups. When it is on, Alien drops three default tools — `browser`, `cron`, and `message` — from the agent's tool surface for every turn. Nothing else changes.

### Why these three tools

These three tools have the largest descriptions and the most parameter shapes in the default Alien runtime. On a small-context or stricter OpenAI-compatible backend that is the difference between:
...
Removing them does not silently rewire Alien — it just makes the tool list shorter. The model still has `read`, `write`, `edit`, `exec`, `apply_patch`, web search/fetch (when configured), memory, and session/agent tools available.
...
1. `alien infer model run --gateway --model <ref> --prompt "Reply with exactly: pong"` succeeds.
...
If a feature is experimental, Alien should say so plainly in docs and in the
