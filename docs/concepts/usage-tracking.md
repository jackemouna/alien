...
- `/usage cost` in chats: local cost summary aggregated from Alien session logs.
- CLI: `alien status --usage` prints a full per-provider breakdown.
- CLI: `alien channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Alien treats
  ...
  quota, so Alien inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Alien prefers the
...
can supply plugin-specific usage auth logic; otherwise Alien falls back to
