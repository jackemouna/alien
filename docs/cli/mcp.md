---
summary: "Expose Alien channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Alien-backed channels
  - Running `alien mcp serve`
  - Managing Alien-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`alien mcp` has two jobs:

- run Alien as an MCP server with `alien mcp serve`
- manage Alien-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is Alien acting as an MCP server
- `list` / `show` / `set` / `unset` is Alien acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`alien acp`](/cli/acp) when Alien should host a coding harness session itself and route that runtime through ACP.

## Alien as an MCP server

This is the `alien mcp serve` path.

### When to use `serve`

Use `alien mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Alien-backed channel conversations
- you already have a local or remote Alien Gateway with routed sessions
- you want one MCP server that works across Alien's channel backends instead of running separate per-channel bridges

Use [`alien acp`](/cli/acp) instead when Alien should host the coding runtime itself and keep the agent session inside Alien.

### How it works

`alien mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Alien Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `alien mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Alien Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `alien agent` and `alien infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Alien (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Alien already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    alien mcp serve --url wss://gateway-host:18789 --token-file ~/.alien/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    alien mcp serve --url wss://gateway-host:18789 --password-file ~/.alien/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    alien mcp serve --verbose
    alien mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Alien equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Alien channel configuration
...
Alien ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `alien mcp serve`
...
## Alien as an MCP client registry

This is the `alien mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Alien over MCP. They manage Alien-owned MCP server definitions under `mcp.servers` in Alien config.

Those saved definitions are for runtimes that Alien launches or configures later, such as embedded Pi and other runtime adapters. Alien stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Alien config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes Alien `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

### Saved MCP server definitions

Alien also stores a lightweight MCP server registry in config for surfaces that want Alien-managed MCP definitions.

Commands:

- `alien mcp list`
- `alien mcp show [name]`
- `alien mcp set <name> <json>`
- `alien mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `alien mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
alien mcp list
alien mcp show context7 --json
alien mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
alien mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
alien mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Alien uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |

Alien config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `alien mcp set` and repaired by `alien doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
