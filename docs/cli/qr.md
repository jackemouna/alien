---
summary: "CLI reference for `alien qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
alien qr
alien qr --setup-code-only
alien qr --json
alien qr --remote
alien qr --url wss://gateway.example/ws
```
...
- With `--remote`, Alien requires either `gateway.remote.url` or
