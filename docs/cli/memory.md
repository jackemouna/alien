---
summary: "CLI reference for `alien memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
alien memory status
alien memory status --deep
alien memory status --fix
alien memory index --force
alien memory search "meeting notes"
alien memory search --query "deployment" --max-results 20
alien memory promote --limit 10 --min-score 0.75
alien memory promote --apply
alien memory promote --json --min-recall-count 0 --min-unique-queries 0
alien memory promote-explain "router vlan"
alien memory promote-explain "router vlan" --json
alien memory rem-harness
alien memory rem-harness --json
alien memory status --json
alien memory status --deep --index
alien memory status --deep --index --verbose
alien memory status --agent main
alien memory index --agent main --verbose
```
...
```bash
alien memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `alien cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
alien memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
alien memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
