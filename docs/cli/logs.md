---
summary: "CLI reference for `alien logs` (tail gateway logs via RPC)"
read_when:
...
`alien logs` also accepts the standard Gateway client flags:
...
alien logs
alien logs --follow
alien logs --follow --interval 2000
alien logs --limit 500 --max-bytes 500000
alien logs --json
alien logs --plain
alien logs --no-color
alien logs --limit 500
alien logs --local-time
alien logs --follow --local-time
alien logs --url ws://127.0.0.1:18789 --token "$ALIEN_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `alien logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
