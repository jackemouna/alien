...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Alien Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`alien acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Alien channel
conversations instead of hosting an ACP harness session, use
[`alien mcp serve`](/cli/mcp) instead.
...
- Alien acts as an ACP server
- an IDE or ACP client connects to Alien
- Alien forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Alien runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Alien: use `alien acp`
- Alien should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Alien gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
alien acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
alien acp --url wss://gateway-host:18789 --token-file ~/.alien/gateway.token

# Attach to an existing session key
alien acp --session agent:main:main

# Attach by label (must already exist)
alien acp --session-label "support inbox"

# Reset the session key before the first prompt
alien acp --session agent:main:main --reset-session
```
...
alien acp client --server-args --url wss://gateway-host:18789 --token-file ~/.alien/gateway.token

# Override the server command (default: alien)
alien acp client --server "node" --server-args alien.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Alien through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Alien Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `alien acp` over stdio.

Example config (persisted):

```bash
alien config set gateway.remote.url wss://gateway-host:18789
alien config set gateway.remote.token <token>
```
...
```bash
alien acp --session agent:main:main
alien acp --session agent:design:main
alien acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Alien plugin tools or selected
...
[Alien tools MCP bridge](/tools/acp-agents-setup#alien-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Alien bot over ACP, use `acpx` with its built-in `alien` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx alien` at `alien acp`.
3. Target the Alien session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Alien ACP session
acpx alien exec "Summarize the active Alien session state."

# Persistent named session for follow-up turns
acpx alien sessions ensure --name codex-bridge
acpx alien -s codex-bridge --cwd /path/to/repo \
  "Ask my Alien work agent for recent context relevant to this repo."
```

If you want `acpx alien` to target a specific Gateway and session key every
time, override the `alien` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "alien": {
      "command": "env ALIEN_HIDE_BANNER=1 ALIEN_SUPPRESS_NOTES=1 alien acp --url ws://127.0.0.1:18789 --token-file ~/.alien/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Alien checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ALIEN_HIDE_BANNER=1 ALIEN_SUPPRESS_NOTES=1 node alien.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Alien agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Alien ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ALIEN_GATEWAY_TOKEN`, `ALIEN_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ALIEN_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ALIEN_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `alien acp client` sets `ALIEN_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `alien`).
