---
summary: "Alien CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `alien` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`alien` is the main CLI entry point. Each core command has either a
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.alien-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.alien-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`alien update`](/cli/update) (source installs only) |
...
```
alien [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
Plugins can add additional top-level commands (for example `alien voicecall`).
...
`alien status --usage` and the Control UI surface provider usage/quota when
