---
summary: "CLI reference for `alien dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: alien.internal)
title: "DNS"
---
...
```bash
alien dns setup
alien dns setup --domain alien.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `alien.internal`)
...
- zone file path
- current tailnet IPs
- recommended `alien.json` discovery config
...
- If `--domain` is omitted, Alien uses `discovery.wideArea.domain` from config.
