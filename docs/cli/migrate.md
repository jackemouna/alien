---
summary: "CLI reference for `alien migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Alien
...
```bash
alien migrate list
alien migrate claude --dry-run
alien migrate codex --dry-run
alien migrate codex --skill gog-vault77-google-workspace
alien migrate hermes --dry-run
alien migrate hermes
alien migrate apply codex --yes --skill gog-vault77-google-workspace
alien migrate apply codex --yes
alien migrate apply claude --yes
alien migrate apply hermes --yes
alien migrate apply hermes --include-secrets --yes
alien onboard --flow import
alien onboard --import-from claude --import-source ~/.claude
alien onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `alien migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Alien state exists.
</ParamField>
...
`alien migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `alien migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Alien backup before applying the migration. If no local Alien state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Alien agent workspace.
...
- Claude command Markdown files converted into Alien skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Alien does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Alien Codex harness and you want to
...
Running `alien migrate codex` in an interactive terminal previews the full
...
```bash
alien migrate codex --dry-run --skill gog-vault77-google-workspace
alien migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Alien agent workspace when you want per-agent ownership.
...
them into Alien. Config and hook files are copied into the migration report
for manual review.
...
- `SOUL.md` and `AGENTS.md` into the Alien agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Alien file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that Alien cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Alien config or credentials. This preserves opaque or unsafe state without pretending Alien can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `alien.plugin.json`:
...
Provider plugins can use `alien/plugin-sdk/migration` for item construction and summary counts, plus `alien/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `alien onboard --flow import` and `alien setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Alien setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Alien to a new machine.
