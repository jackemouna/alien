---
summary: "CLI reference for `alien security` (audit and fix common security footguns)"
read_when:
...
alien security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Alien is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `alien.browserConfigEpoch`) and recommends `alien sandbox recreate --browser --all`.
...
```bash
alien security audit --json | jq '.summary'
alien security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
alien security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files
  ...
  `*.jsonl`)
- also tightens config include files referenced from `alien.json`
