---
summary: "CLI reference for `alien models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
alien models status
alien models list
alien models set <model-or-alias>
alien models scan
```

`alien models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Alien falls back to matching OAuth/API-key
...
the command uses `ALIEN_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For Codex OAuth troubleshooting, `alien models status`,
`alien models auth list --provider openai-codex`, and
`alien config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, Alien resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Alien
...
By default Alien tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `ALIEN_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Alien could not resolve a probeable
...
```bash
alien models auth add
alien models auth list [--provider <id>] [--json]
alien models auth login --provider <id>
alien models auth setup-token --provider <id>
alien models auth paste-token
```
...
`alien plugins list` to see which providers are installed.
Use `alien models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
alien models auth login --provider openai-codex --set-default
...
- Anthropic note: Anthropic staff told us Alien-style Claude CLI usage is allowed again, so Alien treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Alien token path, but Alien now prefers Claude CLI reuse and `claude -p` when available.
