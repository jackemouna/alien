---
summary: "CLI reference for `alien health` (gateway health snapshot via RPC)"
read_when:
...
alien health
alien health --json
alien health --timeout 2500
alien health --verbose
alien health --debug
```

Notes:

- Default `alien health` asks the running gateway for its health snapshot. When the
