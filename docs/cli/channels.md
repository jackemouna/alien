---
summary: "CLI reference for `alien channels` (accounts, status, login/logout, logs)"
read_when:
...
alien channels capabilities
alien channels capabilities --channel discord --target channel:123
alien channels capabilities --channel discord --target channel:<voice-channel-id>
alien channels resolve --channel slack "#general" "@jane"
alien channels logs --channel all
```
...
Do not use `alien sessions`, Gateway `sessions.list`, or the agent
...
```bash
alien channels add --channel telegram --token <bot-token>
alien channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
alien channels remove --channel telegram --delete
```

<Tip>
`alien channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Alien uses the channel's default install source without opening the interactive plugin install prompt.

When you run `alien channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `alien agents bindings`, `alien agents bind`, and `alien agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Alien promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `alien status --deep` for a broad probe.
- Use `alien doctor` for guided fixes.
- `alien channels list` prints `Claude: HTTP 403 ... user:profile` → usage snapshot needs the `user:profile` scope. Use `--no-usage`, or provide a claude.ai session key (`CLAUDE_WEB_SESSION_KEY` / `CLAUDE_WEB_COOKIE`), or re-auth via Claude CLI.
- `alien channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
alien channels resolve --channel slack "#general" "@jane"
alien channels resolve --channel discord "My Server/#support" "@someone"
alien channels resolve --channel matrix "Project Room"
```
