---
summary: "CLI reference for `alien daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `alien daemon ...` in scripts
...
`alien daemon ...` maps to the same service control surface as `alien gateway ...` service commands.

## Usage

```bash
alien daemon status
alien daemon install
alien daemon start
alien daemon stop
alien daemon restart
alien daemon uninstall
```
...
Use [`alien gateway`](/cli/gateway) for current docs and examples.
