---
summary: "CLI reference for `alien secrets` (reload, audit, configure, apply)"
read_when:
...
Use `alien secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
alien secrets reload
alien secrets reload --json
alien secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Alien state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `alien.json` refs)
...
alien secrets audit
alien secrets audit --check
alien secrets audit --json
alien secrets audit --allow-exec
```
...
```bash
alien secrets configure
alien secrets configure --plan-out /tmp/alien-secrets-plan.json
alien secrets configure --apply --yes
alien secrets configure --providers-only
alien secrets configure --skip-provider-setup
alien secrets configure --agent ops
alien secrets configure --json
```
...
- `configure` targets secret-bearing fields in `alien.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Alien fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
alien secrets apply --from /tmp/alien-secrets-plan.json --json
```
...
- `alien.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.alien/.env` known secret keys whose values were migrated
