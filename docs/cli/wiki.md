---
summary: "CLI reference for `alien wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `alien wiki`
title: "Wiki"
---
...
Use `alien wiki` when you want a compiled knowledge vault with:
...
alien wiki doctor
alien wiki init
alien wiki ingest ./notes/alpha.md
alien wiki compile
alien wiki lint
alien wiki search "alpha"
alien wiki search "who should I ask about Teams?" --mode route-question
alien wiki get entity.alpha --from 1 --lines 80

alien wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

alien wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

alien wiki bridge import
alien wiki unsafe-local import

alien wiki obsidian status
alien wiki obsidian search "alpha"
alien wiki obsidian open syntheses/alpha-summary.md
alien wiki obsidian command workspace:quick-switcher
alien wiki obsidian daily
```
...
- `.alien-wiki/cache/agent-digest.json`
- `.alien-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `alien memory search` when the
...
```bash
alien wiki search "bgroux" --mode find-person
alien wiki search "who knows Teams rollout?" --mode route-question
alien wiki search "maintainer-whois" --mode source-evidence
alien wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
alien wiki get entity.alpha
alien wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`alien wiki` behavior is shaped by:
