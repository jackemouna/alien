---
summary: "CLI reference for `alien pairing` (approve/list pairing requests)"
read_when:
...
alien pairing list --channel telegram --account work
alien pairing list telegram --json

alien pairing approve <code>
alien pairing approve telegram <code>
alien pairing approve --channel telegram --account work <code> --notify
```
...
- `alien pairing approve --channel <channel> <code>`
- `alien pairing approve <code>` when exactly one pairing-capable channel is configured
...
- If `commands.ownerAllowFrom` is empty when you approve a pairing code, Alien also records the approved sender as the command owner, using a channel-scoped entry such as `telegram:123456789`.
...
- If you approved a sender before this bootstrap existed, run `alien doctor`; it warns when no command owner is configured and shows the `alien config set commands.ownerAllowFrom ...` command to fix it.
