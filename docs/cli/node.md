---
summary: "CLI reference for `alien node` (headless node host)"
read_when:
...
`alien node run` and `alien node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `ALIEN_GATEWAY_TOKEN` / `ALIEN_GATEWAY_PASSWORD` are checked first.
...
network, set `ALIEN_ALLOW_INSECURE_PRIVATE_WS=1`. Without it, node startup
fails closed and asks you to use `wss://`, an SSH tunnel, or Tailscale.
This is a process-environment opt-in, not an `alien.json` config key.
`alien node install` persists it into the supervised node service when it is
...
```bash
alien node install --host <gateway-host> --port 18789
```
...
alien node stop
alien node restart
alien node uninstall
```

Use `alien node run` for a foreground node host (no service).
...
- `alien approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Alien prepares a canonical `systemRunPlan`
