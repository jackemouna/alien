---
summary: "Run Alien on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Alien
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Alien Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Alien on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Alien bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/alien-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `ALIEN_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `ALIEN_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/alien-compile-cache`
...
For the standard `alien onboard --install-daemon` path, edit the user unit:
...
`alien-gateway.service` via `sudo systemctl edit alien-gateway.service`.
