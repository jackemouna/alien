...
read_when:
  - You are building an Alien plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Alien plugin manifest** only.
...
Alien auto-detects those bundle layouts too, but they are not validated
against the `alien.plugin.json` schema described here.

For compatible bundles, Alien currently reads bundle metadata plus declared
...
Alien runtime expectations.

Every native Alien plugin **must** ship a `alien.plugin.json` file in the
**plugin root**. Alien uses this manifest to validate configuration
...
`alien.plugin.json` is the metadata Alien reads **before it loads your
...
- QA runner metadata the shared `alien qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Alien still reads this during the deprecation window.                                                 |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                          |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Alien can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                            |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `alien qa` host before plugin runtime loads.                                                                                                                                      |
...
Alien reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Alien can
...
If a tool has no `toolMetadata`, Alien preserves the existing behavior and
...
Alien reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Alien falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Alien
...
the shared `alien qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `alien qa`, for example `matrix`.    |
...
Alien also includes `setup.providers[].envVars` in generic provider auth and
...
Alien can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Alien treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `alien.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Alien reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Alien can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `alien migrate`.          |
...
Alien reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Alien can still load the plugin, but
...
the same defaults through `package.json#alien.channel.commands` alongside
...
When `channels.chat` is configured, Alien considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Alien disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Alien
...
Use `modelSupport` when Alien should infer your provider plugin from
...
Use `modelCatalog` when Alien should know provider model metadata before
...
provider-filtered list uses an alias, Alien can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Alien model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Alien
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Alien provider id, for example `z-ai` for a `zai` provider. |
...
### Alien Provider Index

The Alien Provider Index is Alien-owned preview metadata for providers
...
4. Alien Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `alien doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `alien.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `alien` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Alien must know it before loading plugin code, put it in `alien.plugin.json`
...
`alien` block instead of `alien.plugin.json`.
`alien.bundle` and `alien.bundle.json` are not Alien plugin contracts;
native plugins must use `alien.plugin.json` plus the supported
`package.json#alien` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `alien.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `alien.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `alien.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `alien.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `alien.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `alien.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `alien.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `alien.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `alien.install.clawhubSpec` / `alien.install.npmSpec` / `alien.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `alien.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `alien.install.minHostVersion`                                                          | Minimum supported Alien host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `alien.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `alien.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `alien.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#alien.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `alien.plugin.json`.

`alien.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-alien-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `alien.setupEntry` when status, channel list,
...
entrypoint fields. For example, `alien.runtimeExtensions` cannot make an
escaping `alien.extensions` path loadable.

`alien.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`alien.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`alien.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Alien discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.

Precedence, highest to lowest:

1. **Config-selected** — a path explicitly pinned in `plugins.entries.<id>`
2. **Bundled** — plugins shipped with Alien
3. **Global install** — plugins installed into the global Alien plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `alien.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Alien plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `AlienPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
