...
An **agent harness** is the low level executor for one prepared Alien agent
...
runtime and the normal Alien provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Alien
  session transcript
...
Before a harness is selected, Alien has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Alien transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Alien-owned
...
**Import:** `alien/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "alien/plugin-sdk/agent-harness";
...
Alien chooses a harness after provider/model resolution:
...
2. `ALIEN_AGENT_RUNTIME=<id>` forces a registered harness with that id for
   sessions that are not already pinned.
3. `ALIEN_AGENT_RUNTIME=pi` forces the built-in PI harness.
4. `ALIEN_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
5. If no registered harness matches, Alien uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, Alien does not
...
Alien. The harness then claims that provider in `supports(...)`.
...
- app-server request: Alien sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs continue to use the
normal Alien provider path unless you force the Codex harness with
...
Alien requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Alien only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Alien to the newer tested stable line.
...
`alien/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Alien's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded Alien
...
compaction, and app-server execution. Alien still owns the chat channel,
...
By default, Alien runs embedded agents with Alien Pi. In `auto` mode,
...
`ALIEN_AGENT_RUNTIME` still overrides the configured runtime.
...
Keep that binding explicitly associated with the Alien session, and keep
mirroring user-visible assistant/tool output into the Alien transcript.

The Alien transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Alien can
clear it when the owning Alien session is reset.

## Tool and media results

Core constructs the Alien tool list and passes it into the prepared attempt.
