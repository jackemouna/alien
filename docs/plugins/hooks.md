...
Plugin hooks are in-process extension points for Alien plugins. Use them
...
current plugin options; Alien injects it per handler without mutating the
...
- Alien strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, Alien stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when Alien can
...
identifier when Alien can derive one from the session key or delivery
metadata.
...
Alien continues; it does not cancel plugin-owned network work unless the
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Alien can derive a
...
Codex native `Stop` hooks are relayed into this hook as Alien
...
reach the next model turn exactly once. Alien drains queued injections before
...
context when syncing external wake schedulers, and keep Alien as the
