...
read_when:
  - You see the ALIEN_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ALIEN_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before Alien 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Alien plugin
---

Alien has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`alien/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`alien/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`alien/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`alien/extension-api`** - a bridge that gave plugins direct access to
...
Alien does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`alien/plugin-sdk/\<subpath\>`)
...
`alien/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `alien doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `alien/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `AlienConfig` | `alien/plugin-sdk/config-types` |
    | Already-loaded config assertions and plugin-entry config lookup | `alien/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `alien/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `alien/plugin-sdk/config-mutation` |
    | Session store helpers | `alien/plugin-sdk/session-store-runtime` |
    | Markdown table config | `alien/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `alien/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `alien/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `alien/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `alien/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `alien/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "alien/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "alien/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "alien/plugin-sdk/channel-reply-pipeline";
    ...
    import { runEmbeddedPiAgent } from "alien/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `alien/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `alien/plugin-sdk/system-event-runtime` |
    | Heartbeat event and visibility helpers | `alien/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `alien/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `alien/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `alien/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `alien/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `alien/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `alien/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `alien/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `alien/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `alien/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `alien/plugin-sdk/error-runtime` |
    | Transport readiness waits | `alien/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `alien/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `alien/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `alien/plugin-sdk/number-runtime` |
    | Process-local async lock | `alien/plugin-sdk/async-lock-runtime` |
    | File locks | `alien/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `alien/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `AlienSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | Alien-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@alien/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`alien/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`alien/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "alien/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "alien/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `alien/plugin-sdk/channel-inbound` or
    `alien/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `alien/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `alien/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `alien/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `alien/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Alien no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Alien downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="AlienSchemaType alias → AlienConfig">
    `AlienSchemaType` re-exported from `alien/plugin-sdk` is now a
    one-line alias for `AlienConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { AlienSchemaType } from "alien/plugin-sdk";
    // After
    import type { AlienConfig } from "alien/plugin-sdk/config-schema";
    ```
...
```bash
ALIEN_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 alien gateway run
ALIEN_SUPPRESS_EXTENSION_API_WARNING=1 alien gateway run
```
