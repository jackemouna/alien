---
summary: "Adds Amazon Bedrock Mantle model provider support to Alien."
read_when:
  - You are installing, configuring, or auditing the amazon-bedrock-mantle plugin
title: "Amazon Bedrock Mantle plugin"
---

# Amazon Bedrock Mantle plugin

Adds Amazon Bedrock Mantle model provider support to Alien.

## Distribution

- Package: `@alien/amazon-bedrock-mantle-provider`
- Install route: included in Alien

## Surface

providers: amazon-bedrock-mantle

## Related docs

- [amazon-bedrock-mantle](/providers/bedrock-mantle)
