---
summary: "Set up Codex Computer Use for Codex-mode Alien agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Alien agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Alien
...
Use this page when Alien is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Alien.app and Peekaboo

Alien.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Alien.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Alien agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Alien node and exposes mobile
...
Alien-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Alien's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Alien command:

```bash
cua-driver mcp-config --client alien
```

or register the stdio server yourself:

```bash
alien mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal Alien MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Alien
...
With this config, Alien checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Alien asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Alien also tries to
...
plugin command surface is available. These are Alien chat/runtime commands,
not `alien codex ...` CLI subcommands:
...
Alien uses the same app-server API that Codex itself exposes. The
...
During install, Alien polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Alien prefers
...
`computer-use` is registered, Alien tries to add the standard bundled
...
Alien reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Alien says Computer Use
...
Alien intentionally fails closed when `computerUse.enabled` is true. A
...
tool hook could not reach an active Alien relay through the local bridge or
Gateway fallback. Start a fresh Alien session with `/new` or `/reset`. If it
