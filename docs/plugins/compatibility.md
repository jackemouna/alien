...
read_when:
  - You maintain an Alien plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Alien keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core Alien repo as a separate
...
```sh
alien-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. Alien
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `alien` package.

### Maintainer acceptance lane

Use Blacksmith Testbox for the installable-package acceptance lane when validating
the external inspector against Alien plugin packages. Run it from a clean
Alien checkout after the package is built:

```sh
blacksmith testbox warmup ci-check-testbox.yml --ref main --idle-timeout 90
blacksmith testbox run --id <tbx_id> "pnpm install && pnpm build && npm exec --yes @alien/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
blacksmith testbox run --id <tbx_id> "npm exec --yes @alien/plugin-inspector@0.1.0 -- ./extensions/discord --json"
blacksmith testbox run --id <tbx_id> "npm exec --yes @alien/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
...
Alien should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `alien/plugin-sdk/compat`
- legacy hook-only plugin shapes and `before_agent_start`
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `alien/extension-api`,
  `alien/plugin-sdk/channel-runtime`, `alien/plugin-sdk/command-auth`
  status builders, `alien/plugin-sdk/test-utils` (replaced by focused
  `alien/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `AlienSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `alien/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `alien plugins registry --refresh` and
  `alien doctor --fix`
