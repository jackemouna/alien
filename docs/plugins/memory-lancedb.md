...
Set `dimensions` for non-standard embedding models. Alien knows the
...
`memory_forget` query path, and `alien ltm search`. Auto-recall prefers the
...
```bash
alien ltm list
alien ltm search "project preferences"
alien ltm stats
```

The plugin also extends `alien memory` with a non-vector `query` subcommand
that runs against the LanceDB table directly:

```bash
alien memory query --cols id,text,createdAt --limit 20
alien memory query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.alien/memory/lancedb`. Override the
path with `dbPath`:
...
Alien treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Alien and restart the
Gateway.
...
```bash
alien ltm stats
alien ltm search "recent preference"
```
