---
summary: "Step-by-step guide to building a messaging channel plugin for Alien"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Alien to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Alien to a
...
<Info>
  If you have not built any Alien plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Alien keeps one
...
`defineChannelMessageAdapter` from `alien/plugin-sdk/channel-message`. The
...
receive/send lifecycle helpers on `alien/plugin-sdk/channel-message`.
...
Use `alien/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `alien/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `alien/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `alien/plugin-sdk/approval-auth-runtime`
- `alien/plugin-sdk/approval-client-runtime`
- `alien/plugin-sdk/approval-delivery-runtime`
- `alien/plugin-sdk/approval-gateway-runtime`
- `alien/plugin-sdk/approval-handler-adapter-runtime`
- `alien/plugin-sdk/approval-handler-runtime`
- `alien/plugin-sdk/approval-native-runtime`
- `alien/plugin-sdk/approval-reply-runtime`
- `alien/plugin-sdk/channel-runtime-context`

Likewise, prefer `alien/plugin-sdk/setup-runtime`,
`alien/plugin-sdk/setup-adapter-runtime`,
`alien/plugin-sdk/reply-runtime`,
`alien/plugin-sdk/reply-dispatch-runtime`,
`alien/plugin-sdk/reply-reference`, and
`alien/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `alien/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `alien/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `alien/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `alien.setupEntry` in
...
- use the broader `alien/plugin-sdk/setup` seam only when you also need the
...
- `alien/plugin-sdk/account-core`,
  `alien/plugin-sdk/account-id`,
  `alien/plugin-sdk/account-resolution`, and
  `alien/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `alien/plugin-sdk/inbound-envelope` and
  `alien/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `alien/plugin-sdk/messaging-targets` for target parsing/matching
- `alien/plugin-sdk/outbound-media` and
  `alien/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `alien/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `alien/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `alien/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `alien/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `alien/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `alien/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`alien/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`alien/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#alien-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/alien-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "alien": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Alien to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { AlienConfig } from "alien/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "alien/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Alien
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "alien/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Alien loads this instead of the full entry when the channel is disabled
    ...
    `alien/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Alien. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Alien.
...
├── package.json              # alien.channel metadata
├── alien.plugin.json      # Manifest with config schema
