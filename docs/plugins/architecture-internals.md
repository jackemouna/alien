...
At startup, Alien does roughly this:
...
The manifest is the control-plane source of truth. Alien uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Alien
...
Alien does not cache plugin discovery results or direct manifest registry
...
Alien still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Alien calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Alien no longer calls, such as
...
| --  | _(built-in model lookup)_         | Alien tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on Alien's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Alien's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Alien adds those
...
- Alien only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Alien plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-alien-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-alien-scopes` only when the header is explicitly present
  - if `x-alien-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-alien-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `alien/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `alien/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `alien/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `alien/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `alien/plugin-sdk/config-schema` | Root `alien.json` Zod schema (`AlienSchema`) |
...
<Info>
`alien/plugin-sdk/channel-runtime`, `alien/plugin-sdk/config-runtime`,
and `alien/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `alien/plugin-sdk/*` subpaths. Never
...
`alien/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Alien writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Alien's
...
- if both `catalog` and `discovery` are registered, Alien uses `catalog`
...
- Read-only command paths such as `alien status`, `alien status --all`,
  `alien channels status`, `alien channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `alien.extensions`:
...
Security guardrail: every `alien.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `alien plugins install` installs plugin dependencies with a
...
Optional: `alien.setupEntry` can point at a lightweight setup-only module.
When Alien needs setup surfaces for a disabled channel plugin, or
...
Optional: `alien.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Alien load the
...
Channel plugins can advertise setup/discovery metadata via `alien.channel` and
install hints via `alien.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@alien/nextcloud-talk",
  "alien": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `alien.channel` fields beyond the minimal example:
...
Alien can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.alien/mpm/plugins.json`
- `~/.alien/mpm/catalog.json`
- `~/.alien/plugins/catalog.json`

Or point `ALIEN_PLUGIN_CATALOG_PATHS` (or `ALIEN_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "alien": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `alien.install` block. The
...
2. add typed plugin registration/runtime surfaces
   Extend `AlienPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Alien stays opinionated without becoming hardcoded to one
