...
Message presentation is Alien's shared contract for rich outbound chat UI.
...
```bash
alien message send --channel slack \
  --target channel:C123 \
...
```bash
alien message send --channel telegram \
...
Use helpers from `alien/plugin-sdk/interactive-runtime` when bridging older
code:
