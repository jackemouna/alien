...
The plugin SDK is exposed as a set of narrow subpaths under `alien/plugin-sdk/`.
...
| `plugin-sdk/config-schema`                | `AlienSchema`                                                                                                                                                             |
...
| `plugin-sdk/plugin-test-api`              | Minimal `AlienPluginApi` mock builder for direct plugin registration unit tests                                                                                           |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `alien.json` Zod schema export (`AlienSchema`) |
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Bundled Alien channel config schemas for maintained bundled plugins only |
    ...
    | `plugin-sdk/discord` | Deprecated Discord compatibility facade for published `@alien/discord@2026.3.13` and tracked owner compatibility; new plugins should use generic channel SDK subpaths |
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-auth` | `createProviderApiKeyAuthMethod`, `ensureApiKeyFromOptionEnvOrPrompt`, `upsertAuthProfile`, `upsertApiKeyProfile`, `writeOAuthCredentials`, deprecated `resolveAlienAgentDir` compatibility export |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/config-types` | Type-only config surface for plugin config shapes such as `AlienConfig` and channel/provider config types |
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Alien tool bridge helpers, runtime-plan tool policy helpers, terminal outcome classification, tool progress formatting/detail helpers, and attempt result utilities |
    ...
    | `plugin-sdk/agent-runtime` | Agent dir/identity/workspace helpers, including `resolveAgentDir`, `resolveDefaultAgentDir`, and deprecated `resolveAlienAgentDir` compatibility export |
