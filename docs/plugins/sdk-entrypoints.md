...
when Alien loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Alien will use a
...
**Import:** `alien/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `AlienPluginConfigSchema \| () => AlienPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: AlienPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `alien.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Alien resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `alien/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: AlienPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: AlienPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Alien may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Alien
...
Alien loads this instead of the full entry when a channel is disabled,
...
- `alien/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `alien/plugin-sdk/channel-setup` for optional-install setup surfaces
- `alien/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`alien/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "alien/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so Alien can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Alien to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Alien rejects descriptor names outside
...
Alien classifies loaded plugins by their registration behavior:
...
Use `alien plugins inspect <id>` to see a plugin's shape.
