...
A sane workflow for working on the Pi integration in Alien.
...
```bash
ALIEN_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm alien agent --message "Hello" --thinking low`
...
State lives under the Alien state directory. Default is `~/.alien`. If `ALIEN_STATE_DIR` is set, use that directory instead.

To reset everything:

- `alien.json` for config
