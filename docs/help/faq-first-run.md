...
read_when:
  ...
  - Cannot access docs.alien.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://alien.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Alien **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/alien/alien/issues](https://github.com/alien/alien/issues)
    [https://github.com/alien/alien/pulls](https://github.com/alien/alien/pulls)
    ...
    - `alien status`: quick snapshot of gateway/agent health + basic config.
    - `alien models status`: checks provider auth + model availability.
    - `alien doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `alien status --all`, `alien logs --follow`,
    `alien gateway status`, `alien health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Alien">
    ...
    If you don't have a global install yet, run it via `pnpm alien onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `ALIEN_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `ALIEN_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `alien doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `alien gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `alien gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Alien now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Alien on the new machine.
    2. Copy `$ALIEN_STATE_DIR` (default: `~/.alien`) from the old machine.
    3. Copy your workspace (default: `~/.alien/workspace`).
    4. Run `alien doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.alien/` (for example `~/.alien/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/alien/alien/blob/main/CHANGELOG.md](https://github.com/alien/alien/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.alien.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.alien.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.alien.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/alien/alien/tree/main/docs](https://github.com/alien/alien/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alien.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alien.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://alien.ai/install.ps1](https://alien.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://alien.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://alien.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://alien.ai/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://alien.ai/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or alien not recognized">
    ...
    **2) alien is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Alien, track/report it in:

    - [Issue #30640](https://github.com/alien/alien/issues/30640)
  ...
  <Accordion title="How do I install Alien on Linux?">
  ...
  <Accordion title="How do I install Alien on a VPS?">
  ...
  <Accordion title="Can I ask Alien to update itself?">
    ...
    alien update
    alien update status
    alien update --channel stable|beta|dev
    alien update --tag <dist-tag|version>
  ...
  <Accordion title="What does onboarding actually do?">
    `alien onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Alien with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Alien, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Alien**: Anthropic staff
      told us this usage is allowed again, and Alien is treating `claude -p`
    ...
    Alien also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Alien-style Claude CLI usage is allowed again, so
    Alien treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Alien treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Alien token path, but Alien now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Alien, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Alien Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Alien can keep replying while a provider is rate-limited.
  ...
  <Accordion title="Why does Alien still mention openai-codex?">
    ...
    `alien models auth login --provider openai-codex`. For native Codex
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Alien can show the currently visible provider usage/quota windows in
    `alien models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Alien fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Alien. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `alien.json`.
    ...
    2. Enable the plugin: `alien plugins enable google`
    3. Login: `alien models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Alien needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Alien runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Alien, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `alien nodes status` / `alien nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `alien doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `alien logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Alien instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Alien via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes. Use `alien update --channel ...` when Alien is already installed.
    This **does not delete your data** - it only changes the Alien code install.
    Your state (`~/.alien`) and workspace (`~/.alien/workspace`) stay untouched.
    ...
    ```bash
    curl -fsSL https://alien.ai/install.sh | bash -s -- --install-method git
    curl -fsSL https://alien.ai/install.sh | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Alien-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Alien on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Alien is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Alien in a VM and what are the requirements?">
