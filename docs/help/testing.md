...
Alien has three Vitest suites (unit/integration, e2e, live) and a small set
...
- [Matrix QA](/concepts/qa-matrix) - reference for `pnpm alien qa matrix`.
...
- Linux VM-backed QA lane: `pnpm alien qa suite --runner multipass --scenario channel-chat-baseline`
...
- Runtime performance reports: dispatch `Alien Performance` with
  ...
  `alien/clawgrit-reports` when `CLAWGRIT_REPORTS_TOKEN` is configured. The
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `ALIEN_LIVE_MODEL_FILE_PROBE=0` or
    `ALIEN_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Alien Scheduled Live And E2E Checks` and manual
    `Alien Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `Alien Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/alien-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server harness,
    ...
    `ALIEN_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when isolating other Codex
    app-server failures. For a focused sub-agent check, disable the other probes:
    `ALIEN_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 ALIEN_LIVE_CODEX_HARNESS_MCP_PROBE=0 ALIEN_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 ALIEN_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `ALIEN_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty Alien state dir, routes bare `alien` to
...
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `alien models list --provider moonshot --json`, then run an isolated
  `alien agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`media`, `e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `Alien Release
...
`ghcr.io/alien/alien-live-media-runner:ubuntu-24.04`, which already has
`ffmpeg` and `ffprobe`. Docker live model/backend shards use the shared
`ghcr.io/alien/alien-live-test:<sha>` image built once per selected
commit, then pull it with `ALIEN_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab. It
    ...
    `alien-testbox-env` helper is present.
...
- `pnpm alien qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `ALIEN_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. It verifies hidden Alien runtime context is persisted as a
    ...
    `alien doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an Alien package candidate in Docker, runs installed-package
  ...
  - Defaults to `ALIEN_NPM_TELEGRAM_PACKAGE_SPEC=alien@beta`; set
    `ALIEN_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/alien-current.tgz` or
    `ALIEN_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead of
    installing from the registry.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm alien qa telegram`. For CI/release automation, set
    `ALIEN_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `ALIEN_QA_CONVEX_SITE_URL` and the role secret. If
    `ALIEN_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
  ...
  - The wrapper validates Telegram or Convex credential env on the host before
    Docker build/install work. Set `ALIEN_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1`
    only when deliberately debugging pre-credential setup.
  - `ALIEN_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `ALIEN_QA_CREDENTIAL_ROLE` for this lane only.
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  the normalized `alien-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/alien/-/alien-VERSION.tgz \
...
- `pnpm test:docker:plugins`
  - Packs and installs the current Alien build in Docker, starts the Gateway
  ...
  - Also installs a known older npm baseline, enables Telegram before running
    `alien update --tag <candidate>`, and verifies the candidate's
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests. Each
    ...
    the installed `alien update` command in the same guest and verifies the
  ...
  - The OpenAI lane uses `openai/gpt-5.5` for the live agent-turn proof by
    ...
    `ALIEN_PARALLELS_OPENAI_MODEL` when deliberately validating another
    OpenAI model.
  ...
  - The script writes nested lane logs under `/tmp/alien-parallels-npm-update.*`.
...
- `pnpm alien qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `ALIEN_QA_TELEGRAM_GROUP_ID`, `ALIEN_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `ALIEN_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `ALIEN_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
  - Exits non-zero when any scenario fails. Use `--allow-failures` when you
    want artifacts without a failing exit code.
...
When `--credential-source convex` (or `ALIEN_QA_CREDENTIAL_SOURCE=convex`) is enabled for
`alien qa telegram`, QA lab acquires an exclusive lease from a Convex-backed pool, heartbeats
...
- `ALIEN_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `ALIEN_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `ALIEN_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `ALIEN_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `ALIEN_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `ALIEN_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `ALIEN_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `ALIEN_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `ALIEN_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `ALIEN_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `ALIEN_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`ALIEN_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`ALIEN_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm alien qa credentials doctor
pnpm alien qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm alien qa credentials list --kind telegram
pnpm alien qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`ALIEN_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `ALIEN_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-alien-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `ALIEN_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `ALIEN_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `ALIEN_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `ALIEN_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `ALIEN_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `ALIEN_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Alien's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `ALIEN_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `ALIEN_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `ALIEN_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.alien`.
- Set `ALIEN_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `ALIEN_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `ALIEN_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `ALIEN_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `ALIEN_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `ALIEN_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `ALIEN_LIVE_GATEWAY_SMOKE=1`,
  `ALIEN_LIVE_GATEWAY_MAX_MODELS=8`,
  `ALIEN_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `ALIEN_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
  explicitly want the larger exhaustive scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs Alien once as an npm tarball through `scripts/package-alien-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `ALIEN_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `ALIEN_DOCKER_ALL_LIVE_LIMIT=9`, `ALIEN_DOCKER_ALL_NPM_LIMIT=10`, and `ALIEN_DOCKER_ALL_SERVICE_LIMIT=7`; tune `ALIEN_DOCKER_ALL_WEIGHT_LIMIT` or `ALIEN_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Alien E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `ALIEN_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Alien tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `ALIEN_CURRENT_PACKAGE_TGZ=/path/to/alien-*.tgz`, skip the host rebuild with `ALIEN_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `ALIEN_NPM_ONBOARD_CHANNEL=discord` or `ALIEN_NPM_ONBOARD_CHANNEL=slack`.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed Alien tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
- Upgrade survivor smoke: `pnpm test:docker:upgrade-survivor` installs the packed Alien tarball over a dirty old-user fixture with agents, channel config, plugin allowlists, stale plugin dependency state, and existing workspace/session files. It runs package update plus non-interactive doctor without live provider or channel keys, then starts a loopback Gateway and checks config/state preservation plus startup/status budgets.
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `alien@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `ALIEN_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `ALIEN_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `alien@2026.5.2 alien@2026.4.23 alien@2026.4.15`, and expand issue-shaped fixtures with `ALIEN_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external Alien plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation expands the release-soak package gate to `last-stable-4 2026.4.23 2026.5.2 2026.4.15` plus `reported-issues`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `alien infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `ALIEN_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/alien-*.tgz`, skip the host build with `ALIEN_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `ALIEN_BUN_GLOBAL_SMOKE_DIST_IMAGE=alien-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `ALIEN_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `ALIEN_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `ALIEN_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `ALIEN_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=alien-dockerfile-smoke:local ALIEN_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `ALIEN_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `ALIEN_PLUGINS_E2E_CLAWHUB_SPEC` and `ALIEN_PLUGINS_E2E_CLAWHUB_ID`. Without `ALIEN_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed Alien tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
```bash
ALIEN_DOCKER_E2E_IMAGE=alien-docker-e2e-functional:local pnpm test:docker:e2e-build
ALIEN_DOCKER_E2E_IMAGE=alien-docker-e2e-functional:local ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `ALIEN_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `ALIEN_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__alien_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `ALIEN_SKIP_CHANNELS=1` so gateway live probes do not start
...
`ALIEN_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Alien gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `alien/default`, then sends a
...
This lane expects a usable live model key, and `ALIEN_PROFILE_FILE`
...
container, starts a second container that spawns `alien mcp serve`, then
...
- `ALIEN_CONFIG_DIR=...` (default: `~/.alien`) mounted to `/home/node/.alien`
- `ALIEN_WORKSPACE_DIR=...` (default: `~/.alien/workspace`) mounted to `/home/node/.alien/workspace`
- `ALIEN_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `ALIEN_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `ALIEN_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `ALIEN_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/alien/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `ALIEN_LIVE_PROVIDERS` / `ALIEN_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `ALIEN_DOCKER_AUTH_DIRS=all`, `ALIEN_DOCKER_AUTH_DIRS=none`, or a comma list like `ALIEN_DOCKER_AUTH_DIRS=.claude,.codex`
- `ALIEN_LIVE_GATEWAY_MODELS=...` / `ALIEN_LIVE_MODELS=...` to narrow the run
- `ALIEN_LIVE_GATEWAY_PROVIDERS=...` / `ALIEN_LIVE_PROVIDERS=...` to filter providers in-container
- `ALIEN_SKIP_DOCKER_BUILD=1` to reuse an existing `alien:local-live` image for reruns that do not need a rebuild
- `ALIEN_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `ALIEN_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `ALIEN_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
