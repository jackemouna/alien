...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Alien's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.5` or `openai-codex/gpt-5.5`). If you omit the provider, Alien first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Alien falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `alien models set ...` (updates just model config)
    - `alien configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.alien/alien.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `alien doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `alien onboard` and choose `Ollama`
    ...
    - for manual switching, use `alien models list` and `alien models set ollama/<model>`
  ...
  <Accordion title="What do Alien, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `alien models status`.
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Native Codex coding agent:** set `agents.defaults.model.primary` to `openai/gpt-5.5` and `agents.defaults.agentRuntime.id` to `"codex"`. Sign in with `alien models auth login --provider openai-codex` when you want ChatGPT/Codex subscription auth.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Alien release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Alien ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.alien/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `alien agents add <id>` and configure auth during the wizard.
    - Or copy only portable static `api_key` / `token` profiles from the main agent's auth store into the new agent's auth store.
    - For OAuth profiles, sign in from the new agent when it needs its own account; otherwise Alien can read through to the default/main agent without cloning refresh tokens.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Alien can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. Alien
    ...
    explicit billing text on `401` or `403`, Alien can still keep that in
    ...
    `organization spending limit exceeded`), Alien treats it as
    ...
    unknown/error in it". Alien does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.alien/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.alien/agent/*` (migrated by `alien doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.alien/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `alien models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `alien models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.alien/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        alien models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Alien will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Alien now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    To inspect saved profiles without dumping secrets, run `alien models auth list` (optionally `--provider <id>` or `--json`). See [Models CLI](/cli/models#auth-profiles) for details.

  </Accordion>

  <Accordion title="What are typical profile IDs?">
    Alien uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Alien may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `alien models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    alien models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    alien models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    alien models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    alien models auth order set --provider anthropic --agent main anthropic:default
    ```

    To verify what will actually be tried, use:

    ```bash
    alien models status --probe
    ```
