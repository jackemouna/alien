...
This is handy when you need to toggle obscure settings without editing `alien.json`.
...
Use `ALIEN_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
ALIEN_PLUGIN_LIFECYCLE_TRACE=1 alien plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm alien ...`
...
`ALIEN_RUN_NODE_CPU_PROF_DIR`:

```bash
ALIEN_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm alien status
```
...
```bash
ALIEN_TRACE_SYNC_IO=1 pnpm alien gateway --force
```

`pnpm gateway:watch` enables this flag by default for the watched Gateway child.
Set `ALIEN_TRACE_SYNC_IO=0` to suppress Node sync I/O trace output in watch
mode.
...
`alien-gateway-watch-main` (or a profile/port-specific variant such as
`alien-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t alien-gateway-watch-main
```
...
# or
ALIEN_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
ALIEN_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`ALIEN_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`ALIEN_PROFILE`, `ALIEN_CONFIG_PATH`, `ALIEN_STATE_DIR`,
`ALIEN_GATEWAY_PORT`, and `ALIEN_SKIP_CHANNELS` into the pane. Put
...
`alien doctor --fix --non-interactive` once and restarts the Gateway child.
Use `ALIEN_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `alien.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.alien-dev` and
...
```bash
pnpm gateway:dev
ALIEN_PROFILE=dev alien tui
```

If you don't have a global install yet, run the CLI via `pnpm alien ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `ALIEN_PROFILE=dev`
   - `ALIEN_STATE_DIR=~/.alien-dev`
   - `ALIEN_CONFIG_PATH=~/.alien-dev/alien.json`
   - `ALIEN_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`ALIEN_SKIP_CHANNELS=1`).
...
```bash
ALIEN_PROFILE=dev alien gateway --dev --reset
```
...
## Raw stream logging (Alien)

Alien can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.alien/logs/raw-stream.jsonl
```
...
```bash
ALIEN_RAW_STREAM=1
ALIEN_RAW_STREAM_PATH=~/.alien/logs/raw-stream.jsonl
```

Default file:

`~/.alien/logs/raw-stream.jsonl`
...
- If you need to use the built Alien CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./alien.mjs` or create a shell alias like `alias alien-build="node $(pwd)/alien.mjs"`
