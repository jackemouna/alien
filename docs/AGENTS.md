...
- Docs are hosted on Mintlify (`https://docs.alien.ai`).
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `alien/docs` repo (often cloned locally as `../alien-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `alien/docs`.
