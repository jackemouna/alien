...
`~/.alien/alien.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.alien/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `alien setup --node-manager` is narrower and currently accepts `npm`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.alien.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.alien/skills` → bundled skills →
