...
host-local `ask: "always"` in `~/.alien/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `alien approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `alien exec-policy show`                                      | Local-machine merged view.                                                             |
| `alien exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `alien` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Alien also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.alien/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, Alien treats inline code-eval forms as approval-only
...
**both** policy layers - requested exec policy in Alien config
...
- In YOLO mode, Alien does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when Alien's requested exec
...
- Local `~/.alien/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `alien approvals set --gateway` or
`alien approvals set --node <id|name|ip>`.
...
- `alien exec-policy` does not synchronize node approvals.
- `alien exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `alien approvals --node ...`.
...
specific argument shape. Alien evaluates the regular expression
...
entries instead of hand-editing the encoded value. If Alien cannot
...
edit its local `~/.alien/exec-approvals.json` directly.

CLI: `alien approvals` supports gateway or node editing - see
...
When an async exec approval is denied, Alien prevents the agent from
