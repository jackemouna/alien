...
Alien has two cooperating guardrails for repetitive tool-call patterns:
...
When a loop is detected, Alien reports a loop event and either dampens or blocks the next tool-cycle depending on severity. This protects users from runaway token spend and lockups while preserving normal tool access.
