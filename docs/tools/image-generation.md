...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai-codex` OAuth profile is configured, Alien routes image
...
nearby geometry option instead of the exact requested one, Alien remaps to
...
Alien tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when Alien can
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai-codex` OAuth profile is configured, Alien reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Alien
    ...
    Alien forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible Alien maps
    ...
    transparency-capable OpenAI image model. Alien routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    Alien forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Alien-managed image attachments

    Alien intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Alien image generation" size=3840x2160 count=1
```
  </Tab>
...
`alien infer image edit`; `--openai-background` remains as an
