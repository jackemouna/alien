...
read_when:
  ...
  - Debugging why alien is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Alien-managed)"
---

Alien can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `alien` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **alien** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`alien`, `work`, `remote`, ...).
...
alien browser --browser-profile alien doctor --deep
alien browser --browser-profile alien status
...
If `alien browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `alien browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `alien browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `alien` vs `user`

- `alien`: managed, isolated browser (no extension required).
- `user`: built-in Chrome MCP attach profile for your **real signed-in Chrome**
  session.

For agent browser tool calls:

- Default: use the isolated `alien` browser.
...
Set `browser.defaultProfile: "alien"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.alien/alien.json`.
...
```json5
{
  browser: {
    ...
    profiles: {
      alien: { cdpPort: 18800, color: "#FF4500" },
      work: {
        cdpPort: 18801,
        color: "#0066CC",
        headless: true,
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). Overriding `gateway.port` or `ALIEN_GATEWAY_PORT` shifts the derived ports in the same family.
- Local `alien` profiles auto-assign `cdpPort`/`cdpUrl`; set those only for remote CDP. `cdpUrl` defaults to the managed local CDP port when unset.
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Alien pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Alien-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
...
- `POST /start?headless=true` and `alien browser start --headless` request a
  ...
  remote CDP profiles reject the override because Alien does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `alien browser status --json`
  reports `headlessSource` as `env`, `profile`, `config`,
  `request`, `linux-display-fallback`, or `default`.
- `ALIEN_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `ALIEN_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- Default profile is `alien` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
Alien uses it automatically. Set `browser.executablePath` to override
auto-detection. Top-level and per-profile `executablePath` values accept `~`
for your OS home directory:

```bash
alien config set browser.executablePath "/usr/bin/google-chrome"
alien config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
```
...
Per-profile `executablePath` only affects local managed profiles that Alien
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Alien will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local Alien-managed browser profile.
- `headless` only affects local managed profiles that Alien launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `alien browser stop` stops the browser process that
  Alien launched
- attach-only and remote CDP profiles: `alien browser stop` closes the active
  ...
  though no browser process was launched by Alien
...
Alien preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Alien can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Alien treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Alien can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Alien
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and Alien runs on the host, treat
...
Alien process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-Alien WebSocket base, such
...
an address Alien cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, Alien treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by Alien.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Alien accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  Alien calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Alien connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Alien tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Alien
  ...
  accepts it, Alien falls back to that root as well. This lets a bare `ws://`
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-alien-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Alien
  generates a runtime-only gateway token for that startup. Configure
  `gateway.auth.token`, `gateway.auth.password`, `ALIEN_GATEWAY_TOKEN`, or
  `ALIEN_GATEWAY_PASSWORD` explicitly if clients need a stable secret across
  restarts.
- Alien does **not** auto-generate that token when `gateway.auth.mode` is
...
Alien supports multiple named profiles (routing configs). Profiles can be:

- **alien-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `alien` profile is auto-created if missing.
...
Alien can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Alien attaches.
...
```bash
alien browser --browser-profile user start
alien browser --browser-profile user status
alien browser --browser-profile user tabs
alien browser --browser-profile user snapshot --format ai
```
...
- `alien doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `alien` profile because it can
  act inside your signed-in browser session.
- Alien does not launch the browser for this driver; it only attaches.
- Alien uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, Alien skips
...
Compared to the managed `alien` profile, existing-session drivers are more constrained:
...
When launching locally, Alien picks the first available:
...
control API** plus a matching `alien browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means Alien cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "alien" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by alien` when a
...
- For the local loopback `alien` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Alien's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (alien, chrome, or remote CDP).
