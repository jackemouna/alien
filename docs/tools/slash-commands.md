...
When a conversation or thread is bound to an ACP session, normal follow-up text routes to that ACP harness. Gateway management commands still stay local: `/acp ...` always reaches the Alien ACP command handler, and `/status` plus `/unfocus` stay local whenever command handling is enabled for the surface.
...
On Discord, native command specs may include `descriptionLocalizations`, which Alien publishes as Discord `description_localizations` and includes in reconcile comparisons.
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `alien.json`).
</ParamField>
<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes Alien-managed MCP config under `mcp.servers`).
</ParamField>
...
<AccordionGroup>
  <Accordion title="Sessions and runs">
    ...
    - `/export-trajectory [path]` asks for exec approval, then exports a JSONL [trajectory bundle](/tools/trajectory) for the current session. Use it when you need the prompt, tool, and transcript timeline for one Alien session. In group chats, the approval prompt and export result go to the owner privately. Alias: `/trajectory`.
  ...
  <Accordion title="Discovery and status">
    ...
    - `/diagnostics [note]` is the owner-only support-report flow for Gateway bugs and Codex harness runs. It asks for explicit exec approval every time before running `alien gateway diagnostics export --json`; do not approve diagnostics with an allow-all rule. After approval, it sends a pasteable report with the local bundle path, manifest summary, privacy notes, and relevant session ids. In group chats, the approval prompt and report go to the owner privately. When the active session uses the OpenAI Codex harness, the same approval also sends relevant Codex feedback to OpenAI servers and the completed reply lists the Alien session ids, Codex thread ids, and `codex resume <thread-id>` commands. See [Diagnostics Export](/gateway/diagnostics).
  ...
  <Accordion title="Owner-only writes and admin">
    - `/config show|get|set|unset` reads or writes `alien.json`. Owner-only. Requires `commands.config: true`.
    - `/mcp show|get|set|unset` reads or writes Alien-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
    ...
    - `/restart` restarts Alien when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
<AccordionGroup>
  <Accordion title="Argument and parser notes">
    ...
    - For full provider usage breakdown, use `alien status --usage`.
    - `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
    - In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
    - `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Alien session logs.
    - `/restart` is enabled by default; set `commands.restart: false` to disable it.
    - `/plugins install <spec>` accepts the same plugin specs as `alien plugins install`: local path/archive, npm package, `git:<repo>`, or `clawhub:<pkg>`, then requests a Gateway restart because plugin source modules changed.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is actually running the session: `Alien Pi Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
...
```
/debug show
/debug set messages.responsePrefix="[alien]"
...
<Note>
Overrides apply immediately to new config reads, but do **not** write to `alien.json`. Use `/debug reset` to clear all overrides and return to the on-disk config.
</Note>
...
`/config` writes to your on-disk config (`alien.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[alien]"
...
`/mcp` writes Alien-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
<Note>
`/mcp` stores config in Alien config, not Pi-owned project settings. Runtime adapters decide which transports are actually executable.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` is still supported for a single `/alien`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
