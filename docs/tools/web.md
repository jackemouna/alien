...
Alien also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Alien plugin SDK" });
    ```
...
Direct OpenAI Responses models use OpenAI's hosted `web_search` tool automatically when Alien web search is enabled and no managed provider is pinned. This is provider-owned behavior in the bundled OpenAI plugin and only applies to native OpenAI API traffic, not OpenAI-compatible proxy base URLs or Azure routes. Set `tools.web.search.provider` to another provider such as `brave` to keep the managed `web_search` tool for OpenAI models, or set `tools.web.search.enabled: false` to disable both managed search and native OpenAI search.

## Native Codex web search

Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Alien's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Alien keeps the normal managed `web_search` behavior.

## Network safety

Managed `web_search` provider calls use Alien's guarded fetch path. For
trusted provider API hosts, Alien allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, Alien checks providers in this order and uses the
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Alien resolves only the
...
the owning plugin is unavailable, Alien keeps startup resilient and reports a
warning so you can run `alien doctor --fix` to install or enable the plugin.
...
- or omit that field and let Alien auto-detect the first ready web-fetch
...
When you choose **Kimi** during `alien onboard` or
`alien configure --section web`, Alien can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `alien doctor --fix`.
When you choose Grok during `alien onboard` or `alien configure --section web`,
Alien can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Alien does not
...
<Tabs>
  <Tab title="Config file">
    Run `alien configure --section web` or set the key directly:
...
optional structured filters. Alien only enables the built-in xAI `x_search`
