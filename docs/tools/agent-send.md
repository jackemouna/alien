...
`alien agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    alien agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    alien agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    alien agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    alien agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    alien agent --agent ops --message "Generate report" \
...
alien agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `alien agent` flag and option reference.
  </Card>
