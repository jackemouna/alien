...
read_when:
  - Running Alien Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Alien Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/alien/alien/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Alien and Chrome are on the same machine
...
- Alien points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Alien is not the problem yet.
...
Fix that before touching Alien config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Alien at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Alien to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Alien to succeed
...
```bash
alien browser open https://example.com --browser-profile remote
alien browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `alien browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `alien browser stop --browser-profile remote`
...
3. Alien config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Alien config or Control UI auth
