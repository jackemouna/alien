---
summary: "Typed workflow runtime for Alien with resumable approval gates."
title: Lobster
read_when:
...
Lobster is a workflow shell that lets Alien run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.

Lobster is one authoring layer above detached background work. For flow orchestration above individual tasks, see [Task Flow](/automation/taskflow) (`alien tasks flow`). For the task activity ledger, see [`alien tasks`](/automation/tasks).
...
- **One call instead of many**: Alien runs one Lobster tool call and gets a structured result.
...
Alien runs Lobster workflows **in-process** using an embedded runner. No external CLI subprocess is spawned; the workflow engine executes inside the gateway process and returns a JSON envelope directly.
...
gog.gmail.search --query 'newer_than:1d' \
  | alien.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
alien.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Alien tool calls, set `pipeline` to the file path.
...
If you need the standalone Lobster CLI for development or external pipelines, install it from the [Lobster repo](https://github.com/alien/lobster) and ensure `lobster` is on `PATH`.
...
→ alien calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ alien calls gmail.send
...
- **No secrets** - Lobster doesn't manage OAuth; it calls Alien tools that do.
