...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, Alien best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Manual-spawn delivery resilience">
    - Alien hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Alien first tries to wake/steer that run instead of starting a second visible reply path.
    - If the requester-agent completion handoff fails or produces no visible output, Alien treats delivery as failed and falls back to queue routing/retry. It does not raw-send the child result directly to the external chat.
    - If direct handoff cannot be used, it falls back to queue routing.
    - If queue routing is still not available, the announce is retried with a short exponential backoff before final give-up.
    - Completion delivery keeps the resolved requester route: thread-bound or conversation-bound completion routes win when available; if the completion origin only provides a channel, Alien fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Alien hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Alien config agents from `agents_list`.
...
- **Run timeout:** if `sessions_spawn.runTimeoutSeconds` is omitted, Alien uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
<Steps>
  ...
  <Step title="Bind">
    Alien creates or binds a thread to that session target in the active channel.
  </Step>
...
- If a nested requester subagent session is gone, Alien falls back to that session's requester when available.
...
target agent first. After that, Alien applies the sub-agent restriction
layer.
...
Alien does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, Alien persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`alien tasks maintenance --apply` to reconcile the task record, or
`alien doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
