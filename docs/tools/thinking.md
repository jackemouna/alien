...
- Provider notes:
  ...
  - OpenAI GPT models map `/think` through model-specific Responses API effort support. `/think off` sends `reasoning.effort: "none"` only when the target model supports it; otherwise Alien omits the disabled reasoning payload instead of sending an unsupported value.
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; Alien normalizes incompatible values to `auto`.
...
- Alien resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode sends the same `service_tier=priority` flag on Codex Responses. Alien keeps one shared `/fast` toggle across both auth paths.
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. Alien still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
...
- Provider plugins that proxy Claude models should reuse `resolveClaudeThinkingProfile(modelId)` from `alien/plugin-sdk/provider-model-shared` so direct Anthropic and proxy catalogs stay aligned.
