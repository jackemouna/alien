...
For session-backed agent runs, Alien starts music generation as a
...
completion agent writes only a private final reply, Alien falls back to a
...
<Note>
Not all providers support all parameters. Alien still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Alien
...
- **Status lookup:** `alien tasks list` or `alien tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Alien injects an internal completion event back
...
```bash
ALIEN_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
