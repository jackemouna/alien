...
read_when:
  - Looking for an overview of Alien's media capabilities
...
Alien generates images, videos, and music, understands inbound media
...
For async tools, Alien submits the request to the provider, returns a task
...
Alien wakes the agent with the generated media paths so it can tell the
user and, when required by source-delivery policy, relay the result through
the message tool. For message-tool-only group/channel routes, Alien treats
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until Alien has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in Alien until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
