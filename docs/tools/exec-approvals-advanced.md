...
- allowlist entries live in host-local `~/.alien/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `alien approvals allowlist ...`).
- `alien security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `alien doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Alien still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, Alien sends a followup `agent` turn to the same session.
...
the originating channel, Alien keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, Alien sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.alien.approval` custom event content on the first prompt
  event so Alien-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. Alien first tries a private route on the
...
topics, Alien preserves the topic for the approval prompt and the post-approval follow-up.
