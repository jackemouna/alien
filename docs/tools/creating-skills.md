...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.alien/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Alien picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    alien agent --message "give me a greeting"
    ```
...
| `metadata.alien.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.)                    |
| `metadata.alien.requires.bins`   | No       | Required binaries on PATH                                      |
| `metadata.alien.requires.config` | No       | Required config keys                                           |
...
- **Test locally** — use `alien agent --message "..."` to test before sharing
...
| `~/.alien/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Alien) | Low        | Global                |
