...
The tool is only registered when Alien can resolve a PDF-capable model config for the agent:
...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  Alien can actually authenticate that provider for the agent.
...
- `file://` URL
- `http://` and `https://` URL
- Alien-managed inbound refs such as `media://inbound/<id>`
...
- Managed inbound refs and replayed paths under Alien's inbound media store are allowed with workspace-only file policy.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, Alien drops the rendered images and continues with the
  extracted text.
