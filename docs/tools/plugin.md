---
summary: "Install, configure, and manage Alien plugins"
read_when:
...
search, and more. Some plugins are **core** (shipped with Alien), others
...
temporary set of Alien-owned plugin packages while that migration finishes.
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    alien plugins install npm-pack:./alien-plugin-1.2.3.tgz
    ...
    alien plugins install ./my-plugin.tgz
    ```
...
`alien doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`alien.install.allowInvalidConfigRecovery`.
During Gateway startup, invalid plugin config fails closed like any other invalid
config. Run `alien doctor --fix` to quarantine the bad plugin config by
...
Run `alien doctor --fix` to remove the stale channel/plugin entries; unknown
...
Gateway startup skips plugin discovery/load work and `alien doctor` preserves
...
installed under Alien's managed plugin roots. npm dependencies may be hoisted
within Alien's managed npm root; install/update scans that managed root before
trust and uninstall removes npm-managed packages through npm. External plugins
and custom load paths must still be installed through `alien plugins install`.
Use `alien plugins list --json` to see the static `dependencyStatus` for each
...
and config validation follows with `plugin present but blocked`, Alien found
...
Alien as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Alien config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Alien as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/alien-config/npm
```

After fixing ownership, rerun `alien doctor --fix` or
`alien plugins registry --refresh` so the persisted plugin registry matches
the repaired files.

For npm installs, mutable selectors such as `latest` or a dist-tag are resolved
before installation and then pinned to the exact verified version in Alien's
managed npm root. After npm finishes, Alien verifies the installed
...
Managed npm roots also inherit Alien's package-level npm `overrides`, so
security pins that protect the packaged host also apply to hoisted external
plugin dependencies.

Source checkouts are pnpm workspaces. If you clone Alien to hack on bundled
plugins, run `pnpm install`; Alien then loads bundled plugins from
`extensions/<id>` so edits and package-local dependencies are used directly.
Plain npm root installs are for packaged Alien, not source checkout
development.

## Plugin types

Alien recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `alien.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to Alien features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `alien plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
Native plugin npm packages must declare `alien.extensions` in `package.json`.
...
npm packages installed into Alien's managed plugin root.
...
Alien needs at runtime. That is a plugin packaging issue, not a local config
...
Use `alien.runtimeExtensions` when published runtime files do not live at the
...
publish `alien.setupEntry`, use `alien.runtimeSetupEntry` for its built
...
### Alien-owned npm packages during migration

ClawHub is the primary distribution path for most plugins. Current packaged
Alien releases already bundle many official plugins, so those do not need
separate npm installs in normal setups. Until every Alien-owned plugin has
migrated to ClawHub, Alien still ships some `@alien/*` plugin packages on
npm for older/custom installs and direct npm workflows.

If npm reports an `@alien/*` plugin package as deprecated, that package
version is from an older external package train. Use the bundled plugin from
current Alien or a local checkout until a newer npm package is published.

| Plugin          | Package                    | Docs                                       |
| --------------- | -------------------------- | ------------------------------------------ |
| BlueBubbles     | `@alien/bluebubbles`    | [BlueBubbles](/channels/bluebubbles)       |
| Discord         | `@alien/discord`        | [Discord](/channels/discord)               |
| Feishu          | `@alien/feishu`         | [Feishu](/channels/feishu)                 |
| Matrix          | `@alien/matrix`         | [Matrix](/channels/matrix)                 |
| Mattermost      | `@alien/mattermost`     | [Mattermost](/channels/mattermost)         |
| Microsoft Teams | `@alien/msteams`        | [Microsoft Teams](/channels/msteams)       |
| Nextcloud Talk  | `@alien/nextcloud-talk` | [Nextcloud Talk](/channels/nextcloud-talk) |
| Nostr           | `@alien/nostr`          | [Nostr](/channels/nostr)                   |
| Synology Chat   | `@alien/synology-chat`  | [Synology Chat](/channels/synology-chat)   |
| Tlon            | `@alien/tlon`           | [Tlon](/channels/tlon)                     |
| WhatsApp        | `@alien/whatsapp`       | [WhatsApp](/channels/whatsapp)             |
| Zalo            | `@alien/zalo`           | [Zalo](/channels/zalo)                     |
| Zalo Personal   | `@alien/zalouser`       | [Zalo Personal](/plugins/zalouser)         |

### Core (shipped with Alien)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` - bundled browser plugin for the browser tool, `alien browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
to `plugins.allow` or remove `plugins.allow`; `alien doctor` warns about this
shape.
...
`alien plugins list` is a local plugin registry/config snapshot. An
...
`alien gateway run` process, or use `alien gateway restart` against the
running Gateway when the reload reports a failure.

<Accordion title="Plugin states: disabled vs missing vs invalid">
  - **Disabled**: plugin exists but enablement rules turned it off. Config is preserved.
  - **Missing**: config references a plugin id that discovery did not find.
  - **Invalid**: plugin exists but its config does not match the declared schema. Gateway startup skips only that plugin; `alien doctor --fix` can quarantine the invalid entry by disabling it and removing its config payload.

</Accordion>

## Discovery and precedence

Alien scans for plugins in this order (first match wins):

<Steps>
  <Step title="Config paths">
    `plugins.load.paths` - explicit file or directory paths. Paths that point
    back at Alien's own packaged bundled plugin directories are ignored;
    run `alien doctor --fix` to remove those stale aliases.
  </Step>

  <Step title="Workspace plugins">
    `\<workspace\>/.alien/<plugin-root>/*.ts` and `\<workspace\>/.alien/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global plugins">
    `~/.alien/<plugin-root>/*.ts` and `~/.alien/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Alien. Many are enabled by default (model providers, speech).
...
`/app/extensions/synology-chat`, Alien treats that mounted source directory
...
Set `ALIEN_DISABLE_BUNDLED_SOURCE_OVERLAYS=1` to force packaged dist bundles
...
- Run `alien gateway status --deep --require-rpc` and confirm the active
...
- Restart the live Gateway after plugin install/config/code changes. In wrapper
  ...
  `alien gateway run` process.
- Use `alien plugins inspect <id> --runtime --json` to confirm hook registrations and
...
- For proof of the effective session model, use `alien sessions` or the
...
```bash
alien config set logging.level trace
alien logs --follow
```
...
Alien caches successful plugin tool factory results for repeated resolutions
...
- Run `alien plugins list --enabled --verbose` to see every enabled plugin
  and origin.
- Run `alien plugins inspect <id> --runtime --json` for each suspected plugin and
  compare `channels`, `channelConfigs`, `tools`, and diagnostics.
- Run `alien plugins registry --refresh` after installing or removing
...
- If the duplicate is accidental, disable one side with
  ...
  install.
- If you explicitly enabled both plugins, Alien keeps that request and
...
```bash
alien plugins list                       # compact inventory
alien plugins list --enabled            # only enabled plugins
alien plugins list --verbose            # per-plugin detail lines
alien plugins list --json               # machine-readable inventory
alien plugins search <query>            # search ClawHub plugin catalog
alien plugins inspect <id>              # static detail
alien plugins inspect <id> --runtime    # registered hooks/tools/CLI/gateway methods
alien plugins inspect <id> --json       # machine-readable
alien plugins inspect --all             # fleet-wide table
alien plugins info <id>                 # inspect alias
alien plugins doctor                    # diagnostics
alien plugins registry                  # inspect persisted registry state
alien plugins registry --refresh        # rebuild persisted registry
alien doctor --fix                      # repair plugin registry state

alien plugins install <package>         # install from npm by default
alien plugins install clawhub:<pkg>     # install from ClawHub only
alien plugins install npm:<pkg>         # install from npm only
alien plugins install git:<repo>        # install from git
alien plugins install git:<repo>@<ref>  # install from git ref
alien plugins install <spec> --force    # overwrite existing install
alien plugins install <path>            # install from local path
alien plugins install -l <path>         # link (no copy) for dev
alien plugins install <plugin> --marketplace <source>
alien plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
alien plugins install <spec> --pin      # record exact resolved npm spec
alien plugins install <spec> --dangerously-force-unsafe-install
alien plugins update <id-or-npm-spec> # update one plugin
alien plugins update <id-or-npm-spec> --dangerously-force-unsafe-install
alien plugins update --all            # update all
alien plugins uninstall <id>          # remove config and plugin index records
...
alien plugins inspect <id> --runtime --json

# Run plugin-owned CLI commands directly from the Alien root CLI.
alien <plugin-command> --help
...
Bundled plugins ship with Alien. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `alien plugins enable <id>`.

`--force` overwrites an existing installed plugin or hook pack in place. Use
`alien plugins update <id-or-npm-spec>` for routine upgrades of tracked npm
plugins. It is not supported with `--link`, which reuses the source path instead
of copying over a managed install target.

When `plugins.allow` is already set, `alien plugins install` adds the
...
Alien keeps a persisted local plugin registry as the cold read model for
...
the registry is missing, stale, or invalid, `alien plugins registry
...
In Nix mode (`ALIEN_NIX_MODE=1`), plugin lifecycle mutators are disabled.
Manage plugin package selection and config through the Nix source for the
install instead; for nix-alien, start with the agent-first
[Quick Start](https://github.com/alien/nix-alien#quick-start).
`alien plugins update <id-or-npm-spec>` applies to tracked installs. Passing
...
the resolved version and recorded artifact identity, Alien skips the update
...
Alien does not yet expose LTS or monthly support plugin channels. Planned
...
override instead, while `alien skills install` remains the separate ClawHub
...
`alien plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`alien plugins` CLI reference](/cli/plugins) for full details.
...
Alien loads the entry object and calls `register(api)` during plugin
...
Alien may evaluate the trusted plugin entry or channel plugin module to build
