...
Alien supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Alien reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.alien/.env`.

If you omit `baseUrl`, Alien defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Alien defaults to `kimi-k2.6`.
...
Alien treats Kimi `web_search` as successful only after Moonshot returns
...
Alien returns a structured `kimi_web_search_ungrounded` error instead of
