...
Alien supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Alien also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `alien configure --section web`. It stores the key in
`~/.alien/alien.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.alien/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
