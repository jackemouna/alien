...
Alien supports **Ollama Web Search** as a bundled `web_search` provider. It
...
- an Ollama host that is reachable from Alien
- `ollama signin`
...
If no explicit Ollama base URL is set, Alien uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Alien reuses
...
- If the Ollama host is auth-protected, Alien reuses the normal Ollama
  provider API key when present.
- If `baseUrl` is `https://ollama.com`, Alien calls
...
- If the configured host does not expose web search and `OLLAMA_API_KEY` is set,
  Alien can fall back to `https://ollama.com/api/web_search` without sending
  that env key to the local host.
- Alien warns during setup if Ollama is unreachable or not signed in, but
