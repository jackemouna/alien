---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, Alien ACP, OpenCode) through the ACP backend"
read_when:
...
let Alien run external coding harnesses (for example Pi, Claude Code,
Cursor, Copilot, Droid, Alien ACP, OpenCode, Gemini CLI, and other
...
directly to existing Alien channel conversations, use
[`alien mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ Alien | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an Alien Gateway session _as_ an ACP server for an editor or client                   | [`alien acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to Alien over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No Alien tools, no ACP controls, no harness runtime                                                                                                                               |
...
Alien only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. Alien owns routing,
...
Alien plugin tools and built-in Alien tools are **not** exposed to
...
| `alien` | Alien Gateway bridge through `alien acp` | Lets an ACP-aware harness talk back to an Alien Gateway session.                 |
| `pi`       | Pi/embedded Alien runtime                   | Used for Alien-native harness experiments.                                       |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but Alien
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the Alien session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from Alien's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    Alien dynamic tools still execute through Alien, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, Alien injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through Alien approvals. Codex `Stop` hooks are relayed to
    Alien `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    Alien picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    Alien config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an Alien agent
    is configured with `runtime.type="acp"`, Alien uses
...
plugin is enabled. Use **sub-agents** when you want Alien-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Alien native sub-agent runtime  |
...
1. Alien ACP session control plane.
2. Official `@alien/acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state Alien routes to.
...
spawned ACP session - no child thread, same chat surface. Alien keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; Alien returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, Alien inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by Alien even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - Alien binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, Alien returns a clear
...
- Alien ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, Alien inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, Alien uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, Alien may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  normalize Alien Codex refs such as `openai-codex/gpt-5.4` to Codex
  ...
  `session/set_model`; otherwise Alien/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What Alien sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. Alien agents get Alien's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_ALIEN_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in Alien where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. Alien converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see Alien-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, Alien uses an agent-to-agent (A2A) follow-up path
    ...
    Alien skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an Alien channel session key; Alien still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Alien session you are creating, so `mode: "session"` still requires `thread: true`.
...
- Alien's sandbox policy does **not** wrap ACP harness execution.
- Alien still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced Alien-native work.
...
If no target resolves, Alien returns a clear error
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, Alien normalizes `openai-codex/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai-codex/gpt-5.4/high` to `reasoning_effort`. |
| `/acp set thinking <level>`  | runtime config key `thinking`        | For Codex ACP, Alien sends the corresponding `reasoning_effort` where the adapter supports one.                                                                             |
...
aliases), the plugin-tools and Alien-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | Alien is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| Harness sees `<<<BEGIN_ALIEN_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update Alien and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
