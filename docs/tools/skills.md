...
Alien uses **[AgentSkills](https://agentskills.io)-compatible** skill
folders to teach the agent how to use tools. Each skill is a directory
containing a `SKILL.md` with YAML frontmatter and instructions. Alien
...
Alien loads skills from these sources, **highest precedence first**:
...
| 4   | Managed/local skills  | `~/.alien/skills`             |
...
Codex CLI's native `$CODEX_HOME/skills` directory is not one of these Alien
...
Use `alien migrate codex --dry-run` to inventory them and
`alien migrate codex` to choose skill directories with an interactive
checkbox prompt before copying them into the current Alien agent workspace.
...
| Shared managed/local | `~/.alien/skills`                        | All agents on that machine  |
...
`alien.plugin.json` (paths relative to the plugin root). Plugin skills
...
`metadata.alien.requires.config` on the plugin's config entry.
...
[ClawHub](https://clawhub.ai) is the public skills registry for Alien.
Use native `alien skills` commands for discover/install/update, or the
separate `clawhub` CLI for publish/sync workflows. Full guide:
[ClawHub](/tools/clawhub).

| Action                             | Command                                |
| ---------------------------------- | -------------------------------------- |
| Install a skill into the workspace | `alien skills install <skill-slug>` |
| Update all installed skills        | `alien skills update --all`         |
| Sync (scan + publish updates)      | `clawhub sync --all`                   |

Native `alien skills install` installs into the active workspace
...
configured Alien workspace). Alien picks that up as
...
`alien skills install <slug>` remains only the install path; publishers
...
- `alien skills install <slug>` is different - it downloads a ClawHub skill folder into the workspace and does not use the installer-metadata path above.
...
Alien follows the AgentSkills spec for layout/intent. The parser used
...
<ParamField path="homepage" type="string">
  URL surfaced as "Website" in the macOS Skills UI. Also supported via `metadata.alien.homepage`.
</ParamField>
<ParamField path="user-invocable" type="boolean" default="true">
  When `true`, the skill is exposed as a user slash command.
</ParamField>
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Alien keeps the skill's instructions out of the agent's normal
...
Alien filters skills at load time using `metadata` (single-line JSON):
...
<ParamField path="requires.config" type="string[]">
  List of `alien.json` paths that must be truthy.
</ParamField>
...
If no `metadata.alien` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

<Note>
Legacy `metadata.clawdbot` blocks are still accepted when
`metadata.alien` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Alien lists each entry so you can see the available artifacts.
    - Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
    - Node installs honor `skills.install.nodeManager` in `alien.json` (default: npm; options: npm/pnpm/yarn/bun). This only affects skill installs; the Gateway runtime should still be Node - Bun is not recommended for WhatsApp/Telegram.
    - Gateway-backed installer selection is preference-driven: when install specs mix kinds, Alien prefers Homebrew when `skills.install.preferBrew` is enabled and `brew` exists, then `uv`, then the configured node manager, then other fallbacks like `go` or `download`.
    - If every install spec is `download`, Alien surfaces all download options instead of collapsing to one preferred installer.

  </Accordion>
  <Accordion title="Per-installer details">
    - **Go installs:** if `go` is missing and `brew` is available, the gateway installs Go via Homebrew first and sets `GOBIN` to Homebrew's `bin` when possible.
    - **Download installs:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.alien/tools/<skillKey>`).
...
under `skills.entries` in `~/.alien/alien.json`:
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience for skills that declare `metadata.alien.primaryEnv`. Supports plaintext or SecretRef.
</ParamField>
...
defines `metadata.alien.skillKey`, use that key under `skills.entries`.

<Note>
For stock image generation/editing inside Alien, use the core
...
When an agent run starts, Alien:
...
For the bundled `claude-cli` backend, Alien also materializes the same
...
Alien still owns precedence, per-agent allowlists, gating, and
`skills.entries.*` env/API key injection. Other CLI backends use the
prompt catalog only.

## Snapshots and refresh

Alien snapshots the eligible skills **when a session starts** and
...
session, Alien refreshes the snapshot so visible skills stay aligned
with the current agent.

### Skills watcher

By default, Alien watches skill folders and bumps the skills snapshot
...
Alien can treat macOS-only skills as eligible when the required
...
probes, Alien clears its cached bin matches so agents no longer see
skills that cannot currently run there.

## Token impact

When skills are eligible, Alien injects a compact XML list of available
...
Alien ships a baseline set of skills as **bundled skills** with the
install (npm package or Alien.app). `~/.alien/skills` exists for
