...
- `gateway`/`node` approvals are controlled by `~/.alien/exec-approvals.json`.
...
- `timeout` applies to foreground, background, `yieldMs`, gateway, sandbox, and node `system.run` execution. If omitted, Alien uses `tools.exec.timeoutSec`; explicit `timeout: 0` disables the exec process timeout for that call.
...
- Alien sets `ALIEN_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- `alien channels login` is blocked from `exec` because it is an interactive channel-auth flow; run it in a terminal on the gateway host, or use the channel-native login tool from chat when one exists.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.alien/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#yolo-mode-no-approval).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Alien prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`alien security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `alien doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`alien security audit` and `alien doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
