...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Alien
...
Today this applies to PI embedded runs and Alien dynamic tools in the Codex
app-server harness. Tokenjuice hooks Alien's tool-result middleware and
...
```bash
alien config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
alien plugins enable tokenjuice
```

Alien already ships the plugin. There is no separate `plugins install`
or `tokenjuice install alien` step.
...
```bash
alien config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
alien plugins disable tokenjuice
```
