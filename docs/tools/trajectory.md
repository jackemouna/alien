---
summary: "Export redacted trajectory bundles for debugging an Alien agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Alien session
...
Trajectory capture is Alien's per-session flight recorder. It records a
...
Alien writes the bundle under the workspace:

```text
.alien/trajectory-exports/alien-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.alien/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Alien sends the
...
Trajectory capture is on by default for Alien agent runs.
...
| `metadata.json`       | Alien version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Alien also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `ALIEN_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export ALIEN_TRAJECTORY_DIR=/var/lib/alien/trajectories
```

When this variable is set, Alien writes one JSONL file per session id in that
directory.
...
Set `ALIEN_TRAJECTORY=0` before starting Alien:

```bash
export ALIEN_TRAJECTORY=0
```
...
Alien redacts sensitive values before writing export files:
...
- confirm Alien was started without `ALIEN_TRAJECTORY=0`
- check whether `ALIEN_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.alien/trajectory-exports/`
