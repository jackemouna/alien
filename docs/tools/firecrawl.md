...
Alien can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `alien configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `alien doctor --fix`.
...
when you run Firecrawl yourself. Alien accepts `http://` only for loopback,
...
Alien always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Alien
