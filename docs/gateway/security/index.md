...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  Alien is **not** a hostile multi-tenant security boundary for multiple
...
Alien security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `alien security audit`
...
alien security audit --fix
alien security audit --json
```
...
Alien is both a product and an experiment: you're wiring frontier-model behavior into real messaging surfaces and real tools. **There is no "perfectly secure" setup.** The goal is to be deliberate about:
...
Alien assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.alien`, including `alien.json`), treat them as a trusted operator.
...
- Alien's product default for trusted single-operator setups is that host exec on `gateway`/`node` is allowed without approval prompts (`security="full"`, `ask="off"` unless you tighten it). That default is intentional UX, not a vulnerability by itself.
...
Alien can run many agents inside one Gateway, but those agents still sit
...
If you run `--deep`, Alien also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.alien/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.alien/credentials/<channel>-allowFrom.json` (default account)
  - `~/.alien/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.alien/agents/<agentId>/agent/auth-profiles.json`
- **Codex runtime state**: `~/.alien/agents/<agentId>/agent/codex-home/`
- **File-backed secrets payload (optional)**: `~/.alien/secrets.json`
- **Legacy OAuth import**: `~/.alien/credentials/oauth.json`
...
`alien security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`alien security audit` raises `config.insecure_or_dangerous_flags` when
...
gateway:
  ...
  auth:
    mode: password
    password: ${ALIEN_GATEWAY_PASSWORD}
```
...
- Alien gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Alien responses.
...
Alien stores session transcripts on disk under `~/.alien/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.alien` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Alien cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Alien can refresh the skills list mid-session:
...
- If you install or update plugins (`alien plugins install <package>`, `alien plugins update <id>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - Alien runs a built-in dangerous-code scan before install/update. `critical` findings block by default.
  - npm and git plugin installs run package-manager dependency convergence only during the explicit install/update flow. Local paths and archives are treated as self-contained plugin packages; Alien copies/references them without running `npm install`.
  ...
  - Gateway-backed skill dependency installs follow the same dangerous/suspicious split: built-in `critical` findings block unless the caller explicitly sets `dangerouslyForceUnsafeInstall`, while suspicious findings still warn only. `alien skills install` remains the separate ClawHub skill download/install flow.
...
```bash
alien pairing list <channel>
alien pairing approve <channel> <code>
```
...
By default, Alien routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Alien has two separate "who can trigger me?" layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.alien/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- "Paste the full contents of ~/.alien or your logs."

## External content special-token sanitization

Alien strips common self-hosted LLM chat-template special-token literals from wrapped external content and metadata before they reach the model. Covered marker families include Qwen/ChatML, Llama, Gemma, Mistral, Phi, and GPT-OSS role/turn tokens.
...
Alien includes explicit bypass flags that disable external-content safety wrapping:
...
Alien strips common model-family special-token literals from wrapped
...
- `~/.alien/alien.json`: `600` (user read/write only)
- `~/.alien`: `700` (user only)

`alien doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `ALIEN_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__alien__/canvas/` and `/__alien__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Alien with Docker on a VPS, remember that published container ports
...
When the bundled `bonjour` plugin is enabled, the Gateway broadcasts its presence via mDNS (`_alien-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
5. **Environment variable** (alternative): set `ALIEN_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `alien doctor --generate-gateway-token`.
...
paths, set `ALIEN_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
break-glass. This is intentionally process environment only, not an
`alien.json` config key.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `ALIEN_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `ALIEN_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Alien
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Alien verifies the identity by resolving the
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) and owner semantics for agent turns; narrower `x-alien-scopes` values do not reduce that shared-secret path.
...
- In those identity-bearing modes, omitting `x-alien-scopes` falls back to the normal operator default scope set; send the header explicitly when you want a narrower scope set.
...
- Alien will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.alien/` (or `$ALIEN_STATE_DIR/`) may contain secrets or private data:

- `alien.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
Alien loads workspace-local `.env` files for agents and tools, but never lets those files silently override gateway runtime controls.

- Any key that starts with `ALIEN_*` is blocked from untrusted workspace `.env` files.
...
Why: workspace `.env` files frequently live next to agent code, get committed by accident, or get written by tools. Blocking the whole `ALIEN_*` prefix means adding a new `ALIEN_*` flag later can never regress into silent inheritance from workspace state.
...
- When sharing diagnostics, prefer `alien status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@alien", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.alien`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.alien/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `alien` profile).
...
Alien's browser navigation policy is strict by default: private/internal destinations stay blocked unless you explicitly opt in.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.alien/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.alien/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.alien/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "none",
        },
        // Session tools can reveal sensitive data from transcripts. By default Alien limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `alien gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `ALIEN_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/alien/alien-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.alien/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `alien security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Alien version
...
Found a vulnerability in Alien? Please report responsibly:

1. Email: [security@alien.ai](mailto:security@alien.ai)
