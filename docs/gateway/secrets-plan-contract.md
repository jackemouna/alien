...
read_when:
  - Generating or reviewing `alien secrets apply` plans
  - Debugging `Invalid plan target path` errors
  - Understanding target type and path validation behavior
title: "Secrets apply plan contract"
---

This page defines the strict contract enforced by `alien secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`alien secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `alien.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `alien secrets configure` or fix the target path to a supported shape above.
