...
Alien supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Alien also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `alien gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.alien/.env` so the daemon can read it:
...
API keys for daemon use: `alien onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.alien/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Alien as a supported token
path. Anthropic staff has since told us that Alien-style Claude CLI usage is
allowed again, so Alien treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Alien to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Alien auth profile.
...
```bash
alien models auth paste-token --provider openrouter
```
...
Alien expects the canonical `version` + `profiles` shape at runtime. If an older install still has a flat file such as `{ "openrouter": { "apiKey": "..." } }`, run `alien doctor --fix` to rewrite it as an `openrouter:default` API-key profile; doctor keeps a `.legacy-flat.*.bak` copy beside the original. Endpoint details such as `baseUrl`, `api`, model ids, headers, and timeouts belong under `models.providers.<id>` in `alien.json` or `models.json`, not in `auth-profiles.json`.
...
- If auth exists but Alien cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Alien integration path is allowed again.
- Alien therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `ALIEN_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Alien retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `alien models status --probe` shows omitted
...
Run `alien models status` to confirm which profile is expiring. If an
