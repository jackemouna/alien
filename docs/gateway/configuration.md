...
read_when:
  - Setting up Alien for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

Alien reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.alien/alien.json`.
The active config path must be a regular file. Symlinked `alien.json`
layouts are unsupported for Alien-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `ALIEN_CONFIG_PATH` directly at the real file.

If the file is missing, Alien uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `alien onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    alien onboard       # full onboarding flow
    alien configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.alien/alien.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Alien only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`alien config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`alien doctor`, `alien logs`, `alien health`, `alien status`)
- Run `alien doctor` to see exact issues
- Run `alien doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `alien.json`
...
Run `alien doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `alien config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `ALIEN_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `alien.json`.
    ...
    ```bash
    alien config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ALIEN_APNS_RELAY_BASE_URL` and `ALIEN_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `ALIEN_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-alien-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Alien-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Alien updates that included file and leaves `alien.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Alien-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `alien.json`. To share a tree across machines or users, set
      `ALIEN_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.alien/alien.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `alien.json`. Alien-owned config
...
config`, inspect the config, run `alien config validate`, then run `alien
...
When you edit a source file that is referenced through `$include`, Alien plans
...
```bash
alien gateway call config.get --params '{}'  # capture payload.hash
alien gateway call config.patch --params '{
...
Alien reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.alien/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Alien runs your login shell and imports only the missing keys:
...
Env var equivalent: `ALIEN_LOAD_SHELL_ENV=1`
</Accordion>
