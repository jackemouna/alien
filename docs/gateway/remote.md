...
- Use the macOS app's **Remote over SSH** mode (Settings → General → Alien runs).
...
- `alien health` and `alien status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `alien gateway status`, `alien gateway health`, `alien gateway probe`, and `alien gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `ALIEN_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`ALIEN_GATEWAY_URL`) may use env credentials only (`ALIEN_GATEWAY_TOKEN` / `ALIEN_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `ALIEN_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `ALIEN_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `ALIEN_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `ALIEN_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `ALIEN_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `ALIEN_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
  break-glass. There is no `alien.json` equivalent; this must be process
...
```bash
alien config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.alien.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.alien.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.alien.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.alien.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.alien.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.alien.ssh-tunnel
```
