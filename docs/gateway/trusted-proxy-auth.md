...
read_when:
  - Running Alien behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Alien
...
- You run Alien behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth).
...
<Steps>
  ...
  <Step title="Gateway verifies trusted source">
    Alien checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`).
  </Step>
  <Step title="Gateway extracts identity">
    Alien extracts the user identity from the configured header.
  </Step>
...
- Internal Gateway clients that do not travel through the reverse proxy should use `gateway.auth.password` / `ALIEN_GATEWAY_PASSWORD`, not trusted-proxy identity headers.
...
<Tabs>
  <Tab title="Proxy TLS termination (recommended)">
    ...
    - Alien can stay on loopback HTTP behind the proxy.
  ...
  <Tab title="Gateway TLS termination">
    If Alien itself serves HTTPS directly (no TLS-terminating proxy), set:
...
<AccordionGroup>
  <Accordion title="Pomerium">
    ...
    ```yaml
    routes:
      - from: https://alien.example.com
        to: http://alien-gateway:18789
        policy:
          - allow:
              or:
                - email:
...
Alien rejects ambiguous configurations where both a `gateway.auth.token` (or `ALIEN_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
Loopback trusted-proxy identity headers still fail closed: same-host callers are not silently authenticated as proxy users. Internal Alien callers that bypass the proxy may authenticate with `gateway.auth.password` / `ALIEN_GATEWAY_PASSWORD` instead. Token fallback remains intentionally unsupported in trusted-proxy mode.

## Operator scopes header

Trusted-proxy auth is an **identity-bearing** HTTP mode, so callers may optionally declare operator scopes with `x-alien-scopes`.

Examples:

- `x-alien-scopes: operator.read`
- `x-alien-scopes: operator.read,operator.write`
- `x-alien-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Alien honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-alien-scopes` is absent, their runtime scope falls back to `operator.write`.
...
Practical rule: send `x-alien-scopes` explicitly when you want a trusted-proxy request to be narrower than the defaults, or when a gateway-auth plugin route needs something stronger than write scope.
...
`alien security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
<AccordionGroup>
  ...
  <Accordion title="trusted_proxy_loopback_source">
    Alien rejected a loopback-source trusted-proxy request.
...
<Steps>
  ...
  <Step title="Update Alien config">
    Update Alien config with trusted-proxy auth.
  </Step>
  ...
  <Step title="Audit">
    Run `alien security audit` and review findings.
  </Step>
</Steps>
