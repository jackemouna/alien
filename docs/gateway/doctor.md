...
`alien doctor` is the repair + migration tool for Alien. It fixes stale config/state, checks health, and provides actionable repair steps.
...
<AccordionGroup>
  ...
  <Accordion title="State and integrity">
    ...
    - Extra workspace dir detection (`~/alien`).
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Channel-specific permission checks live under `alien channels capabilities`; for example, Discord voice channel permissions are audited with `alien channels capabilities --channel discord --target channel:<channel-id>`.
...
The Control UI Dreams scene includes **Backfill**, **Reset**, and **Clear Grounded** actions for the grounded dreaming workflow. These actions use gateway doctor-style RPC methods, but they are **not** part of `alien doctor` CLI repair/migration.
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    When the config contains deprecated keys, other commands refuse to run and ask you to run `alien doctor`.

    Doctor will:

    - Explain which legacy keys were found.
    - Show the migration it applied.
    - Rewrite `~/.alien/alien.json` with the updated schema.

    Gateway startup refuses legacy config formats and asks you to run `alien doctor --fix`; it does not rewrite `alien.json` on startup. Cron job store migrations are also handled by `alien doctor --fix`.
  ...
  <Accordion title="2f. Codex route repair">
    Doctor checks for legacy `openai-codex/*` model refs. Native Codex harness routing uses canonical `openai/*` model refs plus `agentRuntime.id: "codex"` so the turn goes through the Codex app-server harness instead of the Alien PI OpenAI path.
  ...
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    - Sessions store + transcripts:
      - from `~/.alien/sessions/` to `~/.alien/agents/<agentId>/sessions/`
    - Agent dir:
      - from `~/.alien/agent/` to `~/.alien/agents/<agentId>/agent/`
    - WhatsApp auth state (Baileys):
      - from legacy `~/.alien/credentials/*.json` (except `oauth.json`)
      - to `~/.alien/credentials/whatsapp/<accountId>/...` (default account id: `default`)

    These migrations are best-effort and idempotent; doctor will emit warnings when it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates the legacy sessions + agent dir on startup so history/auth/models land in the per-agent path without a manual doctor run. WhatsApp auth is intentionally only migrated via `alien doctor`. Talk provider/provider-map normalization now compares by structural equality, so key-order-only diffs no longer trigger repeat no-op `doctor --fix` changes.
  ...
  <Accordion title="3b. Legacy cron store migrations">
    Doctor also checks the cron job store (`~/.alien/cron/jobs.json` by default, or `cron.store` when overridden) for old job shapes that the scheduler still accepts for compatibility.
    ...
    On Linux, doctor also warns when the user's crontab still invokes legacy `~/.alien/bin/ensure-whatsapp.sh`. That host-local script is not maintained by current Alien and can write false `Gateway inactive` messages to `~/.alien/logs/whatsapp-health.log` when cron cannot reach the systemd user bus. Remove the stale crontab entry with `crontab -e`; use `alien channels status --probe`, `alien doctor`, and `alien gateway status` for current health checks.
  ...
  <Accordion title="3d. Session transcript branch repair">
    Doctor scans agent session JSONL files for the duplicated branch shape created by the 2026.4.24 prompt transcript rewrite bug: an abandoned user turn with Alien internal runtime context plus an active sibling containing the same visible user prompt. In `--fix` / `--repair` mode, doctor backs up each affected file next to the original and rewrites the transcript to the active branch so gateway history and memory readers no longer see duplicate turns.
  </Accordion>
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Multiple state dirs**: warns when multiple `~/.alien` folders exist across home directories or when `ALIEN_STATE_DIR` points elsewhere (history can split between installs).
    - **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run it on the remote host (the state lives there).
    - **Config file permissions**: warns if `~/.alien/alien.json` is group/world readable and offers to tighten to `600`.
  ...
  <Accordion title="5. Model auth health (OAuth expiry)">
    ...
    When an OAuth refresh fails permanently (for example `refresh_token_reused`, `invalid_grant`, or a provider telling you to sign in again), doctor reports that re-auth is required and prints the exact `alien models auth login --provider ...` command to run.
  ...
  <Accordion title="7b. Plugin install cleanup">
    Doctor removes legacy Alien-generated plugin dependency staging state in `alien doctor --fix` / `alien doctor --repair` mode. This covers stale generated dependency roots, old install-stage directories, package-local debris from earlier bundled-plugin dependency repair code, and orphaned or recovered managed npm copies of bundled `@alien/*` plugins that can shadow the current bundled manifest.

    Doctor can also reinstall missing downloadable plugins when config references them but the local plugin registry cannot find them. Examples include material `plugins.entries`, configured channel/provider/search settings, and configured agent runtimes. During package updates, doctor avoids running package-manager plugin repair while the core package is being swapped; run `alien doctor --fix` again after the update if a configured plugin still needs recovery. Gateway startup and config reload do not run package managers; plugin installs remain explicit doctor/install/update work.

  </Accordion>
  <Accordion title="8. Gateway service migrations and cleanup hints">
    Doctor detects legacy gateway services (launchd/systemd/schtasks) and offers to remove them and install the Alien service using the current gateway port. It can also scan for extra gateway-like services and print cleanup hints. Profile-named Alien gateway services are considered first-class and are not flagged as "extra."

    On Linux, if the user-level gateway service is missing but a system-level Alien gateway service exists, doctor does not install a second user-level service automatically. Inspect with `alien gateway status --deep` or `alien doctor --deep`, then remove the duplicate or set `ALIEN_SERVICE_REPAIR_POLICY=external` when a system supervisor owns the gateway lifecycle.

  </Accordion>
  <Accordion title="8b. Startup Matrix migration">
    When a Matrix channel account has a pending or actionable legacy state migration, doctor (in `--fix` / `--repair` mode) creates a pre-migration snapshot and then runs the best-effort migration steps: legacy Matrix state migration and legacy encrypted-state preparation. Both steps are non-fatal; errors are logged and startup continues. In read-only mode (`alien doctor` without `--fix`) this check is skipped entirely.
  </Accordion>
  <Accordion title="8c. Device pairing and auth drift">
    ...
    - inspect pending requests with `alien devices list`
    - approve the exact request with `alien devices approve <requestId>`
    - rotate a fresh token with `alien devices rotate --device <deviceId> --role <role>`
    - remove and re-approve a stale record with `alien devices remove <deviceId>`
  ...
  <Accordion title="11. Workspace status (skills, plugins, and legacy dirs)">
    ...
    - **Legacy workspace dirs**: warns when `~/alien` or other legacy workspace directories exist alongside the current workspace.
  ...
  <Accordion title="11c. Shell completion">
    ...
    Run `alien completion --write-state` to regenerate the cache manually.
  ...
  <Accordion title="12. Gateway auth checks (local token)">
    ...
    - `alien doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
  ...
  <Accordion title="12b. Read-only SecretRef-aware repairs">
    ...
    - `alien doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
  ...
  <Accordion title="13b. Memory search readiness">
    ...
    Use `alien memory status --deep` to verify embedding readiness at runtime.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - `alien doctor` prompts before rewriting supervisor config.
    - `alien doctor --yes` accepts the default repair prompts.
    - `alien doctor --repair` applies recommended fixes without prompts.
    - `alien doctor --repair --force` overwrites custom supervisor configs.
    - `ALIEN_SERVICE_REPAIR_POLICY=external` keeps doctor read-only for gateway service lifecycle. It still reports service health and runs non-service repairs, but skips service install/start/restart/bootstrap, supervisor config rewrites, and legacy service cleanup because an external supervisor owns that lifecycle.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older Alien binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
    - You can always force a full rewrite via `alien gateway install --force`.
