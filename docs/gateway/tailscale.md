...
Alien can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Alien requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this Alien Serve/Funnel
mode. `off` means Alien is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `ALIEN_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ALIEN_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Alien verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Alien only treats a request as Serve when it arrives from loopback with
...
Prefer `ALIEN_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
alien gateway --tailscale serve
alien gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Alien to undo `tailscale serve`
