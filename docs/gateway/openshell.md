---
summary: "Use OpenShell as a managed sandbox backend for Alien agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Alien. Instead of running Docker
containers locally, Alien delegates sandbox lifecycle to the `openshell` CLI,
...
- Alien Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Alien creates an OpenShell
...
- You edit files locally outside Alien and want those changes visible in the
...
- When the sandbox is first created, Alien seeds the remote workspace from
...
- Alien does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Alien after the initial seed, the remote sandbox does **not** see those changes. Use `alien sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"alien"`  | Sandbox source for first-time create                  |
...
1. Alien calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Alien calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `alien sandbox` commands
