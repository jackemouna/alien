...
Alien's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`alien agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-alien-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-alien-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "alien"`, `model: "alien/default"`, `model: "alien/<agentId>"`, or `x-alien-agent-id`
- use `x-alien-model` when you want to override the selected agent's backend model
- use `x-alien-session-key` for explicit session routing
- use `x-alien-message-channel` when you want a non-default synthetic ingress channel context
...
For the canonical explanation of how agent-target models, `alien/default`, embeddings pass-through, and backend model overrides fit together, see [OpenAI Chat Completions](/gateway/openai-http-api#agent-first-model-contract) and [Model list and agent routing](/gateway/openai-http-api#model-list-and-agent-routing).
...
- `previous_response_id`: Alien reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Alien normalizes common OpenAI-style aliases before those counters reach
