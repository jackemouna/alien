...
- Default rolling log file is under `/tmp/alien/` (one file per day): `alien-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.alien/alien.json`:
  - `logging.file`
  - `logging.level`
...
Alien can mask sensitive tokens before log or transcript output leaves the
...
`alien gateway` supports a per-gateway style switch:
