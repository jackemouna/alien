...
Alien can create a local diagnostics zip for bug reports. It combines
...
2. Alien sends the diagnostics preamble and asks for one explicit exec
   approval. The approval runs `alien gateway diagnostics export --json`.
   Do not approve diagnostics through an allow-all rule.
3. After approval, Alien replies with a pasteable report containing the local
   bundle path, manifest summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Alien does not
...
that the diagnostics flow was sent privately. If Alien cannot find a private
owner route, the command fails closed and asks the owner to run it from a DM.

When the active Alien session is using the native OpenAI Codex harness,
the same exec approval also covers an OpenAI feedback upload for the Codex
runtime threads Alien knows about. That upload is separate from the local
...
the channels, Alien session ids, Codex thread ids, and local resume commands
for the threads that were sent to OpenAI servers. If you deny or ignore the
approval, Alien does not run the export, does not send Codex feedback, and
...
Persisted bundles live under `~/.alien/logs/stability/` when events exist.

## Useful options

```bash
alien gateway diagnostics export \
  --output alien-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
