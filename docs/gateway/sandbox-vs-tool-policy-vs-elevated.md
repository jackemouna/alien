...
Alien has three related (but different) controls:
...
Use the inspector to see what Alien is _actually_ doing:
...
- Alien validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:alien`: all built-in Alien tools (excludes provider plugins)
