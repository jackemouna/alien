...
Alien supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `ALIEN_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Alien runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Alien validates the resolved target path.
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/Alien QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/alien"],
...
- Alien resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__ALIEN_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `alien.json` refs)
...
When reload-time activation fails after a healthy state, Alien enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `alien memory` remote-memory paths and `alien qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `alien status`, `alien status --all`, `alien channels status`, `alien channels resolve`, `alien security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `alien secrets reload`.
...
<Steps>
  <Step title="Audit current state">
    ```bash
    alien secrets audit --check
    ```
  </Step>
  <Step title="Configure SecretRefs">
    ```bash
    alien secrets configure
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
<AccordionGroup>
  <Accordion title="secrets audit">
    ...
    - plaintext values at rest (`alien.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `alien.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `alien secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `alien.json` plus `auth-profiles.json` for one agent scope
    ...
    - `alien secrets configure --providers-only`
    - `alien secrets configure --skip-provider-setup`
    - `alien secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    alien secrets apply --from /tmp/alien-secrets-plan.json
    alien secrets apply --from /tmp/alien-secrets-plan.json --allow-exec
    alien secrets apply --from /tmp/alien-secrets-plan.json --dry-run
    alien secrets apply --from /tmp/alien-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
Alien intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
