---
summary: "Run multiple Alien Gateways on one host (isolation, ports, and profiles)"
read_when:
...
alien --profile rescue onboard
alien --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `alien --profile rescue onboard`:
...
`alien --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.alien/workspace-rescue`)
...
# extra gateway
alien --profile ops setup
alien --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
alien --profile main setup
alien --profile main gateway --port 18789
...
alien --profile ops gateway install --port 19789
```
...
- `ALIEN_CONFIG_PATH` — per-instance config file
- `ALIEN_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `ALIEN_GATEWAY_PORT` / `--port`).
...
```bash
ALIEN_CONFIG_PATH=~/.alien/main.json \
ALIEN_STATE_DIR=~/.alien \
alien gateway --port 18789

ALIEN_CONFIG_PATH=~/.alien/rescue.json \
ALIEN_STATE_DIR=~/.alien-rescue \
alien gateway --port 19789
```
...
alien --profile rescue gateway status --deep
alien --profile rescue gateway probe
alien status
alien --profile rescue status
alien --profile rescue browser status
```
