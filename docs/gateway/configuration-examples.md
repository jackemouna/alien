---
summary: "Schema-accurate configuration examples for common Alien setups"
read_when:
  - Learning how to configure Alien
...
```json5
{
  agent: { workspace: "~/.alien/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.alien/alien.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.alien/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/alien" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.alien/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
