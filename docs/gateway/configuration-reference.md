---
summary: "Gateway config reference for core Alien keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.alien/alien.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main Alien config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `alien config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional - Alien uses safe defaults when omitted.
...
Alien-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Pi and other runtime adapters. The `alien mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `alien mcp set` and
  `alien doctor --fix` normalize into the canonical `transport` field.
...
See [MCP](/cli/mcp#alien-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.alien.install`
...
- Loaded from `~/.alien/extensions`, `<workspace>/.alien/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Alien plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Alien plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Alien option registry.
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Alien applies those as sanitized agent settings, not as raw Alien config patches.
...
`commitments` controls inferred follow-up memory: Alien can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Alien to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Alien treats the loopback port as a
...
- Local managed `alien` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ALIEN_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ALIEN_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `ALIEN_ALLOW_INSECURE_PRIVATE_WS=1`: client-side process-environment
  break-glass override that allows plaintext `ws://` to trusted private-network
  IPs; default remains loopback-only for plaintext. There is no `alien.json`
...
- `ALIEN_APNS_RELAY_BASE_URL` / `ALIEN_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `ALIEN_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `ALIEN_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```bash
ALIEN_CONFIG_PATH=~/.alien/a.json \
ALIEN_STATE_DIR=~/.alien-a \
alien gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.alien-dev` + port `19001`), `--profile <name>` (uses `~/.alien-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/alien/tls/server.crt",
      keyPath: "/etc/alien/tls/server.key",
      caPath: "/etc/alien/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.alien/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-alien-token: <token>`.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.alien/hooks/transforms`; workspace skill directories are rejected. If `alien doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ALIEN_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.alien/workspace/canvas",
    liveReload: true,
    // enabled: false, // or ALIEN_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__alien__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__alien__/a2ui/`
...
- Also serves A2UI at `/__alien__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `alien`. Override with `ALIEN_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.alien/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `alien dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.alien/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ALIEN_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `alien.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.alien/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/alien-vault-resolver",
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `alien doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.alien/credentials/oauth.json`.
...
- Default log file: `/tmp/alien/alien-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). Alien keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.alien/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, Alien uses the safer extended embedded-run window of at least 10 minutes and 5x `stuckSessionWarnMs`.
...
- `ALIEN_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. Alien then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$ALIEN_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Alien.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `ALIEN_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `alien doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `alien.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
- Alien-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `alien.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for Alien-owned writes; those writes fail closed instead of flattening the config.
