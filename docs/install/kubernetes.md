---
summary: "Deploy Alien Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Alien on a Kubernetes cluster
  - You want to test Alien in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running Alien on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Alien is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/alien 18789:18789 -n alien
...
```bash
kubectl get secret alien-secrets -n alien -o jsonpath='{.data.ALIEN_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: alien (configurable via ALIEN_NAMESPACE)
├── Deployment/alien        # Single pod, init container + gateway
├── Service/alien           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/alien-config  # alien.json + AGENTS.md
└── Secret/alien-secrets    # Gateway token + API keys
```
...
Edit `alien.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret alien-secrets -n alien \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/alien -n alien
```

### Custom namespace

```bash
ALIEN_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/alien/alien:latest # or pin to a specific version from https://github.com/alien/alien/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # alien.json + AGENTS.md
