---
summary: "Run Alien Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Alien running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Alien Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Alien 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Alien Gateway in Docker
- Persist `~/.alien` + `~/.alien/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.alien` state includes `alien.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-alien-project --name="Alien Gateway"
    gcloud config set project my-alien-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create alien-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh alien-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Alien repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    ALIEN_IMAGE=alien:latest
    ALIEN_GATEWAY_TOKEN=
    ALIEN_GATEWAY_BIND=lan
    ALIEN_GATEWAY_PORT=18789

    ALIEN_CONFIG_DIR=/home/$USER/.alien
    ALIEN_WORKSPACE_DIR=/home/$USER/.alien/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.alien
    ```

    Set `ALIEN_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Alien uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `ALIEN_GATEWAY_TOKEN`.
    ...
    services:
      alien-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - ALIEN_GATEWAY_BIND=${ALIEN_GATEWAY_BIND}
          - ALIEN_GATEWAY_PORT=${ALIEN_GATEWAY_PORT}
          - ALIEN_GATEWAY_TOKEN=${ALIEN_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${ALIEN_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`ALIEN_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm alien-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh alien-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop alien-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type alien-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start alien-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create alien-deploy \
     --display-name="Alien Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-alien-project \
     --member="serviceAccount:alien-deploy@my-alien-project.iam.gserviceaccount.com" \
