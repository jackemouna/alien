---
summary: "Run Alien Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want Alien running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running Alien in Docker on Hetzner or a similar provider
title: "Hetzner"
---

## Goal

Run a persistent Alien Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Alien 24/7 for ~$5", this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.alien/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.alien
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ALIEN_CONFIG_DIR=/root/.alien
    ALIEN_WORKSPACE_DIR=/root/.alien/workspace
...
- Infrastructure: [alien-terraform-hetzner](https://github.com/andreesg/alien-terraform-hetzner)
- Docker config: [alien-docker-config](https://github.com/andreesg/alien-docker-config)
