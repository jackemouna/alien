---
summary: "Host Alien on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Alien on a Raspberry Pi
  - Running Alien on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Alien Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'alien dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/alien-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/alien-compile-cache
mkdir -p /var/tmp/alien-compile-cache
export ALIEN_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running Alien, add a service drop-in:

```bash
systemctl --user edit alien-gateway.service
```

```ini
[Service]
Environment=ALIEN_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/alien-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart alien-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
Most Alien features work on ARM64 without changes (Node.js, Telegram, WhatsApp/Baileys, Chromium). The binaries that occasionally lack ARM builds are typically optional Go/Rust CLI tools shipped by skills. Verify a missing binary's release page for `linux-arm64` / `aarch64` artifacts before falling back to building from source.

## Persistence and backups

Alien state lives under:

- `~/.alien/` — `alien.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.alien/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u alien-gateway.service --no-pager -n 100` and run `alien doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
