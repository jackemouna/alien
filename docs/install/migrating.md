...
read_when:
  - You are moving Alien to a new laptop or server
  - You are coming from another agent system and want to keep state
  - You are upgrading an in-place plugin
title: "Migration guide"
---

Alien supports three migration paths: importing from another agent system, moving an existing install to a new machine, and upgrading a plugin in place.

## Import from another agent system

Use the bundled migration providers to bring instructions, MCP servers, skills, model config, and (opt-in) API keys into Alien. Plans are previewed before any change, secrets are redacted in reports, and apply is backed by a verified backup.
...
The CLI entry point is [`alien migrate`](/cli/migrate). Onboarding can also offer migration when it detects a known source (`alien onboard --flow import`).

## Move Alien to a new machine

Copy the **state directory** (`~/.alien/` by default) and your **workspace** to preserve:

- **Config** — `alien.json` and all gateway settings.
...
<Tip>
Run `alien status` on the old machine to confirm your state directory path. Custom profiles use `~/.alien-<profile>/` or a path set via `ALIEN_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    alien gateway stop
    cd ~
    tar -czf alien-state.tgz .alien
    ```

    If you use multiple profiles (for example `~/.alien-work`), archive each separately.

  </Step>

  <Step title="Install Alien on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine. It is fine if onboarding creates a fresh `~/.alien/`. You will overwrite it next.
  </Step>
...
```bash
awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.alien/.env
```

`alien doctor` also warns when an enabled default Telegram or Discord account has no configured token and the matching env variable is unavailable to the doctor process.

### Common pitfalls

<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `ALIEN_STATE_DIR` and the new one does not, channels will appear logged out and sessions will be empty. Launch the gateway with the **same** profile or state-dir you migrated, then rerun `alien doctor`.
  </Accordion>

  <Accordion title="Copying only alien.json">
...
- [ ] `alien status` shows the gateway running.
...
- [`alien migrate`](/cli/migrate): CLI reference for cross-system imports.
- [Install overview](/install): all installation methods.
- [Doctor](/gateway/doctor): post-migration health check.
- [Uninstall](/install/uninstall): removing Alien cleanly.
