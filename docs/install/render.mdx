---
summary: "Deploy Alien on Render with Infrastructure-as-Code"
read_when:
  - Deploying Alien to Render
...
Deploy Alien on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/alien/alien)
...
repository configures everything needed to run Alien:
...
services:
  - type: web
    ...
    envVars:
      - key: ALIEN_GATEWAY_PORT
        value: "8080"
      - key: ALIEN_STATE_DIR
        value: /data/.alien
      - key: ALIEN_WORKSPACE_DIR
        value: /data/workspace
      - key: ALIEN_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Alien state
...
`ALIEN_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Alien repository, Render will not auto-deploy your Alien. To update it, run a manual Blueprint sync from the dashboard.
...
For Alien, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Alien state plus any configured
...
- Missing `ALIEN_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ALIEN_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `alien backup create` in the Render shell.
