---
summary: "Install Alien - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Alien, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://alien.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Alien and Node kept under a local prefix such as
`~/.alien`, without depending on a system-wide Node install:

```bash
curl -fsSL https://alien.ai/install-cli.sh | bash
```
...
`alien update --channel dev` and `alien update --channel stable`. See
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g alien@latest
```
...
Or skip the link and use `pnpm alien ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:alien/alien#main
```
...
```bash
alien --version      # confirm the CLI is available
alien doctor         # check for config issues
alien gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `alien onboard --install-daemon` or `alien gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Alien on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Alien completely.
  </Card>
</CardGroup>

## Troubleshooting: `alien` not found

If the install succeeded but `alien` is not found in your terminal:
