---
summary: "Run Alien Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Alien running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Alien Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Alien.
...
- Install Alien with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-alien"
    LOCATION="westus2"
    VNET_NAME="vnet-alien"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-alien-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-alien-vm"
    VM_NAME="vm-alien"
    ADMIN_USERNAME="alien"
    BASTION_NAME="bas-alien"
    BASTION_PIP_NAME="pip-alien-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Alien (in the VM shell)">
    ```bash
    curl -fsSL https://alien.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Alien, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Alien onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Alien Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Alien Azure deployment with the GitHub Copilot model provider: [Alien on Azure with GitHub Copilot](https://github.com/johnsonshi/alien-azure-github-copilot)
