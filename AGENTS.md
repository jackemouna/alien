...
- Repo: `https://github.com/alien/alien`
...
- Extensions cross into core only via `alien/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- CLI: `pnpm alien ...` or `pnpm dev`; build: `pnpm build`.
...
- Vitest flags only; no Jest flags like `--runInBand`. For serial runs use `pnpm test:serial` or `ALIEN_VITEST_MAX_WORKERS=1 pnpm test ...`.
...
- Heavy checks: `ALIEN_LOCAL_CHECK=1`, mode `ALIEN_LOCAL_CHECK_MODE=throttled|full`; CI/shared use `ALIEN_LOCAL_CHECK=0`.
...
- Testbox full-suite profile: `blacksmith testbox run --id <ID> "env NODE_OPTIONS=--max-old-space-size=4096 ALIEN_TEST_PROJECTS_PARALLEL=6 ALIEN_VITEST_MAX_WORKERS=1 pnpm test"`. For installable package proof, prefer the GitHub `Package Acceptance` workflow over ad hoc Testbox commands.
...
- Search/dedupe: prefer `gh search issues 'repo:alien/alien is:open <terms>' --json number,title,state,updatedAt --limit 20`.
...
- Wait matrix:
  ...
  - release/manual only: `Docker Release`, `Alien NPM Release`, `macOS Release`, `Alien Release Checks`, `Cross-OS Release Checks`, `NPM Telegram Beta E2E`.
...
- Naming: **Alien** product/docs; `alien` CLI/package/path/config.
...
- Docs final answers: when doc files changed, end with the relevant full `https://docs.alien.ai/...` URL(s).
- Changelog user-facing only; fixing an issue or landing/merging a PR needs one unless pure test/internal.
- Changelog placement: active version `### Changes`/`### Fixes`; contributor-facing added entries should include at least one `Thanks @author` attribution, using credited human GitHub username(s). Never add `Thanks @codex`, `Thanks @alien`, `Thanks @clawsweeper`, or `Thanks @steipete`; if the real credited human is unknown, leave attribution blank instead of guessing or adding a random person.
...
- PR/issue workflows: `$alien-pr-maintainer`. `/landpr`: `~/.codex/prompts/landpr.md`.

## Security / Release

- Never commit real phone numbers, videos, credentials, live config.
- Secrets: channel/provider creds in `~/.alien/credentials/`; model auth profiles in `~/.alien/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Release docs: `docs/reference/RELEASING.md`; use `$alien-release-maintainer`.
- GHSA/advisories: `$alien-ghsa-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch` (tmux `alien-gateway-watch-main`, auto-attach). Noninteractive: `ALIEN_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch`; attach/stop: `tmux attach -t alien-gateway-watch-main` / `tmux kill-session -t alien-gateway-watch-main`. Managed installs: `alien gateway restart/status --deep`. No launchd/ad-hoc tmux. Logs: `./scripts/clawlog.sh`.
...
- Mobile LAN pairing: plaintext `ws://` loopback-only. Private-network `ws://` needs `ALIEN_ALLOW_INSECURE_PRIVATE_WS=1`; Tailscale/public use `wss://` or tunnel.
- A2UI hash `src/canvas-host/a2ui/.bundle.hash`: generated; ignore unless running `pnpm canvas:a2ui:bundle`; commit separately.

## Ops / Footguns

- Remote install docs: `docs/install/{exe-dev,fly,hetzner}.md`. Parallels smoke: `$alien-parallels-smoke`; Discord roundtrip: `parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `alien doctor`.
