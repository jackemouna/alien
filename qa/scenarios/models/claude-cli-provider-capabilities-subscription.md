...
execution:
  kind: flow
  summary: Run with `pnpm alien qa suite --provider-mode live-frontier --cli-auth-mode subscription --model claude-cli/claude-sonnet-4-6 --alt-model claude-cli/claude-sonnet-4-6 --scenario claude-cli-provider-capabilities-subscription`.
  config:
...
steps:
  - name: confirms the selected live provider and Claude CLI auth mode
    actions:
      ...
      - set: preserveEnv
        value:
          expr: "String(env.gateway.runtimeEnv.ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV ?? '')"
      - assert:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || env.gateway.runtimeEnv.ALIEN_LIVE_CLI_BACKEND_AUTH_MODE === config.authMode"
          message:
            expr: "`expected Claude CLI auth mode ${config.authMode}, got ${env.gateway.runtimeEnv.ALIEN_LIVE_CLI_BACKEND_AUTH_MODE ?? 'unset'}`"
      - assert:
    ...
    detailsExpr: "env.providerMode === 'live-frontier' ? `provider=${selected?.provider} model=${selected?.model} auth=${env.gateway.runtimeEnv.ALIEN_LIVE_CLI_BACKEND_AUTH_MODE} preserve=${preserveEnv}` : `mock-compatible provider=${selected?.provider}`"
