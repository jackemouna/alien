...
execution:
  kind: flow
  summary: Run with `ALIEN_LIVE_SETUP_TOKEN_VALUE=<setup-token> pnpm alien qa suite --provider-mode live-frontier --model anthropic/claude-opus-4-6 --alt-model anthropic/claude-opus-4-6 --scenario anthropic-opus-setup-token-smoke`.
  config:
...
steps:
  - name: confirms regular Anthropic setup-token lane
    actions:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || !env.gateway.runtimeEnv.ALIEN_LIVE_SETUP_TOKEN_VALUE"
