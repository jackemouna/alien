...
coverage:
  ...
  secondary:
    - models.openai
objective: Verify a live OpenAI GPT model can use OpenAI native web_search when Alien web search is enabled in auto mode.
successCriteria:
...
execution:
  kind: flow
  summary: Run with `ALIEN_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" pnpm alien qa suite --provider-mode live-frontier --model openai/gpt-5.5 --alt-model openai/gpt-5.5 --fast --thinking medium --scenario openai-native-web-search-live`.
  config:
...
steps:
  - name: confirms live OpenAI GPT-5.5 web search auto mode
    actions:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || Boolean(env.gateway.runtimeEnv.OPENAI_API_KEY?.trim() || env.gateway.runtimeEnv.ALIEN_LIVE_OPENAI_KEY?.trim())"
          message: expected OPENAI_API_KEY or ALIEN_LIVE_OPENAI_KEY for live OpenAI QA
