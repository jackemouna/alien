...
coverage:
  ...
  secondary:
    - tools.invocation
objective: Verify Alien can expose plugin tools over MCP and a real MCP client can call one successfully.
successCriteria:
...
execution:
  kind: flow
  summary: Verify Alien can expose plugin tools over MCP and a real MCP client can call one successfully.
  config:
