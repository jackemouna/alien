...
coverage:
  ...
  secondary:
    ...
    - agents.create
objective: Verify Crestodian can bootstrap a fresh Alien config, set the default model, create an agent, configure Discord through a SecretRef, validate config, and leave an audit trail.
successCriteria:
...
steps:
  - name: bootstraps config through Crestodian CLI
    actions:
      ...
      - set: configPath
        value:
          expr: "path.join(stateDir, 'alien.json')"
      ...
      - set: crestodianEnv
        value:
          expr: "({ ALIEN_STATE_DIR: stateDir, ALIEN_CONFIG_PATH: configPath, ALIEN_BUNDLED_PLUGINS_DIR: path.join(env.repoRoot, 'dist', 'extensions'), [setupSpec.discordEnv]: setupSpec.discordToken })"
      - call: fs.rm
        args:
          - ref: stateDir
          - recursive: true
