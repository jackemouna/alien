...
objective: Verify an agent can self-update an installed Alien package from 2026.4.26 to latest by using the gateway update.run action, then recover through the forced restart.
successCriteria:
...
execution:
  ...
  config:
    ...
    targetTag: latest
    allowEnv: ALIEN_QA_ALLOW_UPDATE_RUN_SELF
    channelId: qa-room
```
