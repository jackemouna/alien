...
steps:
  - name: emits protected low-cardinality prometheus metrics
    actions:
      ...
      - set: gatewayToken
        value:
          expr: "String(env.gateway.token ?? env.gateway.runtimeEnv.ALIEN_GATEWAY_TOKEN ?? '')"
      - assert:
      ...
      - assert:
          expr: "prometheusText.includes('# TYPE alien_run_completed_total counter')"
          message: "missing run completion counter"
      - assert:
          expr: "prometheusText.includes('# TYPE alien_run_duration_seconds histogram')"
          message: "missing run duration histogram"
      - assert:
          expr: "prometheusText.includes('# TYPE alien_model_call_total counter')"
          message: "missing model call counter"
      - assert:
          expr: "prometheusText.includes('# TYPE alien_harness_run_total counter')"
      ...
      - assert:
          expr: "!prometheusText.includes('alien.content.')"
          message: "prometheus output leaked content attributes"
      - assert:
          expr: "!/alien_prometheus_series_dropped_total(?:\\{[^}]*\\})?\\s+(?!0(?:\\.0+)?(?:\\s|$))/.test(prometheusText)"
