...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "AlienKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "AlienKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.ALIEN_SPARSE_PROFILE || process.env.ALIEN_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because ALIEN_A2UI_SKIP_MISSING=1 or ALIEN_SPARSE_PROFILE is set.",
      );
      return;
    }
