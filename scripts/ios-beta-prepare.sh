...
- regenerates apps/ios/Alien.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${ALIEN_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ALIEN_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ALIEN_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ALIEN_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ALIEN_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing ALIEN_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ALIEN_URL_SLASH)$(ALIEN_URL_SLASH)#g'
)"
...
ALIEN_CODE_SIGN_STYLE = Automatic
ALIEN_DEVELOPMENT_TEAM = ${TEAM_ID}
ALIEN_IOS_SELECTED_TEAM = ${TEAM_ID}
ALIEN_APP_BUNDLE_ID = ai.alien.client
ALIEN_SHARE_BUNDLE_ID = ai.alien.client.share
ALIEN_ACTIVITY_WIDGET_BUNDLE_ID = ai.alien.client.activitywidget
ALIEN_WATCH_APP_BUNDLE_ID = ai.alien.client.watchkitapp
ALIEN_WATCH_EXTENSION_BUNDLE_ID = ai.alien.client.watchkitapp.extension
ALIEN_APP_PROFILE =
ALIEN_SHARE_PROFILE =
ALIEN_PUSH_TRANSPORT = relay
ALIEN_PUSH_DISTRIBUTION = official
ALIEN_URL_SLASH = /
ALIEN_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ALIEN_PUSH_APNS_ENVIRONMENT = production
EOF
