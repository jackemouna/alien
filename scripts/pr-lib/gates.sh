...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${ALIEN_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with ALIEN_VITEST_MAX_WORKERS=$ALIEN_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env ALIEN_VITEST_MAX_WORKERS="$ALIEN_VITEST_MAX_WORKERS" pnpm test
      else
