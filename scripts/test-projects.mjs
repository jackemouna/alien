...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.ALIEN_VITEST_MAX_WORKERS || env.ALIEN_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      ALIEN_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.ALIEN_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.ALIEN_TEST_PROJECTS_PARALLEL &&
        !baseEnv.ALIEN_VITEST_MAX_WORKERS &&
        !baseEnv.ALIEN_TEST_WORKERS &&
