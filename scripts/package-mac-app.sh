...
# Build and bundle Alien into a minimal .app we can open.
# Outputs to dist/Alien.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Alien.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Alien"
MLX_TTS_HELPER_PRODUCT="alien-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.alien.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/alien/alien/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Alien/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AlienBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AlienGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Alien"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Alien"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Alien"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Alien" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Alien/Resources/Alien.icns" "$APP_ROOT/Contents/Resources/Alien.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Alien/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AlienKit resources"
ALIENKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AlienKit_AlienKit.bundle"
if [ -d "$ALIENKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AlienKit_AlienKit.bundle"
  cp -R "$ALIENKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AlienKit_AlienKit.bundle"
else
  echo "WARN: AlienKit resource bundle not found at $ALIENKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Alien"
killall -q Alien 2>/dev/null || true
