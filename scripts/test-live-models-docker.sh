...
if alien_live_truthy "${ALIEN_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  ALIEN_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${ALIEN_LIVE_PROVIDERS:-}" || -n "${ALIEN_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      alien_live_collect_auth_dirs_from_csv "${ALIEN_LIVE_PROVIDERS:-}"
      alien_live_collect_auth_dirs_from_csv "${ALIEN_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      alien_live_collect_auth_files_from_csv "${ALIEN_LIVE_PROVIDERS:-}"
      alien_live_collect_auth_files_from_csv "${ALIEN_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${ALIEN_DOCKER_PROFILE_ENV_ONLY:-0}"
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e ALIEN_LIVE_TEST=1 \
  -e ALIEN_LIVE_MODELS="${ALIEN_LIVE_MODELS:-modern}" \
  -e ALIEN_LIVE_PROVIDERS="${ALIEN_LIVE_PROVIDERS:-}" \
  -e ALIEN_LIVE_MAX_MODELS="${ALIEN_LIVE_MAX_MODELS:-12}" \
  -e ALIEN_LIVE_MODEL_TIMEOUT_MS="${ALIEN_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e ALIEN_LIVE_REQUIRE_PROFILE_KEYS="${ALIEN_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e ALIEN_LIVE_GATEWAY_MODELS="${ALIEN_LIVE_GATEWAY_MODELS:-}" \
  -e ALIEN_LIVE_GATEWAY_PROVIDERS="${ALIEN_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ALIEN_LIVE_GATEWAY_MAX_MODELS="${ALIEN_LIVE_GATEWAY_MAX_MODELS:-}" \
