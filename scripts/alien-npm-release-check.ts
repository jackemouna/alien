...
const EXPECTED_REPOSITORY_URL = "https://github.com/alien/alien";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@alien/fs-safe";
...
const skipPackValidationEnv = "ALIEN_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "alien") {
    errors.push(`package.json name must be "alien"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.alien !== "alien.mjs") {
    errors.push(
      `package.json bin.alien must be "alien.mjs"; found "${pkg.bin?.alien ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`alien-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `alien-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
