...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/alien/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Alien Re-Auth" -c "After getting key, run: ssh $SERVER '~/alien/scripts/mobile-reauth.sh'" --id alien-auth
        ;;
    *)
