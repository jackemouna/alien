...
# Checks both Claude Code and Alien auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ALIEN_AUTH="$HOME/.alien/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    alien models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    alien_status=$(check_alien_auth 2>/dev/null || true)

    claude_expires=0
    alien_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        alien_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        alien_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ALIEN_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$alien_status" \
        --arg be "$alien_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            alien: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$alien_status" == EXPIRED* ]] || [[ "$alien_status" == MISSING* ]]; then
        echo "ALIEN_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$alien_status" == EXPIRING* ]]; then
        echo "ALIEN_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Alien Auth (~/.alien/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ALIEN_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ALIEN_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'alien doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active alien >/dev/null 2>&1; then
    echo -e "Alien service: ${GREEN}running${NC}"
else
    echo -e "Alien service: ${RED}NOT running${NC}"
fi
