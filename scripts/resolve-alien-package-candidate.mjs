...
const DEFAULT_OUTPUT_NAME = "alien-current.tgz";
export const ALIEN_PACKAGE_SPEC_RE =
  /^alien@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-alien-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateAlienPackageSpec(spec) {
  if (!ALIEN_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be alien@alpha, alien@beta, alien@latest, or an exact Alien release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Alien branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `alien-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^alien-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no Alien tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-alien-for-docker.mjs",
        "--source-dir",
      ...
      validateAlienPackageSpec(options.packageSpec);
  ...
  console.error(`Checking Alien package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-alien-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `Alien package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "alien") {
    throw new Error(`package candidate must be named "alien"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
