#!/bin/bash
# Setup Alien Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'alien-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Alien message: Send warning via Alien itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/alien-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/alien-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/alien-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now alien-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status alien-auth-monitor.timer"
echo ""
echo "Quick commands:"
