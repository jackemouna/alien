...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.ALIEN_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestNoOutputTimeoutMs(env = process.env) {
  return parsePositiveInt(env.ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS);
}
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.ALIEN_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.ALIEN_VITEST_MAX_WORKERS ?? env.ALIEN_TEST_WORKERS);
}
