...
function printUsage(): void {
  console.log(`Alien Gateway startup benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "alien.json");
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "alien-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "alien-bench",
    npm_config_update_notifier: "false",
    ALIEN_CONFIG: configPath,
    ALIEN_CONFIG_PATH: configPath,
    ALIEN_GATEWAY_STARTUP_TRACE: "1",
    ALIEN_HOME: root,
    ALIEN_LOCAL_CHECK: "0",
    ALIEN_NO_RESPAWN: "1",
    ALIEN_STATE_DIR: path.join(root, "state"),
    ALIEN_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
  return env;
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "alien-gateway-bench-"));
  ...
  const childArgs = [
    ...(options.cpuProfDir
      ? [
          ...
          options.cpuProfDir,
          "--cpu-prof-name",
          `alien-gateway-${options.benchCase.id}-${options.sampleIndex}-${Date.now()}.cpuprofile`,
        ]
      : []),
    options.entry,
    "gateway",
