...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.ALIEN_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[alien] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[alien] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[alien] This usually means a file in node_modules is empty or truncated.");
  console.error("[alien] Recommended recovery:");
  console.error("[alien]   rm -rf node_modules");
  console.error("[alien]   pnpm store prune");
  console.error("[alien]   pnpm install");
  console.error("");
  console.error("[alien] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.ALIEN_WATCH_MODE = "1";
  childEnv.ALIEN_WATCH_SESSION = watchSession;
  ...
  childEnv.ALIEN_NO_RESPAWN = "1";
  if (isGatewayWatchCommand(deps.args) && childEnv.ALIEN_TRACE_SYNC_IO === undefined) {
    childEnv.ALIEN_TRACE_SYNC_IO = "1";
  }
  if (deps.args.length > 0) {
    childEnv.ALIEN_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `alien doctor --fix --non-interactive` once.",
        deps,
      );
