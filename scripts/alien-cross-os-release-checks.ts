...
const PUBLISHED_INSTALLER_BASE_URL = "https://alien.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "ALIEN_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
const CROSS_OS_AGENT_TURN_OPTIONAL = parseBooleanEnv("ALIEN_CROSS_OS_AGENT_TURN_OPTIONAL", true);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `ALIEN_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.ALIEN_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.alien-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "ALIEN_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.ALIEN_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.ALIEN_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.ALIEN_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `alien@${previousVersion}` : "alien@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.ALIEN_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.ALIEN_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        ALIEN_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `alien-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".alien");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    ALIEN_HOME: lane.homeDir,
    ALIEN_STATE_DIR: lane.stateDir,
    ALIEN_CONFIG_PATH: join(lane.stateDir, "alien.json"),
    ALIEN_DISABLE_BONJOUR: "1",
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.ALIEN_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]alien\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/alien[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "alien@latest") {
    return "";
  }
  if (trimmed.startsWith("alien@")) {
    return trimmed.slice("alien@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "alien@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view alien@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'alien.cmd'),
      (Join-Path $npmPrefix 'alien.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command alien -ErrorAction Stop
...
Write-Output "__ALIEN_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__ALIEN_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed alien path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const cliPath = parseMarkerLine(result.stdout, "__ALIEN_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed alien path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `alien update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.alien/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.ALIEN_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.ALIEN_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.ALIEN_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.ALIEN_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "alien/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.ALIEN_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.ALIEN_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.ALIEN_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing ALIEN_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "ALIEN_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.ALIEN_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.ALIEN_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      ALIEN_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      ALIEN_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Alien Control</title>") &&
          html.includes("<alien-app></alien-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Alien Control</title>")} app=${html.includes("<alien-app></alien-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runAlien(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "alien.mjs");
}
