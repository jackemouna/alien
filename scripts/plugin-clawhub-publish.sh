...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-alien/alien}}"
...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/alien-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${ALIEN_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${ALIEN_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
for attempt in $(seq 1 "${ALIEN_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${ALIEN_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${ALIEN_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${ALIEN_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
