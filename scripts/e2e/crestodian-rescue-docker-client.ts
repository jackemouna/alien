...
import type { AlienConfig } from "../../dist/config/types.alien.js";
...
function makeParams(commandBody: string, cfg: AlienConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: AlienConfig, isGroup = false): Promise<string> {
...
async function main() {
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "alien-crestodian-")));
  const configPath = process.env.ALIEN_CONFIG_PATH ?? path.join(stateDir, "alien.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env ALIEN_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/alien-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/alien-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as AlienConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "ALIEN_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/alien-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/alien-work",
    ),
