...
// against the Alien npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/alien-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      ALIEN_STATE_DIR: "/tmp/alien-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[alien mcp] ${String(chunk)}`);
  });
