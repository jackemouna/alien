...
import { applyDockerOpenAiProviderConfig, type AlienConfig } from "./docker-openai-seed.ts";
...
process.title = "alien-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.ALIEN_STATE_DIR?.trim() || path.join(os.homedir(), ".alien");
  const configPath =
    process.env.ALIEN_CONFIG_PATH?.trim() || path.join(stateDir, "alien.json");
