...
function resolveCredentialSource(env: NodeJS.ProcessEnv) {
  return env.ALIEN_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.ALIEN_QA_CREDENTIAL_SOURCE;
}

function resolveCredentialRole(env: NodeJS.ProcessEnv) {
  return env.ALIEN_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.ALIEN_QA_CREDENTIAL_ROLE;
}

async function resolveTrustedAlienCommand(rawCommand: string) {
  if (!path.isAbsolute(rawCommand)) {
    throw new Error("ALIEN_NPM_TELEGRAM_SUT_COMMAND must be an absolute path.");
  }
  const commandName = path.basename(rawCommand);
  if (commandName !== "alien" && commandName !== "alien.cmd") {
    throw new Error(
      `ALIEN_NPM_TELEGRAM_SUT_COMMAND must point to alien; got: ${commandName}`,
    );
  }
  const npmPrefix = process.env.NPM_CONFIG_PREFIX?.trim();
  if (!npmPrefix) {
    throw new Error("Missing NPM_CONFIG_PREFIX for installed alien command validation.");
  }
  ...
  if (realCommand !== realPrefix && !realCommand.startsWith(`${realPrefix}${path.sep}`)) {
    throw new Error("ALIEN_NPM_TELEGRAM_SUT_COMMAND must resolve inside NPM_CONFIG_PREFIX.");
  }
  return rawCommand;
}
...
async function main() {
  ...
  const rawSutAlienCommand = process.env.ALIEN_NPM_TELEGRAM_SUT_COMMAND?.trim();
  if (!rawSutAlienCommand) {
    throw new Error("Missing ALIEN_NPM_TELEGRAM_SUT_COMMAND.");
  }
  const sutAlienCommand = await resolveTrustedAlienCommand(rawSutAlienCommand);

  const repoRoot = path.resolve(process.env.ALIEN_NPM_TELEGRAM_REPO_ROOT ?? process.cwd());
  const outputDir =
    process.env.ALIEN_NPM_TELEGRAM_OUTPUT_DIR?.trim() ||
  ...
  const result = await runTelegramQaLive({
    ...
    providerMode: process.env.ALIEN_NPM_TELEGRAM_PROVIDER_MODE,
    primaryModel: process.env.ALIEN_NPM_TELEGRAM_MODEL,
    alternateModel: process.env.ALIEN_NPM_TELEGRAM_ALT_MODEL,
    fastMode: parseBoolean(process.env.ALIEN_NPM_TELEGRAM_FAST),
    scenarioIds: splitCsv(process.env.ALIEN_NPM_TELEGRAM_SCENARIOS),
    sutAccountId: process.env.ALIEN_NPM_TELEGRAM_SUT_ACCOUNT,
  ...
  if (
    !parseBoolean(process.env.ALIEN_NPM_TELEGRAM_ALLOW_FAILURES) &&
