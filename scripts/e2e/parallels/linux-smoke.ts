...
class LinuxSmoke {
  ...
  private disableBonjour = parseBoolEnv(process.env.ALIEN_PARALLELS_LINUX_DISABLE_BONJOUR);
  private hostIp = "";
  private hostPort = 0;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("alien-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("alien-parallels-linux-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("alien-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.ALIEN_PARALLELS_LINUX_AGENT_TIMEOUT_S || 1500),
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("alien-main-upgrade.tgz"),
    );
...
printf 'preflight.npmRoot=%s\n' "$(npm root -g 2>/dev/null || true)"`);
  ...
  private installLatestRelease(): void {
    this.guestExec(["curl", "-fsSL", this.options.installUrl, "-o", "/tmp/alien-install.sh"]);
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "ALIEN_NO_ONBOARD=1",
        "bash",
        "/tmp/alien-install.sh",
        "--version",
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["alien", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.alien/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@alien/test-bad-plugin","version":"1.0.0","alien":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/alien.plugin.json" <<'JSON'
...
config_path = Path("/root/.alien/alien.json")
...
PY`);
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " ALIEN_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "alien gateway run" >/dev/null 2>&1 || true
rm -f /tmp/alien-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env ALIEN_HOME=/root ALIEN_STATE_DIR=/root/.alien ALIEN_CONFIG_PATH=/root/.alien/alien.json${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["alien", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["alien", "gateway", "status", "--deep", "--require-rpc"]
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.alien/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.alien/test-bad-plugin`);
  }

  private verifyLocalTurn(): void {
    this.guestExec(["alien", "models", "set", this.auth.modelId]);
...
JSON
alien config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["alien", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} alien agent --local --agent main --session-id "$session_id" --message ${shellQuote(
