...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\alien\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
...
for attempt in 1 2; do
  ...
  rm -f "$HOME/.alien/agents/main/sessions/$session_id.jsonl"
  output_file="$(mktemp)"
  set +e
...
if [ "$agent_ok" != true ]; then
  echo "alien agent finished without OK response" >&2
  exit 1
fi`;
}

function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  return `$script:AlienUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ ALIEN_DISABLE_BUNDLED_PLUGINS = '1'; ALIEN_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1' } {
  Invoke-Alien update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:AlienUpdateExit = $LASTEXITCODE
}
$updateExit = $script:AlienUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-AlienGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-Alien gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-Alien gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-Alien gateway restart *>&1 | Out-Host
...
$sessionPath = Join-Path $env:USERPROFILE '.alien\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-Alien agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking minimal --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
path = Path.home() / ".alien" / "alien.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_alien_gateway_processes() {
  ALIEN_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/alien gateway stop || true
  pkill -f 'alien.*gateway' >/dev/null 2>&1 || true
}
start_alien_gateway() {
  if /opt/homebrew/bin/alien gateway restart; then
    return
  fi
  pkill -f 'alien.*gateway' >/dev/null 2>&1 || true
  rm -f /tmp/alien-parallels-macos-gateway.log
  nohup env ALIEN_HOME="$HOME" ALIEN_STATE_DIR="$HOME/.alien" ALIEN_CONFIG_PATH="$HOME/.alien/alien.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} /opt/homebrew/bin/alien gateway run --bind loopback --port 18789 --force >/tmp/alien-parallels-macos-gateway.log 2>&1 </dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if /opt/homebrew/bin/alien gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/alien-parallels-macos-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_alien_gateway_processes
ALIEN_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ALIEN_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/alien update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("/opt/homebrew/bin/alien", input.expectedNeedle)}
start_alien_gateway
wait_for_gateway
/opt/homebrew/bin/alien models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("/opt/homebrew/bin/alien", input.auth.modelId, "macos")}
/opt/homebrew/bin/alien config set agents.defaults.skipBootstrap true --strict-json
/opt/homebrew/bin/alien config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("/opt/homebrew/bin/alien", input, "parallels-npm-update-macos")}`;
}
...
function Stop-AlienGatewayProcesses {
  Invoke-Alien gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'alien.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "alien update failed with exit code $updateExit" }
  Write-Host "alien update returned a stale post-swap module import; continuing to post-update health checks"
}
...
const configPath = path.join(process.env.HOME || "/root", ".alien", "alien.json");
...
stop_alien_gateway_processes() {
  ALIEN_DISABLE_BUNDLED_PLUGINS=1 alien gateway stop || true
  pkill -f 'alien.*gateway' >/dev/null 2>&1 || true
}
start_alien_gateway() {
  pkill -f "alien gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/alien-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env ALIEN_HOME=/root ALIEN_STATE_DIR=/root/.alien ALIEN_CONFIG_PATH=/root/.alien/alien.json ALIEN_DISABLE_BONJOUR=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
    )} alien gateway run --bind loopback --port 18789 --force >/tmp/alien-parallels-linux-gateway.log 2>&1`,
  )} >/dev/null 2>&1 < /dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if alien gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/alien-parallels-linux-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_alien_gateway_processes
ALIEN_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ALIEN_DISABLE_BUNDLED_PLUGINS=1 alien update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("alien", input.expectedNeedle)}
start_alien_gateway
wait_for_gateway
alien models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("alien", input.auth.modelId, "linux")}
alien config set agents.defaults.skipBootstrap true --strict-json
alien config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("alien", input, "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-Alien --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "alien --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
