...
export function resolveProviderAuth(input: {
  ...
  const providerDefaults: Record<Provider, Omit<ProviderAuth, "apiKeyValue">> = {
    anthropic: {
      ...
      modelId:
        input.modelId ||
        process.env.ALIEN_PARALLELS_ANTHROPIC_MODEL ||
    ...
    minimax: {
      ...
      modelId:
        input.modelId || process.env.ALIEN_PARALLELS_MINIMAX_MODEL || "minimax/MiniMax-M2.7",
    },
    openai: {
      apiKeyEnv: input.apiKeyEnv || "OPENAI_API_KEY",
      authChoice: "openai-api-key",
      authKeyFlag: "openai-api-key",
      modelId: input.modelId || process.env.ALIEN_PARALLELS_OPENAI_MODEL || "openai/gpt-5.5",
    },
  };
...
export function resolveWindowsProviderAuth(input: {
  ...
  const windowsModel = process.env.ALIEN_PARALLELS_WINDOWS_OPENAI_MODEL?.trim();
  if (windowsModel) {
    return { ...auth, modelId: windowsModel };
  }
  if (process.env.ALIEN_PARALLELS_OPENAI_MODEL?.trim()) {
    return auth;
  }
...
export function resolveParallelsModelTimeoutSeconds(platform?: Platform): number {
  const platformEnv =
    platform === undefined
      ? undefined
      : process.env[`ALIEN_PARALLELS_${platform.toUpperCase()}_MODEL_TIMEOUT_S`];
  const defaultSeconds = platform === "macos" || platform === "windows" ? 1800 : 900;
  const raw = Number(
    platformEnv || process.env.ALIEN_PARALLELS_MODEL_TIMEOUT_S || defaultSeconds,
  );
...
export function resolveLatestVersion(versionOverride = ""): string {
  ...
  return run(
    ...
    [
      ...
      "version",
      "--userconfig",
      mkdtempSync(path.join(tmpdir(), "alien-npm-")),
    ],
    {
      quiet: true,
    },
