...
const guestAlien = "/opt/homebrew/bin/alien";
const guestAlienEntry = "/opt/homebrew/lib/node_modules/alien/alien.mjs";
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://alien.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://alien.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("alien-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("alien-parallels-macos-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("alien-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.ALIEN_PARALLELS_MACOS_AGENT_TIMEOUT_S || 2700),
      () => this.verifyTurn(),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      ...
      await this.phase(
        "upgrade.update-dev",
        Number(process.env.ALIEN_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S || 1800),
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'alien.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'alien-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'alien.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g alien >/dev/null 2>&1 || true
rm -rf "$HOME/.alien"
...
curl -fsSL ${shellQuote(this.options.installUrl)} -o /tmp/alien-install.sh
bash /tmp/alien-install.sh --version ${shellQuote(this.installVersion)}
${guestAlien} --version`,
    );
  }
...
${guestAlien} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestAlien, "--version"]);
...
check_path "$root/alien"
check_path "$root/alien/extensions"
if [ -d "$root/alien/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/alien/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/alien-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/alien`)}
export PATH=${shellQuote(`/tmp/alien-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".alien", "alien.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 ALIEN_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ALIEN_DISABLE_BUNDLED_PLUGINS=1 ${guestNode} ${guestAlienEntry} update --channel dev --yes --json
${guestNode} ${guestAlienEntry} --version
${guestNode} ${guestAlienEntry} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestExec([guestNode, guestAlienEntry, "update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'alien.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'alien-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'alien.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} ALIEN_HOME=${shellQuote(home)} ALIEN_STATE_DIR=${shellQuote(`${home}/.alien`)} ALIEN_CONFIG_PATH=${shellQuote(
        `${home}/.alien/alien.json`,
      )} ${guestNode} ${guestAlienEntry} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/alien-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestAlien, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestAlien(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestAlien(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`alien ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/alien-dashboard-smoke.html 2>/dev/null; then
    grep -F '<title>Alien Control</title>' /tmp/alien-dashboard-smoke.html >/dev/null &&
      grep -F '<alien-app></alien-app>' /tmp/alien-dashboard-smoke.html >/dev/null &&
      exit 0
  fi
  sleep 1
done
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestExec([guestNode, guestAlienEntry, "models", "set", this.auth.modelId]);
...
rm -f "$provider_config_batch"`);
    ...
    this.guestExec([guestNode, guestAlienEntry, "config", "set", "tools.profile", "minimal"]);
    this.guestSh(
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestNode} ${guestAlienEntry} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
