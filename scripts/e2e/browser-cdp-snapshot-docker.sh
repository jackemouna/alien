...
BASE_IMAGE="$(docker_e2e_resolve_image "alien-browser-cdp-base-e2e" ALIEN_BROWSER_CDP_BASE_E2E_IMAGE)"
IMAGE_NAME="$(docker_e2e_resolve_image "alien-browser-cdp-snapshot-e2e" ALIEN_BROWSER_CDP_SNAPSHOT_E2E_IMAGE)"
SKIP_BUILD="${ALIEN_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="alien-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${ALIEN_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
...
if [ "${ALIEN_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$SKIP_BUILD" = "1" ]; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/alien-browser-cdp-build.XXXXXX")"
...
ALIEN_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
mkdir -p /tmp/alien-browser-cdp/chrome
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/alien-browser-cdp/ \;
PORT=$PORT CDP_PORT=$CDP_PORT node scripts/e2e/lib/fixture.mjs browser-cdp
chromium --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage \\
  --remote-debugging-address=127.0.0.1 \\
  --remote-debugging-port=$CDP_PORT \\
  --user-data-dir=/tmp/alien-browser-cdp/chrome \\
...
alien_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/browser-cdp-gateway.log" >/dev/null

echo "Waiting for Chromium and Gateway..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "
    source scripts/lib/alien-e2e-instance.sh
    alien_e2e_probe_http_status http://127.0.0.1:$CDP_PORT/json/version
    alien_e2e_probe_tcp 127.0.0.1 $PORT
"; then
...
set -euo pipefail
source /tmp/alien-test-state-env
source scripts/lib/alien-e2e-instance.sh
entry=\"\$(alien_e2e_resolve_entrypoint)\"
