#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Alien Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "alien-openwebui-e2e" ALIEN_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${ALIEN_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ALIEN_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${ALIEN_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${ALIEN_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ALIEN_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ALIEN_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="alien-openwebui-e2e-$$"
GW_NAME="alien-openwebui-gateway-$$"
OW_NAME="alien-openwebui-$$"
DOCKER_COMMAND_TIMEOUT="${ALIEN_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-600s}"
DOCKER_PULL_TIMEOUT="${ALIEN_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"

PROFILE_FILE="${ALIEN_TESTBOX_PROFILE_FILE:-$HOME/.alien-testbox-live.profile}"
...
docker_e2e_docker_cmd run -d \
  ...
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ALIEN_GATEWAY_TOKEN=$TOKEN" \
  -e "ALIEN_OPENWEBUI_MODEL=$MODEL" \
  -e "ALIEN_SKIP_CHANNELS=1" \
  -e "ALIEN_SKIP_GMAIL_WATCHER=1" \
  -e "ALIEN_SKIP_CRON=1" \
  -e "ALIEN_SKIP_CANVAS_HOST=1" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    entry="$(alien_e2e_resolve_entrypoint)"

    openai_api_key="${OPENAI_API_KEY:?OPENAI_API_KEY required}"
    batch_file="$(mktemp /tmp/alien-openwebui-config.XXXXXX.json)"
    ALIEN_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    alien_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "ALIEN_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Alien E2E" \
  -e WEBUI_SECRET_KEY="alien-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Alien E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="alien/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "ALIEN_HTTP_PROBE_BEARER='$TOKEN' ALIEN_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Alien smoke..."
