#!/usr/bin/env bash
# Verifies `alien plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "alien-plugin-update-e2e" ALIEN_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${ALIEN_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${ALIEN_CURRENT_PACKAGE_TGZ:-}")"
...
ALIEN_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
