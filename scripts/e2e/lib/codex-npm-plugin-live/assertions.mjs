...
const allowBetaCompatDiagnostics =
  process.env.ALIEN_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";

function stateDir() {
  return process.env.ALIEN_STATE_DIR || path.join(process.env.HOME, ".alien");
}
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@alien/codex";
  const list = readJson("/tmp/alien-codex-plugins-list.json");
  const inspect = readJson("/tmp/alien-codex-plugin-inspect.json");
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @alien/codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@alien/codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .alien/npm");
  }
...
function assertPreflight() {
  ...
  const output = fs.readFileSync("/tmp/alien-codex-preflight.log", "utf8");
...
function assertAgentTurn() {
  ...
  const stdout = fs.readFileSync("/tmp/alien-codex-agent.json", "utf8");
  const stderr = fs.existsSync("/tmp/alien-codex-agent.err")
    ? fs.readFileSync("/tmp/alien-codex-agent.err", "utf8")
    : "";
  const response = JSON.parse(stdout);
  const text = (response.payloads || []).map((payload) => payload?.text || "").join("\n");
  if (!text.includes(marker)) {
    throw new Error(
      `Alien agent reply did not contain ${marker}:\nstdout=${stdout}\nstderr=${stderr}`,
    );
  }
  ...
  if (typeof entry.sessionFile !== "string" || !fs.existsSync(entry.sessionFile)) {
    throw new Error(`missing Alien session file: ${entry.sessionFile}`);
  }
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/alien-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  if (!Number.isInteger(status) || status === 0) {
    throw new Error(
      `expected Alien agent to fail after Codex uninstall, got status ${process.argv[3]}`,
    );
  }
  const stdout = fs.existsSync("/tmp/alien-codex-agent-after-uninstall.json")
    ? fs.readFileSync("/tmp/alien-codex-agent-after-uninstall.json", "utf8")
    : "";
  const stderr = fs.existsSync("/tmp/alien-codex-agent-after-uninstall.err")
    ? fs.readFileSync("/tmp/alien-codex-agent-after-uninstall.err", "utf8")
    : "";
