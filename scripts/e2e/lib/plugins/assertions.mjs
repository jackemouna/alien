...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.ALIEN_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".alien", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Alien plugin install/update/uninstall commands. Use `alien plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".alien", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(alienPeerPath)) {
    throw new Error(`missing ClawHub alien peer symlink: ${alienPeerPath}`);
  }
  if (!fs.lstatSync(alienPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub alien peer is not a symlink: ${alienPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(alienPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub alien peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@alien/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@alien/demo-plugin-npm") {
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.ALIEN_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.ALIEN_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".alien", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".alien", "alien.json");
