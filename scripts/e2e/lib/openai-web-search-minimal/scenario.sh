...
export ALIEN_SKIP_GMAIL_WATCHER=1
export ALIEN_SKIP_CRON=1
export ALIEN_SKIP_CANVAS_HOST=1
export ALIEN_SKIP_BROWSER_CONTROL_SERVER=1
export ALIEN_SKIP_ACPX_RUNTIME=1
export ALIEN_SKIP_ACPX_RUNTIME_PROBE=1

PORT="${PORT:?missing PORT}"
MOCK_PORT="${MOCK_PORT:?missing MOCK_PORT}"
TOKEN="${ALIEN_GATEWAY_TOKEN:?missing ALIEN_GATEWAY_TOKEN}"
SUCCESS_MARKER="ALIEN_SCHEMA_E2E_OK"
RAW_SCHEMA_ERROR="400 The following tools cannot be used with reasoning.effort 'minimal': web_search."
MOCK_REQUEST_LOG="/tmp/alien-openai-web-search-minimal-requests.jsonl"
GATEWAY_LOG="/tmp/alien-openai-web-search-minimal-gateway.log"
mock_pid=""
gateway_pid=""
...
dump_debug_logs() {
  ...
  for file in \
    "$GATEWAY_LOG" \
    /tmp/alien-openai-web-search-minimal-mock.log \
    /tmp/alien-openai-web-search-minimal-client-success.log \
    /tmp/alien-openai-web-search-minimal-client-reject.log \
    "$MOCK_REQUEST_LOG" \
    "$ALIEN_STATE_DIR/alien.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
MOCK_PORT="$MOCK_PORT" \
  ...
  node scripts/e2e/lib/openai-web-search-minimal/mock-server.mjs >/tmp/alien-openai-web-search-minimal-mock.log 2>&1 &
mock_pid="$!"
...
PORT="$PORT" ALIEN_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs success >/tmp/alien-openai-web-search-minimal-client-success.log 2>&1

node scripts/e2e/lib/openai-web-search-minimal/assertions.mjs assert-success-request "$MOCK_REQUEST_LOG"

PORT="$PORT" ALIEN_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs reject >/tmp/alien-openai-web-search-minimal-client-reject.log 2>&1
