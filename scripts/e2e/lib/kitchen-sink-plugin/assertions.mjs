...
function scanLogs() {
  const roots = ["/tmp", path.join(process.env.HOME, ".alien")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /alien-kitchen-sink-/u.test(path.basename(entry))) {
...
function assertClawHubExternalInstallContract(installPath) {
  const alienPeerPath = path.join(installPath, "node_modules", "alien");
  if (!fs.existsSync(alienPeerPath)) {
    throw new Error(`missing kitchen-sink alien peer symlink: ${alienPeerPath}`);
  }
  if (!fs.lstatSync(alienPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink alien peer is not a symlink: ${alienPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(alienPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink alien peer ${linkedHostRoot} to target ${hostRoot}`);
  }
