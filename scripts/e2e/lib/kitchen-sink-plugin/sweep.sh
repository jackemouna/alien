...
start_kitchen_sink_clawhub_fixture_server() {
  ...
  local wait_attempts="${ALIEN_CLAWHUB_FIXTURE_WAIT_ATTEMPTS:-600}"
  for _ in $(seq 1 "$wait_attempts"); do
    if [[ -s "$server_port_file" ]]; then
      export ALIEN_CLAWHUB_URL="http://127.0.0.1:$(cat "$server_port_file")"
...
run_success_scenario() {
  ...
  if [ -n "${KITCHEN_SINK_PREINSTALL_SPEC:-}" ]; then
    run_logged_print "kitchen-sink-preinstall-${KITCHEN_SINK_LABEL}" node "$ALIEN_ENTRY" plugins install "$KITCHEN_SINK_PREINSTALL_SPEC"
    assert_kitchen_sink_cutover_preinstalled
    install_args+=("--force")
  fi
  run_logged_print "kitchen-sink-install-${KITCHEN_SINK_LABEL}" node "$ALIEN_ENTRY" plugins install "${install_args[@]}"
  configure_kitchen_sink_runtime
  run_logged_print "kitchen-sink-enable-${KITCHEN_SINK_LABEL}" node "$ALIEN_ENTRY" plugins enable "$KITCHEN_SINK_ID"
  node "$ALIEN_ENTRY" plugins list --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-plugins.json"
  node "$ALIEN_ENTRY" plugins inspect "$KITCHEN_SINK_ID" --runtime --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect.json"
  node "$ALIEN_ENTRY" plugins inspect --all --runtime --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect-all.json"
  assert_kitchen_sink_installed
  if [ "$KITCHEN_SINK_SOURCE" = "clawhub" ]; then
    run_logged_print "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" node "$ALIEN_ENTRY" plugins uninstall "$KITCHEN_SINK_SPEC" --force
  else
    run_logged_print "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" node "$ALIEN_ENTRY" plugins uninstall "$KITCHEN_SINK_ID" --force
  fi
  remove_kitchen_sink_channel_config
  node "$ALIEN_ENTRY" plugins list --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json"
  assert_kitchen_sink_removed
}

run_failure_scenario() {
  echo "Testing expected ${KITCHEN_SINK_LABEL} install failure from ${KITCHEN_SINK_SPEC}..."
  run_expect_failure "install-${KITCHEN_SINK_LABEL}" node "$ALIEN_ENTRY" plugins install "$KITCHEN_SINK_SPEC"
  remove_kitchen_sink_channel_config
  node "$ALIEN_ENTRY" plugins list --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json"
  assert_kitchen_sink_removed
}
...
if [[ "$KITCHEN_SINK_SCENARIOS" == *"clawhub:"* ]]; then
  if [[ "${ALIEN_KITCHEN_SINK_LIVE_CLAWHUB:-0}" = "1" ]]; then
    export ALIEN_CLAWHUB_URL="${ALIEN_CLAWHUB_URL:-${CLAWHUB_URL:-https://clawhub.ai}}"
  else
    if [[ -n "${ALIEN_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]; then
      echo "Ignoring ambient ClawHub URL for fixture-mode kitchen-sink E2E; set ALIEN_KITCHEN_SINK_LIVE_CLAWHUB=1 for live ClawHub."
    fi
    unset ALIEN_CLAWHUB_URL CLAWHUB_URL
    clawhub_fixture_dir="$(mktemp -d "/tmp/alien-kitchen-sink-clawhub.XXXXXX")"
...
while IFS='|' read -r label spec plugin_id source expectation surface_mode personality preinstall_spec; do
  ...
  export ALIEN_KITCHEN_SINK_PERSONALITY="${personality:-}"
