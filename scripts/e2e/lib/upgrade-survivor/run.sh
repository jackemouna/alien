...
ARTIFACT_ROOT="$(dirname "${ALIEN_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/alien-upgrade-survivor-artifacts/summary.json}")"
...
SUMMARY_JSON="${ALIEN_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${ALIEN_UPGRADE_SURVIVOR_BASELINE:?missing ALIEN_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${ALIEN_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${ALIEN_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${ALIEN_CURRENT_PACKAGE_TGZ:-}}"
...
export ALIEN_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "ALIEN_UPGRADE_SURVIVOR_BASELINE must be alien@latest, alien@beta, alien@alpha, an exact Alien release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "ALIEN_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    alien@*)
      baseline_spec="$raw"
      baseline_version="${raw#alien@}"
      ;;
    *@*)
      echo "ALIEN_UPGRADE_SURVIVOR_BASELINE must be alien@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="alien@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "ALIEN_UPGRADE_SURVIVOR_BASELINE must be alien@latest, alien@beta, alien@alpha, alien@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "ALIEN_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.ALIEN_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.ALIEN_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.ALIEN_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      alien_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/alien\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${ALIEN_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "ALIEN_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@alien-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${ALIEN_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$ALIEN_STATE_DIR/extensions/opik-alien"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-alien",
...
JSON
  cat >"$shadow_root/alien.plugin.json" <<'JSON'
{
  "id": "opik-alien",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.alien-runtime-deps.json" \
      "$plugin_dir/.alien-runtime-deps-stamp.json" \
      "$plugin_dir/.alien-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.alien-install-stage-upgrade-survivor" \
      "$plugin_dir/.alien-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ALIEN_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ALIEN_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/alien-upgrade-survivor-dep" \
      "$plugin_dir/.alien-runtime-deps-copy-upgrade-survivor/node_modules/alien-upgrade-survivor-dep" \
      "$plugin_dir/.alien-install-stage-upgrade-survivor" \
      "$plugin_dir/.alien-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/alien-upgrade-survivor-dep" \
      "$ALIEN_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/alien-upgrade-survivor-dep" \
      "$ALIEN_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/alien-upgrade-survivor-dep"
    printf '{"name":"alien-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/alien-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.alien-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.alien-runtime-deps-stamp.json"
    printf '{"name":"alien-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.alien-runtime-deps-copy-upgrade-survivor/node_modules/alien-upgrade-survivor-dep/package.json"
    printf '{"name":"alien-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/alien-upgrade-survivor-dep/package.json"
    printf '{"name":"alien-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ALIEN_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/alien-upgrade-survivor-dep/package.json"
    printf '{"name":"alien-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ALIEN_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/alien-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"alien-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
install_baseline() {
  ...
  if ! command -v alien >/dev/null; then
    echo "baseline install did not expose alien on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(alien --version 2>&1)"; then
    echo "baseline alien --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline alien --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  ...
  alien_test_state_create "$ARTIFACT_ROOT/state-home" minimal
  export ALIEN_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! alien config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
...
log_file="${ALIEN_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/alien-systemctl-shim.log}"
pid_file="${ALIEN_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/alien-systemctl-shim.pid}"
daemon_log="${ALIEN_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/alien-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/alien-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! env -u ALIEN_GATEWAY_TOKEN -u ALIEN_GATEWAY_PASSWORD alien gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
...
const stateDir = process.env.ALIEN_STATE_DIR;
if (!stateDir) {
  throw new Error("missing ALIEN_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  mkdir -p "$ALIEN_STATE_DIR"
  local dotenv_path="$ALIEN_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$ALIEN_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  ALIEN_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing ALIEN_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  if ! env -u ALIEN_GATEWAY_TOKEN -u ALIEN_GATEWAY_PASSWORD alien "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
...
run_doctor() {
  if ! alien doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
...
validate_post_doctor_config() {
  if ! alien config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${ALIEN_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$ALIEN_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  args+=(--out "$out_file")
...
start_gateway() {
  local port=18789
  local budget="${ALIEN_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u ALIEN_GATEWAY_TOKEN -u ALIEN_GATEWAY_PASSWORD alien gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_probes() {
  ...
  export ALIEN_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING="discord,telegram,whatsapp,feishu,matrix"
  readyz_seconds="$(probe_gateway_endpoint /readyz ready "$READYZ_JSON")"
  unset ALIEN_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING
}

check_gateway_status() {
  local port=18789
  local budget="${ALIEN_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! alien gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
