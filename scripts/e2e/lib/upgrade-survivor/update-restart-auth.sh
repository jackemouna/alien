...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >"$ALIEN_STATE_DIR/gateway.systemd.env"
}
...
prepare_update_restart_probe_current_install() {
  ...
  env -u ALIEN_GATEWAY_TOKEN -u ALIEN_GATEWAY_PASSWORD alien gateway --port "$port" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
  printf '%s\n' "$gateway_pid" >"$ALIEN_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE"
  alien_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360
  ready_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  start_seconds=$(((ready_epoch - start_epoch + 999) / 1000))
  write_update_restart_service_auth_env
  if ! env -u ALIEN_GATEWAY_TOKEN -u ALIEN_GATEWAY_PASSWORD alien gateway install --force --json >"$ALIEN_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" 2>"$ALIEN_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR"; then
    echo "gateway service install failed" >&2
    cat "$ALIEN_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR" >&2 || true
    cat "$ALIEN_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" >&2 || true
    return 1
  fi
}
