    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/alien-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.ALIEN_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
function runAlien(step) {
  const result = spawnSync("alien", step.argv, {
    encoding: "utf8",
    env: process.env,
  });
  return {
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runAlien(adaptedStep);
