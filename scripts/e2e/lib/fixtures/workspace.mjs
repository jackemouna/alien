...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.ALIEN_WORKSPACE_DIR || path.join(process.env.HOME, ".alien", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.ALIEN_STATE_DIR, "ALIEN_STATE_DIR");
  ...
  writeJson(path.join(stateDir, "alien.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.ALIEN_STATE_DIR, "alien.json"))?.agents?.list ?? [];
