...
const timeoutMs = Number(process.env.ALIEN_HTTP_PROBE_TIMEOUT_MS ?? 30_000);
const controller = new AbortController();
const timer = setTimeout(() => controller.abort(), timeoutMs);

try {
  const headers = {};
  if (process.env.ALIEN_HTTP_PROBE_BEARER) {
    headers.authorization = `Bearer ${process.env.ALIEN_HTTP_PROBE_BEARER}`;
  }
