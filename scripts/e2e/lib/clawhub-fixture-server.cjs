...
const packageName = "@alien/kitchen-sink";
const pluginId = "alien-kitchen-sink-fixture";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "alien-clawhub-fixture-"));
  try {
...
`,
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          name: packageName,
          displayName: "Alien Kitchen Sink",
          family: "code-plugin",
          runtimeId: pluginId,
          channel: "official",
          isOfficial: true,
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/alien/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
async function main() {
  ...
  zip.file("package/alien.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "Alien Kitchen Sink",
