...
function loadManifestEntries() {
  const explicit = (process.env.ALIEN_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  const manifestEntries = fs
    ...
    .map((entry) => {
      const manifestPath = path.join(extensionRoot, entry.name, "alien.plugin.json");
...
function selectedManifestEntries() {
  ...
  const total = Number.parseInt(process.env.ALIEN_BUNDLED_PLUGIN_SWEEP_TOTAL || "1", 10);
  const index = Number.parseInt(process.env.ALIEN_BUNDLED_PLUGIN_SWEEP_INDEX || "0", 10);
  if (!Number.isInteger(total) || total < 1) {
    throw new Error(
      `ALIEN_BUNDLED_PLUGIN_SWEEP_TOTAL must be >= 1, got ${process.env.ALIEN_BUNDLED_PLUGIN_SWEEP_TOTAL}`,
    );
  }
  if (!Number.isInteger(index) || index < 0 || index >= total) {
    throw new Error(
      `ALIEN_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.ALIEN_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
...
function assertInstalled(pluginId, pluginDir, requiresConfig) {
  ...
  const indexPath = path.join(process.env.HOME, ".alien", "plugins", "installs.json");
...
function assertUninstalled(pluginId, pluginDir) {
  ...
  const managedPath = path.join(process.env.HOME, ".alien", "extensions", pluginId);
