...
const WATCHDOG_MS = readPositiveInt(process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS, 1000);
const READY_TIMEOUT_MS = readPositiveInt(
  process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_READY_MS,
  900000,
);
const RPC_TIMEOUT_MS = readPositiveInt(process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_RPC_MS, 60000);
const RPC_READY_TIMEOUT_MS = readPositiveInt(
  process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS,
  210000,
);
...
function manifestPath(pluginDir) {
  return path.join(process.cwd(), "dist", "extensions", pluginDir, "alien.plugin.json");
}
...
function configPathFromEnv(env = process.env) {
  return (
    env.ALIEN_CONFIG_PATH || path.join(env.HOME || os.homedir(), ".alien", "alien.json")
  );
}
  ...
  const child = childProcess.spawn(
    ...
    ],
    {
      env: {
        ...
        ...params.env,
        ALIEN_NO_ONBOARD: "1",
        ALIEN_SKIP_CHANNELS: params.skipChannels ? "1" : "0",
...
async function rpcCall(method, params, options) {
  const rpcStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-plugin-runtime-rpc-"));
  const args = [
    options.entrypoint,
    "gateway",
    "call",
...
async function smokePlugin(pluginId, pluginDir, requiresConfig, pluginIndex) {
  ...
  const entrypoint = process.env.ALIEN_ENTRY;
  if (!entrypoint) {
    throw new Error("missing ALIEN_ENTRY");
  }
  const manifest = loadManifest(pluginDir);
  const plan = buildPluginPlan(manifest);
  const port =
    readPositiveInt(process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_PORT_BASE, 19000) + pluginIndex * 3;
  ...
  const logPath = `/tmp/alien-plugin-runtime-${pluginIndex}-${pluginId}.log`;
...
async function smokeTtsGlobalDisable(pluginId, pluginDir, provider, pluginIndex) {
  ...
  const port =
    readPositiveInt(process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_PORT_BASE, 19000) +
    pluginIndex * 3 +
    1;
  ...
  const logPath = `/tmp/alien-plugin-runtime-${pluginIndex}-${pluginId}-tts-disabled.log`;
...
async function smokeOpenAiTts(pluginIndex) {
  ...
  const logPath = `/tmp/alien-plugin-runtime-${pluginIndex}-openai-tts-live.log`;
  const child = startGateway({ entrypoint, port, logPath, env, skipChannels: true });
  try {
    await waitForReady({ child, port, logPath });
    await assertBaseGatewayProbes({ entrypoint, port, env });
...
function createIsolatedStateEnv(label) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `alien-${label}-`));
