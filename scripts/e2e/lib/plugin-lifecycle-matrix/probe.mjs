...
function alienPath(...parts) {
  return path.join(home, ".alien", ...parts);
}
...
function records() {
  const index = readJson(alienPath("plugins", "installs.json"));
...
function config() {
  return readJson(process.env.ALIEN_CONFIG_PATH ?? alienPath("alien.json"));
}
