...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/alien-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/alien-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${ALIEN_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${ALIEN_SECRET_ENV_NAME:-}" ] && [ -n "${ALIEN_SECRET_FILE:-}" ] && [ -f "$ALIEN_SECRET_FILE" ]; then
  secret_value="$(cat "$ALIEN_SECRET_FILE")"
  rm -f "$ALIEN_SECRET_FILE"
  export "${ALIEN_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "ALIEN_SECRET_ENV_NAME=$api_key_env" \
      "ALIEN_SECRET_FILE=$secret_path" \
      "ALIEN_WRAPPER_FILE=$wrapper_path" \
