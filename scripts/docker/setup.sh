...
EXTRA_MOUNTS="${ALIEN_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${ALIEN_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${ALIEN_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${ALIEN_DOCKER_SOCKET:-}"
TIMEZONE="${ALIEN_TZ:-}"
RAW_SKIP_ONBOARDING="${ALIEN_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
...
read_config_gateway_token() {
  local config_path="$ALIEN_CONFIG_DIR/alien.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == ALIEN_GATEWAY_TOKEN=* ]]; then
      token="${line#ALIEN_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${ALIEN_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$ALIEN_GATEWAY_PORT" "$ALIEN_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$ALIEN_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$ALIEN_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network alien-cli service because it
  ...
  run_prestart_gateway --entrypoint node alien-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" alien-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "ALIEN_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "ALIEN_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
ALIEN_CONFIG_DIR="${ALIEN_CONFIG_DIR:-$HOME/.alien}"
ALIEN_WORKSPACE_DIR="${ALIEN_WORKSPACE_DIR:-$HOME/.alien/workspace}"

validate_mount_path_value "ALIEN_CONFIG_DIR" "$ALIEN_CONFIG_DIR"
validate_mount_path_value "ALIEN_WORKSPACE_DIR" "$ALIEN_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "ALIEN_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "ALIEN_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "ALIEN_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "ALIEN_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "ALIEN_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "ALIEN_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$ALIEN_CONFIG_DIR"
mkdir -p "$ALIEN_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$ALIEN_CONFIG_DIR/identity"
mkdir -p "$ALIEN_CONFIG_DIR/agents/main/agent"
mkdir -p "$ALIEN_CONFIG_DIR/agents/main/sessions"

export ALIEN_CONFIG_DIR
export ALIEN_WORKSPACE_DIR
export ALIEN_GATEWAY_PORT="${ALIEN_GATEWAY_PORT:-18789}"
export ALIEN_BRIDGE_PORT="${ALIEN_BRIDGE_PORT:-18790}"
export ALIEN_GATEWAY_BIND="${ALIEN_GATEWAY_BIND:-lan}"
export ALIEN_DISABLE_BONJOUR="${ALIEN_DISABLE_BONJOUR:-}"
export ALIEN_IMAGE="$IMAGE_NAME"
export ALIEN_DOCKER_APT_PACKAGES="${ALIEN_DOCKER_APT_PACKAGES:-}"
export ALIEN_EXTENSIONS="${ALIEN_EXTENSIONS:-}"
export ALIEN_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export ALIEN_HOME_VOLUME="$HOME_VOLUME_NAME"
export ALIEN_ALLOW_INSECURE_PRIVATE_WS="${ALIEN_ALLOW_INSECURE_PRIVATE_WS:-}"
export ALIEN_SANDBOX="$SANDBOX_ENABLED"
export ALIEN_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export ALIEN_DOCKER_SETUP=1
export ALIEN_TZ="$TIMEZONE"
...
export ALIEN_OTEL_PRELOADED="${ALIEN_OTEL_PRELOADED:-}"
export ALIEN_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${ALIEN_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ALIEN_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $ALIEN_CONFIG_DIR/alien.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      ALIEN_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      ALIEN_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      ALIEN_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${ALIEN_CONFIG_DIR}:/home/node/.alien"
    gateway_workspace_mount="${ALIEN_WORKSPACE_DIR}:/home/node/.alien/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${ALIEN_INSTALL_DOCKER_CLI:-}" ]]; then
    export ALIEN_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  ALIEN_BRIDGE_PORT \
  ALIEN_GATEWAY_BIND \
  ALIEN_DISABLE_BONJOUR \
  ALIEN_GATEWAY_TOKEN \
  ALIEN_IMAGE \
  ALIEN_EXTRA_MOUNTS \
  ALIEN_HOME_VOLUME \
  ALIEN_DOCKER_APT_PACKAGES \
  ALIEN_EXTENSIONS \
  ALIEN_SANDBOX \
  ALIEN_DOCKER_SOCKET \
  DOCKER_GID \
  ALIEN_INSTALL_DOCKER_CLI \
  ...
  ALIEN_OTEL_PRELOADED \
  ALIEN_SKIP_ONBOARDING

if [[ "$IMAGE_NAME" == "alien:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "ALIEN_DOCKER_APT_PACKAGES=${ALIEN_DOCKER_APT_PACKAGES}" \
    --build-arg "ALIEN_EXTENSIONS=${ALIEN_EXTENSIONS}" \
    --build-arg "ALIEN_INSTALL_DOCKER_CLI=${ALIEN_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Alien metadata subdirectory
# (.alien/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh alien-gateway -c \
  'find /home/node/.alien -xdev -exec chown node:node {} +; \
   [ -d /home/node/.alien/workspace/.alien ] && chown -R node:node /home/node/.alien/workspace/.alien || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (ALIEN_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from ALIEN_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $ALIEN_GATEWAY_BIND"
  if is_truthy_value "$ALIEN_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (ALIEN_DISABLE_BONJOUR=$ALIEN_DISABLE_BONJOUR)."
  elif [[ -z "$ALIEN_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (ALIEN_DISABLE_BONJOUR=$ALIEN_DISABLE_BONJOUR)."
  fi
  echo "Gateway token: $ALIEN_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm alien-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm alien-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm alien-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.alien.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d alien-gateway

# --- Sandbox setup (opt-in via ALIEN_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$sandbox_dockerfile" ]]; then
    echo "Building sandbox image: alien-sandbox:bookworm-slim"
    run_docker_build \
      -t "alien-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker alien-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg ALIEN_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (ALIEN_IMAGE=alien:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: ALIEN_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Alien config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.alien.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate alien-gateway
  fi
else
...
echo "Config: $ALIEN_CONFIG_DIR"
echo "Workspace: $ALIEN_WORKSPACE_DIR"
echo "Token: $ALIEN_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f alien-gateway"
echo "  ${COMPOSE_HINT} exec alien-gateway node dist/index.js health --token \"$ALIEN_GATEWAY_TOKEN\""
