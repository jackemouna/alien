...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  ALIEN_SKIP_ACPX_RUNTIME: "1",
  ALIEN_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$ALIEN_WATCH_PID_FILE"',
    'mkdir -p "$ALIEN_HOME/.alien"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$ALIEN_HOME/.alien/alien.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ALIEN_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ALIEN_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-gateway-watch-"));
