...
function parseArgs(argv) {
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("ALIEN_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("ALIEN_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
  ...
  const envIds = normalizeCsv(process.env.ALIEN_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function alienCommand(repoRoot, args) {
  return {
...
function sourceAlienCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    ALIEN_LOG_DIR: path.join(runRoot, "logs"),
    ALIEN_QA_SUITE_PROGRESS: process.env.ALIEN_QA_SUITE_PROGRESS ?? "1",
    ...
    for (const { phase, args } of commands) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...alienCommand(params.repoRoot, ["plugins", ...args]),
    ...
    for (const alias of plugin.cliCommandAliases) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...alienCommand(params.repoRoot, [command, "--help"]),
...
function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = runMeasuredCommand({
      ...
      ...sourceAlienCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-plugin-gauntlet-"));
