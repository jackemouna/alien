 ...
 * `alien update`. Use it to verify the completion installation flow
...
\`alien update\`, \`alien doctor\`, and \`alien onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Alien Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`alien completion --install\` later to enable.`));
    return;
  }
