...
tsx_version="${ALIEN_RELEASE_TSX_VERSION:-${TSX_VERSION:-4.21.0}}"
script_path="${ALIEN_RELEASE_CHECKS_SCRIPT:-workflow/scripts/alien-cross-os-release-checks.ts}"
...
temp_root="${ALIEN_RELEASE_TSX_TOOL_ROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}}"
if command -v cygpath >/dev/null 2>&1; then
  temp_root="$(cygpath -u "${temp_root}")"
fi

tool_dir="${ALIEN_RELEASE_TSX_TOOL_DIR:-${temp_root}/alien-release-tsx-${tsx_version}}"
