...
REMOTE_URL="${ALIEN_REF_REMOTE:-https://github.com/alien/alien.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-alien-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Alien branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Alien ref: ${REF}" >&2
exit 1
