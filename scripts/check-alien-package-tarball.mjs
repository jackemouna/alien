...
function usage() {
  return "Usage: node scripts/check-alien-package-tarball.mjs <alien.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`Alien package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.ALIEN_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-alien-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Alien package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Alien package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Alien package tarball integrity passed.");
