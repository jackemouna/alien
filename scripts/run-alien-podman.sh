#!/usr/bin/env bash
# Rootless Alien in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-alien-podman.sh launch        # Start gateway
#   ./scripts/run-alien-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   alien --container alien dashboard --no-open
#   alien --container alien channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ALIEN_GATEWAY_TOKEN|ALIEN_PODMAN_CONTAINER|ALIEN_PODMAN_IMAGE|ALIEN_IMAGE|ALIEN_PODMAN_PULL|ALIEN_PODMAN_GATEWAY_HOST_PORT|ALIEN_GATEWAY_PORT|ALIEN_PODMAN_BRIDGE_HOST_PORT|ALIEN_BRIDGE_PORT|ALIEN_GATEWAY_BIND|ALIEN_PODMAN_USERNS|ALIEN_BIND_MOUNT_OPTIONS|ALIEN_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-alien-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${ALIEN_CONFIG_DIR:-$EFFECTIVE_HOME/.alien}"
ENV_FILE="${ALIEN_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${ALIEN_CONFIG_DIR:-$EFFECTIVE_HOME/.alien}"
ENV_FILE="${ALIEN_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${ALIEN_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${ALIEN_PODMAN_CONTAINER:-alien}"
ALIEN_IMAGE="${ALIEN_PODMAN_IMAGE:-${ALIEN_IMAGE:-alien:local}}"
PODMAN_PULL="${ALIEN_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${ALIEN_PODMAN_GATEWAY_HOST_PORT:-${ALIEN_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${ALIEN_PODMAN_BRIDGE_HOST_PORT:-${ALIEN_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${ALIEN_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$ALIEN_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  ALIEN_CONTAINER="" ALIEN_CONFIG_DIR="$config_dir" \
    alien config get gateway.bind 2>/dev/null || true
}
...
# ALIEN_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${ALIEN_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v alien >/dev/null 2>&1; then
    echo "Warning: alien not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    ALIEN_CONTAINER="" ALIEN_CONFIG_DIR="$config_dir" \
      alien config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    ALIEN_CONTAINER="" ALIEN_CONFIG_DIR="$config_dir" \
      alien config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  ALIEN_CONTAINER="" ALIEN_CONFIG_DIR="$config_dir" \
    alien config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${ALIEN_GATEWAY_TOKEN:-}" ]]; then
  export ALIEN_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "ALIEN_GATEWAY_TOKEN" "$ALIEN_GATEWAY_TOKEN"
  echo "Generated ALIEN_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/alien.json"
...
PODMAN_USERNS="${ALIEN_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported ALIEN_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (ALIEN_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${ALIEN_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${ALIEN_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$ALIEN_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.alien/.npm \
    -e ALIEN_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.alien:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.alien/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$ALIEN_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME alien dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME alien devices approve --latest  # if pairing required"
