...
const DEFAULT_WORKFLOW = "alien-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef = process.env.ALIEN_DOCKER_E2E_WORKFLOW_REF || process.env.GITHUB_REF_NAME;
...
if (isLocalJson) {
  ...
  const outputDir =
    options.dir || path.join(os.tmpdir(), `alien-docker-e2e-rerun-${options.input}`);
