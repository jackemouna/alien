...
export function isLocalCheckEnabled(env) {
  const raw = env.ALIEN_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (defaultProjectRun) {
    ...
    insertBeforeSeparator(
      nextArgs,
      "--tsBuildInfoFile",
      nextEnv.ALIEN_TSGO_BUILD_INFO_FILE ?? DEFAULT_LOCAL_TSGO_BUILD_INFO_FILE,
    );
  }
  ...
  if (nextEnv.ALIEN_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.ALIEN_TSGO_PPROF_DIR);
  }
...
export function shouldAcquireLocalHeavyCheckLockForOxlint(
  ...
  if (env.ALIEN_OXLINT_FORCE_LOCK === "1") {
    return true;
  }
...
export function shouldAcquireLocalHeavyCheckLockForTsgo(args, env = process.env) {
  if (env.ALIEN_TSGO_FORCE_LOCK === "1") {
    return true;
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const timeoutMs = readPositiveInt(
    env.ALIEN_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
  );
  const pollMs = readPositiveInt(env.ALIEN_HEAVY_CHECK_LOCK_POLL_MS, DEFAULT_LOCK_POLL_MS);
  const progressMs = readPositiveInt(
    env.ALIEN_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
  );
  const staleLockMs = readPositiveInt(
    env.ALIEN_HEAVY_CHECK_STALE_LOCK_MS,
...
function readLocalCheckMode(env, defaultMode) {
  const raw = env.ALIEN_LOCAL_CHECK_MODE?.trim().toLowerCase();
