...
alien_e2e_eval_test_state_from_b64() { eval "$(printf '%s' "${1:?missing Alien test-state script}" | base64 -d)"; }
alien_e2e_resolve_entrypoint() {
  local entry
  for entry in dist/index.mjs dist/index.js; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "Alien entrypoint not found under dist/" >&2
  return 1
}
alien_e2e_package_root() {
  local prefix="${1:-}"
  if [ -n "$prefix" ]; then
...
alien_e2e_package_entrypoint() {
  local root="${1:?missing package root}"
  local entry
  for entry in "$root/dist/index.mjs" "$root/dist/index.js"; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "Alien package entrypoint not found under $root/dist/" >&2
  return 1
}
alien_e2e_install_package() {
  local log_file="$1"
  local label="${2:-mounted Alien package}"
  local prefix="${3:-}"
  local package_tgz="${ALIEN_CURRENT_PACKAGE_TGZ:?missing ALIEN_CURRENT_PACKAGE_TGZ}"
  local args=(-g)
  if [ -n "$prefix" ]; then
    args+=("--prefix" "$prefix")
  fi
...
alien_e2e_assert_package_extensions() {
  local root="$1"
  shift
  local extension
  for extension in "$@"; do
...
alien_e2e_write_state_env() {
  local target="${1:-/tmp/alien-test-state-env}"
  {
    printf 'export HOME=%q\n' "$HOME"
    printf 'export ALIEN_HOME=%q\n' "$ALIEN_HOME"
    printf 'export ALIEN_STATE_DIR=%q\n' "$ALIEN_STATE_DIR"
    printf 'export ALIEN_CONFIG_PATH=%q\n' "$ALIEN_CONFIG_PATH"
    printf 'export ALIEN_AGENT_DIR=%q\n' "${ALIEN_AGENT_DIR-}"
...
TRASH
  chmod +x /tmp/alien-bin/trash
}
alien_e2e_stop_process() {
  local pid="${1:-}" _
  [ -n "$pid" ] || return 0
...
alien_e2e_start_mock_openai() { MOCK_PORT="$1" node scripts/e2e/mock-openai-server.mjs >"$2" 2>&1 & printf '%s\n' "$!"; }
alien_e2e_wait_mock_openai() {
...
alien_e2e_start_gateway() { node "$1" gateway --port "$2" --bind loopback --allow-unconfigured >"$3" 2>&1 & printf '%s\n' "$!"; }
alien_e2e_exec_gateway() { exec node "$1" gateway --port "$2" --bind "${3:-loopback}" --allow-unconfigured >"$4" 2>&1; }
alien_e2e_wait_gateway_ready() {
...
alien_e2e_probe_http_status() {
  node -e 'fetch(process.argv[1]).then(r=>process.exit(r.status===Number(process.argv[2])?0:1)).catch(()=>process.exit(1))' "$1" "${2:-200}"
}
alien_e2e_assert_file() { [ -f "$1" ] || { echo "Missing file: $1"; exit 1; }; }
alien_e2e_assert_dir() { [ -d "$1" ] || { echo "Missing dir: $1"; exit 1; }; }
alien_e2e_assert_log_not_contains() {
  ! grep -q "$2" "$1" || { echo "Unexpected log output: $2"; exit 1; }
}
alien_e2e_run_logged() {
  local label="$1" log_path="/tmp/alien-onboard-${1}.log"
  shift
...
alien_e2e_dump_logs() {
  ...
  for path in "$@"; do
    [ -f "$path" ] || continue
    echo "--- $path ---"; tail -n "${ALIEN_E2E_LOG_TAIL_LINES:-120}" "$path" || true
  done
}
