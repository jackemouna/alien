...
function usage() {
  return `Usage:
  node scripts/lib/alien-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/alien-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/alien-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "alien-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "alien-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'ALIEN_TEST_STATE_JSON'`,
    json,
    "ALIEN_TEST_STATE_JSON",
  ].join("\n");
}
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `alien-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `alien-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'ALIEN_TEST_STATE_TMP_ROOT="${ALIEN_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    "export ALIEN_TEST_STATE_TMP_ROOT",
    'mkdir -p "$ALIEN_TEST_STATE_TMP_ROOT"',
    `ALIEN_TEST_STATE_HOME="$(mktemp -d "$ALIEN_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$ALIEN_TEST_STATE_HOME"',
    'export USERPROFILE="$ALIEN_TEST_STATE_HOME"',
    'export ALIEN_HOME="$ALIEN_TEST_STATE_HOME"',
    'export ALIEN_STATE_DIR="$ALIEN_TEST_STATE_HOME/.alien"',
    'export ALIEN_CONFIG_PATH="$ALIEN_STATE_DIR/alien.json"',
    'export ALIEN_TEST_WORKSPACE_DIR="$ALIEN_TEST_STATE_HOME/workspace"',
    'mkdir -p "$ALIEN_STATE_DIR" "$ALIEN_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$ALIEN_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `alien_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown Alien test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      ALIEN_TEST_STATE_HOME="$raw_label"
      mkdir -p "$ALIEN_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${ALIEN_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      mkdir -p "$tmp_root"
      ALIEN_TEST_STATE_HOME="$(mktemp -d "$tmp_root/alien-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$ALIEN_TEST_STATE_HOME"
  export USERPROFILE="$ALIEN_TEST_STATE_HOME"
  ...
  export ALIEN_CONFIG_PATH="$ALIEN_STATE_DIR/alien.json"
  export ALIEN_TEST_WORKSPACE_DIR="$ALIEN_TEST_STATE_HOME/workspace"
  unset ALIEN_AGENT_DIR
  unset PI_CODING_AGENT_DIR
  unset ALIEN_SERVICE_REPAIR_POLICY
  mkdir -p "$ALIEN_STATE_DIR" "$ALIEN_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$ALIEN_CONFIG_PATH" <<'ALIEN_TEST_STATE_JSON'
{}
ALIEN_TEST_STATE_JSON
      ;;
    update-stable)
...
ALIEN_TEST_STATE_JSON
      ;;
    external-service)
      export ALIEN_SERVICE_REPAIR_POLICY="external"
      cat > "$ALIEN_CONFIG_PATH" <<'ALIEN_TEST_STATE_JSON'
{}
ALIEN_TEST_STATE_JSON
      ;;
