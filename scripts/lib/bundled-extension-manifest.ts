...
export function collectBundledExtensionManifestErrors(extensions: BundledExtension[]): string[] {
  ...
  for (const extension of extensions) {
    const install = extension.packageJson.alien?.install;
    if (install !== undefined && !isRecord(install)) {
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | alien.install must be an object`,
      );
      continue;
    }
    ...
    if (
      ...
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | alien.install.npmSpec must be a non-empty string`,
      );
    }
