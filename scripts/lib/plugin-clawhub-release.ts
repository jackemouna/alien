...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/alien-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.alien?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.alien?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
export async function collectClawHubAlienOwnerErrors(params: {
  ...
  const requiredOwnerHandle = params.requiredOwnerHandle ?? "alien";
  const errors: string[] = [];

  await Promise.all(
    params.plugins.map(async (plugin) => {
      if (!plugin.packageName.startsWith("@alien/")) {
        return;
      }
      ...
      if (response.status === 404) {
        errors.push(
          `${plugin.packageName}: ClawHub package row must already exist under @${requiredOwnerHandle} before Alien release publish.`,
        );
        return;
      }
      if (!response.ok) {
