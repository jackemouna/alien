...
function isSystemThrottleDisabled(env) {
  const normalized = env.ALIEN_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.ALIEN_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.ALIEN_VITEST_MAX_WORKERS ?? env.ALIEN_TEST_WORKERS);
  if (override !== null) {
