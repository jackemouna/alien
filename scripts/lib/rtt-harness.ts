...
const ALIEN_PACKAGE_SPEC_RE =
  /^alien@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;
...
export function validateAlienPackageSpec(spec: string) {
  if (!ALIEN_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `Package spec must be alien@main, alien@alpha, alien@beta, alien@latest, or an exact Alien release version; got: ${spec}`,
    );
  }
  return spec;
}
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    ALIEN_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...(params.packageTgz ? { ALIEN_NPM_TELEGRAM_PACKAGE_TGZ: params.packageTgz } : {}),
    ALIEN_NPM_TELEGRAM_PACKAGE_LABEL: `${params.spec} (${params.version})`,
    ALIEN_NPM_TELEGRAM_PROVIDER_MODE: params.providerMode,
    ALIEN_NPM_TELEGRAM_SCENARIOS: params.scenarios.join(","),
    ALIEN_NPM_TELEGRAM_OUTPUT_DIR: params.rawOutputDir,
    ALIEN_NPM_TELEGRAM_FAST: params.baseEnv.ALIEN_NPM_TELEGRAM_FAST ?? "1",
    ALIEN_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    ALIEN_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    ALIEN_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    ALIEN_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
...
export async function assertHarnessRoot(harnessRoot: string) {
  ...
  try {
    ...
    throw new Error(`Missing Alien Telegram npm harness: ${scriptPath}`);
  }
}
...
export async function resolveMainVersion(harnessRoot: string) {
  ...
  if (typeof packageJson.version !== "string" || packageJson.version.trim().length === 0) {
    throw new Error("Alien package.json must contain a non-empty version.");
  }
