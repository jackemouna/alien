...
const upgradeSurvivorCommand = "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor";
const updateRestartAuthCommand =
  "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth";
...
function liveDockerScriptCommand(script, envPrefix = "", options = {}) {
  ...
  const skipBuild = options.skipBuild === false ? "" : "ALIEN_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c 'harness="\${ALIEN_DOCKER_E2E_TRUSTED_HARNESS_DIR:-}"; if [ -z "$harness" ]; then if [ -d .release-harness/scripts ]; then harness=.release-harness; else harness=.; fi; fi; ALIEN_LIVE_DOCKER_REPO_ROOT="\${ALIEN_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `ALIEN_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} ALIEN_BUNDLED_PLUGIN_SWEEP_INDEX=${index} ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
export const mainLanes = [
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "ALIEN_IMAGE=alien:local-live-gateway ALIEN_DOCKER_BUILD_EXTENSIONS=matrix ALIEN_LIVE_GATEWAY_PROVIDERS=claude-cli,codex-cli,google-gemini-cli",
      { skipBuild: false },
    ),
    {
      providers: ["claude-cli", "codex-cli", "google-gemini-cli"],
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "ALIEN_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "ALIEN_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
  ...
  serviceLane("onboard", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane(
    "npm-onboard-channel-agent",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-discord-channel-agent",
    "ALIEN_NPM_ONBOARD_CHANNEL=discord ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-slack-channel-agent",
    "ALIEN_NPM_ONBOARD_CHANNEL=slack ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  serviceLane("gateway-network", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  serviceLane(
    "agents-delete-shared-workspace",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  serviceLane("mcp-channels", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  lane("pi-bundle-mcp-tools", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:pi-bundle-mcp-tools", {
    stateScenario: "empty",
  }),
  lane("crestodian-rescue", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-rescue", {
    stateScenario: "empty",
  }),
  lane("crestodian-planner", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-planner", {
    stateScenario: "empty",
  }),
  serviceLane(
    "cron-mcp-cleanup",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
    { resources: ["npm"], stateScenario: "empty", weight: 3 },
  ),
  npmLane("doctor-switch", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch", {
    stateScenario: "empty",
    weight: 3,
  }),
  npmLane(
    "update-channel-switch",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
    {
  ...
  npmLane(
    ...
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
    {
      stateScenario: "upgrade-survivor",
  ...
  npmLane("update-migration", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration", {
    stateScenario: "upgrade-survivor",
    timeoutMs: 30 * 60 * 1000,
    weight: 3,
  }),
  lane("plugins", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    "plugins-offline",
    "ALIEN_PLUGINS_E2E_CLAWHUB=0 ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      weight: 6,
  ...
  npmLane("plugin-update", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
  lane("openai-image-auth", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-image-auth", {
    stateScenario: "empty",
  }),
  lane(
    "crestodian-first-run",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-first-run",
    { stateScenario: "empty" },
  ),
  lane(
    "session-runtime-context",
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:session-runtime-context",
  ),
  lane("commitments-safety", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:commitments-safety", {
    stateScenario: "empty",
  }),
...
export const tailLanes = [
  serviceLane(
    ...
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "ALIEN_LIVE_CODEX_BIND=1 ALIEN_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts",
    ),
    {
      cacheKey: "codex-harness",
      provider: "codex-cli",
  ...
  liveLane(
    ...
    "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-codex-npm-plugin",
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "ALIEN_LIVE_CLI_BACKEND_AUTH=api-key ALIEN_LIVE_CLI_BACKEND_MODEL=codex-cli/gpt-5.4",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "ALIEN_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "ALIEN_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "ALIEN_LIVE_ACP_BIND_AGENT=droid ALIEN_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "ALIEN_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "ALIEN_LIVE_ACP_BIND_AGENT=opencode ALIEN_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const releasePathPackageInstallOpenAiLanes = [
  npmLane(
    "install-e2e-openai",
    "ALIEN_INSTALL_TAG=beta ALIEN_E2E_MODELS=openai ALIEN_INSTALL_E2E_IMAGE=alien-install-e2e-openai:local pnpm test:install:e2e",
    {
      resources: ["service"],
      weight: 3,
    },
...
const releasePathPackageInstallAnthropicLanes = [
  npmLane(
    ...
    "ALIEN_INSTALL_TAG=beta ALIEN_E2E_MODELS=anthropic ALIEN_INSTALL_E2E_IMAGE=alien-install-e2e-anthropic:local pnpm test:install:e2e",
    {
      resources: ["service"],
      weight: 3,
    },
...
const primaryReleasePathChunks = {
  core: [
    lane("qr", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr"),
    serviceLane("onboard", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
      stateScenario: "empty",
      weight: 2,
    }),
    ...
    lane(
      "pi-bundle-mcp-tools",
      "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:pi-bundle-mcp-tools",
      { stateScenario: "empty" },
    ),
    serviceLane("mcp-channels", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
      resources: ["npm"],
...
function openWebUILane() {
  return liveLane("openwebui", "ALIEN_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    needsLiveImage: false,
    provider: "openai",
    resources: ["service"],
...
export function releasePathChunkLanes(chunk, options = {}) {
  ...
  if (!base) {
    throw new Error(
      `ALIEN_DOCKER_ALL_CHUNK must be one of: ${[
