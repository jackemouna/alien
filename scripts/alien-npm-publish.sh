...
if [[ "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/alien-npm-publish.sh --publish [package.tgz]" >&2
  exit 2
fi
...
mapfile -t publish_plan < <(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${ALIEN_NPM_PUBLISH_TAG:-}" \
    node --import tsx --input-type=module <<'EOF'
import { resolveNpmPublishPlan } from "./scripts/alien-npm-release-check.ts";
