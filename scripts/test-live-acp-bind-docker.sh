...
ACP_AGENT_LIST_RAW="${ALIEN_LIVE_ACP_BIND_AGENTS:-${ALIEN_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
TEMP_DIRS=()
...
alien_live_acp_bind_append_build_extension() {
...
alien_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    claude) printf '%s\n' "claude-cli" ;;
    codex) printf '%s\n' "codex-cli" ;;
    droid) printf '%s\n' "droid" ;;
    ...
    opencode) printf '%s\n' "opencode" ;;
    *)
      echo "Unsupported ALIEN_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, droid, gemini, or opencode)" >&2
      return 1
      ;;
  esac
}

alien_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    claude) printf '%s' "${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND_CLAUDE:-${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    codex) printf '%s' "${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND_CODEX:-${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    droid) printf '%s' "${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND_DROID:-${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    gemini) printf '%s' "${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    opencode) printf '%s' "${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND_OPENCODE:-${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
agent="${ALIEN_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${ALIEN_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${ALIEN_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${ALIEN_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${ALIEN_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
NODE
  ...
  opencode)
    ...
    export OPENCODE_CONFIG_CONTENT="$(
      node -e 'process.stdout.write(JSON.stringify({model: process.env.ALIEN_LIVE_ACP_BIND_OPENCODE_MODEL || "opencode/kimi-k2.6"}))'
    )"
    ;;
  *)
    echo "Unsupported ALIEN_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
cd "$tmp_dir"
export ALIEN_LIVE_ACP_BIND_AGENT_COMMAND="${ALIEN_LIVE_ACP_BIND_AGENT_COMMAND:-}"
pnpm test:live src/gateway/gateway-acp-bind.live.test.ts
EOF

alien_live_acp_bind_append_build_extension acpx
...
for token in "${ACP_AGENT_TOKENS[@]}"; do
  agent="$(alien_live_trim "$token")"
  [[ -n "$agent" ]] || continue
  alien_live_acp_bind_resolve_auth_provider "$agent" >/dev/null
  ACP_AGENTS+=("$agent")
done

if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use ALIEN_LIVE_ACP_BIND_AGENTS=claude,codex,droid,gemini,opencode." >&2
  exit 1
fi

for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  AUTH_PROVIDER="$(alien_live_acp_bind_resolve_auth_provider "$ACP_AGENT")"
  AGENT_COMMAND="$(alien_live_acp_bind_resolve_agent_command "$ACP_AGENT")"
  ...
  if [[ -n "${ALIEN_DOCKER_AUTH_DIRS:-}" ]]; then
    ...
    while IFS= read -r auth_dir; do
      [[ -n "$auth_dir" ]] || continue
      AUTH_DIRS+=("$auth_dir")
    done < <(alien_live_collect_auth_dirs_from_csv "$AUTH_PROVIDER")
    while IFS= read -r auth_file; do
      [[ -n "$auth_file" ]] || continue
      AUTH_FILES+=("$auth_file")
    done < <(alien_live_collect_auth_files_from_csv "$AUTH_PROVIDER")
  fi
  ...
  DOCKER_RUN_ARGS=(docker run --rm -t \
    ...
    -e ALIEN_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e ALIEN_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e FACTORY_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e ALIEN_LIVE_TEST=1 \
    -e ALIEN_LIVE_ACP_BIND=1 \
    -e ALIEN_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e ALIEN_LIVE_ACP_BIND_OPENCODE_MODEL="${ALIEN_LIVE_ACP_BIND_OPENCODE_MODEL:-opencode/kimi-k2.6}" \
    -e ALIEN_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND")
