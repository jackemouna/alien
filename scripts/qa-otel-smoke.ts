...
function spawnAlien(args: string[], env: NodeJS.ProcessEnv): ChildProcess {
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "alien-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.ALIEN_QA_SUITE_PROGRESS = env.ALIEN_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function assertSmoke(params: {
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("alien.content."));
  ...
  const modelSpans = params.spans.filter((span) => span.name === "alien.model.call");
  const modelErrorSpans = modelSpans.filter((span) => {
    const serialized = JSON.stringify(span.attributes);
    return (
      Object.hasOwn(span.attributes, "error.type") ||
      Object.hasOwn(span.attributes, "alien.errorCategory") ||
      serialized.includes("StreamAbandoned")
    );
  });
  if (modelSpans.length === 0) {
    failures.push("no alien.model.call span was exported");
  }
...
async function main() {
  ...
  let childExitCode = 1;
  try {
    const child = spawnAlien(buildQaArgs(options), buildQaEnv(port));
