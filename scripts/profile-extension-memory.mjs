...
const RSS_MARKER = "__ALIEN_MAX_RSS_KB__=";
...
async function main() {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "alien-extension-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "alien-extension-memory.json");
