...
const DISABLE_POSTINSTALL_ENV = "ALIEN_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "ALIEN_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallAlienHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.ALIEN_HOME?.trim();
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.alien-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const openClawHome = resolvePostinstallAlienHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  const configPath = env?.ALIEN_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, openClawHome)));
  }
  addStateRoot(join(openClawHome, ".alien"));
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.alien-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneAlienCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune Alien compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
