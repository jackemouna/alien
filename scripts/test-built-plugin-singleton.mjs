...
const { clearPluginCommands, getPluginCommandSpecs, loadAlienPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadAlienPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "alien.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'alien/plugin-sdk';",
...
const registry = loadAlienPlugins({
  ...
  env: {
    ...process.env,
    ALIEN_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
