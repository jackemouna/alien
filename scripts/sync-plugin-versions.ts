...
const ALIEN_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncAlienDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.alien;
  if (!current || current === "workspace:*" || !ALIEN_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.alien?.compat;
  const current = compat?.pluginApi;
  if (!current || !ALIEN_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildAlienVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.alien?.build;
  const current = build?.alienVersion;
  if (!current) {
    return false;
  }
  ...
  build.alienVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncAlienDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncAlienDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildAlienVersionChanged = syncBuildAlienVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildAlienVersionChanged;
    if (!packageChanged) {
