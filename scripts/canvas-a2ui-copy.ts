...
function getA2uiPaths(env = process.env) {
  const srcDir = env.ALIEN_A2UI_SRC_DIR ?? path.join(repoRoot, "src", "canvas-host", "a2ui");
  const outDir = env.ALIEN_A2UI_OUT_DIR ?? path.join(repoRoot, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

function shouldSkipMissingA2uiAssets(env = process.env): boolean {
  return env.ALIEN_A2UI_SKIP_MISSING === "1" || Boolean(env.ALIEN_SPARSE_PROFILE);
}
...
export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because ALIEN_A2UI_SKIP_MISSING=1 or ALIEN_SPARSE_PROFILE is set.`,
      );
      return;
    }
