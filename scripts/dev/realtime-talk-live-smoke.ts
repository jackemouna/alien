...
const OPENAI_REALTIME_MODEL =
  process.env.ALIEN_REALTIME_OPENAI_MODEL?.trim() || "gpt-realtime-1.5";
const OPENAI_REALTIME_VOICE = process.env.ALIEN_REALTIME_OPENAI_VOICE?.trim() || "alloy";
const GOOGLE_REALTIME_MODEL =
  process.env.ALIEN_REALTIME_GOOGLE_MODEL?.trim() ||
  "gemini-2.5-flash-native-audio-preview-12-2025";
const GOOGLE_REALTIME_VOICE = process.env.ALIEN_REALTIME_GOOGLE_VOICE?.trim() || "Kore";
    ...
    config: {
      ...
      liveConnectConstraints: {
        ...
        config: {
          ...
          systemInstruction: "Alien browser Talk live smoke.",
...
async function smokeGatewayRelayBrowser(browser: Browser): Promise<SmokeResult> {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), "alien-realtime-talk-"));
  try {
