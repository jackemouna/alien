...
export DISPLAY=:1
export HOME=/tmp/alien-home
export XDG_CONFIG_HOME="${HOME}/.config"
export XDG_CACHE_HOME="${HOME}/.cache"

CDP_PORT="${ALIEN_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${ALIEN_BROWSER_CDP_SOURCE_RANGE:-}"
VNC_PORT="${ALIEN_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${ALIEN_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${ALIEN_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${ALIEN_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${ALIEN_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${ALIEN_BROWSER_NOVNC_PASSWORD:-}"

DISABLE_GRAPHICS_FLAGS="${ALIEN_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${ALIEN_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${ALIEN_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
AUTO_START_TIMEOUT_MS="${ALIEN_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_SOURCE_RANGE}" ]]; then
  ...
  echo "[sandbox-browser] Set ALIEN_BROWSER_CDP_SOURCE_RANGE to an explicit CIDR to enable CDP access." >&2
else
