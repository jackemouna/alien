#!/usr/bin/env node
// Builds the Alien package artifact used by Docker E2E.
...
async function newestAlienTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Alien tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building Alien package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building Alien Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing Alien package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing Alien package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestAlienTarball(outputDir, packOutput);
  ...
  console.error("==> Checking Alien package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-alien-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
