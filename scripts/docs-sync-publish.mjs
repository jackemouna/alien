...
const SYNC_SUPPORT_FILES = [
  {
    ...
    target: path.join(".alien-sync", "check-docs-mdx.mjs"),
  },
  {
    source: path.join(ROOT, "scripts", "lib", "mintlify-accordion.mjs"),
    target: path.join(".alien-sync", "lib", "mintlify-accordion.mjs"),
  },
  {
    source: path.join(ROOT, ".github", "codex", "prompts", "docs-mdx-repair.md"),
    target: path.join(".alien-sync", "docs-mdx-repair.md"),
  },
];
...
function writeSyncMetadata(targetRoot, args) {
  ...
  writeJson(path.join(targetRoot, ".alien-sync", "source.json"), metadata);
}
