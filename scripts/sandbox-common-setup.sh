...
BASE_IMAGE="${BASE_IMAGE:-alien-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-alien-sandbox-common:bookworm-slim}"
...
ALIEN_DOCKER_BUILD_USE_BUILDX="${ALIEN_DOCKER_BUILD_USE_BUILDX:-0}"
ALIEN_DOCKER_BUILD_CACHE_FROM="${ALIEN_DOCKER_BUILD_CACHE_FROM:-}"
ALIEN_DOCKER_BUILD_CACHE_TO="${ALIEN_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=alien.sandbox=1)
NOTE
