...
const PLUGIN_SDK_SPECIFIER_PATTERN =
  /\b(?:from\s*["']|import\s*\(\s*["']|require\s*\(\s*["']|vi\.(?:mock|doMock)\s*\(\s*["'])(alien\/plugin-sdk\/([a-z0-9][a-z0-9-]*))["']/g;
...
function extractCompatTokens(record: PluginCompatRecord): string[] {
  ...
  for (const value of values) {
    ...
    for (const match of value.matchAll(/\balien\/[a-z0-9/-]+\b/g)) {
      tokens.add(match[0]);
    }
    for (const match of value.matchAll(/\bALIEN_[A-Z0-9_]+\b/g)) {
      tokens.add(match[0]);
    }
