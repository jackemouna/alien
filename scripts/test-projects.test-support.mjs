...
const INCLUDE_FILE_ENV_KEY = "ALIEN_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "ALIEN_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-alien-runtime)?\.ts$/u,
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/lib/alien-test-state.mjs", ["test/scripts/alien-test-state.test.ts"]],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/alien-test-state.test.ts", ["test/scripts/alien-test-state.test.ts"]],
  [
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/alien-test-state.ts", ["src/test-utils/alien-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "ALIEN_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "ALIEN_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = Number.parseInt(process.env.ALIEN_TEST_PROJECTS_PARALLEL ?? "", 10);
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.ALIEN_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.ALIEN_VITEST_MAX_WORKERS ?? env.ALIEN_TEST_WORKERS,
  );
...
export function resolveParallelFullSuiteConcurrency(specCount, env = process.env, hostInfo) {
  const override = parsePositiveInt(env.ALIEN_TEST_PROJECTS_PARALLEL);
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.ALIEN_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.ALIEN_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `alien-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.ALIEN_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.ALIEN_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
