...
SERVER="${ALIEN_SERVER:-l36}"

# Check auth status
termux-toast "Checking Alien auth..."

STATUS=$(ssh "$SERVER" '$HOME/alien/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/alien/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ALIEN_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/alien/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Alien won't work until re-auth"
                ;;
        esac
        ;;
