...
DOCKER_BUILD_EXTENSIONS="${ALIEN_DOCKER_BUILD_EXTENSIONS:-${ALIEN_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "ALIEN_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${ALIEN_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because ALIEN_DOCKER_BUILD_ON_MISSING/ALIEN_TESTBOX allows fallback."
fi
