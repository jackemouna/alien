...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./alien-npm-release-check.ts";
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`alien@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `alien-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "alien");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed alien binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`alien-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error(
      "Usage: node --import tsx scripts/alien-npm-postpublish-verify.ts <version>",
    );
  }
  ...
  console.log(
    `alien-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`alien-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
