#!/usr/bin/env bash
# ClawDock - Docker helpers for Alien
# Inspired by Simon Willison's "Running Alien in Docker"
# https://til.simonwillison.net/llms/alien-docker
#
# Installation:
...
# Common paths to check for Alien
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/alien"
  "${HOME}/workspace/alien"
  "${HOME}/projects/alien"
  "${HOME}/dev/alien"
  "${HOME}/code/alien"
  "${HOME}/src/alien"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "👾 Found Alien at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/alien"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Alien not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/alien/alien.git ~/alien"
    echo "  cd ~/alien && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ALIEN_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d alien-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart alien-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f alien-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.alien"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show alien.json
  if [[ -f "${config_dir}/alien.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/alien.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/alien.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No alien.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec alien-gateway \
    bash -c 'echo "alias alien=\"./alien.mjs\"" > /tmp/.bashrc_alien && bash --rcfile /tmp/.bashrc_alien'
}
...
clawdock-cli() {
  _clawdock_compose run --rm alien-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build alien-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d alien-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build alien-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ALIEN_GATEWAY_TOKEN=$token" alien-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" alien-gateway \
    bash -c './alien.mjs config set gateway.remote.token "$TOKEN" && ./alien.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec alien-gateway \
    bash -c "./alien.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart alien-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm alien-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec alien-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'alien config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}👾 ClawDock - Docker Helpers for Alien${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (alien alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to alien project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.alien)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'alien channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'alien status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.alien.ai${_CLR_RESET}"
  echo ""
}
