...
const jitiFactoryOverrideKey = Symbol.for("alien.channelConfigSurfaceJitiFactoryOverride");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.ALIEN_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing ALIEN_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        ALIEN_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadViaJiti = (candidatePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "alien/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap({
        modulePath: resolvedPath,
        pluginSdkResolution: "src",
      }),
