...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "alien-startup-memory-"));
...
const cases = [
  {
    ...
    label: "--help",
    args: ["alien.mjs", "--help"],
    limitMb: Number(process.env.ALIEN_STARTUP_MEMORY_HELP_MB ?? DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["alien.mjs", "status", "--json"],
    limitMb: Number(
      process.env.ALIEN_STARTUP_MEMORY_STATUS_JSON_MB ?? DEFAULT_LIMITS_MB.statusJson,
    ),
  },
  {
    id: "gatewayStatus",
    label: "gateway status",
    args: ["alien.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.ALIEN_STARTUP_MEMORY_GATEWAY_STATUS_MB ?? DEFAULT_LIMITS_MB.gatewayStatus,
    ),
  },
];
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Alien startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
