...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[alien] warning: detected ${packageManager} for install lifecycle.`,
    "[alien] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[alien] prefer: corepack pnpm install",
  ].join("\n");
}
