...
#
# Suggested cron: */30 * * * * /home/admin/alien/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Alien notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., alien-alerts)
...
STATE_FILE="$HOME/.alien/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Alien if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use alien
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Alien to $NOTIFY_PHONE..."
            alien send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Alien is down. Run: ssh l36 '~/alien/scripts/mobile-reauth.sh'" "urgent"
    exit 1
