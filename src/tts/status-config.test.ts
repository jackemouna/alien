...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-tts-status-"));
});
...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  process.env.ALIEN_STATE_DIR = path.join(home, ".alien");
  try {
    await run(home);
  } finally {
    restoreEnv("HOME", previousHome);
    restoreEnv("USERPROFILE", previousUserProfile);
    restoreEnv("ALIEN_HOME", previousAlienHome);
    restoreEnv("ALIEN_STATE_DIR", previousStateDir);
  }
}
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".alien", "settings", "tts.json");
  ...
  it("derives the default prefs path from ALIEN_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".alien-dev");
      ...
      vi.stubEnv("ALIEN_CONFIG_PATH", path.join(stateDir, "alien.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
