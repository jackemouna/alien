...
describe("shouldAttemptTtsPayload", () => {
  ...
  beforeAll(() => {
    root = mkdtempSync(path.join(tmpdir(), "alien-tts-config-"));
  });
  ...
  beforeEach(() => {
    originalPrefsPath = process.env.ALIEN_TTS_PREFS;
    dir = path.join(root, `case-${caseId++}`);
    mkdirSync(dir, { recursive: true });
    prefsPath = path.join(dir, "tts.json");
    process.env.ALIEN_TTS_PREFS = prefsPath;
  });

  afterEach(() => {
    if (originalPrefsPath === undefined) {
      delete process.env.ALIEN_TTS_PREFS;
    } else {
      process.env.ALIEN_TTS_PREFS = originalPrefsPath;
    }
  });

  it("skips TTS when config, prefs, and session state leave auto mode off", () => {
    expect(shouldAttemptTtsPayload({ cfg: {} as AlienConfig })).toBe(false);
  });
  ...
  it("honors session auto state before prefs and config", () => {
    ...
    const cfg = { messages: { tts: { auto: "off" } } } as AlienConfig;
  ...
  it("uses local prefs before config auto mode", () => {
    ...
    writeFileSync(prefsPath, JSON.stringify({ tts: { auto: "off" } }));
    expect(
      shouldAttemptTtsPayload({ cfg: { messages: { tts: { enabled: true } } } as AlienConfig }),
    ).toBe(false);
  });
