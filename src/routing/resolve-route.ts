...
function listAgents(cfg: AlienConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: AlienConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<AlienConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AlienConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: AlienConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AlienConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<AlienConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AlienConfig,
  {
    bindingsRef: AlienConfig["bindings"];
    agentsRef: AlienConfig["agents"];
    sessionRef: AlienConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AlienConfig): Map<string, ResolvedAgentRoute> {
