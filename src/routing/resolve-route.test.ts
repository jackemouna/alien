...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: AlienConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: AlienConfig };
    expected: ResolvedRouteExpectation;
  }) => {
  ...
  test.each([
    ...
    },
    {
      ...
      cfg: {
        agents: {
          list: [{ id: "home", default: true, workspace: "~/alien-home" }],
        },
      } satisfies AlienConfig,
      channel: "whatsapp" as const,
      accountId: "biz",
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<AlienConfig["bindings"]>[number];
