...
async function makeSettings() {
  testRoot = await mkdtemp(join(tmpdir(), "alien-debug-proxy-managed-proxy-"));
  return {
    enabled: true,
    required: false,
    dbPath: ":memory:",
...
describe("debug proxy managed-proxy direct upstream policy", () => {
  const originalProxyActive = process.env["ALIEN_PROXY_ACTIVE"];
  const originalAllowDirect =
    process.env["ALIEN_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];

  beforeEach(async () => {
    await cleanupTestDirs();
    delete process.env["ALIEN_PROXY_ACTIVE"];
    delete process.env["ALIEN_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];
  });
  ...
  afterEach(async () => {
    if (originalProxyActive === undefined) {
      ...
      process.env["ALIEN_PROXY_ACTIVE"] = originalProxyActive;
    }
  ...
  it("uses shared truthy parsing for managed proxy mode", () => {
    process.env["ALIEN_PROXY_ACTIVE"] = "true";
  ...
  it("allows direct upstreams with explicit diagnostic override", () => {
    process.env["ALIEN_PROXY_ACTIVE"] = "1";
    process.env["ALIEN_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"] = "1";
