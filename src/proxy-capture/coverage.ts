...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[alien proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[alien proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`alien proxy coverage\` for details.`,
  );
}
