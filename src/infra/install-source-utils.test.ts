...
const TEMP_DIR_PREFIX = "alien-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("alien-install-source-utils-");
  const archivePath = path.join(cwd, "alien-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("alien-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-alien-archive.tgz",
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "alien-plugin@1.2.3",
          name: "alien-plugin",
          version: "1.2.3",
          filename: "alien-plugin-1.2.3.tgz",
    ...
    const result = await runPack("alien-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "alien-plugin",
        version: "1.2.3",
        resolvedSpec: "alien-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "alien-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "alien-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nalien-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"alien-plugin@1.2.3","name":"alien-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "alien-plugin",
        version: "1.2.3",
        resolvedSpec: "alien-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@alien/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@alien/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@alien/whatsapp",
      "docs.alien.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("alien-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "alien-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@alien/plugin-demo@2.0.0",
            filename: "alien-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@alien/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "alien-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@alien/plugin-demo@2.0.0",
      },
    });
  });
