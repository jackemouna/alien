...
import { compareAlienReleaseVersions } from "./npm-registry-spec.js";
...
export async function fetchNpmPackageTargetStatus(params: {
  ...
  try {
    const res = await fetchWithTimeout(
      `https://registry.npmjs.org/alien/${encodeURIComponent(target)}`,
      {},
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const openClawReleaseCmp = compareAlienReleaseVersions(a, b);
