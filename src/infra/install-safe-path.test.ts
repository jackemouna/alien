...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@alien/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@alien/matrix", id: "matrix", expected: true },
    { packageName: "@alien/matrix", id: "@alien/matrix", expected: true },
    { packageName: "@alien/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@alien/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@alien__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "alien-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "alien-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "alien-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "alien-install-safe-" }, async (parentDir) => {
