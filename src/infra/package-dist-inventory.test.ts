...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-pack-" }, async (packageRoot) => {
  ...
  it("keeps transient plugin dependency trees out of the inventory", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-plugin-deps-" }, async (packageRoot) => {
  ...
  it("omits packaged extension node_modules while keeping extension runtime files", async () => {
    await withTempDir(
      { prefix: "alien-dist-inventory-extension-node-modules-" },
      async (packageRoot) => {
  ...
  it("keeps publishable externalized bundled plugin dist trees out of the inventory", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-externalized-" }, async (packageRoot) => {
  ...
  it("keeps publishable core-package runtime plugin dist trees in the inventory", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-core-runtime-" }, async (packageRoot) => {
  ...
  it("reports runtime-created install staging dirs during installed dist verification", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-stage-" }, async (packageRoot) => {
      ...
      await expect(collectPackageDistInventoryErrors(packageRoot)).resolves.toEqual([
        ...
        "unexpected packaged dist file dist/extensions/browser/.alien-install-stage-AbC123/node_modules/playwright-core/package.json",
      ]);
    });
  });
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/brave/.alien-install-stage/node_modules/typebox/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/Extensions/browser/.ALIEN-INSTALL-STAGE-AbC123/node_modules/playwright-core/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
    ...
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/browser/.alien-runtime-deps-copy-AbC123/package.json",
      ),
    ).toBe(false);
    expect(
      isLegacyPluginDependencyInstallStagePath("dist/extensions/.alien-install-stage"),
    ).toBe(false);
  });

  it("rejects pre-populated install-stage debris at publish time", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-stage-publish-" }, async (packageRoot) => {
      ...
      await expect(collectLegacyPluginDependencyStagingDebrisPaths(packageRoot)).resolves.toEqual([
        "dist/extensions/browser/.alien-install-stage-AbC123",
        "dist/extensions/evil/.alien-install-stage",
      ]);
  ...
  it("rejects mixed-case install-stage debris on case-sensitive release builders", async () => {
    await withTempDir(
      { prefix: "alien-dist-inventory-stage-extensions-case-" },
      async (packageRoot) => {
    ...
    await withTempDir(
      { prefix: "alien-dist-inventory-stage-root-case-" },
      async (packageRoot) => {
        const mixedCaseStage = path.join(
          packageRoot,
          "Dist",
  ...
  it("treats a missing dist/extensions tree as no staging debris", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-no-extensions-" }, async (packageRoot) => {
  ...
  it("fails closed when the inventory is missing", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-missing-" }, async (packageRoot) => {
      await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
      await expect(collectPackageDistInventoryErrors(packageRoot)).resolves.toEqual([
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "alien-dist-inventory-symlink-" }, async (packageRoot) => {
