...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".alien");
...
function sharedMainOpsConfig(sharedStorePath: string): AlienConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: AlienConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { ALIEN_STATE_DIR: stateDir };
