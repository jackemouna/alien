...
const DENIED_CANARY_HEADER = "x-alien-proxy-validation-canary";
...
export type ResolveProxyValidationConfigOptions = {
  config?: ProxyConfig;
  env?: NodeJS.ProcessEnv | Partial<Record<"ALIEN_PROXY_URL", string | undefined>>;
...
function validateProxyUrl(value: string | undefined): string[] {
  if (!value) {
    return ["proxy validation requires proxy.proxyUrl, --proxy-url, or ALIEN_PROXY_URL"];
  }
...
function validateProxyEnabled(source: ProxyValidationConfigSource, enabled: boolean): string[] {
  ...
  if (source === "env") {
    return ["proxy validation requires proxy.enabled to be true for ALIEN_PROXY_URL"];
  }
...
export function resolveProxyValidationConfig(
  ...
  const envUrl = normalizeProxyUrl(options.env?.ALIEN_PROXY_URL);
  if (envUrl) {
    return {
      enabled: options.config?.enabled === true,
      proxyUrl: envUrl,
