...
describe("proxy validation", () => {
  it("resolves proxy URL overrides before config and ALIEN_PROXY_URL", () => {
    const result = resolveProxyValidationConfig({
      ...
      env: {
        ALIEN_PROXY_URL: "http://env-proxy.example:3128",
      },
    });
  ...
  it("resolves config proxy URLs before ALIEN_PROXY_URL", () => {
    const result = resolveProxyValidationConfig({
      config: {
        enabled: true,
        proxyUrl: "http://config-proxy.example:3128",
  ...
  it("uses ALIEN_PROXY_URL when enabled config has no URL", () => {
    const result = resolveProxyValidationConfig({
      config: { enabled: true },
      env: {
        ALIEN_PROXY_URL: "http://env-proxy.example:3128",
  ...
  it("reports disabled proxy config when only ALIEN_PROXY_URL is present", async () => {
    const fetchCheck = vi.fn();
    ...
    expect(result).toEqual({
      ...
      config: {
        ...
        proxyUrl: "http://env-proxy.example:3128",
        source: "env",
        errors: ["proxy validation requires proxy.enabled to be true for ALIEN_PROXY_URL"],
      },
      checks: [],
    });
  });
