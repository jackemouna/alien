...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Alien to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "alien.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ALIEN_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ALIEN_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ALIEN_APNS_TEAM_ID and ALIEN_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ALIEN_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ALIEN_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ALIEN_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ALIEN_APNS_PRIVATE_KEY_P8 or ALIEN_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ALIEN_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
