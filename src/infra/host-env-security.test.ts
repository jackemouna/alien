...
import { ALIEN_CLI_ENV_VALUE } from "./alien-exec-env.js";
...
describe("sanitizeHostExecEnv", () => {
  ...
  it("blocks PATH and dangerous override values", () => {
    ...
    expect(env.ALIEN_CLI).toBe(ALIEN_CLI_ENV_VALUE);
...
describe("shell wrapper exploit regression", () => {
  it("blocks SHELLOPTS/PS4 chain after sanitization", async () => {
    ...
    const marker = path.join(os.tmpdir(), `alien-ps4-marker-${process.pid}-${Date.now()}`);
    try {
      fs.unlinkSync(marker);
    } catch {
      // no-op
...
describe("git env exploit regression", () => {
  it("blocks inherited GIT_SEQUENCE_EDITOR so git rebase -i cannot execute helper payloads", async () => {
    ...
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-git-sequence-editor-${process.pid}-${Date.now()}-`),
    );
    const safeRepoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-git-sequence-editor-safe-${process.pid}-${Date.now()}-`),
    );
    const editorPath = path.join(repoDir, "sequence-editor.sh");
    const safeEditorPath = path.join(safeRepoDir, "sequence-editor.sh");
    const marker = path.join(
      os.tmpdir(),
      `alien-git-sequence-editor-marker-${process.pid}-${Date.now()}`,
    );
  ...
  it("blocks inherited GIT_EXEC_PATH so git cannot execute helper payloads", async () => {
    ...
    const helperDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-git-exec-path-${process.pid}-${Date.now()}-`),
    );
    const helperPath = path.join(helperDir, "git-remote-https");
    const marker = path.join(
      os.tmpdir(),
      `alien-git-exec-path-marker-${process.pid}-${Date.now()}`,
    );
    try {
      clearMarker(marker);
  ...
  it("blocks inherited GIT_TEMPLATE_DIR so git clone cannot install hook payloads", async () => {
    ...
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-git-template-source-${process.pid}-${Date.now()}-`),
    );
    const cloneDir = path.join(
      os.tmpdir(),
      `alien-git-template-clone-${process.pid}-${Date.now()}`,
    );
    const safeCloneDir = path.join(
      os.tmpdir(),
      `alien-git-template-safe-clone-${process.pid}-${Date.now()}`,
    );
    const templateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-git-template-dir-${process.pid}-${Date.now()}-`),
    );
    const hooksDir = path.join(templateDir, "hooks");
    const marker = path.join(
      os.tmpdir(),
      `alien-git-template-marker-${process.pid}-${Date.now()}`,
    );
  ...
  it("blocks GIT_SSH_COMMAND override so git cannot execute helper payloads", async () => {
    ...
    const marker = path.join(os.tmpdir(), `alien-git-ssh-command-${process.pid}-${Date.now()}`);
    clearMarker(marker);
...
describe("compiler override exploit regression", () => {
  it("blocks CC overrides so make cannot execute a substituted compiler", async () => {
    ...
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-compiler-override-${process.pid}-${Date.now()}-`),
    );
    const exploitPath = path.join(tempDir, "evil-cc");
    const marker = path.join(
      os.tmpdir(),
      `alien-compiler-override-marker-${process.pid}-${Date.now()}`,
    );
...
describe("make env exploit regression", () => {
  it("blocks MAKEFLAGS overrides so make cannot evaluate shell payloads from env", async () => {
    ...
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `alien-makeflags-override-${process.pid}-${Date.now()}-`),
    );
    const exploitPath = path.join(tempDir, "evil-makeflags.sh");
    const marker = path.join(os.tmpdir(), `alien-makeflags-marker-${process.pid}-${Date.now()}`);
