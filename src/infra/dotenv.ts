...
const BLOCKED_WORKSPACE_DOTENV_KEYS = new Set([
  ...
  "ALIEN_BROWSER_CONTROL_MODULE",
  "ALIEN_BUNDLED_HOOKS_DIR",
  "ALIEN_BUNDLED_PLUGINS_DIR",
  "ALIEN_BUNDLED_SKILLS_DIR",
  "ALIEN_CACHE_TRACE",
...
const BLOCKED_WORKSPACE_DOTENV_PREFIXES = [
  ...
  "CLAWHUB_",
  "OPENAI_API_KEY_",
  // Workspace .env is untrusted; reserve the full Alien runtime namespace
  // for shell/global config so new ALIEN_* controls are fail-closed by default.
  "ALIEN_",
  "ALIEN_CLAWHUB_",
  "ALIEN_DISABLE_",
  "ALIEN_SKIP_",
...
function shouldBlockRuntimeDotEnvKey(key: string): boolean {
  // The global ~/.alien/.env (or ALIEN_STATE_DIR/.env) is a trusted
...
export function loadGlobalRuntimeDotEnvFiles(opts?: { quiet?: boolean; stateEnvPath?: string }) {
  ...
  const hasExplicitNonDefaultStateDir =
    process.env.ALIEN_STATE_DIR?.trim() !== undefined &&
...
export function loadDotEnv(opts?: { quiet?: boolean }) {
  ...
  // Then load global fallback: ~/.alien/.env (or ALIEN_STATE_DIR/.env),
