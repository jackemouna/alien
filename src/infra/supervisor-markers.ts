const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "ALIEN_LAUNCHD_LABEL"],
  systemd: ["ALIEN_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["ALIEN_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.ALIEN_SERVICE_MARKER?.trim();
