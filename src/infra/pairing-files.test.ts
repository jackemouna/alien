...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/alien-state", "devices")).toEqual({
      dir: path.join("/tmp/alien-state", "devices"),
      pendingPath: path.join("/tmp/alien-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/alien-state", "devices", "paired.json"),
    });
  });
