...
function expectMalformedAgentAskUsesDefaults(agentAsk: unknown): void {
  ...
  expect(summary.ask).toMatchObject({
    requested: "off",
    host: "always",
    hostSource: "~/.alien/exec-approvals.json defaults.ask",
    effective: "always",
...
describe("exec approvals policy helpers", () => {
  ...
  it("explains stricter host security and ask precedence", () => {
    ...
    expect(summary.security).toMatchObject({
      ...
      host: "allowlist",
      effective: "allowlist",
      hostSource: "~/.alien/exec-approvals.json defaults.security",
    ...
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.alien/exec-approvals.json defaults.askFallback",
    });
  });
  ...
  it("attributes host policy to wildcard agent entries before defaults", () => {
    ...
    expect(summary.security).toMatchObject({
      host: "allowlist",
      hostSource: "~/.alien/exec-approvals.json agents.*.security",
    });
    expect(summary.ask).toMatchObject({
      host: "always",
      hostSource: "~/.alien/exec-approvals.json agents.*.ask",
    });
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.alien/exec-approvals.json agents.*.askFallback",
    });
  });
  ...
  it("reports askFallback from the Alien default when approvals omit it", () => {
    const summary = resolveExecPolicyScopeSummary({
      approvals: {
        version: 1,
        agents: {},
  ...
  it("avoids a duplicate default-agent scope when main only appears in approvals", () => {
    ...
    expect(snapshots[0]?.security).toMatchObject({
      host: "allowlist",
      hostSource: "~/.alien/exec-approvals.json agents.main.security",
    });
    expect(snapshots[0]?.ask).toMatchObject({
      host: "always",
      hostSource: "~/.alien/exec-approvals.json agents.main.ask",
    });
  });
