...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ALIEN_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*alien-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "alien";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "alien");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "alien-gw");
  ...
  records.push(`_alien-gw._tcp IN PTR ${instanceLabel}._alien-gw._tcp`);
  records.push(`${instanceLabel}._alien-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._alien-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; alien-content-hash: ${contentHash}\n${contentBody}`;
}
