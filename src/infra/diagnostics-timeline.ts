...
const ALIEN_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "alien.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.ALIEN_DIAGNOSTICS)) &&
    typeof env.ALIEN_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.ALIEN_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: ALIEN_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.ALIEN_DIAGNOSTICS_RUN_ID ? { runId: env.ALIEN_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.ALIEN_DIAGNOSTICS_ENV ? { envName: env.ALIEN_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.ALIEN_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
