...
describe("tmp-alien-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__alienTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_ALIEN_TMP_DIR, resolvePreferredAlienTmpDir } from "./src/infra/tmp-alien-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_ALIEN_TMP_DIR,
            resolverType: typeof resolvePreferredAlienTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-alien-dir-browser-entry.ts",
      },
      write: false,
    });
