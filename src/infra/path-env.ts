...
function candidateBinDirs(opts: EnsureAlienPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Alien process is on.
  try {
  ...
  // Bundled macOS app: `alien` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "alien");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ALIEN_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "alien"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `alien`
...
/**
 * Best-effort PATH bootstrap so skills that require the `alien` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAlienCliOnPath(opts: EnsureAlienPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ALIEN_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.ALIEN_PATH_BOOTSTRAPPED = "1";
