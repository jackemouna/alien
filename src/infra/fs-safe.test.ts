...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("alien-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("alien-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("alien-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("alien-fs-safe-root-");
    const sourceDir = await tempDirs.make("alien-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("alien-fs-safe-root-");
    const outside = await tempDirs.make("alien-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    process.env.ALIEN_HOME = fakeHome;
    try {
    ...
    const root = await tempDirs.make("alien-tilde-test-");
    process.env.HOME = root;
    process.env.ALIEN_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("alien-tilde-outside-");
