...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.ALIEN_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Alien. You can keep running the web gateway without it: \`${formatCliCommand("alien gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
