...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "ALIEN_LIVE_ANTHROPIC_KEYS",
  "ALIEN_LIVE_GEMINI_KEY",
  "ALIEN_LIVE_OPENAI_KEY",
  "ALIEN_GATEWAY_TOKEN",
  "ALIEN_GATEWAY_PASSWORD",
  "ALIEN_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "ALIEN_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "ALIEN_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "ALIEN_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "ALIEN_BUNDLED_SKILLS_DIR=./attacker-skills",
  "ALIEN_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "alien-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.alien/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.alien/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".alien");
        process.env.ALIEN_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "alien", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ALIEN_STATE_DIR=./evil-state",
            "ALIEN_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.ALIEN_STATE_DIR).toBe(stateDir);
        expect(process.env.ALIEN_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ALIEN_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "ALIEN_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "ALIEN_LIVE_GEMINI_KEY=sk-gemini-live",
            "ALIEN_LIVE_OPENAI_KEY=sk-openai-live",
            "ALIEN_GATEWAY_TOKEN=attacker-token",
            "ALIEN_GATEWAY_PASSWORD=attacker-password",
            "ALIEN_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.ALIEN_STATE_DIR).toBeUndefined();
  ...
  it("blocks path-override vars (ALIEN_AGENT_DIR, ALIEN_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, ALIEN_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        delete process.env.ALIEN_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ALIEN_AGENT_DIR).toBeUndefined();
        expect(process.env.ALIEN_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.ALIEN_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks ALIEN_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "ALIEN_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.ALIEN_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ALIEN_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ALIEN_PINNED_PYTHON=./attacker-python",
            "ALIEN_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.ALIEN_PINNED_PYTHON;
        delete process.env.ALIEN_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ALIEN_PINNED_PYTHON).toBeUndefined();
        expect(process.env.ALIEN_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ALIEN_PINNED_PYTHON=/trusted/python",
            "ALIEN_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.ALIEN_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.ALIEN_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ALIEN_LIVE_GEMINI_KEY=sk-gemini-live",
            "ALIEN_LIVE_OPENAI_KEY=sk-openai-live",
            "ALIEN_GATEWAY_TOKEN=trusted-token",
            "ALIEN_GATEWAY_PASSWORD=trusted-password",
            "ALIEN_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.ALIEN_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.ALIEN_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.ALIEN_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.ALIEN_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.ALIEN_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.ALIEN_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.ALIEN_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "ALIEN_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks ALIEN_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "ALIEN_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when ALIEN_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.ALIEN_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "alien", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.ALIEN_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "alien-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      process.env.HOME = base;
      delete process.env.ALIEN_STATE_DIR;
      delete process.env.ALIEN_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "ALIEN_WORKSPACE_DIR",
          "ALIEN_MDNS_HOSTNAME",
          "ALIEN_SESSION_CACHE_TTL_MS",
          "ALIEN_UPDATE_PACKAGE_SPEC",
          "ALIEN_GATEWAY_PORT",
          "ALIEN_GATEWAY_URL",
          "ALIEN_CLAWHUB_URL",
          "CLAWHUB_URL",
          ...
          "ALIEN_SKIP_CRON",
          "ALIEN_RAW_STREAM",
          "ALIEN_RAW_STREAM_PATH",
          "ALIEN_CACHE_TRACE",
          "ALIEN_CACHE_TRACE_FILE",
          "ALIEN_CACHE_TRACE_MESSAGES",
          "ALIEN_CACHE_TRACE_PROMPT",
          "ALIEN_CACHE_TRACE_SYSTEM",
          "ALIEN_SHOW_SECRETS",
          "ALIEN_PLUGIN_CATALOG_PATHS",
          "ALIEN_MPM_CATALOG_PATHS",
          "ALIEN_NODE_EXEC_HOST",
          "ALIEN_NODE_EXEC_FALLBACK",
          "ALIEN_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("alien/alien");
