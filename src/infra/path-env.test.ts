...
import { ensureAlienCliOnPath } from "./path-env.js";
...
describe("ensureAlienCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ALIEN_PATH_BOOTSTRAPPED",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/alien-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "alien");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureAlienCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.ALIEN_PATH_BOOTSTRAPPED;
    delete process.env.ALIEN_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling alien exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/alien-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "alien");
      ...
      if (envValue === undefined) {
        ...
        process.env.ALIEN_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/alien-path/case-linuxbrew");
