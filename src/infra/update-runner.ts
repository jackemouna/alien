...
const DEFAULT_PACKAGE_NAME = "alien";
...
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "alien-update-preflight-";
...
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "ALIEN_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["alien", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "alien.mjs");
      ...
      const doctorStep = await runStep(
        step("alien doctor", doctorArgv, gitRoot, {
