...
describe("executable path helpers", () => {
  it("detects executable files and rejects directories or non-executables", async () => {
    await withTempDir({ prefix: "alien-exec-path-" }, async (base) => {
  ...
  it("does not treat drive-less rooted windows paths as cwd-relative executables", () => {
    ...
    expect(
      resolveExecutablePath(String.raw`:\Users\demo\AI\system\alien\git.exe`, {
        cwd: String.raw`C:\Users\demo\AI\system\alien`,
      }),
    ).toBeUndefined();
    expect(
      resolveExecutablePath(String.raw`:/Users/demo/AI/system/alien/git.exe`, {
