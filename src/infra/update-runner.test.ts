...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "alien-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:alien-update-preflight-|ocu-pf-)/;
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "alien.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "alien.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "alien", version: "1.0.0" };
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g alien@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g alien@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "alien.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "alien.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("alien-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "alien.mjs")} doctor --non-interactive --fix`;
    ...
    expect(pnpmEnvPaths.some((value) => value.includes("alien-update-pnpm-"))).toBe(true);
  });
  ...
  it("runs dev preflight lint in constrained mode when explicitly enabled", async () => {
    ...
    const result = await withEnvAsync({ ALIEN_UPDATE_PREFLIGHT_LINT: "1" }, async () =>
    ...
    expect(lintEnv[0]).toMatchObject({
      ALIEN_LOCAL_CHECK: "1",
      ALIEN_LOCAL_CHECK_MODE: "throttled",
      ALIEN_OXLINT_SHARDS_SERIAL: "1",
    });
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    try {
      const runCommand = async (
        ...
        if (key === "pnpm install") {
          if (options?.cwd && /(?:alien-update-preflight-|ocu-pf-)/.test(options.cwd)) {
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "alien.mjs")} doctor --non-interactive --fix`;
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "alien", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "alien", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
        ...
        if (normalizedInstallCommand === params.installCommand) {
          const packageRoot =
            process.platform === "win32"
              ? path.join(installPrefix, "node_modules", "alien")
              : path.join(installPrefix, "lib", "node_modules", "alien");
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g alien@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g alien@beta --no-fund --no-audit --loglevel=error",
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand:
        "npm i -g github:alien/alien#main --no-fund --no-audit --loglevel=error",
      tag: "main",
    });
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g alien@latest --no-fund --no-audit --loglevel=error",
      onInstall: async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("alien doctor");
    expect(doctorEnv?.ALIEN_UPDATE_IN_PROGRESS).toBe("1");
    expect(doctorEnv?.ALIEN_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
  });
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g alien@latest --no-fund --no-audit --loglevel=error");
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "alien");
    const staleDir = path.join(nodeModules, ".alien-stale");
  ...
  it("fails global npm update when the installed version misses the requested correction", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: "npm i -g alien@2026.3.23-2 --no-fund --no-audit --loglevel=error",
      tag: "2026.3.23-2",
    });
    ...
    expect(calls).toContain("npm i -g alien@2026.3.23-2 --no-fund --no-audit --loglevel=error");
  });

  it("fails global npm update when bundled runtime sidecars are missing after install", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = "npm i -g alien@latest --no-fund --no-audit --loglevel=error";
    ...
    const { runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async (options) => {
        ...
        if (options?.installPrefix) {
          ...
          await fs.writeFile(path.join(binDir, "alien"), "#!/bin/sh\n", "utf-8");
        }
      },
    });
  ...
  it("uses ALIEN_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/alien-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: expectedInstallCommand,
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g alien@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g alien@latest")).toBe(true);
    });
  });

  it("rejects git roots that are not a alien checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-alien-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when alien.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "alien.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("alien doctor entry");
  });
