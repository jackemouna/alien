...
function createAlienBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "alien-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createAlienBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses alien-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "alien-gateway" },
          { pid: process.pid, cmd: "alien-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of alien", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/alien/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: alien-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "alien-gateway" },
          { pid: unrelatedStalePid, cmd: "alien-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "alien-gateway" },
            { pid: grandparentPid, cmd: "alien-gateway" },
            { pid: benignStalePid, cmd: "alien-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "alien-gateway" },
            { pid: gatewayGrandparentPid, cmd: "alien-gateway" },
          ]),
          stderr: "",
        });
    ...
    it("excludes pids whose command does not include 'alien'", () => {
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["alien", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple alien pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "alien-gateway" },
          { pid: pid2, cmd: "alien-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-alien pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks alien processes — non-alien listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty alien stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty alien stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + alien pid in stdout — container-restricted lsof reports partial results
          return createAlienBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "alien-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'alien'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("alien")
      // inside the mid-loop flush: a non-alien cmd between two entries must not
      // be pushed, but the following alien entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-alien entry first, then alien entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid alien entry must still be found.
      const stdout = `p0\ncalien-gateway\np${stalePid}\ncalien-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero alien pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-alien stdout (line 145)", () => {
    it("treats status 1 + non-alien stdout as port-free (not an alien process)", () => {
      // status 1 + non-empty stdout where no alien pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill alien pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-alien output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no alien pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
