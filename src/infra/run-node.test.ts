...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "alien.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "alien.plugin.json");
...
function statusCommandSpawn() {
  return [process.execPath, "alien.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "alien.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "alien.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "alien.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "alien.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          ALIEN_FORCE_BUILD: "1",
          ALIEN_RUNNER_LOG: "1",
          ALIEN_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[alien]");
      expect(spawnCalls.at(-1)?.args).toEqual(["alien.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ALIEN_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "alien.mjs" ? childStderr : "",
        });
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ALIEN_RUNNER_LOG: "0",
          ALIEN_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          ALIEN_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched Alien child when requested", async () => {
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ALIEN_RUNNER_LOG: "0",
          ALIEN_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=alien-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["alien.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ALIEN_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
      expect(fsSync.existsSync(profileDir)).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched Alien child when requested", async () => {
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "alien.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["alien.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.ALIEN_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"alien":{"extensions":["./index.ts"]}}\n',
      ...
      await expect(
        Promise.all([
          runStatusCommand({
            ...
            spawnSync,
            env: {
              ALIEN_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
            },
            runRuntimePostBuild,
          }),
          runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active alien child and returns 143", async () => {
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("skips rebuilding when extension package metadata is newer than the build stamp", async () => {
    await withTempDir({ prefix: "alien-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","alien":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","alien":{"extensions":["./stale.js"]}}\n',
        },
