...
describe("resolveLocalVitestMaxWorkers", () => {
  ...
  it("lets ALIEN_VITEST_MAX_WORKERS override the inferred cap", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          ALIEN_VITEST_MAX_WORKERS: "2",
        },
        {
          cpuCount: 10,
          loadAverage1m: 0,
  ...
  it("respects the legacy ALIEN_TEST_WORKERS override too", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          ALIEN_TEST_WORKERS: "3",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0,
...
describe("resolveLocalVitestScheduling", () => {
  ...
  it("allows disabling the system throttle probe explicitly", () => {
    expect(
      resolveLocalVitestScheduling(
        {
          ALIEN_VITEST_DISABLE_SYSTEM_THROTTLE: "1",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0.5,
...
describe("test scripts", () => {
  it("keeps test scripts on the native thread-first configs", () => {
    ...
    expect(pkg.scripts?.["test:serial"]).toBe(
      "ALIEN_TEST_PROJECTS_SERIAL=1 ALIEN_VITEST_MAX_WORKERS=1 node scripts/test-projects.mjs",
    );
    ...
    expect(pkg.scripts?.["test:gateway"]).toBe(
      "ALIEN_GATEWAY_PROJECT_SHARDS=1 node scripts/run-vitest.mjs run --config test/vitest/vitest.gateway.config.ts",
    );
