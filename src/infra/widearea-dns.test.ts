...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "alien.internal", expected: "alien.internal." },
    { value: "alien.internal.", expected: "alien.internal." },
    { value: "  alien.internal  ", expected: "alien.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { ALIEN_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { ALIEN_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("alien.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "alien.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN alien.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_alien-gw._tcp IN PTR studio-london._alien-gw._tcp`,
      `studio-london._alien-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Alien)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/alien`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN alien.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._alien-gw._tcp IN TXT`,
        `displayName=Mac Studio (Alien)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("alien.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("alien.internal."),
