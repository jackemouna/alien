...
import { resolveAlienPackageRoot } from "./alien-root.js";
...
function resolveAutoUpdatePolicy(cfg: AlienConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: AlienConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.ALIEN_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("alien update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by ALIEN_NO_AUTO_UPDATE", {
