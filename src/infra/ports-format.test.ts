...
describe("ports-format", () => {
  it.each([
    ...
    [{ command: "ssh" }, "ssh"],
    [{ commandLine: "node /Users/me/Projects/alien/dist/entry.js gateway" }, "gateway"],
  ...
  it("builds ordered hints for mixed listener kinds and multiplicity", () => {
    expect(
      buildPortHints(
        [
          { commandLine: "node dist/index.js alien gateway" },
  ...
  it("treats single-process loopback dual-stack gateway listeners as benign", () => {
    const listeners = [
      { pid: 4242, commandLine: "alien-gateway", address: "127.0.0.1:18789" },
      { pid: 4242, commandLine: "alien-gateway", address: "[::1]:18789" },
    ];
