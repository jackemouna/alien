...
const resolvePreferredAlienTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".alien", "gateway.cmd");
  }),
);
...
vi.mock("./tmp-alien-dir.js", () => ({
  resolvePreferredAlienTmpDir: () => resolvePreferredAlienTmpDirMock(),
}));
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredAlienTmpDirMock.mockReset();
    resolvePreferredAlienTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ ALIEN_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "Alien Gateway (work)"']),
    });
    ...
    expect(script).toContain(
      'alien restart attempt source=windows-task-handoff target="Alien Gateway (work)"',
    );
    expect(script).toContain('schtasks /Run /TN "Alien Gateway (work)" >>');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers ALIEN_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Alien Gateway (custom)" >>');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredAlienTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ ALIEN_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-state-"));
