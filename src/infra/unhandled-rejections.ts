...
// Plugins resolve `alien/plugin-sdk/runtime` through their own staged
...
const HANDLERS_GLOBAL_KEY = Symbol.for("alien.unhandledRejection.handlers");
const EXCEPTION_HANDLERS_GLOBAL_KEY = Symbol.for("alien.uncaughtException.handlers");
...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[alien] Unhandled rejection handler failed:",
...
export function isUncaughtExceptionHandled(error: unknown): boolean {
  for (const handler of exceptionHandlers) {
    try {
      ...
      console.error(
        "[alien] Uncaught exception handler failed:",
        err instanceof Error ? (err.stack ?? err.message) : err,
      );
    }
  }
...
export function installUnhandledRejectionHandler(): void {
  ...
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[alien] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[alien] FATAL unhandled rejection:", formatUncaughtError(reason));
      exitWithTerminalRestore("fatal unhandled rejection", reason, "fatal_unhandled_rejection");
      return;
    }

    if (isConfigError(reason)) {
      console.error("[alien] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
    ...
    if (isTransientUnhandledRejectionError(reason)) {
      console.warn(
        "[alien] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[alien] Unhandled promise rejection:", formatUncaughtError(reason));
