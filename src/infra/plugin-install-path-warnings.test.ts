...
const MATRIX_REPO_INSTALL_COMMAND = `alien plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/alien-matrix-missing",
        installPath: "/tmp/alien-matrix-missing",
      },
    });
    ...
    expect(
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/alien-matrix-missing",
      'Reinstall with "alien plugins install @alien/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/alien-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<alien plugins install @alien/matrix>" when you want to return to the standard Matrix plugin.',
