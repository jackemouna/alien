...
export function emitGatewayRestart(reasonOverride?: string): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerAlienRestart() uses schtasks to restart the gateway.
...
export function triggerAlienRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ALIEN_SYSTEMD_UNIT,
      process.env.ALIEN_PROFILE,
    );
  ...
  const label =
    process.env.ALIEN_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ALIEN_PROFILE);
