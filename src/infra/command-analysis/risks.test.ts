...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "ALIEN_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "alien", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("alien channels login --channel whatsapp");
  });
