...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__alien_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "alien-root");
...
function setPackageRoot(root: string, name = "alien") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./alien-root.js").resolveAlienPackageRootSync,
  asyncResolver: typeof import("./alien-root.js").resolveAlienPackageRoot,
  opts: Parameters<typeof import("./alien-root.js").resolveAlienPackageRootSync>[0],
...
vi.mock("./alien-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveAlienPackageRoot", () => {
  let resolveAlienPackageRoot: typeof import("./alien-root.js").resolveAlienPackageRoot;
  let resolveAlienPackageRootSync: typeof import("./alien-root.js").resolveAlienPackageRootSync;
  let clearAlienPackageRootCaches: typeof import("./alien-root.js").__testing.clearAlienPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveAlienPackageRoot,
      resolveAlienPackageRootSync,
      __testing: { clearAlienPackageRootCaches },
    } = await import("./alien-root.js"));
  });

  beforeEach(() => {
    clearAlienPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "alien");
        const pkgRoot = path.join(project, "node_modules", "alien");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "alien");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "alien.mjs")));
    ...
    {
      name: "falls through from a non-alien moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-alien");
    ...
    {
      name: "returns null for non-alien package roots",
      setup: () => {
        const pkgRoot = fx("not-alien");
        setPackageRoot(pkgRoot, "not-alien");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "alien.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "alien");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
