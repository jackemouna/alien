...
import {
  ...
  isAlienStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@alien/voice-call",
    "@alien/voice-call@1.2.3",
    "@alien/voice-call@1.2.3-beta.4",
    "@alien/voice-call@latest",
    "@alien/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    ...
    {
      spec: "git+ssh://github.com/alien/alien",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@alien/voice-call@2026.5.3-1",
      expected: {
        name: "@alien/voice-call",
        raw: "@alien/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@alien/voice-call@1.2.3-beta.1",
      expected: {
        name: "@alien/voice-call",
        raw: "@alien/voice-call@1.2.3-beta.1",
  ...
  ])("detects Alien stable correction versions for %s", ({ value, expected }) => {
    expect(isAlienStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares Alien release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareAlienReleaseVersions(left, right)).toBe(expected);
  });
});
  ...
  it.each([
    {
      spec: "@alien/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@alien/voice-call@beta"`,
    },
    {
      spec: "@alien/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
