...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AlienConfig | null = null;
  ...
  get(key: string, cfg: AlienConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AlienConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AlienConfig): void {
