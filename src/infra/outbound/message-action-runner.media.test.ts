...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Alien tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideAlien = path.join(os.tmpdir(), "outside-alien", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
