...
describe("stripInternalRuntimeScaffolding", () => {
  ...
  it("fails closed on unmatched runtime context delimiters", () => {
    expect(
      stripInternalRuntimeScaffolding(
        ["visible", "<<<BEGIN_ALIEN_INTERNAL_CONTEXT>>>", "internal metadata"].join("\n"),
      ),
    ).toBe("visible");
  });
  ...
  it("preserves inline delimiter mentions", () => {
    expect(
      stripInternalRuntimeScaffolding("visible <<<END_ALIEN_INTERNAL_CONTEXT>>> inline mention"),
    ).toBe("visible <<<END_ALIEN_INTERNAL_CONTEXT>>> inline mention");
  ...
  it("removes stray standalone marker lines", () => {
    expect(
      stripInternalRuntimeScaffolding(
        ["visible", "<<<END_ALIEN_INTERNAL_CONTEXT>>>", "after"].join("\n"),
      ),
    ).toBe("visible\nafter");
    expect(
      stripInternalRuntimeScaffolding(
