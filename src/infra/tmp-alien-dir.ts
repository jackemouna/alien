...
export const POSIX_ALIEN_TMP_DIR = "/tmp/alien";
...
export type ResolvePreferredAlienTmpDirOptions = {
...
export function resolvePreferredAlienTmpDir(
  options: ResolvePreferredAlienTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "alien" : `alien-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[alien] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Alien temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Alien temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_ALIEN_TMP_DIR;
