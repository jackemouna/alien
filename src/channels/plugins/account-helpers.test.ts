...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): AlienConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as AlienConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: AlienConfig) => string[];
  input: AlienConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: AlienConfig, expected: string) {
