...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AlienConfig, policy: ChannelAccessPolicy) => AlienConfig;
  resolveAllowlist: (params: { cfg: AlienConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AlienConfig; resolved: TResolved }) => AlienConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AlienConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AlienConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AlienConfig, policy: ChannelAccessPolicy): AlienConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AlienConfig, policy: ChannelAccessPolicy): AlienConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AlienConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AlienConfig; resolved: string[] }) => {
      calls.push("apply");
