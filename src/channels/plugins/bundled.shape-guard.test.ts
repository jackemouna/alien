  ...
  return fs
    ...
    .filter(
      (entryPath) =>
        ...
        fs.existsSync(path.join(entryPath, "alien.plugin.json")),
    );
}

afterEach(() => {
  delete (globalThis as { __alienBundledChannelReenter?: () => void })
    .__alienBundledChannelReenter;
  vi.resetModules();
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-metadata-"));
    ...
    try {
      process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-override-"));
    const previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ALIEN_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("caches undefined bundled plugin loads as unavailable", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-null-load-"));
    const previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "alien/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "alien/plugin-sdk/core"') ||
        source.includes('from "alien/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.alien?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "alien/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("alien/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "alien/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-reentrant-"));
    const modulePath = path.join(pluginDir, "index.cjs");
    fs.writeFileSync(
      modulePath,
      `
const reenter = globalThis.__alienBundledChannelReenter;
...
`,
    ...
    let reentered = false;
    (
      globalThis as { __alienBundledChannelReenter?: () => void }
    ).__alienBundledChannelReenter = () => {
      if (!reentered) {
        reentered = true;
        expect(bundled.listBundledChannelPlugins()).toEqual([]);
