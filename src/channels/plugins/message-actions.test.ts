...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as AlienConfig).toSorted()).toEqual([
      "delivery-pin",
      "presentation",
    ]);
    expect(channelSupportsMessageCapability({} as AlienConfig, "presentation")).toBe(true);
    expect(channelSupportsMessageCapability({} as AlienConfig, "delivery-pin")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as AlienConfig, channel: "demo-buttons" },
        "presentation",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as AlienConfig, channel: "demo-cards" },
        "presentation",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      channelSupportsMessageCapabilityForChannel({ cfg: {} as AlienConfig }, "delivery-pin"),
    ).toBe(false);
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as AlienConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as AlienConfig)).toEqual(["presentation"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as AlienConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as AlienConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as AlienConfig)).toEqual([]);
