    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@alien/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that Alien can discover during onboarding and install on demand via `alien plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-alien-plugin",
                  description:
                    "Alien WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  alien: {
                    channel: {
            ...
            expected: {
              ...
              installSource: {
                ...
                npm: {
                  ...
                  packageName: "@wecom/wecom-alien-plugin",
                  selector: "1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-prerelease-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(catalogPath, {
            ...createCatalogEntry({
              packageName: "@alien/prerelease-demo-channel",
          ...
          return {
            ...
            catalogPaths: [catalogPath],
            expected: {
              install: { npmSpec: "@alien/prerelease-demo-channel@2026.5.3-beta.1" },
              installSource: {
                npm: {
                  spec: "@alien/prerelease-demo-channel@2026.5.3-beta.1",
                  packageName: "@alien/prerelease-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-clawhub-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "clawhub",
                clawhub: {
                  spec: "clawhub:alien/clawhub-chat@2026.5.2",
                  packageName: "alien/clawhub-chat",
                  version: "2026.5.2",
                  exactVersion: true,
                },
                warnings: [],
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  description:
                    "Alien Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  alien: {
                    channel: {
                    ...
                    install: {
                      npmSpec: "alien-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
          ...
          return {
            channelId: "alien-plugin-yuanbao",
            catalogPaths: [catalogPath],
            expected: {
              id: "alien-plugin-yuanbao",
              meta: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredAlienTmpDir(), "alien-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@alien/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              ALIEN_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              ALIEN_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@alien/default-env-demo",
