  ...
  describe(`${params.pluginId} bundled metadata-only channel catalog contract`, () => {
    it("includes the bundled metadata-only channel entry when the runtime entrypoint is omitted", () => {
      const workspaceDir = fs.mkdtempSync(
        path.join(resolvePreferredAlienTmpDir(), "alien-bundled-catalog-"),
      );
      const bundledDir = path.join(workspaceDir, ".alien", "extensions", params.pluginId);
      fs.mkdirSync(bundledDir, { recursive: true });
      fs.writeFileSync(
        path.join(workspaceDir, "package.json"),
        JSON.stringify({ name: "alien" }),
        "utf8",
      );
      fs.writeFileSync(
      ...
      fs.writeFileSync(
        path.join(bundledDir, "alien.plugin.json"),
      ...
      const entry = listChannelPluginCatalogEntries({
        workspaceDir,
        env: createCatalogFixtureEnv({ ALIEN_DISABLE_BUNDLED_PLUGINS: "1" }),
  ...
  describe(`${params.channelId} official fallback channel catalog contract`, () => {
    it("includes shipped official channel catalog entries when bundled metadata is omitted", () => {
      const dir = fs.mkdtempSync(
        path.join(resolvePreferredAlienTmpDir(), "alien-official-catalog-"),
      );
    ...
    it("lets external catalogs override shipped fallback channel metadata", () => {
      const dir = fs.mkdtempSync(
        path.join(resolvePreferredAlienTmpDir(), "alien-fallback-catalog-"),
      );
    ...
    it("surfaces package-name drift in external channel catalog install metadata", () => {
      const dir = fs.mkdtempSync(
        path.join(resolvePreferredAlienTmpDir(), "alien-drifted-catalog-"),
      );
