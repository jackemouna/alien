...
const SAME_CHANNEL_SDK_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']alien\/plugin-sdk\/discord["']/, /plugin-sdk-internal\/discord/],
  },
  {
    path: bundledPluginFile("slack", "src/shared.ts"),
    forbiddenPatterns: [/["']alien\/plugin-sdk\/slack["']/, /plugin-sdk-internal\/slack/],
  },
  {
    path: bundledPluginFile("telegram", "src/shared.ts"),
    forbiddenPatterns: [/["']alien\/plugin-sdk\/telegram["']/, /plugin-sdk-internal\/telegram/],
  },
  {
    path: bundledPluginFile("telegram", "src/account-inspect.ts"),
    forbiddenPatterns: [/["']alien\/plugin-sdk\/account-resolution["']/],
  },
  {
  ...
  },
  {
    ...
    forbiddenPatterns: [/["']alien\/plugin-sdk\/imessage["']/, /plugin-sdk-internal\/imessage/],
  },
  {
    path: bundledPluginFile("whatsapp", "src/shared.ts"),
    forbiddenPatterns: [/["']alien\/plugin-sdk\/whatsapp["']/, /plugin-sdk-internal\/whatsapp/],
  },
  {
    path: bundledPluginFile("signal", "src/shared.ts"),
    forbiddenPatterns: [/["']alien\/plugin-sdk\/signal["']/, /plugin-sdk-internal\/signal/],
  },
  {
...
const CHANNEL_CONFIG_SCHEMA_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']alien\/plugin-sdk\/core["']/],
  },
];
...
function expectNoCrossPluginSdkFacadeImports(file: string, imports: string[]): void {
  ...
  for (const specifier of imports) {
    if (!specifier.startsWith("alien/plugin-sdk/")) {
      continue;
    }
    const targetSubpath = specifier.slice("alien/plugin-sdk/".length);
...
describe("channel import guardrails", () => {
  ...
  it("keeps bundled extension source files off root and compat plugin-sdk imports", () => {
    for (const file of collectExtensionSourceFiles()) {
      ...
      expect(text, `${file} should not import alien/plugin-sdk root`).not.toMatch(
        /["']alien\/plugin-sdk["']/,
      );
      expect(text, `${file} should not import alien/plugin-sdk/compat`).not.toMatch(
        /["']alien\/plugin-sdk\/compat["']/,
      );
    }
  });
  ...
  it("keeps internalized extension helper surfaces behind local api barrels", () => {
    for (const extensionId of LOCAL_EXTENSION_API_BARREL_GUARDS) {
      for (const file of collectExtensionFiles(extensionId)) {
        ...
        ).not.toMatch(new RegExp(`["']alien/plugin-sdk/${extensionId}(?:["'/])`, "u"));
      }
    }
  });
});
