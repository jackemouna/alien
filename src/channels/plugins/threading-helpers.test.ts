...
describe("createStaticReplyToModeResolver", () => {
  it.each(["off", "all"] as const)("always returns the configured mode %s", (mode) => {
    expect(createStaticReplyToModeResolver(mode)({ cfg: {} as AlienConfig })).toBe(mode);
  });
});
  ...
  it.each([
    {
      ...
      cfg: { channels: { "demo-top-level": { replyToMode: "first" } } } as AlienConfig,
      expected: "first",
    },
    {
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as AlienConfig, chatType: "group" })).toBe("first");
