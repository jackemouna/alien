...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://alien.ai");
  });
  ...
  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    ...
    startupRegistry.channels = [
      {
        pluginId: "alien-weixin",
        plugin: { id: "alien-weixin", meta: { aliases: ["weixin"] } },
        source: "test",
      },
    ] as never;
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["alien-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("alien-weixin");
  });
