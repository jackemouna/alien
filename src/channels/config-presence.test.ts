...
const matrixPresenceOptions = {
  ...
  persistedAuthStateProbe: {
    ...
    hasState: ({ channelId, env }: { channelId: string; env?: NodeJS.ProcessEnv }) =>
      channelId === "matrix" && Boolean(env?.ALIEN_STATE_DIR?.includes("persisted-matrix")),
  },
};

function makeTempStateDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-channel-config-presence-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("config presence", () => {
  ...
  it("detects persisted Matrix credentials without config or env", () => {
    const stateDir = makeTempStateDir().replace(
      "alien-channel-config-presence-",
      "persisted-matrix-",
    );
    fs.mkdirSync(stateDir, { recursive: true });
    tempDirs.push(stateDir);
    const env = { ALIEN_STATE_DIR: stateDir } as NodeJS.ProcessEnv;
