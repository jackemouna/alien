...
// resolveAlienPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/alien-root.js", () => ({
  resolveAlienPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveAlienPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
function useBundledPluginsDir(extensionsRoot: string | undefined): void {
  if (extensionsRoot) {
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = extensionsRoot;
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "alien" });
...
function seedChannelPkg(
  ...
  writeJsonFile(path.join(pluginDir, "alien.plugin.json"), {
    id: opts.id,
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // ALIEN_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an ALIEN_BUNDLED_PLUGINS_DIR override pointing at
