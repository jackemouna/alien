...
const TEMP_MEDIA_PREFIX = "alien-media-";
...
function createGroqAudioConfig(): AlienConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AlienConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AlienConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AlienConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredAlienTmpDir();
