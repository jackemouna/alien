...
function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`alien models auth login --provider ${choice.providerId}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("alien configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("alien configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
