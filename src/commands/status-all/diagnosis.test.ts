...
describe("status-all diagnosis port checks", () => {
  it("labels Alien Tailscale exposure separately from daemon state", async () => {
  ...
  it("treats same-process dual-stack loopback listeners as healthy", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "alien-gateway", address: "127.0.0.1:18789" },
      { pid: 5001, commandLine: "alien-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "alien-gateway", address: "127.0.0.1:18789" },
      { pid: 5002, commandLine: "alien-gateway", address: "[::1]:18789" },
    ]);
