...
export function stripUnknownConfigKeys(config: AlienConfig): {
  config: AlienConfig;
  removed: string[];
} {
  const parsed = AlienSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: AlienConfig): void {
