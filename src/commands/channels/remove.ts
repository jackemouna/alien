...
export async function channelsRemoveCommand(
  ...
  let cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as AlienConfig;
  ...
  if (!plugin) {
    if (resolvedPluginState?.catalogEntry) {
      runtime.error(
        `Channel plugin "${resolvedPluginState.catalogEntry.id}" is not installed. Run "alien channels add --channel ${resolvedPluginState.catalogEntry.id}" first.`,
      );
      runtime.exit(1);
      return;
    }
