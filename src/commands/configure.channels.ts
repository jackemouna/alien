...
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in alien.json.",
          `Tip: \`${formatCliCommand("alien channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    if (Object.keys(nextChannels).length) {
      next.channels = nextChannels as AlienConfig["channels"];
    } else {
