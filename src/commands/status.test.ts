...
beforeAll(() => {
  envSnapshot = captureEnv(["ALIEN_PROFILE"]);
  process.env.ALIEN_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByAlien: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/alien/pnpm-lock.yaml",
        markerPath: "/tmp/alien/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.ALIEN_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.alien.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.alien.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.ALIEN_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/alien-root.js", () => ({
  resolveAlienPackageRoot: vi.fn().mockResolvedValue("/tmp/alien"),
  resolveAlienPackageRootSync: vi.fn(() => "/tmp/alien"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("alien status --all") ||
          line.includes("alien --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("alien --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("ALIEN_GATEWAY_TOKEN", "abcd1234", async () => {
