...
const testConfigStore = new Map<string, AlienConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.ALIEN_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.ALIEN_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("ALIEN_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "alien.json");
}
...
function readTestConfig<T = AlienConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  replaceConfigFile: async ({ nextConfig }: { nextConfig: AlienConfig }) => {
    testConfigStore.set(resolveTestConfigPath(), nextConfig);
  },
  resolveGatewayPort: (cfg: AlienConfig) => cfg.gateway?.port ?? 18789,
}));
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/alien-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "alien"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("alien-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "alien");
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `alien gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `alien gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.ALIEN_STATE_DIR = stateDir;
      process.env.ALIEN_CONFIG_PATH = path.join(stateDir, "alien.json");
