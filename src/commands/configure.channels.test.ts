...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirm).toHaveBeenCalledWith(
    expect.objectContaining({
      message: `Delete ${label} configuration from ~/.alien/alien.json?`,
    }),
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from alien.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from alien.json", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram configuration from ~/.alien/alien.json?",
      }),
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete done configuration from ~/.alien/alien.json?",
      }),
    );
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram\\nBot configuration from ~/.alien/alien.json?",
      }),
    );
    expect(note).toHaveBeenCalledWith(
