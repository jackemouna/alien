...
import { bundledPluginRoot, bundledPluginRootAt } from "alien/plugin-sdk/test-fixtures";
...
const discoverAlienPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverAlienPlugins: (args: unknown) => discoverAlienPlugins(args),
}));

import fs from "node:fs";
import type { ChannelPluginCatalogEntry } from "../../channels/plugins/catalog.js";
import type { AlienConfig } from "../../config/config.js";
import { loadAlienPlugins } from "../../plugins/loader.js";
...
const bundledChatNpmSpec = "@alien/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/alien", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/alien-test/${id}`,
    source: `/tmp/alien-test/${id}/index.ts`,
    manifestPath: `/tmp/alien-test/${id}/alien.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadAlienPlugins).toHaveBeenCalledWith(
    expect.not.objectContaining({
      onlyPluginIds: [params.pluginId],
    }),
  );
  const firstLoadCall = vi.mocked(loadAlienPlugins).mock.calls[0]?.[0] as
...
beforeEach(() => {
  ...
  discoverAlienPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: AlienConfig = { update: { channel } };
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: AlienConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/alien-ledger-channel";
    process.env.ALIEN_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: AlienConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-alien-plugin",
      targetDir: "/tmp/wecom-alien-plugin",
      version: "2026.5.4-beta.1",
      npmResolution: {
        name: "@alien/wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@alien/wecom@2026.5.4-beta.1",
      },
    });
    ...
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "@alien/wecom@beta",
        expectedPluginId: "wecom-alien-plugin",
      }),
    );
    expect(result.cfg.plugins?.installs?.["wecom-alien-plugin"]?.spec).toBe("@alien/wecom");
  });
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: bundledPluginRootAt("/opt/alien", "bundled-chat"),
          }),
        ]),
      }),
    );
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...
        install: {
          clawhubSpec: "clawhub:alien/clawhub-chat@2026.5.2",
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: [
          expect.objectContaining({
            value: "clawhub",
            label: "Download from ClawHub (clawhub:alien/clawhub-chat@2026.5.2)",
          }),
          expect.objectContaining({
            value: "skip",
          }),
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@alien/wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-alien-plugin",
      installPath: "/tmp/wecom-alien-plugin",
    });
    vi.mocked(fs.existsSync).mockReturnValue(false);
    resolveBundledPluginSources.mockReturnValue(new Map());
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-alien-plugin");
  });
