...
const hasUsableCustomProviderApiKey = vi.hoisted(() =>
  vi.fn<(_cfg?: AlienConfig, _provider?: string, _env?: NodeJS.ProcessEnv) => boolean>(
    () => false,
  ),
);
...
describe("promptDefaultModel", () => {
  it("adds auth-route hints for OpenAI API and Codex OAuth models", async () => {
    ...
    await promptDefaultModel({
      config: { agents: { defaults: {} } } as AlienConfig,
      prompter,
      allowKeep: false,
      includeManual: false,
  ...
  it("supports configuring vLLM during setup", async () => {
    ...
    const prompter = makePrompter({ select });
    const config = { agents: { defaults: {} } } as AlienConfig;
