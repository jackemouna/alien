...
function parseTimeoutSeconds(opts: { cfg: AlienConfig; timeout?: string }) {
  const raw =
...
async function agentViaGatewayCommand(opts: AgentCliOpts, runtime: RuntimeEnv) {
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("alien agents list")}" to see configured agents.`,
      );
    }
  }
