...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  it("does not fall back to stale ALIEN_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.ALIEN_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.ALIEN_GATEWAY_TOKEN = "stale-env-token";
    process.env.ALIEN_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
