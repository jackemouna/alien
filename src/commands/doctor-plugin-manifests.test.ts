...
function makeTrustedBundledPluginsDir() {
  ...
  const dir = fs.mkdtempSync(path.join(fixturesRoot, "alien-doctor-plugin-manifests-"));
  tempDirs.push(dir);
  return dir;
}

function configWithPluginLoadPath(pluginRoot: string): AlienConfig {
  return {
    plugins: {
      load: {
        paths: [pluginRoot],
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "alien.plugin.json"), "utf-8")) as {
