...
function createSetupDeps(home: string) {
  ...
  return {
    ...
    ensureAgentWorkspace: vi.fn(async (params?: { dir?: string }) => ({
      dir: params?.dir ?? path.join(home, ".alien", "workspace"),
    })),
    ...
    mkdir: vi.fn(async () => {}),
    resolveSessionTranscriptsDir: vi.fn(() => path.join(home, ".alien", "sessions")),
...
describe("setupCommand", () => {
  it("writes gateway.mode=local on first run", async () => {
    await withTempHome(async (home) => {
      ...
      const workspace = path.join(home, ".alien", "workspace");
  ...
  it("explains that plain setup only initializes local files", async () => {
    await withTempHome(async (home) => {
      ...
      expect(logs).toContain("alien configure");
      expect(logs).toContain("alien setup --wizard");
    });
  });
