...
async function runRepair(cfg: AlienConfig) {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      ALIEN_GATEWAY_TOKEN: "stale-token",
    },
  });
  mocks.auditGatewayServiceConfig.mockResolvedValue({
    ok: false,
    issues: [
      {
        ...
        message: "Gateway service ALIEN_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: AlienConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.ALIEN_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("passes planned managed env keys into service audit for legacy inline secret detection", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...
      workingDirectory: "/tmp",
      environment: {
        ALIEN_SERVICE_MANAGED_ENV_KEYS: "TAVILY_API_KEY",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
  ...
  it("repairs gateway services whose pinned port differs from current config", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: ["/usr/bin/node", "/usr/local/bin/alien", "gateway", "--port", "18888"],
      workingDirectory: "/tmp",
      environment: {},
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
  ...
  it("uses ALIEN_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ ALIEN_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/alien/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/alien@2026.3.12/node_modules/alien/dist/index.js",
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/alien/")) {
          return value.replace(
            "/global/5/node_modules/alien/",
            "/global/5/node_modules/.pnpm/alien@2026.3.12/node_modules/alien/",
          );
        }
        return value;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/alien/../alien/dist/index.js",
      installEntrypoint: "/opt/alien/dist/index.js",
  ...
  it("keeps wrapper-managed gateway services aligned during entrypoint drift checks", async () => {
    const wrapperPath = "/usr/local/bin/alien-doppler";
    mocks.readCommand.mockResolvedValue({
      programArguments: [wrapperPath, "gateway", "--port", "18789"],
      environment: {
        ALIEN_WRAPPER: wrapperPath,
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: true,
    ...
    mocks.buildGatewayInstallPlan.mockImplementation(async ({ env }) => ({
      programArguments: [env.ALIEN_WRAPPER, "gateway", "--port", "18789"],
      environment: {
        ALIEN_WRAPPER: env.ALIEN_WRAPPER,
      },
    }));
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      "Gateway service invokes ALIEN_WRAPPER: /usr/local/bin/alien-doppler",
      "Gateway",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/alien/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/alien/dist/index.js",
    });
  ...
  it("skips entrypoint rewrites for an active systemd unit", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      ...createGatewayCommand("/opt/old-alien/dist/index.js"),
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...createGatewayCommand("/opt/new-alien/dist/index.js"),
      workingDirectory: "/tmp",
    });
  ...
  it("leaves all service metadata unchanged when an active unit has command drift plus other issues", async () => {
    mockProcessPlatform("linux");
    mocks.readCommand.mockResolvedValue({
      programArguments: ["/usr/bin/alien", "run"],
      environment: {},
      sourcePath: "/home/test/.config/systemd/user/alien-gateway.service",
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
  ...
  it("skips entrypoint rewrite in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/alien/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/alien/dist/index.js",
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      expect.stringContaining("alien gateway install --force"),
      "Gateway service config",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            ALIEN_GATEWAY_TOKEN: "env-file-token",
          },
  ...
  it("warns when the gateway service entrypoint resolves to a source checkout", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "alien-doctor-service-layout-"));
      try {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "alien", version: "0.0.0-test" }),
          "utf8",
        );
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Alien gateway next.",
    );
  });
