...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("alien security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("alien status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("alien tasks maintenance --apply")}`,
