...
const mocks = vi.hoisted(() => ({
  ...
  applyPrimaryModel: vi.fn((cfg: AlienConfig, model: string) => ({
    ...cfg,
    agents: {
      ...cfg.agents,
      defaults: {
  ...
  applyModelAllowlist: vi.fn(
    (cfg: AlienConfig, models: string[], opts: { scopeKeys?: string[] } = {}) => {
  ...
  applyModelFallbacksFromSelection: vi.fn(
    (cfg: AlienConfig, selection: string[], opts: { scopeKeys?: string[] } = {}) => {
      const defaults = cfg.agents?.defaults;
