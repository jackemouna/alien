...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.ALIEN_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.ALIEN_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from alien.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<AlienConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Alien update wizard" : "Alien configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.alien.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("alien doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbePromise = (async () => {
      ...
      return probeGatewayReachable({
        url: localUrl,
        token: process.env.ALIEN_GATEWAY_TOKEN ?? baseLocalProbeToken,
        password: process.env.ALIEN_GATEWAY_PASSWORD ?? baseLocalProbePassword,
