...
function createTelegramCfg(botToken: string, enabled?: boolean): AlienConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "external-chat",
    pluginId: "@alien/external-chat-plugin",
    meta: {
      id: "external-chat",
      label: "External Chat",
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: AlienConfig }) =>
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: AlienConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: AlienConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: AlienConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as AlienConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("../commands/channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: AlienConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("shows explicit dmScope config command in channel primer", async () => {
    ...
    await runSetupChannels({} as AlienConfig, prompter);
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@alien/external-chat-plugin": { enabled: true },
          },
        },
      } as AlienConfig,
      prompter,
        ...
        if (channel === "external-chat") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: AlienConfig) =>
                  Object.keys(
                    (
                      cfg.channels?.["external-chat"] as
                        | { accounts?: Record<string, unknown> }
                ...
                resolveAccount: (cfg: AlienConfig, accountId: string) =>
                  (
                    cfg.channels?.["external-chat"] as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: AlienConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: AlienConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
