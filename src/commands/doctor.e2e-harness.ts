...
export const resolveAlienPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadAlienPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("alien/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("alien/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("alien/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveAlienPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.ALIEN_UPDATE_IN_PROGRESS;
  process.env.ALIEN_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-doctor-state-"));
