...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".alien-work");
    const defaultStateDir = path.join(homeDir, ".alien");
    const profileConfigPath = path.join(profileStateDir, "alien.json");
    ...
    vi.stubEnv("ALIEN_HOME", homeDir);
    vi.stubEnv("ALIEN_PROFILE", "work");
    vi.stubEnv("ALIEN_STATE_DIR", profileStateDir);
    vi.stubEnv("ALIEN_CONFIG_PATH", profileConfigPath);
