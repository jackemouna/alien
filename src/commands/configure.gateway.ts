...
export async function promptGatewayConfig(
  ...
  if (authMode === "token") {
    ...
    if (tokenInputMode === "ref") {
      ...
      note(`Validated ${envVarName}. Alien will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Alien trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.alien.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
