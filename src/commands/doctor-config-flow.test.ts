    ...
    findLegacyConfigIssues: (raw: unknown, sourceRaw?: unknown, extraRules: LegacyRule[] = []) => {
      ...
      if ("heartbeat" in root) {
        addIssue(
          issues,
          ["heartbeat"],
          'heartbeat is legacy; use agents.defaults.heartbeat and channels.defaults.heartbeat. Run "alien doctor --fix".',
        );
      }
      if ("memorySearch" in root) {
        addIssue(
          issues,
          ["memorySearch"],
          'memorySearch is legacy; use agents.defaults.memorySearch. Run "alien doctor --fix".',
        );
      }
      ...
      if (gateway && "bind" in gateway) {
        addIssue(
          issues,
          ["gateway", "bind"],
          'gateway.bind host aliases are legacy; use the canonical bind mode. Run "alien doctor --fix".',
        );
      }
      ...
      if (sessionThreadBindings && "ttlHours" in sessionThreadBindings) {
        addIssue(
          ...
          'session.threadBindings.ttlHours is legacy; use session.threadBindings.idleHours. Run "alien doctor --fix".',
        );
      }
      ...
      if (sessionMaintenance && "rotateBytes" in sessionMaintenance) {
        addIssue(
          issues,
          ["session", "maintenance"],
          'session.maintenance.rotateBytes is deprecated and ignored; run "alien doctor --fix" to remove it.',
        );
      }
      ...
      if (xSearch && "apiKey" in xSearch) {
        addIssue(
          ...
          'tools.web.x_search.apiKey is legacy; use plugins.entries.xai.config.webSearch.apiKey. Run "alien doctor --fix".',
        );
      }
      ...
      if (sandbox && "perSession" in sandbox) {
        addIssue(
          ...
          'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope. Run "alien doctor --fix".',
        );
      }
      ...
      for (const [channelId, channelRaw] of Object.entries(channels ?? {})) {
        ...
        if (hasLegacyStreamingAlias(channel)) {
          addIssue(
            ...
            channelId === "googlechat"
              ? `channels.${channelId}.streamMode is legacy and no longer used. Run "alien doctor --fix".`
              : `channels.${channelId}.streamMode, channels.${channelId}.streaming aliases are legacy. Run "alien doctor --fix".`,
          );
        }
        ...
        if (threadBindings && "ttlHours" in threadBindings) {
          addIssue(
            ...
            'channels.<id>.threadBindings.ttlHours is legacy; use channels.<id>.threadBindings.idleHours. Run "alien doctor --fix".',
          );
        }
        if (channelId === "slack") {
          for (const roomRaw of Object.values(asRecord(channel.channels) ?? {})) {
            if ("allow" in (asRecord(roomRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "slack"],
                'channels.slack.channels.<id>.allow is legacy; use enabled. Run "alien doctor --fix".',
              );
            }
          }
        }
          ...
          for (const spaceRaw of Object.values(asRecord(channel.groups) ?? {})) {
            if ("allow" in (asRecord(spaceRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "googlechat"],
                'channels.googlechat.groups.<id>.allow is legacy; use enabled. Run "alien doctor --fix".',
              );
            }
          }
        }
            ...
            for (const roomRaw of Object.values(asRecord(guild?.channels) ?? {})) {
              if ("allow" in (asRecord(roomRaw) ?? {})) {
                addIssue(
                  issues,
                  ["channels", "discord"],
                  'channels.discord.guilds.<id>.channels.<id>.allow is legacy; use enabled. Run "alien doctor --fix".',
                );
              }
            }
          }
    ...
    listPluginDoctorLegacyConfigRules: () => [
      {
        ...
        message:
          'channels.telegram.groupMentionsOnly was removed; use channels.telegram.groups."*".requireMention instead. Run "alien doctor --fix".',
      },
      {
        path: ["talk"],
        message:
          "talk.voiceId/talk.voiceAliases/talk.modelId/talk.outputFormat/talk.apiKey are legacy; use talk.providers.<provider> and run alien doctor --fix.",
...
vi.mock("./doctor-config-preflight.js", async () => {
  ...
  function resolveConfigPath() {
    const stateDir =
      process.env.ALIEN_STATE_DIR ||
      (process.env.HOME ? path.join(process.env.HOME, ".alien") : "");
    return process.env.ALIEN_CONFIG_PATH || path.join(stateDir, "alien.json");
  }
...
describe("doctor config flow", () => {
  ...
  it("warns when hooks transformsDir points outside the hook transforms root", async () => {
    const doctorWarnings = await collectDoctorWarnings({
      hooks: {
        enabled: true,
        token: "hook-secret",
        transformsDir: "/virtual/.alien/workspace/skills/linear-webhook",
        mappings: [
          {
            match: { path: "linear" },
            action: "agent",
            messageTemplate: "Linear event",
            transform: { module: "./alien-linear-transform.js" },
          },
        ],
      },
    });
    ...
    expect(warning).toContain("/virtual/.alien/workspace/skills/linear-webhook");
    expect(warning).toContain("/virtual/.alien/hooks/transforms");
  ...
  it("keeps discord streaming aliases on disk during repair so downgrades stay recoverable", async () => {
    await withTempHome(
      async (home) => {
        const configDir = path.join(home, ".alien");
        const configPath = path.join(configDir, "alien.json");
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            message.includes('Run "alien doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockClear();
