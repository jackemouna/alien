...
import { replaceConfigFile, type AlienConfig } from "../config/config.js";
...
import { ALIEN_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function buildGatewayServiceRepairEnv(
  ...
  const wrapperPath = command?.environment?.[ALIEN_WRAPPER_ENV_KEY]?.trim();
  if (!wrapperPath || Object.hasOwn(process.env, ALIEN_WRAPPER_ENV_KEY)) {
    return process.env;
  }
  return {
    ...process.env,
    [ALIEN_WRAPPER_ENV_KEY]: wrapperPath,
  };
}
...
function resolveGatewayServiceWrapperPath(
  ...
  return normalizeOptionalString(command?.environment?.[ALIEN_WRAPPER_ENV_KEY]) ?? null;
}
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "alien-gateway.service";
}
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${ALIEN_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  if (serviceLayout?.entrypointSourceCheckout) {
    note(
      [
        ...
        "Run `alien doctor --fix` from the intended package install, or reinstall the gateway service with `alien gateway install --force`.",
      ].join("\n"),
      "Gateway service config",
    );
  }
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service ALIEN_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (serviceRewriteBlocked) {
    note(
      "Gateway service is running; leaving supervisor metadata unchanged. Stop the service first or use `alien gateway install --force` when you want to replace the active launcher.",
      "Gateway service config",
    );
    return;
  }
  ...
  if (!repair) {
    note(
      "Run `alien gateway install --force` when you want to replace the gateway service definition.",
      "Gateway service config",
    );
    return;
  }
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Alien gateway next.");
      }
    }
  }
