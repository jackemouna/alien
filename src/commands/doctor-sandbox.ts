...
function resolveSandboxDockerImage(cfg: AlienConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: AlienConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return backend || "docker";
}

function resolveSandboxBrowserImage(cfg: AlienConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: AlienConfig, image: string): AlienConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: AlienConfig, image: string): AlienConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export async function maybeRepairSandboxImages(
  ...
  if (!dockerAvailable) {
    const lines = [
      ...
      "- Disable sandbox mode: alien config set agents.defaults.sandbox.mode off",
    ];
    note(lines.join("\n"), "Sandbox");
    return cfg;
  }
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("alien doctor --fix")} to migrate them to sharded registry files.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
...
export function noteSandboxScopeWarnings(cfg: AlienConfig) {
