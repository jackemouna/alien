...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-alien-backup";
...
describe("backupVerifyCommand", () => {
  ...
  it("verifies a valid backup archive", async () => {
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-backup-verify-out-"));
    try {
      ...
      const payloadArchivePath = `${archiveRoot}/payload/posix/tmp/.alien/state.txt`;
      await fs.writeFile(
        manifestPath,
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-alien-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.alien",
            archivePath: `${rootName}/payload/posix/tmp/.alien`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    ...
    try {
      ...
      const stateArchivePath = `${archiveRoot}/payload/posix/tmp/.alien/state.txt`;
  ...
  it("rejects duplicate manifest and payload entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.alien/payload.txt`;
    for (const options of [
      {
        tempPrefix: "alien-backup-duplicate-manifest-",
        payloads: [{ fileName: "payload.txt", contents: "payload\n" }],
        buildTarEntries: ({
          manifestPath,
          payloadPaths,
      ...
      {
        tempPrefix: "alien-backup-duplicate-payload-",
        payloads: [
