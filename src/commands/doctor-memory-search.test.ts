...
describe("noteMemorySearchHealth", () => {
  ...
  beforeEach(() => {
    ...
    resolveActiveMemoryBackendConfig.mockImplementation(({ cfg }: { cfg: AlienConfig }) =>
  ...
  it("does not warn when QMD backend is active", async () => {
    const qmdCfg = { memory: { backend: "qmd", qmd: { command: "qmd" } } } as AlienConfig;
    resolveMemorySearchConfig.mockReturnValue({
      provider: "auto",
      local: {},
      remote: {},
  ...
  it("does not warn when key-optional provider (lmstudio) probe was skipped (skipped: true)", async () => {
    // When `alien doctor` runs without --deep, the probe is skipped and returns
    ...
    // Regression test for: https://github.com/alien/alien/issues/74608
    resolveMemorySearchConfig.mockReturnValue({
      provider: "lmstudio",
      local: {},
      remote: {},
  ...
  it("warns for key-optional provider (lmstudio) when gateway probe timed out", async () => {
    ...
    // Regression guard: https://github.com/alien/alien/issues/74608
    resolveMemorySearchConfig.mockReturnValue({
      provider: "lmstudio",
      local: {},
      remote: {},
  ...
  it("uses model configure hint when gateway probe is unavailable and API key is missing", async () => {
    ...
    expect(message).toContain("alien configure --section model");
    expect(message).not.toContain("alien auth add --provider");
  });
...
describe("memory recall doctor integration", () => {
  ...
  it("runs dreaming artifact repair during doctor --fix", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValueOnce({
      changed: true,
      archiveDir: "/tmp/agent-default/workspace/.alien-repair/dreaming/2026-04-11T21-35-00-000Z",
      archivedDreamsDiary: false,
...
describe("detectLegacyWorkspaceDirs", () => {
  it("returns active workspace and no legacy dirs", () => {
    const workspaceDir = "/home/user/alien";
