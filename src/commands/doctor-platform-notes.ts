...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".alien", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: AlienConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["ALIEN_GATEWAY_TOKEN", await getenv("ALIEN_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["ALIEN_GATEWAY_PASSWORD", await getenv("ALIEN_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "ALIEN_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.ALIEN_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- ALIEN_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
