...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "alien-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): AlienConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<AlienConfig["acp"]>>,
) {
  const cfg = createAcpEnabledConfig(home, storePath);
  cfg.acp = {
    ...cfg.acp,
...
describe("agentCommand ACP runtime routing", () => {
  ...
  it("blocks ACP turns when disabled by policy", async () => {
    ...
    ] satisfies Array<Partial<NonNullable<AlienConfig["acp"]>>>) {
