...
describe("readServiceStatusSummary", () => {
  it("marks Alien-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["alien", "gateway", "run"] })),
    ...
    expect(summary.managedByAlien).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByAlien).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.ALIEN_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ALIEN_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["alien", "gateway", "run", "--port", "18789"],
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "alien-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "alien-gateway.service");
      ...
      expect(summary.layout).toMatchObject({
        sourcePath: serviceFile,
        sourcePathReal: path.join(realRoot, "alien-gateway.service"),
        entrypoint,
