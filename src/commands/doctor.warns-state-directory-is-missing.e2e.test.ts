...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ALIEN_STATE_DIR = missingDir;
    await doctorCommand(createDoctorRuntime(), {
      nonInteractive: true,
      workspaceSuggestions: false,
    });
  ...
  it("skips gateway auth warning when ALIEN_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.ALIEN_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("alien config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain(
      "alien config set gateway.auth.mode password",
    );
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.ALIEN_GATEWAY_TOKEN;
    delete process.env.ALIEN_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.ALIEN_GATEWAY_TOKEN = previousToken;
      }
    }
