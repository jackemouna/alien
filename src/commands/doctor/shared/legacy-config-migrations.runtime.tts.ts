...
const LEGACY_TTS_PROVIDER_RULES: LegacyConfigRule[] = [
  {
    path: ["messages", "tts"],
    message:
      'messages.tts legacy provider aliases/keys are legacy; use provider: "microsoft" and messages.tts.providers.<provider>. Run "alien doctor --fix".',
    match: (value) => hasLegacyTtsProviderKeys(value),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts legacy provider aliases/keys are legacy; use provider: "microsoft" and plugins.entries.voice-call.config.tts.providers.<provider>. Run "alien doctor --fix".',
...
const LEGACY_TTS_ENABLED_RULES: LegacyConfigRule[] = [
  {
    path: ["messages", "tts"],
    message: 'messages.tts.enabled is legacy; use messages.tts.auto. Run "alien doctor --fix".',
    match: (value) => hasLegacyTtsEnabled(value),
  },
  {
    path: ["agents"],
    message: 'agents.*.tts.enabled is legacy; use agents.*.tts.auto. Run "alien doctor --fix".',
    match: (value) => hasLegacyTtsEnabledInAgentLocations(value),
  },
  {
    path: ["channels"],
    message:
      'channels.*.tts.enabled is legacy; use channels.*.tts.auto. Run "alien doctor --fix".',
    match: (value) => hasLegacyTtsEnabledInChannelLocations(value),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts.enabled is legacy; use plugins.entries.voice-call.config.tts.auto. Run "alien doctor --fix".',
