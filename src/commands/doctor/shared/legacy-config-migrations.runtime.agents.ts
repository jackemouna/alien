...
const MEMORY_SEARCH_RULE: LegacyConfigRule = {
  path: ["memorySearch"],
  message:
    'top-level memorySearch was moved; use agents.defaults.memorySearch instead. Run "alien doctor --fix".',
};
...
const LEGACY_SANDBOX_SCOPE_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope instead. Run "alien doctor --fix".',
    match: (value) => hasLegacySandboxPerSession(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].sandbox.perSession is legacy; use agents.list[].sandbox.scope instead. Run "alien doctor --fix".',
...
const LEGACY_AGENT_RUNTIME_POLICY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.agentRuntime.fallback is no longer supported; explicit runtimes fail closed and auto mode owns PI fallback. Run "alien doctor --fix".',
  },
  {
    path: ["agents", "defaults", "embeddedHarness"],
    message:
      'agents.defaults.embeddedHarness is legacy; use agents.defaults.agentRuntime instead. Run "alien doctor --fix".',
    match: (value) => getRecord(value) !== null,
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].agentRuntime.fallback is no longer supported; explicit runtimes fail closed and auto mode owns PI fallback. Run "alien doctor --fix".',
    match: (value) => hasAgentListRuntimeFallback(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].embeddedHarness is legacy; use agents.list[].agentRuntime instead. Run "alien doctor --fix".',
...
const LEGACY_AGENT_LLM_TIMEOUT_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.llm is legacy; use models.providers.<id>.timeoutSeconds for slow model/provider timeouts. Run "alien doctor --fix".',
    match: (value) => getRecord(value) !== null,
  },
];
