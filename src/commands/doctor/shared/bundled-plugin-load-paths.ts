...
function resolveBundledWorkspaceDir(cfg: AlienConfig): string | undefined {
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Alien loads the packaged bundled plugin from "${hit.toPath}".`,
  );
