...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentDir: (_config: unknown, agentId: string) =>
    `${process.env.ALIEN_STATE_DIR ?? "/tmp/alien-state"}/agents/${agentId}/agent`,
  resolveAgentWorkspaceDir: (_config: unknown, agentId: string) =>
    `/tmp/alien-workspaces/${agentId}`,
}));

vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/alien-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.ALIEN_AGENT_DIR || "__main__";
}
...
async function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<AlienConfig>;
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as AlienConfig } : {}),
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as AlienConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(resolveAgentDir({} as AlienConfig, "main"));
  }
  ...
  beforeAll(async () => {
    authTestRoot = (await setupAuthTestEnv("alien-auth-")).stateDir;
