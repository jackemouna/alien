...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.alien", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.alien", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/alien-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/alien/.alien",
    });

    expect(result).toEqual({
      path: "/mnt/slow/alien/.alien",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".alien"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.alien`;
