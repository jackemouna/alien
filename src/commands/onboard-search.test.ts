...
function getWebSearchConfig(config: AlienConfig | undefined, pluginId: string) {
  return (config as WebSearchConfigRecord | undefined)?.plugins?.entries?.[pluginId]?.config
    ?.webSearch;
}

function ensureWebSearchConfig(config: AlienConfig, pluginId: string) {
...
function createSearchProviderEntry(id: string): PluginWebSearchProviderEntry {
  ...
  const entry: PluginWebSearchProviderEntry = {
    ...
    applySelectionConfig: (config) => {
      const next: AlienConfig = { ...config, plugins: { ...config.plugins } };
      const entries = { ...next.plugins?.entries } as NonNullable<
        NonNullable<AlienConfig["plugins"]>["entries"]
      >;
...
function createPerplexityConfig(apiKey: string, enabled?: boolean): AlienConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: AlienConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): AlienConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: AlienConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: AlienConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
