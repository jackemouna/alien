...
describe("resolveUnusableProfileHint", () => {
  ...
  it("formats permanent OAuth refresh failures as reauth-required", () => {
    expect(
      ...
      "- openai-codex:default: re-auth required [refresh_token_reused] — Run `alien models auth login --provider openai-codex`.",
    );
  });
  ...
  it("formats non-permanent OAuth refresh failures as retry-then-reauth guidance", () => {
    expect(
      ...
      "- openai-codex:default: OAuth refresh failed — Try again; if this persists, run `alien models auth login --provider openai-codex`.",
    );
  });
  ...
  it("drops the provider-specific command when the parsed provider is unsafe", () => {
    expect(
      ...
      "- openai-codex:default: re-auth required [invalid_grant] — Run `alien models auth login --provider openai-codex`.",
    );
  });
});
