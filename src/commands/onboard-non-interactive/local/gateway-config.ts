...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.ALIEN_GATEWAY_TOKEN);
  ...
  // plaintext > ambient ALIEN_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like ALIEN_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }
      ...
      // branch, an ambient ALIEN_GATEWAY_TOKEN (or randomToken() fallback)
