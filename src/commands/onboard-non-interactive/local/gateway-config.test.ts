...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "ALIEN_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): AlienConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  const originalEnvToken = process.env.ALIEN_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken === undefined) {
      ...
      process.env.ALIEN_GATEWAY_TOKEN = originalEnvToken;
    }
  ...
  it("prefers existing plaintext token over ambient ALIEN_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd ALIEN_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    process.env.ALIEN_GATEWAY_TOKEN = "stale-env-token";
    const nextConfig = createTokenConfig("existing-user-token");
  ...
  it("uses ALIEN_GATEWAY_TOKEN to fill an empty config on first-run", () => {
  ...
  it("preserves an existing SecretRef even when ambient ALIEN_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "ALIEN_GATEWAY_TOKEN_NEW_REF";
