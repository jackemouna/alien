...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `alien gateway run` or `alien agent --local`.",
              ]
            : [`Run \`${formatCliCommand("alien gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("alien gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("alien configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.alien.ai/tools/web`,
    );
  }
}
