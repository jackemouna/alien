...
vi.mock("../daemon/program-args.js", () => ({
  ALIEN_WRAPPER_ENV_KEY: "ALIEN_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveAlienWrapperPath: mocks.resolveAlienWrapperPath,
}));
...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/alien/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\alien\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/alien/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { ALIEN_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveAlienWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.alien/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ ALIEN_PORT: "3000" });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".alien"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledWith(
      expect.objectContaining({
        platform: "darwin",
      }),
  ...
  it("passes ALIEN_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/alien-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ALIEN_PORT: "3000",
    ...
    expect(plan.environment.ALIEN_WRAPPER).toBe(wrapperPath);
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.ALIEN_PORT).toBe("3000");
    expect(plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        ALIEN_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "ALIEN_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.ALIEN_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".alien"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ALIEN_PORT: "3000",
    ...
    expect(plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".alien"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains managed .env values for macOS LaunchAgent env files", async () => {
    ...
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        HOME: "/from-service",
        ALIEN_LAUNCHD_LABEL: "ai.alien.gateway",
        ALIEN_PORT: "3000",
      },
    });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { ALIEN_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.ALIEN_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        ALIEN_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.ALIEN_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.ALIEN_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:alien|alien)( --profile isolated)? gateway install/,
    );
  });
});
