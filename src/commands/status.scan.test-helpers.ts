...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/alien-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = AlienConfig>(
  overrides: T = {} as T,
): AlienConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): AlienConfig {
