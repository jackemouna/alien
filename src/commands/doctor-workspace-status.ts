...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("alien tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("alien tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: AlienConfig) {
