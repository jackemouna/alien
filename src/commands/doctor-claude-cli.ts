...
function usesClaudeCliModelSelection(cfg: AlienConfig): boolean {
...
function resolveClaudeCliCommand(cfg: AlienConfig): string {
...
function formatWorkspaceHealthLine(
  ...
  if (health === "missing") {
    return `- ${label}: ${display} (missing; Alien will create it on first run).`;
  }
...
function resolveClaudeCliAgentIds(cfg: AlienConfig, env: NodeJS.ProcessEnv): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- Alien auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Alien auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- Alien auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
