...
const SEARCH_TOOL = "https://docs.alien.ai/mcp.SearchAlien";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.alien.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('alien docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.alien.ai/");
      runtime.log(`Search: ${formatCliCommand('alien docs "your query"')}`);
    }
    return;
  }
