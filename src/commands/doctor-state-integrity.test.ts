...
type EnvSnapshot = {
  HOME?: string;
  ALIEN_HOME?: string;
  ALIEN_STATE_DIR?: string;
  ALIEN_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ALIEN_HOME: process.env.ALIEN_HOME,
    ALIEN_STATE_DIR: process.env.ALIEN_STATE_DIR,
    ALIEN_OAUTH_DIR: process.env.ALIEN_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: AlienConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("ALIEN_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: AlienConfig) {
...
async function runStateIntegrityText(cfg: AlienConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "alien-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ALIEN_HOME = tempHome;
    process.env.ALIEN_STATE_DIR = path.join(tempHome, ".alien");
    delete process.env.ALIEN_OAUTH_DIR;
    fs.mkdirSync(process.env.ALIEN_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when ALIEN_OAUTH_DIR is explicitly configured", async () => {
    process.env.ALIEN_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: AlienConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("alien tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.ALIEN_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.ALIEN_HOME = symlinkHome;
        process.env.ALIEN_STATE_DIR = path.join(symlinkHome, ".alien");
  ...
  it("prints alien-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: AlienConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/alien sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/alien sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /alien sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-tui-pointer-clear-"));
    try {
