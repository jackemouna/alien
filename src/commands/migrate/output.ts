...
export function assertConflictFreePlan(plan: MigrationPlan, providerId: string): void {
  if (plan.summary.conflicts > 0) {
    throw new Error(
      `Migration has ${formatCount(plan.summary.conflicts, "conflict")}. Re-run with --overwrite after reviewing alien migrate plan ${providerId}.`,
    );
  }
}
...
export function writeApplyResult(
  ...
  if (result.backupPath) {
    ...
    runtime.log("Backup: skipped (no existing Alien state found)");
  }
  if (result.reportDir) {
