...
const enablePluginInConfig = vi.hoisted(() =>
  vi.fn<(cfg: AlienConfig, pluginId: string) => PluginEnableResult>((cfg, pluginId) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })),
...
const recordPluginInstall = vi.hoisted(() =>
  vi.fn((cfg: AlienConfig, update: { pluginId: string }) => ({
    ...cfg,
    plugins: {
      ...cfg.plugins,
      installs: {
...
describe("ensureOnboardingPluginInstalled", () => {
  ...
  it("refuses non-skipped installs in Nix mode before package work", async () => {
    ...
    try {
      await expect(
        ensureOnboardingPluginInstalled({
          ...
          entry: {
            ...
            label: "Demo Provider",
            install: {
              npmSpec: "@alien/demo-plugin@1.2.3",
            },
          },
          promptInstall: false,
          prompter: {
      ...
      ).rejects.toThrow("ALIEN_NIX_MODE=1");
    } finally {
    ...
    const result = await ensureOnboardingPluginInstalled({
      ...
      entry: {
        ...
        install: {
          ...
          npmSpec: "@alien/demo-plugin@2026.5.2",
  ...
  it("passes npm specs and optional expected integrity to npm installs with progress", async () => {
    const npmResolution = {
      ...
      version: "1.2.3",
      resolvedSpec: "@wecom/wecom-alien-plugin@1.2.3",
    ...
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "@wecom/wecom-alien-plugin@1.2.3",
        expectedPluginId: "demo-plugin",
        expectedIntegrity: "sha512-wecom",
        trustedSourceLinkedOfficialInstall: true,
        timeoutMs: 300_000,
  ...
  it("defaults dual-source remote installs to npm unless ClawHub is explicit", async () => {
    ...
    expect(captured?.options).toEqual([
      ...
      { value: "npm", label: "Download from npm (@alien/demo-plugin@2026.5.2)" },
      { value: "skip", label: "Skip for now" },
    ]);
    expect(captured?.initialValue).toBe("npm");
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("falls back from ClawHub to npm when the ClawHub package is unavailable", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: "2026.5.2",
        resolvedSpec: "@alien/demo-plugin@2026.5.2",
    ...
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "@alien/demo-plugin@2026.5.2",
        expectedPluginId: "demo-plugin",
      }),
    );
    expect(result.installed).toBe(true);
  ...
  it("does not offer local installs when the workspace only has a spoofed .git marker", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-spoofed-git-" }, async (temp) => {
  ...
  it("allows local installs for real gitdir checkouts and sanitizes prompt text", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-gitdir-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("does not add local plugin paths when enablement is blocked by policy", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-blocked-enable-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("allows local installs for linked git worktrees", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-worktree-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
  ...
  it("records local install source metadata when a local path is selected", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-local-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("hides the npm download option for bundled plugins so the menu matches non-npm channels", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-bundled-prompt-" }, async (temp) => {
      ...
      expect(captured).toBeDefined();
      // "Download from npm (@alien/tlon)" must NOT appear: the bundled
  ...
  it("enables bundled plugins without adding their bundled directory as a local install", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-bundled-record-" }, async (temp) => {
  ...
  it("records local install source metadata when npm install falls back to local", async () => {
    await withTempDir(
      { prefix: "alien-onboarding-install-npm-fallback-record-" },
      async (temp) => {
        const workspaceDir = path.join(temp, "workspace");
        const pluginDir = path.join(workspaceDir, "plugins", "demo");
        await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("records absolute local catalog paths as workspace-relative source metadata", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-portable-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("keeps local installs available when cwd is a git repo but workspaceDir is not", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-cwd-git-" }, async (temp) => {
  ...
  it("rejects local install paths outside the trusted workspace roots", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-outside-root-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
  ...
  it("rejects local install paths when relative resolution looks cross-drive", async () => {
    await withTempDir({ prefix: "alien-onboarding-install-cross-drive-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
