...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("alien plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("alien plugins install")}\`.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in Alien.");
    runtime.log("Alien prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this Alien path is allowed again.");
  }
}
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("alien plugins install")}\`.`,
    );
  }
