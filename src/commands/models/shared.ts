...
export async function loadValidConfigOrThrow(): Promise<AlienConfig> {
...
export async function updateConfig(
  mutator: (cfg: AlienConfig) => AlienConfig,
): Promise<AlienConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: AlienConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AlienConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("alien agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
