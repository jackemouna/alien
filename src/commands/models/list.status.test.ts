...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/alien-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/alien-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/alien-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/alien-dev/alien.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/alien-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/alien-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/alien-dev/alien.json");
    expect(payload.auth.storePath).toBe("/tmp/alien-agent/auth-profiles.json");
  ...
  it("honors ALIEN_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    const previous = process.env.ALIEN_AGENT_DIR;
    process.env.ALIEN_AGENT_DIR = "/tmp/alien-isolated-agent";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ALIEN_AGENT_DIR = previous;
      }
    }

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/alien-isolated-agent");
    const payload = JSON.parse(String((localRuntime.log as Mock).mock.calls[0]?.[0]));
    expect(payload.agentDir).toBe("/tmp/alien-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/alien-isolated-agent/auth-profiles.json");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/alien-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/alien-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/alien-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/alien-agent/workspace"
          ? {
