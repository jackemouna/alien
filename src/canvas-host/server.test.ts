...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("alienCanvasA2UIAction");
    expect(out).toContain("alienSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(response.body).toContain("alienSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.alienA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      ...
      expect(html).toContain("alien-a2ui-host");
      expect(html).toContain("alienCanvasA2UIAction");

      const bundleRes = await captureA2uiResponse(`${A2UI_PATH}/a2ui.bundle.js`);
      const js = bundleRes.body;
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("alienA2UI");
