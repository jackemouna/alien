...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.alienCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.alienCanvasA2UIAction &&
        typeof window.alienCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.alienSendUserAction === "function";
  ...
  window.addEventListener("alien:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Alien node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.alienSendUserAction === "function"
        ? window.alienSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ALIEN_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ALIEN_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Alien Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
