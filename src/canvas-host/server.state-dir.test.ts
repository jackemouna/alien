...
describe("canvas host state dir defaults", () => {
  ...
  it("uses ALIEN_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("alien-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Alien Canvas");
      } finally {
        await handler.close();
      }
    });
