...
var AlienA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--alien-a2ui-inset-top, 0px)
        var(--alien-a2ui-inset-right, 0px)
        var(--alien-a2ui-inset-bottom, 0px)
        var(--alien-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--alien-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--alien-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--alien-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--alien-a2ui-empty-top, var(--alien-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["alien:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["alien:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__alienLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.alienCanvasA2UIAction ?? globalThis.alienCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.alienCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("alien-a2ui-host")) customElements.define("alien-a2ui-host", AlienA2UIHost);
