...
export function buildConfigureCandidates(config: AlienConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: AlienConfig;
  authoredAlienConfig?: AlienConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredAlienConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const alienCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `alien.json` as const,
  ...
  return [...alienCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "alien secrets configure",
