...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-secrets-audit-"));
  const stateDir = path.join(rootDir, ".alien");
...
describe("secrets audit", () => {
  ...
  it("scans active agent-dir override models.json even when outside state dir", async () => {
    ...
    const report = await runSecretsAudit({
      env: {
        ...fixture.env,
        ALIEN_AGENT_DIR: externalAgentDir,
      },
    });
    expect(
      hasFinding(
  ...
  it("does not flag non-sensitive routing headers in alien config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
