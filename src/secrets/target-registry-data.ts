...
function createPluginAlienConfigSecretTargetEntry(
  pluginId: string,
  configPath: string,
  ...
  for (const record of bundledPlugins) {
    for (const config of WEB_PROVIDER_SECRET_CONFIGS) {
      if (
        ...
        entries.push(createPluginAlienConfigSecretTargetEntry(record.id, config.configPath));
      }
    }
  }
...
function listBundledPluginConfigSecretTargetRegistryEntries(
  ...
  for (const record of bundledPlugins) {
    ...
    for (const secretInput of secretInputs) {
      const entry = createPluginAlienConfigSecretTargetEntry(record.id, secretInput.path);
...
export function getSourceSecretTargetRegistry(): SecretTargetRegistryEntry[] {
  return loadSecretTargetRegistryFromPluginMetadata({
    env: {
      ...process.env,
      ALIEN_BUNDLED_PLUGINS_DIR: process.env.ALIEN_BUNDLED_PLUGINS_DIR ?? "extensions",
    },
