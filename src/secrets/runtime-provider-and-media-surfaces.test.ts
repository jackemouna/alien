...
function createOpenAiFileModelsConfig(): NonNullable<AlienConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime provider and media surfaces", () => {
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "alien-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "alien-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
