...
function toCandidate(
  ...
  defaults: NonNullable<AlienConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: AlienConfig): {
  config: AlienConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as AlienConfig & Record<string, unknown>;
