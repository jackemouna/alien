...
function getSecretProviders(config: AlienConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: AlienConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: AlienConfig): Array<{ value: SecretRefSource; label: string }> {
...
function resolveConfigureAgentId(config: AlienConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: AlienConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredAlienConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "alien.json",
