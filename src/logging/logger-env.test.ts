...
const logPathTracker = createSuiteLogPathTracker("alien-test-env-log-level-");

describe("ALIEN_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.ALIEN_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.ALIEN_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.ALIEN_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.ALIEN_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("ALIEN_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid ALIEN_LOG_LEVEL="nope"');
  });
});
