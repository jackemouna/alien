...
type LoggerConfigLoader = () => AlienConfig["logging"] | undefined;
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: AlienConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ALIEN_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[alien] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
