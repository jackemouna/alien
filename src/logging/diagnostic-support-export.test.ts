...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "alien.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["alien", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              ALIEN_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$ALIEN_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/alien-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.ALIEN_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\alien`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$ALIEN_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\alien\\dist\\index.js`,
              "--config",
              `${stateDir}\\alien.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\alien\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\alien\\\\dist\\\\index.js");
    expect(serialized).toContain("$ALIEN_STATE_DIR\\\\alien.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\alien\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/alien.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/alien-sanitized.jsonl");
