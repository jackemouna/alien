...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("skips mutating config loads for config schema", async () => {
    process.argv = ["node", "alien", "config", "schema"];
  ...
  it("supports JSON5 comments and trailing commas", () => {
    writeConfig(`{
      // users commonly keep comments in alien.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("returns undefined for missing or malformed config files", () => {
    process.env.ALIEN_CONFIG_PATH = path.join(os.tmpdir(), "alien-missing-config.json");
    expect(readLoggingConfig()).toBeUndefined();
