...
function resetGatewayCallMocks() {
  ...
  const loadConfigForTests = getRuntimeConfig as unknown as () => AlienConfig;
...
describe("callGateway url resolution", () => {
  ...
  it("uses ALIEN_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.ALIEN_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.ALIEN_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses env ALIEN_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.ALIEN_GATEWAY_URL;
    try {
      process.env.ALIEN_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env ALIEN_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.ALIEN_GATEWAY_URL;
      } else {
        process.env.ALIEN_GATEWAY_URL = prevUrl;
      }
    }
  });

  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-gateway-call-"));
    process.env.ALIEN_STATE_DIR = tempStateDir;
    process.env.ALIEN_CONFIG_PATH = path.join(tempStateDir, "missing-config.json");
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      __testing.setDepsForTests({
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("alien doctor --fix");
  });

  it("allows ws:// private remote URLs only when ALIEN_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostname remote URLs when ALIEN_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ALIEN_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://alien-gateway.ai:18789" },
      },
    });
    resolveGatewayPort.mockReturnValue(18789);
    ...
    expect(details.url).toBe("ws://alien-gateway.ai:18789");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
...
describe("callGateway error details", () => {
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["ALIEN_HANDSHAKE_TIMEOUT_MS"]);
    try {
      process.env.ALIEN_HANDSHAKE_TIMEOUT_MS = "30000";
      startMode = "silent";
      setLocalLoopbackGatewayConfig();
  ...
  it("waits for gateway client teardown before resolving", async () => {
    ...
    __testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => AlienConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.ALIEN_GATEWAY_TOKEN = "env-token";
    process.env.ALIEN_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.ALIEN_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ALIEN_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.ALIEN_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
