...
const saveMediaBufferMock = vi.hoisted(() =>
  vi.fn(async (_buffer: Buffer, mime?: string, _subdir?: string) => ({
    ...
    path: `/tmp/alien-test-media/inbound/fake.${mime?.split("/")[1] ?? "bin"}`,
    size: 0,
    contentType: mime,
  })),
);
  ...
  it("rejects oversized images before offload", async () => {
    ...
    await expect(
      parseMessageWithAttachments(
        ...
        { maxBytes: resolveChatAttachmentMaxBytes({} as AlienConfig), log: { warn: () => {} } },
      ),
...
describe("resolveChatAttachmentMaxBytes", () => {
  ...
  const cfgWithMediaMaxMb = (value: unknown): AlienConfig =>
    ({ agents: { defaults: { mediaMaxMb: value } } }) as unknown as AlienConfig;
  ...
  it("falls back to DEFAULT_CHAT_ATTACHMENT_MAX_MB when unset", () => {
    expect(resolveChatAttachmentMaxBytes({} as AlienConfig)).toBe(DEFAULT_BYTES);
    expect(resolveChatAttachmentMaxBytes({ agents: {} } as unknown as AlienConfig)).toBe(
      DEFAULT_BYTES,
    );
  });
