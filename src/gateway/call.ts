...
function loadGatewayConfig(): AlienConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: AlienConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.ALIEN_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.ALIEN_TEST_HANDSHAKE_TIMEOUT_MS);
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ALIEN_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as AlienConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `alien doctor` for diagnostics.";
  }
  return message;
}
