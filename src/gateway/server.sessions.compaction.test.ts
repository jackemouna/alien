...
test("sessions.patch preserves nested model ids under provider overrides", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-gw-sessions-nested-"));
  ...
  await withEnvAsync({ ALIEN_CONFIG_PATH: undefined }, async () => {
    ...
    await withEnvAsync({ ALIEN_CONFIG_PATH: configPath }, async () => {
