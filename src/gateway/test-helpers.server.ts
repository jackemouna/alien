...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ALIEN_CONFIG_PATH) {
    configPaths.add(process.env.ALIEN_CONFIG_PATH);
  }
  if (process.env.ALIEN_STATE_DIR) {
    configPaths.add(path.join(process.env.ALIEN_STATE_DIR, "alien.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alien-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ALIEN_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "alien-test-no-bundled-extensions")
    : "alien-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".alien-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".alien-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".alien-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>alien-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__alienTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__alienTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["ALIEN_GATEWAY_TOKEN"]);
  const prev = process.env.ALIEN_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ALIEN_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ALIEN_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
