...
describe("OpenAI-compatible models HTTP API (e2e)", () => {
  it("serves /v1/models when compatibility endpoints are enabled", async () => {
    ...
    expect(json.data?.map((entry) => entry.id)).toContain("alien");
    expect(json.data?.map((entry) => entry.id)).toContain("alien/default");
    expect(
      json.data?.every((entry) => typeof entry.id === "string" && entry.id?.startsWith("alien")),
    ).toBe(true);
  });
  ...
  it("rejects operator scopes that lack read access", async () => {
    const res = await getModels("/v1/models", { "x-alien-scopes": "operator.approvals" });
  ...
  it("rejects requests with no declared operator scopes", async () => {
    const res = await getModels("/v1/models", { "x-alien-scopes": "" });
    expect(res.status).toBe(403);
    await expect(res.json()).resolves.toMatchObject({
      ok: false,
      error: {
