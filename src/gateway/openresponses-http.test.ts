...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("ALIEN_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "alien", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "alien", input: "hi" },
        { "x-alien-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "alien/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "alien/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "alien", input: "hi" },
        { "x-alien-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-alien-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats write-scoped HTTP callers as non-owner and admin-scoped callers as owner", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "alien", input: "hi" },
      { "x-alien-scopes": "operator.admin, operator.write" },
    );
