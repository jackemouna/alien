...
describe("gateway server models + voicewake", () => {
  const listModels = async (params?: { view?: "default" | "configured" | "all" }) =>
    withEnvAsync({ ALIEN_DISABLE_BUNDLED_PLUGINS: "1" }, async () =>
      params
  ...
  const withModelsConfig = async <T>(config: unknown, run: () => Promise<T>): Promise<T> => {
    ...
    if (!configPath) {
      throw new Error("Missing ALIEN_CONFIG_PATH");
    }
  ...
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("alien-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["alien", "claude", "computer"]);
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".alien", "settings", "voicewake.json"), "utf8"),
  ...
  test("voicewake.routing.get/set persists and broadcasts", { timeout: 60_000 }, async () => {
    await withTempHome(async (homeDir) => {
      ...
      const onDisk = JSON.parse(
        await fs.readFile(
          path.join(homeDir, ".alien", "settings", "voicewake-routing.json"),
          "utf8",
        ),
      ) as { routes?: unknown };
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    await withEnvAsync({ ALIEN_GATEWAY_TOKEN: "secret" }, async () => {
      testTailnetIPv4.value = "100.64.0.1";
      testState.gatewayBind = "lan";
      const canvasPort = await getFreePort();
      testState.canvasHostPort = canvasPort;
      await withEnvAsync({ ALIEN_CANVAS_HOST_PORT: String(canvasPort) }, async () => {
