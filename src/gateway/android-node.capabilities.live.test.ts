...
const LIVE_ANDROID_NODE = isTruthyEnvValue(process.env.ALIEN_LIVE_ANDROID_NODE);
...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(process.env.ALIEN_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(process.env.ALIEN_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(process.env.ALIEN_ANDROID_GATEWAY_PASSWORD);
...
describe("resolvePolicyConfigForRun", () => {
  ...
  it("still uses local config loading for local loopback runs", async () => {
    const localConfig = { gateway: { bind: "127.0.0.1" } } as unknown as AlienConfig;
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(process.env.ALIEN_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((node) => node.nodeId === nodeIdOverride);
    if (!match) {
      throw new Error(`ALIEN_ANDROID_NODE_ID not found in node.list: ${nodeIdOverride}`);
    }
    return match;
  }

  const nodeNameOverride = readString(process.env.ALIEN_ANDROID_NODE_NAME)?.toLowerCase();
  if (nodeNameOverride) {
    const match = nodes.find(
      (node) => readString(node.displayName)?.toLowerCase() === nodeNameOverride,
    );
    if (!match) {
      throw new Error(`ALIEN_ANDROID_NODE_NAME not found in node.list: ${nodeNameOverride}`);
    }
    return match;
  }
    ...
    if (!target.connected || !target.paired) {
      ...
      throw new Error(
        [
          ...
          pendingHint,
          "precondition: open app, keep foreground, ensure pairing approved (`alien nodes pending` / `alien nodes approve <requestId>`)",
        ].join("\n"),
      );
    }
