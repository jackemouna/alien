...
export const ALIEN_MODEL_ID = "alien";
export const ALIEN_DEFAULT_MODEL_ID = "alien/default";

function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-alien-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-alien-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === ALIEN_MODEL_ID || lowered === ALIEN_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^alien[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `alien` or `alien/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-alien-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-alien-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-alien-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-alien-message-channel")) ??
