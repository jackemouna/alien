...
import type { ConfigFileSnapshot, ModelDefinitionConfig, AlienConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: AlienConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: AlienConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: AlienConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/alien-startup-recovery.json";
const validConfig = {
  gateway: {
    mode: "local",
  },
  ...
  it("logs config warnings without failing startup", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        'tools.web.search.provider: web_search provider is not available: brave (install or enable plugin "brave", then run alien doctor --fix)',
      ),
    );
  });
