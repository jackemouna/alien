...
export function attachAlienTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__alien && typeof record.__alien === "object" && !Array.isArray(record.__alien)
      ? (record.__alien as Record<string, unknown>)
      : {};
  return {
    ...record,
    __alien: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __alien: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachAlienTranscriptMeta(message, { seq: firstSeq + index }),
  );
...
function parsedSessionEntryToMessage(parsed: unknown, seq: number): unknown {
  ...
  if (entry.message) {
    return attachAlienTranscriptMeta(entry.message, {
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "alien" && model === "delivery-mirror";
