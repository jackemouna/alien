...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "alien-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.ALIEN_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as AlienConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "alien-test-config-"));
  const configPath = path.join(dir, "alien.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ALIEN_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
