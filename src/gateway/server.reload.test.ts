...
const hoisted = vi.hoisted(() => {
  ...
  const resolveAlienPackageRootSync = vi.fn((_params: unknown) => "/package");
...
vi.mock("../infra/alien-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/alien-root.js")>();
  return {
    ...actual,
    resolveAlienPackageRootSync: hoisted.resolveAlienPackageRootSync,
  };
});
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    prevSkipChannels = process.env.ALIEN_SKIP_CHANNELS;
    prevSkipGmail = process.env.ALIEN_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.ALIEN_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.ALIEN_SKIP_CHANNELS = "0";
    ...
    hoisted.resolveAlienPackageRootSync.mockClear();
    hoisted.resolveAlienPackageRootSync.mockReturnValue("/package");
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ALIEN_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ALIEN_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ALIEN_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.ALIEN_SKIP_PROVIDERS;
    } else {
      process.env.ALIEN_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("ALIEN_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ ALIEN_TEST_MINIMAL_GATEWAY: undefined }, async () =>
...
`,
    ...
    const previousGatewayTokenEnv = process.env.ALIEN_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.ALIEN_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
