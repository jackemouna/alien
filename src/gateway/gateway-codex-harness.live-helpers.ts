export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Alien session status reports the active model as:",
] as const;
...
export const EXPECTED_CODEX_STATUS_COMMAND_TEXT = [
  ...
  "Alien status shown above.",
  "Status shown above.",
] as const;

export function isExpectedCodexStatusCommandText(text: string): boolean {
  const normalized = text.toLowerCase();
  const mentionsAlienStatus =
    normalized.includes("alien is running on") ||
    /alien\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("alien status:") ||
  ...
  return (
    ...
    (mentionsAlienStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from alien status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live alien config shows") ||
