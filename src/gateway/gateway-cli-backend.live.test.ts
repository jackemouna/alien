...
import { clearRuntimeConfigSnapshot, type AlienConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.ALIEN_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.ALIEN_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.ALIEN_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.ALIEN_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.ALIEN_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "ALIEN_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
...
function openAiProviderConfigForCodexCli(
  modelKey: string,
): NonNullable<NonNullable<AlienConfig["models"]>["providers"]>["openai"] {
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.ALIEN_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      ...
      const cliCommand = process.env.ALIEN_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `ALIEN_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "ALIEN_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.ALIEN_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.ALIEN_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.ALIEN_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "ALIEN_LIVE_CLI_BACKEND_IMAGE_MODE requires ALIEN_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-live-cli-"));
      ...
      const disableMcpConfig = process.env.ALIEN_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      const cfgWithCliBackends = cfg as AlienConfig & {
        agents?: {
          defaults?: {
