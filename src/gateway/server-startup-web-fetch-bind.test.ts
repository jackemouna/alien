...
async function writeConfig(config: AlienConfig): Promise<void> {
...
describe("gateway startup web fetch config", () => {
  it("binds HTTP with credential-free tools.web.fetch config without fetch provider discovery", async () => {
    const previousMinimal = process.env.ALIEN_TEST_MINIMAL_GATEWAY;
    process.env.ALIEN_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.ALIEN_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
});
