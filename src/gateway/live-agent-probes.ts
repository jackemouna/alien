...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Alien MCP tool `alien-tools/cron` (server `alien-tools`, tool `cron`). " +
      "If the harness shows Claude-style MCP names, use `mcp__alien-tools__cron` or `mcp__alien_tools__cron`. " +
      ...
      "Do the actual tool call; I will verify externally with the Alien cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Alien MCP tool `alien-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__alien-tools__cron` or `mcp__alien_tools__cron`. " +
      ...
      "I will verify externally with the Alien cron CLI."
    );
  }
  return (
    "Your previous Alien cron MCP tool call was cancelled before the job was created. " +
    "Retry the Alien MCP tool `alien-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__alien-tools__cron` or `mcp__alien_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runAlienCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  delete childEnv.VITEST;
  delete childEnv.VITEST_MODE;
  delete childEnv.VITEST_POOL_ID;
  delete childEnv.VITEST_WORKER_ID;
  const { stdout, stderr } = await execFileAsync(process.execPath, ["alien.mjs", ...args], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: 30_000,
    maxBuffer: 1024 * 1024,
  ...
  if (!trimmed) {
    throw new Error(
      [
        `alien ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `alien ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runAlienCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
