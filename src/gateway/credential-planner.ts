...
type GatewaySecretDefaults = NonNullable<AlienConfig["secrets"]>["defaults"];

export const trimToUndefined = normalizeOptionalString;

/**
 * Like trimToUndefined but also rejects unresolved env var placeholders (e.g. `${VAR}`).
 * This prevents literal placeholder strings like `${ALIEN_GATEWAY_TOKEN}` from being
...
export function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.ALIEN_GATEWAY_TOKEN));
}

export function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.ALIEN_GATEWAY_PASSWORD));
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const envPassword = trimToUndefined(env.ALIEN_GATEWAY_PASSWORD);
