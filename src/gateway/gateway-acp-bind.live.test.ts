...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.ALIEN_LIVE_ACP_BIND);
...
function shouldRequireBoundAssistantTranscript(liveAgent: LiveAcpAgent): boolean {
  return (
    liveAgent === "droid" ||
    liveAgent === "opencode" ||
    isTruthyEnvValue(process.env.ALIEN_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT)
  );
}

function shouldRequireCronMcpProbe(): boolean {
  return isTruthyEnvValue(process.env.ALIEN_LIVE_ACP_BIND_REQUIRE_CRON);
}
...
function resolveLiveParentModel(): string {
  return normalizeOpenAiModelRef(
    process.env.ALIEN_LIVE_ACP_BIND_PARENT_MODEL?.trim() ||
      process.env.ALIEN_LIVE_ACP_BIND_CODEX_MODEL?.trim() ||
...
async function prepareCodexHomeForLiveBindTest(): Promise<void> {
  ...
  const model = process.env.ALIEN_LIVE_ACP_BIND_CODEX_MODEL?.trim() || DEFAULT_LIVE_CODEX_MODEL;
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.ALIEN_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.ALIEN_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.ALIEN_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "alien.json");
      ...
      try {
        ...
        if (
          shouldRunLiveImageProbe({
            agent: liveAgent,
            override: process.env.ALIEN_LIVE_ACP_BIND_IMAGE_PROBE,
          })
        ) {
