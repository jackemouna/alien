...
const WRITE_SCOPE_HEADER = { "x-alien-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-alien-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("alien/default");
  ...
  it("rejects invalid agent targets", async () => {
    ...
    expect(json.error).toEqual({
      ...
      message: "Invalid `model`. Use `alien` or `alien/<agentId>`.",
    });
  });

  it("rejects disallowed x-alien-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "alien/default",
        input: "hello",
      },
      { "x-alien-model": "ollama/nomic-embed-text" },
    );
    expect(res.status).toBe(400);
    const json = (await res.json()) as { error?: { type?: string; message?: string } };
    expect(json.error).toEqual({
