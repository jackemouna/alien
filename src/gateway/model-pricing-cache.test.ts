...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listAlienPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listAlienPluginManifestMetadata: (
      params?: Parameters<typeof actual.listAlienPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listAlienPluginManifestMetadata(params);
      return actual.listAlienPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listAlienPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as AlienConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listAlienPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/alien.plugin.json",
    ...overrides,
  };
}
