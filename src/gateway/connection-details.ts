...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: AlienConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.ALIEN_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.ALIEN_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set ALIEN_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: alien doctor --fix",
