...
describe("gateway auth", () => {
  ...
  it("resolves token/password from ALIEN gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          ALIEN_GATEWAY_TOKEN: "env-token",
          ALIEN_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides ALIEN_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        ALIEN_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
