...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("alien.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("alien.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `alien-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.ALIEN_CONFIG_PATH = path.join(root, "alien.json");
}
