...
const loadAlienPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadLogger(): {
  ...
  const call = loadAlienPlugins.mock.calls.at(-1)?.[0] as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadAlienPlugins.mockReturnValue(createRegistry([]));
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadAlienPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", async () => {
    ...
    loadAlienPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", async () => {
    ...
    expect(loadAlienPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.alien.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadAlienPlugins.mockReturnValue(pluginRegistry);
