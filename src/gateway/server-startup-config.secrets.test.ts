...
function gatewayTokenConfig(config: AlienConfig): AlienConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: AlienConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/alien-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: AlienConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.ALIEN_SKIP_CHANNELS;
  const previousSkipProviders = process.env.ALIEN_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.ALIEN_SKIP_CHANNELS;
    } else {
      process.env.ALIEN_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.ALIEN_SKIP_PROVIDERS;
    } else {
      process.env.ALIEN_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
