...
const connectNodeClient = async (params: {
  ...
  const token = process.env.ALIEN_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("ALIEN_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("ALIEN_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
...
describe("gateway node command allowlist", () => {
  test("enforces command allowlists across node clients", async () => {
    ...
    try {
      const systemDeviceIdentity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `alien-node-system-run-${Date.now()}-${Math.random()}.json`),
      );
      const emptyDeviceIdentity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `alien-node-empty-${Date.now()}-${Math.random()}.json`),
      );
      const allowedDeviceIdentity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `alien-node-allowed-${Date.now()}-${Math.random()}.json`),
      );
  ...
  test("records only allowlisted commands in pending node pairing requests", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `alien-allowlisted-pending-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `alien-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
    const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
  ...
  test("filters system.run for confusable iOS metadata at connect time", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `alien-confusable-node-greek-omicron-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
    const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
