...
const READ_SCOPE_HEADER = { "x-alien-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-session-history-"));
...
async function expectMessageEventMatch(
  ...
  if (params.id !== undefined) {
    expect(
      (event.data as { message?: { __alien?: { id?: string; seq?: number } } }).message
        ?.__alien,
    ).toMatchObject({
      id: params.id,
      seq: params.seq,
...
describe("session history HTTP endpoints", () => {
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __alien?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __alien?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__alien?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __alien?: { seq?: number } }>;
        messages?: Array<{ __alien?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__alien?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(nextData.messages?.[0]?.__alien).toMatchObject({
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(refreshData.messages?.[0]?.__alien?.seq).toBe(2);
  ...
  test("sanitizes phased assistant history entries before returning them", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(body.messages?.[0]?.__alien).toMatchObject({
        id: visibleMessageId,
        seq: 2,
      });
    });
