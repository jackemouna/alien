...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/alien.json" }),
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: AlienConfig) => ({
    ok: true,
    config,
  }));
  prepareSecretsRuntimeSnapshotMock.mockImplementation(
    async ({ config }: { config: AlienConfig }) => ({
      config,
    }),
  );
