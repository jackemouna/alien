...
function resolveRelayNodePushConfig(cfg: AlienConfig) {
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: AlienConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "Alien needs a quick reopen",
        body: "Tap to reopen Alien and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
