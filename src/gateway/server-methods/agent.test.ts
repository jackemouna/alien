...
describe("gateway agent handler", () => {
  ...
  it("does not create task rows for inter-session completion wakes", async () => {
    ...
    await invokeAgent(
      {
        message: [
          "[Mon 2026-04-06 02:42 GMT+1] <<<BEGIN_ALIEN_INTERNAL_CONTEXT>>>",
  ...
  it("terminalizes successful async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "alien-gateway-agent-task-" }, async (root) => {
  ...
  it("terminalizes failed async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "alien-gateway-agent-task-error-" }, async (root) => {
      process.env.ALIEN_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("preserves aborted async gateway agent runs as timed out", async () => {
    await withTempDir({ prefix: "alien-gateway-agent-task-aborted-" }, async (root) => {
      process.env.ALIEN_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("classifies aborted async gateway agent rejections as timed out", async () => {
    await withTempDir({ prefix: "alien-gateway-agent-task-abort-error-" }, async (root) => {
      process.env.ALIEN_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("does not overwrite operator-cancelled async gateway agent tasks after late completion", async () => {
    await withTempDir({ prefix: "alien-gateway-agent-task-cancelled-" }, async (root) => {
      process.env.ALIEN_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("dispatches async gateway agent task creation through the detached task runtime seam", async () => {
    await withTempDir({ prefix: "alien-gateway-agent-seam-" }, async (root) => {
      process.env.ALIEN_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("prepends runtime-loaded startup memory to bare /new agent runs", async () => {
    await withTempDir({ prefix: "alien-gateway-reset-startup-" }, async (workspaceDir) => {
  ...
  it("uses shared bootstrap reset wording for bare /new when workspace bootstrap is pending", async () => {
    await withTempDir({ prefix: "alien-gateway-reset-bootstrap-" }, async (workspaceDir) => {
  ...
  it("resolves bare /new bootstrap state from the effective spawned workspace", async () => {
    await withTempDir(
      { prefix: "alien-gateway-reset-default-" },
      async (defaultWorkspaceDir) => {
        await withTempDir(
          { prefix: "alien-gateway-reset-spawned-" },
  ...
  it("suppresses full bootstrap wording for bare /new on subagent sessions", async () => {
    await withTempDir({ prefix: "alien-gateway-reset-subagent-" }, async (workspaceDir) => {
      await fs.writeFile(`${workspaceDir}/BOOTSTRAP.md`, "bootstrap ritual", "utf-8");
      mocks.loadConfigReturn = {
        agents: {
          defaults: {
  ...
  it.each(["all", "non-main"] as const)(
    ...
    async (sandboxMode) => {
      ...
      try {
        await withTempDir(
          { prefix: "alien-gateway-startup-canonical-" },
          async (canonicalWorkspaceDir) => {
            await withTempDir(
              { prefix: "alien-gateway-startup-inherited-" },
  ...
  it("uses request model override when resolving bare /new bootstrap file access", async () => {
    await withTempDir(
      { prefix: "alien-gateway-reset-model-override-" },
      async (workspaceDir) => {
        await fs.writeFile(`${workspaceDir}/BOOTSTRAP.md`, "bootstrap ritual", "utf-8");
        mocks.loadConfigReturn = {
          agents: {
