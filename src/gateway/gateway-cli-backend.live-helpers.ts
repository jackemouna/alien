...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `ALIEN_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("ALIEN_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.ALIEN_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.ALIEN_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "ALIEN_LIVE_CLI_BACKEND_ARGS",
      process.env.ALIEN_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `ALIEN_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "ALIEN_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.ALIEN_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.ALIEN_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.ALIEN_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.ALIEN_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.ALIEN_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.ALIEN_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.ALIEN_SKIP_PROVIDERS = "1";
  process.env.ALIEN_SKIP_GMAIL_WATCHER = "1";
  process.env.ALIEN_SKIP_CRON = "1";
  process.env.ALIEN_SKIP_CANVAS_HOST = "1";
  process.env.ALIEN_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ALIEN_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("ALIEN_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("ALIEN_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("ALIEN_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("ALIEN_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("ALIEN_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("ALIEN_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("ALIEN_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("ALIEN_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("ALIEN_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("ALIEN_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("ALIEN_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
