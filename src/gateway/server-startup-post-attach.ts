...
import type { loadAlienPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "ALIEN_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: AlienConfig): GatewayMemoryStartupPolicy {
...
function resolveRestartSentinelPathFast(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.ALIEN_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.ALIEN_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".alien");
  if (env.ALIEN_TEST_FAST === "1" || fs.existsSync(newStateDir)) {
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadAlienPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { ensureAlienModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureAlienModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: AlienConfig;
  pluginRegistry: ReturnType<typeof loadAlienPlugins>;
  ...
  await measureStartup(params.startupTrace, "sidecars.channels", async () => {
    if (!skipChannels) {
      ...
      params.logChannels.info(
        "skipping channel start (ALIEN_SKIP_CHANNELS=1 or ALIEN_SKIP_PROVIDERS=1)",
      );
    }
  });
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: AlienConfig;
    pluginRegistry: ReturnType<typeof loadAlienPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
