...
function resolveAutoExplicitProviders(cfg: AlienConfig): Set<string> {
  return new Set(
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== ALIEN_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-alien-model")) ||
