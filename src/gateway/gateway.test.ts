...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "alien-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".alien", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["ALIEN_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "alien");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.ALIEN_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".alien");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "alien-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alien-wizard-home-"));
      const configPath = path.join(tempHome, ".alien", "alien.json");
      process.env.HOME = tempHome;
      process.env.ALIEN_STATE_DIR = path.join(tempHome, ".alien");
      process.env.ALIEN_CONFIG_PATH = configPath;
      process.env.ALIEN_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alien-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".alien", "alien.json");
      const bundledPluginsDir = path.join(tempHome, "alien-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.ALIEN_STATE_DIR = path.join(tempHome, ".alien");
      process.env.ALIEN_CONFIG_PATH = configPath;
      process.env.ALIEN_SKIP_CHANNELS = "1";
