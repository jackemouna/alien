...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.ALIEN_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: AlienConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: AlienConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: AlienConfig, requestedAgentId: string) => {
