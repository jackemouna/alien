...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-"));
    try {
  ...
  async function runTrustedProxyAssistantMediaRequest(params: {
    ...
    return await runAssistantMediaRequest({
      url: `/__alien__/assistant-media?${params.meta ? "meta=1&" : ""}source=${encodeURIComponent(params.filePath)}`,
      method: "GET",
  ...
  async function withAllowedAssistantMediaRoot<T>(params: {
    ...
    const tmpRoot = await fs.mkdtemp(path.join(resolvePreferredAlienTmpDir(), params.prefix));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-root-"));
    try {
  ...
  async function withPairedOperatorDeviceToken<T>(params: { fn: (token: string) => Promise<T> }) {
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-device-token-"));
    vi.stubEnv("ALIEN_HOME", tempHome);
    try {
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const { res, handled } = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
  ...
  it("serves assistant media from canonical inbound media refs", async () => {
    ...
    try {
      const { res, handled } = await runAssistantMediaRequest({
        url: `/__alien__/assistant-media?source=${encodeURIComponent(`media://inbound/${id}`)}&token=test-token`,
        method: "GET",
        auth: { mode: "token", token: "test-token", allowTailscale: false },
      });
      expect(handled).toBe(true);
  ...
  it("reports assistant media metadata for canonical inbound media refs", async () => {
    ...
    try {
      const { res, handled, end } = await runAssistantMediaRequest({
        url: `/__alien__/assistant-media?meta=1&source=${encodeURIComponent(`media://inbound/${id}`)}&token=test-token`,
        method: "GET",
        auth: { mode: "token", token: "test-token", allowTailscale: false },
      });
      expect(handled).toBe(true);
  ...
  it("rejects assistant local media outside allowed preview roots", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-media-blocked-"));
    try {
  ...
  it("reports assistant local media availability metadata", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const meta = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
          headers: {
        ...
        const media = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(payload.mediaTicket ?? "")}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const refresh = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(payload.mediaTicket ?? "")}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
  ...
  it("rejects assistant local media with an invalid scoped media ticket", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=v1.invalid.invalid`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
  ...
  it("reports assistant local media availability failures with a reason", async () => {
    const { res, handled, end } = await runAssistantMediaRequest({
      url: `/__alien__/assistant-media?meta=1&source=${encodeURIComponent("/Users/test/Documents/private.pdf")}&token=test-token`,
      method: "GET",
      auth: { mode: "token", token: "test-token", allowTailscale: false },
    });
    expect(handled).toBe(true);
  ...
  it("rejects assistant local media without a valid auth token when auth is enabled", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__alien__/assistant-media?source=${encodeURIComponent(filePath)}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
      ...
      fn: async (operatorToken) => {
        await withAllowedAssistantMediaRoot({
          ...
          fn: async (tmpRoot) => {
            ...
            await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
            const { res, handled } = await runAssistantMediaRequest({
              url: `/__alien__/assistant-media?source=${encodeURIComponent(filePath)}&token=${encodeURIComponent(operatorToken)}`,
              method: "GET",
              auth: { mode: "token", token: "shared-token", allowTailscale: false },
            });
            expect(handled).toBe(true);
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const { res, handled, end } = await runTrustedProxyAssistantMediaRequest({
          filePath,
          headers: {
            "x-alien-scopes": "operator.approvals",
          },
        });
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          { url: `/alien${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/alien",
            root: { kind: "resolved", path: tmp },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/alien");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/alien/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
        expect(Array.isArray(parsed.localMediaPreviewRoots)).toBe(true);
      },
    });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-avatar-http-"));
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "alien-avatar-http-outside-"));
    try {
  ...
  it("serves local avatar bytes when auth is enabled and the token is valid", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-avatar-auth-"));
    try {
      const avatarPath = path.join(tmp, "main.png");
      await fs.writeFile(avatarPath, "avatar-bytes\n");
  ...
  it("serves local avatar bytes when paired device-token auth is valid", async () => {
    await withPairedOperatorDeviceToken({
      fn: async (operatorToken) => {
        const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-avatar-device-token-"));
        try {
          const avatarPath = path.join(tmp, "main.png");
          await fs.writeFile(avatarPath, "avatar-bytes\n");
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-outside-"));
        try {
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-index-outside-"));
        try {
  ...
  it("rejects hardlinked index.html for non-package control-ui roots", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-ui-index-hardlink-"));
        try {
          const outsideIndex = path.join(outsideDir, "index.html");
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = await handleControlUiHttpRequest(
          ...
          res,
          { basePath: "/alien", root: { kind: "resolved", path: tmp } },
        );
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/alien", "/alien/", "/alien/some-page"]) {
          const { handled, end } = await runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,
