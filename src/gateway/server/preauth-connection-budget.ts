...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__alien_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.ALIEN_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.ALIEN_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
