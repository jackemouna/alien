...
const CODEX_HARNESS_LIVE = process.env.ALIEN_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.ALIEN_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["ALIEN_TRAJECTORY", "ALIEN_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-alien-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.ALIEN_TRAJECTORY = "1";
      process.env.ALIEN_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".alien", "trajectory-exports", "bundle");
