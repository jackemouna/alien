...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ALIEN_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.ALIEN_DISABLE_BONJOUR) &&
  ...
  const tailnetDns = needsTailnetDns
    ...
    : undefined;
  const sshPortEnv = mdnsMinimal ? undefined : process.env.ALIEN_SSH_PORT?.trim();
