...
const { createAlienTools } = await import("../agents/alien-tools.js");
...
type SessionSendTool = ReturnType<typeof createAlienTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAlienTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ALIEN_GATEWAY_PORT", "ALIEN_GATEWAY_TOKEN"]);
  ...
  process.env.ALIEN_GATEWAY_PORT = String(gatewayPort);
  process.env.ALIEN_GATEWAY_TOKEN = gatewayToken;
