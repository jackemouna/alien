 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for Alien Gateway.
 *
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    requestedSessionKey: getHeader(params.req, "x-alien-session-key"),
  });
}
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from Alien.";
