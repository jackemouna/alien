...
export function resolveHooksConfig(cfg: AlienConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AlienConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-alien-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-alien-idempotency-key"]) ||
