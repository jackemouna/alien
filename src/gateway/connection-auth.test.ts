...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when ALIEN env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when ALIEN env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when ALIEN env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
