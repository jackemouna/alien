...
beforeAll(async () => {
  sharedTempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-sessions-config-"));
...
describe("gateway config methods", () => {
  it("rejects config.set when SecretRef resolution fails", async () => {
    const missingEnvVar = `ALIEN_MISSING_SECRETREF_${Date.now()}`;
    delete process.env[missingEnvVar];
    const current = await rpcReq<{
      hash?: string;
  ...
  it("redacts browser cdpUrl credentials from config.get responses", async () => {
    ...
    try {
      ...
      expect(after.payload?.config?.browser?.cdpUrl).toBe("__ALIEN_REDACTED__");
      ...
      if (typeof after.payload?.raw === "string") {
        expect(after.payload.raw).toContain("__ALIEN_REDACTED__");
  ...
  it("does not reject config.set for unresolved auth-profile refs outside submitted config", async () => {
    const missingEnvVar = `ALIEN_MISSING_AUTH_PROFILE_REF_${Date.now()}`;
  ...
  it("rejects config.patch when merged SecretRefs cannot resolve", async () => {
    const missingEnvVar = `ALIEN_MISSING_SECRETREF_PATCH_${Date.now()}`;
    delete process.env[missingEnvVar];
...
describe("gateway config.apply", () => {
  it("rejects config.apply when SecretRef resolution fails", async () => {
    const missingEnvVar = `ALIEN_MISSING_SECRETREF_APPLY_${Date.now()}`;
    delete process.env[missingEnvVar];
    const current = await rpcReq<{
      hash?: string;
      raw?: string | null;
  ...
  it("does not reject config.apply for unresolved auth-profile refs outside submitted config", async () => {
    const missingEnvVar = `ALIEN_MISSING_AUTH_PROFILE_REF_APPLY_${Date.now()}`;
    await writeUnresolvedAuthProfileTokenRef(missingEnvVar);
