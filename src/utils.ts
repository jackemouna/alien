...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".alien");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ALIEN_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ALIEN_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ALIEN_STATE_DIR.
