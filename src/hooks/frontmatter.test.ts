...
`;
    ...
    expect(parsed.alien.emoji).toBe("💾");
    expect(parsed.alien.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.alien.emoji).toBe("📝");
    expect(parsed.alien.events).toEqual(["command"]);
    expect(parsed.alien.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.alien.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"alien": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"alien": {"events": ["test"]}}');
  });
...
describe("resolveAlienMetadata", () => {
  it("extracts alien metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAlienMetadata(frontmatter);
  ...
  it("returns undefined when alien key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        alien: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Alien" },
            { id: "npm", kind: "npm", package: "@alien/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@alien/hook");
  });
    ...
    "alien":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Alien" }],
      },
  }
---
...
`;
    ...
    const alien = resolveAlienMetadata(frontmatter);
    expect(alien).toBeDefined();
    expect(alien?.emoji).toBe("💾");
    expect(alien?.events).toEqual(["command:new", "command:reset"]);
    expect(alien?.requires?.config).toEqual(["workspace.dir"]);
    expect(alien?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const alien = resolveAlienMetadata(frontmatter);
    expect(alien?.emoji).toBe("disk");
    expect(alien?.events).toEqual(["command:new"]);
  });
});
