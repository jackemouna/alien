...
This directory contains hooks that ship with Alien. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.alien/workspace`)
...
homepage: https://docs.alien.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.alien/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.alien/alien.json`:
...
2. Restart gateway: `pkill -9 -f 'alien.*gateway' && pnpm alien gateway`
3. Enable the hook: `alien hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.alien.ai/automation/hooks
