...
homepage: https://docs.alien.ai/automation/hooks#command-logger
metadata:
  {
    "alien":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.alien/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.alien/logs/commands.log ~/.alien/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/alien`:
   ```
   /home/username/.alien/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
