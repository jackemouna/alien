...
async function ensureAlienHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing alien.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json alien.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureAlienHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `alien.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `alien.hooks entry resolves outside package directory: ${entry}`,
      };
    }
