...
export function hasConfiguredInternalHooks(config: AlienConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: AlienConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
