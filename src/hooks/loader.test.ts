...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ALIEN_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    it("detects configured internal hook surfaces", () => {
      expect(hasConfiguredInternalHooks({} satisfies AlienConfig)).toBe(false);
      expect(
    ...
    const expectNoCommandHookRegistration = async (cfg: AlienConfig) => {
    ...
    it("skips hook discovery until internal hooks are configured", async () => {
      for (const cfg of [
        {} satisfies AlienConfig,
        { hooks: {} } satisfies AlienConfig,
        { hooks: { internal: {} } } satisfies AlienConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
