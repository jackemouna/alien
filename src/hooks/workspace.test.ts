...
function setupHardlinkHookWorkspace(hookName: string): {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-hooks-workspace-hardlink-"));
...
describe("hooks workspace", () => {
  it("ignores package.json hook paths that traverse outside package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-hooks-workspace-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
    ...
    const entries = loadHookEntriesFromDir({ dir: hooksRoot, source: "alien-workspace" });
    expect(entries.some((e) => e.hook.name === "outside")).toBe(false);
  });

  it("accepts package.json hook paths within package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-hooks-workspace-ok-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("ignores package.json hook paths that escape via symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-hooks-workspace-link-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("does not let workspace hooks override managed hooks with the same name", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-hooks-collision-"));
    ...
    expect(entries[0]?.hook.source).toBe("alien-managed");
  });

  it("treats configured extraDirs as managed hook sources", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-hooks-extra-"));
    const workspaceDir = path.join(root, "workspace");
    ...
    fs.writeFileSync(
      ...
      ["---", "name: shared-hook", 'metadata: {"alien":{"events":["command:new"]}}', "---"].join(
        "\n",
      ),
    );
