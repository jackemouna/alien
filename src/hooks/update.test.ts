...
describe("updateNpmInstalledHookPacks", () => {
  ...
  it("aborts exact pinned hook pack updates on integrity drift by default", async () => {
      ...
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            ...
            resolvedSpec: "@alien/demo-hooks@1.0.0",
            version: "1.0.0",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @alien/demo-hooks@1.0.0",
          };
        }
        return {
          ok: true,
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for hook pack "demo-hooks" (@alien/demo-hooks@1.0.0): expected sha512-old, got sha512-new',
    );
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          'Failed to update hook pack "demo-hooks": aborted: npm package integrity drift detected for @alien/demo-hooks@1.0.0',
      },
    ]);
  });
});
