...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "alien-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.ALIEN_BUNDLED_HOOKS_DIR;
    process.env.ALIEN_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.ALIEN_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ALIEN_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".alien", "extensions", "sample-bundle");
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"alien":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): AlienConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("alien-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Alien hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".alien", "extensions", "claude-bundle");
