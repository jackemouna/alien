...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "alien-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "alien", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "alien", version: "2.3.4" });
  ...
  it("ignores non-alien package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers ALIEN_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ALIEN_VERSION: "9.9.9",
        ALIEN_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale ALIEN_VERSION for compatibility checks", () => {
    const previous = process.env.ALIEN_VERSION;
    const previousService = process.env.ALIEN_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ALIEN_VERSION = "2026.3.25";
      process.env.ALIEN_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.ALIEN_VERSION = previous;
      process.env.ALIEN_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ALIEN_VERSION: "2026.3.99",
        ALIEN_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        ALIEN_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        ALIEN_VERSION: "2026.3.99",
        ALIEN_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
