...
const ORIGINAL_SKIP_MISSING = process.env.ALIEN_A2UI_SKIP_MISSING;
const ORIGINAL_SPARSE_PROFILE = process.env.ALIEN_SPARSE_PROFILE;

describe("canvas a2ui copy", () => {
  beforeEach(() => {
    delete process.env.ALIEN_A2UI_SKIP_MISSING;
    delete process.env.ALIEN_SPARSE_PROFILE;
  });
  ...
  afterEach(() => {
    if (ORIGINAL_SKIP_MISSING === undefined) {
      ...
      process.env.ALIEN_A2UI_SKIP_MISSING = ORIGINAL_SKIP_MISSING;
    }

    if (ORIGINAL_SPARSE_PROFILE === undefined) {
      delete process.env.ALIEN_SPARSE_PROFILE;
    } else {
      process.env.ALIEN_SPARSE_PROFILE = ORIGINAL_SPARSE_PROFILE;
    }
  });

  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    await withTempDir("alien-a2ui-", run);
  }
  ...
  it("skips missing assets when ALIEN_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.ALIEN_A2UI_SKIP_MISSING = "1";
      await expect(
  ...
  it("skips missing assets when ALIEN_SPARSE_PROFILE is set", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.ALIEN_SPARSE_PROFILE = "core";
      await expect(
        copyA2uiAssets({ srcDir: dir, outDir: path.join(dir, "out") }),
      ).resolves.toBeUndefined();
    });
