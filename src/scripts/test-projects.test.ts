...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/alien-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/alien-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("caps project-level parallelism when the Vitest worker budget is conservative", () => {
    expect(
      resolveParallelFullSuiteConcurrency(58, {
        ALIEN_VITEST_MAX_WORKERS: "1",
      }),
    ).toBe(1);

    expect(
      resolveParallelFullSuiteConcurrency(58, {
        ALIEN_TEST_WORKERS: "1",
      }),
    ).toBe(1);
  });

  it("keeps conservative core full-suite runs on aggregate shards", () => {
    const originalVitestMaxWorkers = process.env.ALIEN_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.ALIEN_TEST_WORKERS;
    const originalProjectParallel = process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    try {
      process.env.ALIEN_VITEST_MAX_WORKERS = "1";
      ...
      if (originalVitestMaxWorkers === undefined) {
        ...
        process.env.ALIEN_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.ALIEN_TEST_WORKERS;
      } else {
        process.env.ALIEN_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.ALIEN_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.ALIEN_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
    }
  });
  ...
  it("uses a bounded local default for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          ALIEN_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(specs[0]?.env).toMatchObject({
      KEEP_ME: "1",
      ALIEN_VITEST_FS_MODULE_CACHE_PATH:
        "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    });
    expect(specs[1]?.env.ALIEN_VITEST_FS_MODULE_CACHE_PATH).toBe(
  ...
  it("preserves explicit Vitest filesystem module cache paths", () => {
    ...
    expect(
      applyParallelVitestCachePaths(specs, {
        cwd: "/repo",
        env: {
          ALIEN_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache",
        },
      }),
    ).toBe(specs);
  });
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("alien-vitest-include-");
    expect(spec?.env.ALIEN_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-contract-include-"));
    try {
      ...
      const specs = createVitestRunSpecs(
        ...
        {
          baseEnv: {
            ALIEN_VITEST_INCLUDE_FILE: includeFile,
