...
const planAlienModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planAlienModelsJson: (...args: unknown[]) => planAlienModelsJsonMock(...args),
  }));
  ...
  ({ ensureAlienModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureAlienModelsJson({}, agentDir);

      expect(planAlienModelsJsonMock).toHaveBeenCalledWith(
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureAlienModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureAlienModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".alien", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(home, ".alien", "agents", "main", "agent", "models.json")),
      ).rejects.toThrow();
    });
  });
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      planAlienModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
      ...
      expect(planAlienModelsJsonMock).toHaveBeenCalledTimes(2);
      expect(planAlienModelsJsonMock).toHaveBeenLastCalledWith(
        expect.objectContaining({
          providerDiscoveryProviderIds: ["anthropic"],
          providerDiscoveryTimeoutMs: 5000,
        }),
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureAlienModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureAlienModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planAlienModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planAlienModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureAlienModelsJson(first);
      await ensureAlienModelsJson(second);
      await ensureAlienModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      await Promise.all([ensureAlienModelsJson(first), ensureAlienModelsJson(second)]);
