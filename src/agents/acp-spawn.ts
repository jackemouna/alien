...
function resolveHeartbeatConfigForAgent(params: {
  cfg: AlienConfig;
  agentId: string;
}): NonNullable<NonNullable<AlienConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Alien config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Alien config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: AlienConfig, agentId: string): boolean {
