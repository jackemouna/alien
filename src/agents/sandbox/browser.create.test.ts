...
vi.mock("../../plugin-sdk/browser-profiles.js", () => ({
  ...
  DEFAULT_ALIEN_BROWSER_COLOR: "#FF4500",
  DEFAULT_ALIEN_BROWSER_PROFILE_NAME: "alien",
  resolveProfile: (
...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    browser: {
      ...
      containerPrefix: "alien-sbx-browser-",
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("ALIEN_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("ALIEN_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^ALIEN_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries.some((entry) => entry.startsWith("ALIEN_BROWSER_NOVNC_PASSWORD="))).toBe(
      false,
    );
  ...
  it("recreates a cached bridge when the SSRF policy changes", async () => {
    ...
    BROWSER_BRIDGES.set("session:test", {
      ...
      containerName: "alien-sbx-browser-session-test-0661d10a",
  ...
  it("stamps the mount format version label on browser containers", async () => {
    ...
    expect(labels).toContain(`alien.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  });
  ...
  it("force-removes the browser container when CDP never becomes reachable", async () => {
    ...
    expect(dockerMocks.execDocker).toHaveBeenCalledWith(
      ["rm", "-f", expect.stringMatching(/^alien-sbx-browser-session-test-/)],
      { allowFailure: true },
    );
  });
  ...
  it("auto-derives CDP source range from Docker network gateway", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries).toContain("ALIEN_BROWSER_CDP_SOURCE_RANGE=172.21.0.1/32");
  });
  ...
  it("uses explicit cdpSourceRange over auto-derived gateway", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries).toContain("ALIEN_BROWSER_CDP_SOURCE_RANGE=10.0.0.0/24");
  ...
  it("uses loopback range for network=none (no IPAM gateway, no peer risk)", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries).toContain("ALIEN_BROWSER_CDP_SOURCE_RANGE=127.0.0.1/32");
  });
});
