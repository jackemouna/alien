...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "alien-sandbox-ssh-"));
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredAlienTmpDir() ?? os.tmpdir());
}
