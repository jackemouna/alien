...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "alien-test-ssh-config"),
    host: "alien-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "alien-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "alien-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/alien/alien-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/alien/alien-ssh-agent-worker");
