...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".alien", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-exec-approvals-"));
  });

  beforeEach(async () => {
    previousHome = process.env.HOME;
    previousUserProfile = process.env.USERPROFILE;
    previousBundledPluginsDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.ALIEN_DISABLE_BUNDLED_PLUGINS;
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-test-bin-"));
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".alien", "skills", "gog");
