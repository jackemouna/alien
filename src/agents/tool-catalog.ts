...
function buildCoreToolGroupMap() {
  ...
  const alienTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAlienGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:alien": alienTools,
