import { createParameterFreeTool } from "alien/plugin-sdk/agent-runtime-test-contracts";
...
describe("AgentRuntimePlan", () => {
  it("records resolved model, auth, transport, tool, delivery, and observability policy", () => {
    const plan = buildAgentRuntimePlan({
      ...
      config: {},
      workspaceDir: "/tmp/alien-runtime-plan",
      model: {
        id: "gpt-5.4",
        name: "GPT-5.4",
  ...
  it("keeps Alien-owned tool-schema normalization reachable from the plan", () => {
    const plan = buildAgentRuntimePlan({
      provider: "openai",
      modelId: "gpt-5.4",
      modelApi: "openai-responses",
