...
const ALIEN_TRANSPORT_TO_CLI_BUNDLE_TYPE: Record<string, string> = {
...
/**
 * User config stores Alien MCP transport names, while CLI backends such as
 ...
 * Alien `transport` shape directly.
 */
export function toCliBundleMcpServerConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  ...
  if (typeof rawTransport === "string") {
    const mapped = ALIEN_TRANSPORT_TO_CLI_BUNDLE_TYPE[rawTransport];
    if (mapped) {
      next.type = mapped;
    }
  }
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // Alien config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
