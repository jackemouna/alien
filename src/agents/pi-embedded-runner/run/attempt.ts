...
import { resolveAlienReferencePaths } from "../../docs-path.js";
...
import { createAlienCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
import {
  ...
  try {
    ...
    const toolsRaw = !toolConstructionPlan.constructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-alien-coding-tools");
    ...
    const openClawReferences = await resolveAlienReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Alien
      // compaction overrides applied in createPreparedEmbeddedPiSettingsManager — same
      // rehydration also restores Pi's auto-compaction (alien#75799), so re-apply
      // both guards.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Alien-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Alien agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        // double-compaction. See: https://github.com/alien/alien/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("alien:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/alien/alien/issues/8643
