...
async function compactEmbeddedPiSessionDirectOnce(
  ...
  await ensureAlienModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const toolsRaw = createAlienCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createAlienCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Alien-managed registrations so custom tools survive startup.
