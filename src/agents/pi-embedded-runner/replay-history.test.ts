...
function alienTranscriptAssistant(model: "delivery-mirror" | "gateway-injected"): AgentMessage {
  return {
    role: "assistant",
...
describe("normalizeAssistantReplayContent", () => {
  ...
  it("filters alien delivery-mirror and gateway-injected assistant messages from replay", () => {
    const messages = [
      userMessage("hello"),
      alienTranscriptAssistant("delivery-mirror"),
      bedrockAssistant([{ type: "text", text: "real reply" }]),
      alienTranscriptAssistant("gateway-injected"),
    ];
    const out = normalizeAssistantReplayContent(messages);
