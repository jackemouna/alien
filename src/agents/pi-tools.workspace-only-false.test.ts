...
describe("FS tools with workspaceOnly=false", () => {
  ...
  const toolsFor = (workspaceOnly: boolean | undefined): AnyAgentTool[] => {
    const read = createAlienReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool);
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-test-"));
  ...
  it("restricts memory-triggered writes to append-only canonical memory files", async () => {
    ...
    const tools = [
      createAlienReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool),
