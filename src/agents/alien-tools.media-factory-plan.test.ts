...
import { resolveOptionalMediaToolFactoryPlan } from "./alien-tools.media-factory-plan.js";
import * as pdfModelConfigModule from "./tools/pdf-tool.model-config.js";

type CreateAlienToolsOptions = Parameters<
  typeof import("./alien-tools.js").createAlienTools
>[0];

async function createAlienToolsForTest(options?: CreateAlienToolsOptions) {
  const { createAlienTools } = await import("./alien-tools.js");
  return createAlienTools(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/alien.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): AlienConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: AlienConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createAlienToolsForTest({
      config,
