...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/alien/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./alien-plugin-tools.js", () => ({
  resolveAlienPluginToolsForOptions: () => [],
}));

vi.mock("./alien-tools.nodes-workspace-guard.js", () => ({
...
describe("createAlienTools TTS config wiring", () => {
  beforeEach(() => {
  ...
  it("passes the resolved shared config into the tts tool", async () => {
    ...
    const { __testing, createAlienTools } = await import("./alien-tools.js");
...
describe("createAlienTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });
