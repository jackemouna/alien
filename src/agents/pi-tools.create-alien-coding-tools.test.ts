...
function expectNoSubagentControlTools(tools: ReturnType<typeof createAlienCodingTools>) {
...
describe("createAlienCodingTools", () => {
  const testConfig: AlienConfig = {};

  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createAlienCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    const createAlienToolsMock = vi.mocked(createAlienTools);
    createAlienToolsMock.mockClear();
    ...
    expect(createAlienToolsMock).toHaveBeenCalledWith(
  ...
  it("skips unrelated tool families when construction is planned from a narrow allowlist", () => {
    ...
    expect(createAlienToolsMock).not.toHaveBeenCalled();
  ...
  it("passes plugin suppression into Alien tool construction plans", () => {
    const createAlienToolsMock = vi.mocked(createAlienTools);
    createAlienToolsMock.mockClear();
  ...
  it("keeps plugin-only construction off the Alien core factory", () => {
    const createAlienToolsMock = vi.mocked(createAlienTools);
    createAlienToolsMock.mockClear();
  ...
  it("passes explicit denylist entries to Alien tool factory planning", () => {
    const createAlienToolsMock = vi.mocked(createAlienTools);
    createAlienToolsMock.mockClear();
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("alien-tools:test-helper")).toBeLessThan(
      stages.indexOf("alien-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createAlienCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createAlienCodingTools({
      config: testConfig,
    ...
    const codexTools = createAlienCodingTools({
      config: testConfig,
    ...
    const disabledConfig: AlienConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createAlienCodingTools({
    ...
    const anthropicTools = createAlienCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: AlienConfig = {
      tools: {
        exec: {
    ...
    const allowed = createAlienCodingTools({
    ...
    const denied = createAlienCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createAlienCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createAlienCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAlienCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createAlienCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createAlienCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createAlienCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createAlienCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(codingTools.some((tool) => tool.name === "message")).toBe(false);

    const cronTools = createAlienCodingTools({
      config: { tools: { profile: "coding" } },
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createAlienCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createAlienCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createAlienCodingTools({
      modelProvider: "xai",
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createAlienCodingTools();
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-read-"));
    try {
      ...
      const contents = "Hello from alien read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "alien-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "alien-workspace"),
    ...
    const tools = createAlienCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-canonical-"));
    try {
      const tools = createAlienCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-legacy-alias-"));
    try {
      const tools = createAlienCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-structured-write-"));
    try {
      const tools = createAlienCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-structured-edit-"));
    try {
