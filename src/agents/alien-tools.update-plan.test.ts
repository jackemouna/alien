...
import { isUpdatePlanToolEnabledForAlienTools } from "./alien-tools.registration.js";
import { createUpdatePlanTool } from "./tools/update-plan-tool.js";

type UpdatePlanGatingParams = Parameters<typeof isUpdatePlanToolEnabledForAlienTools>[0];

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(isUpdatePlanToolEnabledForAlienTools(params)).toBe(expected);
}
...
describe("alien-tools update_plan gating", () => {
  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as AlienConfig }, false);
  });

  it("does not expose update_plan from default tool construction", () => {
    const defaultTools = createAlienTools({
      config: {} as AlienConfig,
      disablePluginTools: true,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
    const emptyAllowlistTools = createAlienTools({
      config: {} as AlienConfig,
      disablePluginTools: true,
      pluginToolAllowlist: [],
      modelProvider: "anthropic",
  ...
  it("registers update_plan when a config allowlist group includes it", () => {
    const tools = createAlienTools({
      config: { tools: { allow: ["group:agents"] } } as AlienConfig,
      disablePluginTools: true,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
