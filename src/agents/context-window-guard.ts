...
export function formatContextWindowWarningMessage(params: {
  ...
  if (params.guard.source === "agentContextTokens") {
    return (
      `${base}; Alien is capped by agents.defaults.contextTokens, so raise that cap ` +
      `if you want to use more of the model context window`
    );
  }
  if (params.guard.source === "modelsConfig") {
    return (
      `${base}; Alien is using the configured model context limit for this model, ` +
...
export function formatContextWindowBlockMessage(params: {
  ...
  if (params.guard.source === "agentContextTokens") {
    return `${base} Alien is capped by agents.defaults.contextTokens. Raise that cap.`;
  }
  if (params.guard.source === "modelsConfig") {
    return (
      `${base} Alien is using the configured model context limit for this model. ` +
  ...
  return (
    ...
    `Alien local/self-hosted runs work best at ${params.guard.warnBelowTokens}+ tokens.`
  );
}
