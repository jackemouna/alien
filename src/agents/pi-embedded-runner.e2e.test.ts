...
const ensureAlienModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureAlienModelsJson: (...args: Parameters<typeof ensureAlienModelsJsonMock>) =>
        ensureAlienModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("alien-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureAlienModelsJsonMock.mockResolvedValue({ wrote: false });
