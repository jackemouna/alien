...
function stripAlienPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "alien") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "alien"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "alien"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "alien") {
      return argv.slice(idx);
    }
  }
  return argv;
...
function parseAlienChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const alienArgv = stripAlienPackageRunner(argv);
  return (
    normalizeCommandBaseName(alienArgv[0]) === "alien" &&
    (alienArgv[1] === "channels" || alienArgv[1] === "channel") &&
    alienArgv[2] === "login"
  );
}
...
function rejectUnsafeControlShellCommand(command: string): void {
  ...
  for (const candidate of candidates) {
    ...
    if (parseAlienChannelsLoginShellCommand(candidate)) {
      throw new Error(
        [
          "exec cannot run interactive Alien channel login commands.",
          "Run `alien channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
        ].join(" "),
      );
    }
  }
