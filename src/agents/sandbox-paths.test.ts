...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-managed-media-"));
...
async function withOutsideHardlinkInAlienTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside alien tmp root",
      media: path.join(os.tmpdir(), "outside-alien", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Alien tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Alien tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAlienTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Alien tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAlienTmp(
