...
function resolveAttachmentLimits(config: AlienConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".alien", "attachments");
  const relDir = path.posix.join(".alien", "attachments", attachmentId);
