...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.alien/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "alien doctor" and "alien approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
