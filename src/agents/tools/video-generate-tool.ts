...
const VideoGenerateToolSchema = Type.Object({
  ...
  filename: Type.Optional(
    Type.String({
      description:
        "Optional output filename hint. Alien preserves the basename and saves under its managed media directory.",
    }),
  ),
  size: Type.Optional(
    Type.String({
  ...
  aspectRatio: Type.Optional(
    Type.String({
      description:
        'Optional aspect ratio hint such as 1:1, 16:9, 9:16, "adaptive", or a provider-specific value. Alien normalizes or ignores unsupported values per provider.',
    }),
  ),
  resolution: Type.Optional(
    Type.String({
      description:
        "Optional resolution hint such as 480P, 720P, 768P, 1080P, 4K, or a provider-specific value. Alien normalizes or ignores unsupported values per provider.",
    }),
  ),
  durationSeconds: Type.Optional(
    Type.Number({
      description:
        "Optional target duration in seconds. Alien may round this to the nearest provider-supported duration.",
      minimum: 1,
    }),
  ),
  audio: Type.Optional(
...
function hasExplicitVideoGenerationModelConfig(cfg?: AlienConfig): boolean {
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: AlienConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
  ...
  return {
    ...
    description:
      "Generate videos using configured providers. Generated videos are saved under Alien-managed media storage and delivered automatically as attachments. Duration requests may be rounded to the nearest provider-supported value.",
