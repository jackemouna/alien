...
type CreateAlienCodingToolsArgs = Parameters<typeof createAlienCodingTools>[0];
type MockAlienToolsOptions = {
  config?: AlienConfig;
  agentDir?: string;
  workspaceDir?: string;
  sandboxRoot?: string;
...
vi.mock("../alien-tools.js", async () => {
  const { createImageTool } = await import("./image-tool.js");
  return {
    createAlienTools: vi.fn((options?: MockAlienToolsOptions) => {
...
async function createAlienCodingToolsWithFreshModules(options?: CreateAlienCodingToolsArgs) {
  ...
  return createAlienCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "alien-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): AlienConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows workspace images via createAlienCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createAlienCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.alien/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-image-managed-inbound-"));
