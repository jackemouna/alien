...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    alien: process.env.ALIEN_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.alien === undefined) {
      delete process.env.ALIEN_GATEWAY_TOKEN;
    } else {
      process.env.ALIEN_GATEWAY_TOKEN = envSnapshot.alien;
    }
  });
  ...
  it("uses ALIEN_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ALIEN_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
