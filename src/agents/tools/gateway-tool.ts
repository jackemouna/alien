...
import { isAlienOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `alien security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as AlienConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as AlienConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isAlienOwnerOnlyCoreToolName("gateway"),
    description:
