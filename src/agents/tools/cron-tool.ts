...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    label: "Cron",
    name: "cron",
    ownerOnly: isAlienOwnerOnlyCoreToolName("cron"),
...
Main-session cron jobs enqueue system events for heartbeat handling. Isolated cron jobs create background task runs that appear in \`alien tasks\`.
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "add": {
          ...
          // See: https://github.com/alien/alien/issues/11310
