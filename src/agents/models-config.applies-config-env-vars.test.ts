...
import {
  planAlienModelsJsonWithDeps,
  resolveProvidersForModelsJsonWithDeps,
} from "./models-config.plan.js";
import type { ProviderConfig } from "./models-config.providers.secrets.js";

const TEST_ENV_VAR = "ALIEN_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/alien-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): AlienConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: AlienConfig) {
...
describe("models-config", () => {
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/alien-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planAlienModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/alien-models-config-env-vars-test",
        env: {},
