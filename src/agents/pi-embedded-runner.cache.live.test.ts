...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.ALIEN_CACHE_TRACE,
      file: process.env.ALIEN_CACHE_TRACE_FILE,
      messages: process.env.ALIEN_CACHE_TRACE_MESSAGES,
      prompt: process.env.ALIEN_CACHE_TRACE_PROMPT,
      system: process.env.ALIEN_CACHE_TRACE_SYSTEM,
    };
    process.env.ALIEN_CACHE_TRACE = "1";
    process.env.ALIEN_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.ALIEN_CACHE_TRACE_MESSAGES = "0";
    process.env.ALIEN_CACHE_TRACE_PROMPT = "0";
    process.env.ALIEN_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("ALIEN_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("ALIEN_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("ALIEN_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("ALIEN_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("ALIEN_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
