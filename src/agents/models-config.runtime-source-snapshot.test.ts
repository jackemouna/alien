...
vi.mock("../plugins/provider-runtime.js", () => ({
  applyProviderConfigDefaultsWithPlugin: (config: AlienConfig) => config,
...
let ensureAlienModelsJson: typeof import("./models-config.js").ensureAlienModelsJson;
let resetModelsJsonReadyCacheForTest: typeof import("./models-config.js").resetModelsJsonReadyCacheForTest;
let planAlienModelsJsonWithDeps: typeof import("./models-config.plan.js").planAlienModelsJsonWithDeps;
let readGeneratedModelsJson: typeof import("./models-config.test-utils.js").readGeneratedModelsJson;
const fixtureSuite = createFixtureSuite("alien-models-runtime-source-");

beforeAll(async () => {
  await fixtureSuite.setup();
  ({ clearConfigCache, clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
    await import("../config/io.js"));
  ({ ensureAlienModelsJson, resetModelsJsonReadyCacheForTest } =
    await import("./models-config.js"));
  ({ planAlienModelsJsonWithDeps } = await import("./models-config.plan.js"));
...
function createOpenAiApiKeySourceConfig(): AlienConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): AlienConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): AlienConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): AlienConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiSourceConfigWithHeadersAndApiKey(): AlienConfig {
...
function createOpenAiRuntimeConfigWithHeadersAndApiKey(): AlienConfig {
...
function withGatewayTokenMode(config: AlienConfig): AlienConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function planGeneratedProviders(params: {
  ...
  const plan = await planAlienModelsJsonWithDeps(
    {
      cfg: params.config,
      sourceConfigForSecrets: params.sourceConfigForSecrets,
      agentDir: "/tmp/alien-models-plan",
      env: {},
      existingRaw: "",
      existingParsed: null,
    },
...
describe("models-config runtime source snapshot", () => {
  it("uses runtime source snapshot markers when passed the active runtime config", async () => {
    ...
    const runtimeConfig: AlienConfig = {
      models: {
        providers: {
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const clonedRuntimeConfig: AlienConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        ...
        await ensureAlienModelsJson(clonedRuntimeConfig, agentDir);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const sourceConfig = createOpenAiApiKeySourceConfig();
      const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
      const firstCandidate: AlienConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      const secondCandidate: AlienConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureAlienModelsJson(firstCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.headers?.["X-Alien-Test"]).toBe("one");

        // Header changes still rewrite models.json, but merge mode preserves the existing baseUrl.
        await ensureAlienModelsJson(secondCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.baseUrl).toBe("https://api.openai.com/v1");
        expect(parsed.providers.openai?.apiKey).toBe("OPENAI_API_KEY"); // pragma: allowlist secret
        expect(parsed.providers.openai?.headers?.["X-Alien-Test"]).toBe("two");
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
