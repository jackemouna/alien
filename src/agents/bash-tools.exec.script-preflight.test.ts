...
import { __setFsSafeTestHooksForTest } from "@alien/fs-safe/test-hooks";
...
const parseAlienChannelsLoginShellCommand = __testing.parseAlienChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("alien-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive Alien channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseAlienChannelsLoginShellCommand("alien channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseAlienChannelsLoginShellCommand(
        "pnpm exec alien channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseAlienChannelsLoginShellCommand("alien channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-alien-channel-login", {
        command: "alien channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive Alien channel login commands/);
    await expect(
      tool.execute("call-wrapped-alien-channel-login", {
        command: "sudo -u alien bash -lc 'alien channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Alien channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'alien channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Alien channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env alien channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Alien channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'alien channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Alien channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("alien-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("alien-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("alien-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("alien-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__alien_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
