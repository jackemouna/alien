...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureAlienModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): AlienConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): AlienConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `alien-auth-rotation-${Date.now()}.log`),
    });
