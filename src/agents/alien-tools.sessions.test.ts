...
import "./test-helpers/fast-alien-tools-sessions.js";
...
function createAlienTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createAlienTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createAlienTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
