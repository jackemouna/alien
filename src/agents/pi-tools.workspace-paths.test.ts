...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("alien-ws-", async (workspaceDir) => {
      await withTempDir("alien-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createAlienCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "alien" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("alien-ws-", async (workspaceDir) => {
      await withTempDir("alien-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("alien-ws-", async (workspaceDir) => {
      const cfg: AlienConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createAlienCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-alien.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("alien-sandbox-", async (sandboxDir) => {
      await withTempDir("alien-workspace-", async (workspaceDir) => {
        ...
        const tools = createAlienCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
