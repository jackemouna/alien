...
async function createTempWorkspaceDir() {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-skill-status-"));
...
function makeEntry(params: {
  ...
  return {
    skill: createFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "alien-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-bundled-"));
    ...
    await withEnvAsync({ ALIEN_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill).toBeDefined();
      expect(skill?.source).toBe("alien-workspace");
