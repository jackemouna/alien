...
function makeFallbacksOnlyCfg(): AlienConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): AlienConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "alien-auth-suite-mock");
...
// https://github.com/alien/alien/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("fails fast on session write-lock timeouts instead of trying model fallbacks", async () => {
    ...
    const lockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "pid=37121",
      lockPath: "/tmp/alien/session.jsonl.lock",
    });
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("alien-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
