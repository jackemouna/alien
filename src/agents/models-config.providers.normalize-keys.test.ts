...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<AlienConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-agent-"));
    try {
      const providers: NonNullable<NonNullable<AlienConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<AlienConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<AlienConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
