...
function createDefaultSpawnConfig(): AlienConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: AlienConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects Alien config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
    ...
    expect(result).toHaveProperty("error", expect.stringContaining("Alien config agent"));
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps Alien ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
