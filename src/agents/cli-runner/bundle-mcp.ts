...
function normalizeAlienLoopbackUrl(value: string): string {
  const match =
    /^(http:\/\/(?:127\.0\.0\.1|localhost|\[::1\])):\d+(\/mcp)$/.exec(value.trim()) ?? undefined;
  if (!match) {
    return value;
  }
  return `${match[1]}:<alien-loopback>${match[2]}`;
}
...
function canonicalizeBundleMcpConfigForResume(config: BundleMcpConfig): BundleMcpConfig {
  const canonicalServers = Object.fromEntries(
    Object.entries(config.mcpServers).map(([name, server]) => {
      if (name !== "alien" || typeof server.url !== "string") {
      ...
      return [
        ...
        sortJsonValue({
          ...server,
          url: normalizeAlienLoopbackUrl(server.url),
        }),
      ];
    }),
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-cli-mcp-"));
