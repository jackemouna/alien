...
describe("loadCliSessionHistoryMessages", () => {
  ...
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-cli-store-"));
    vi.stubEnv("ALIEN_STATE_DIR", stateDir);
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders Alien transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[Alien reseed history truncated]");
