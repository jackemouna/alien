...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from Alien config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "alien-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates Alien transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "alien-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "alien-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "alien-cli-bundle-mcp-user-servers-loopback-",
    );
      ...
      additionalConfig: {
        mcpServers: {
          alien: {
            ...
            headers: { Authorization: "Bearer ${ALIEN_MCP_TOKEN}" },
          },
        },
      },
    });
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "alien-cli-bundle-mcp-user-servers-replace-",
    );
