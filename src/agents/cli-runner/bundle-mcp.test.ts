...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "alien-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".alien", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "alien"]);
    expect(raw.mcpServers?.alien?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.alien?.headers?.Authorization).toBe("Bearer ${ALIEN_MCP_TOKEN}");
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      env: {
        ALIEN_MCP_TOKEN: "loopback-token-123",
        ALIEN_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/alien-bundle-mcp-disabled",
    });
