...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/alien-bundle-mcp-gemini",
    ...
    expect(prepared.env?.ALIEN_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["alien"]);
    expect(raw.mcpServers?.alien?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.alien?.headers?.Authorization).toBe("Bearer loopback-token-123");
