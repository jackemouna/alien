...
const LIVE = isLiveTestEnabled(["ALIEN_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "alien-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("alien_live_probe", "Alien Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Alien-configured streamable-http server", async () => {
    const geminiCommand = process.env.ALIEN_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("alienLiveProbe");
