...
const CLI_BACKEND_PRESERVE_ENV = "ALIEN_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.ALIEN_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.ALIEN_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.ALIEN_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.ALIEN_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.ALIEN_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.ALIEN_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.ALIEN_MCP_TOKEN ||
            env.ALIEN_MCP_SESSION_KEY ||
            env.ALIEN_MCP_SENDER_IS_OWNER
          ) {
