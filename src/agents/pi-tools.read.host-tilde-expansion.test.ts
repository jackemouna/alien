...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("alien-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("alien-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("alien-tilde-test-mkdir-");
  ...
  it("ignores ALIEN_HOME for write operations", async () => {
    const alienHome = await createTempDir("alien-home-override-", os.tmpdir());
    const dir = await createTempDir("alien-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("ALIEN_HOME", alienHome);

    createHostWorkspaceWriteTool(alienHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(alienHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores ALIEN_HOME for mkdir operations", async () => {
    const alienHome = await createTempDir("alien-home-override-", os.tmpdir());
    const dir = await createTempDir("alien-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(alienHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores ALIEN_HOME for readFile operations", async () => {
    const alienHome = await createTempDir("alien-home-override-", os.tmpdir());
    const dir = await createTempDir("alien-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(alienHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores ALIEN_HOME for access operations", async () => {
    const alienHome = await createTempDir("alien-home-override-", os.tmpdir());
    const dir = await createTempDir("alien-tilde-test-edit-");
