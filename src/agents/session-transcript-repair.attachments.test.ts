...
describe("sanitizeToolCallInputs redacts sessions_spawn attachments", () => {
  it("replaces attachments[].content with __ALIEN_REDACTED__", () => {
    ...
    expect(tool?.arguments?.attachments?.[0]?.content).toBe("__ALIEN_REDACTED__");
  ...
  it("redacts ACP-only routing fields from arguments and input payloads", () => {
    ...
    expect(argumentTool?.arguments?.resumeSessionId).toBe("__ALIEN_REDACTED__");
    expect(argumentTool?.arguments?.streamTo).toBe("__ALIEN_REDACTED__");
    expect(inputTool?.input?.resumeSessionId).toBe("__ALIEN_REDACTED__");
    expect(inputTool?.input?.streamTo).toBe("__ALIEN_REDACTED__");
  ...
  it("redacts ACP-only routing fields with non-string payloads", () => {
    ...
    expect(tool?.input?.resumeSessionId).toBe("__ALIEN_REDACTED__");
    expect(tool?.input?.streamTo).toBe("__ALIEN_REDACTED__");
