...
const GOOGLE_LIVE_SINGLE_KEY = "ALIEN_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ALIEN_LIVE_ANTHROPIC_KEY",
    listVar: "ALIEN_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ALIEN_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ALIEN_LIVE_${base}_KEY`;
