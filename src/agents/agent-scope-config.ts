...
export function listAgentEntries(cfg: AlienConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AlienConfig): string[] {
...
export function resolveDefaultAgentId(cfg: AlienConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: AlienConfig, agentId: string): AgentEntry | undefined {
