...
import type { createAlienCodingTools } from "./pi-tools.js";
...
const effectiveInventoryState = vi.hoisted(() => ({
  ...
  createToolsMock: vi.fn<typeof createAlienCodingTools>(
    (_options) =>
      [
        mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
        mockTool({ name: "docs_lookup", label: "Docs Lookup", description: "Search docs" }),
...
vi.mock("./pi-tools.js", () => ({
  createAlienCodingTools: (options?: Parameters<typeof createAlienCodingTools>[0]) =>
...
async function loadHarness(options?: {
  ...
  effectiveInventoryState.createToolsMock =
    ...
    vi.fn<typeof createAlienCodingTools>((_options) => effectiveInventoryState.tools);
  return {
...
describe("resolveEffectiveToolInventory", () => {
  ...
  beforeEach(() => {
    ...
    effectiveInventoryState.createToolsMock = vi.fn<typeof createAlienCodingTools>(
  ...
  it("passes resolved model compat into effective tool creation", async () => {
    const createToolsMock = vi.fn<typeof createAlienCodingTools>(() => [
      mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
    ]);
    const { resolveEffectiveToolInventory } = await loadHarness({
      createToolsMock,
