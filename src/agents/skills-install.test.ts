...
import { resolveAlienMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"alien":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveAlienMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("alien-skills-install-");
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("ALIEN_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Alien-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["ALIEN_STATE_DIR", "ALIEN_CONFIG_PATH"]);
    try {
      process.env.ALIEN_STATE_DIR = "/tmp/untrusted-state";
      process.env.ALIEN_CONFIG_PATH = "/tmp/untrusted-config/alien.json";
