...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseAlienManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyAlienManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveAlienMetadata(
  frontmatter: ParsedSkillFrontmatter,
): AlienSkillMetadata | undefined {
  const metadataObj = resolveAlienManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveAlienManifestRequires(metadataObj);
  const install = resolveAlienManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveAlienManifestOs(metadataObj);
  return {
