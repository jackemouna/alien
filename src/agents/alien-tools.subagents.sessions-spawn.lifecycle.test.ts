...
} from "./alien-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.ALIEN_TEST_FAST;
...
describe("alien-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  afterAll(() => {
    ...
    process.env.ALIEN_TEST_FAST = fastModeEnv.previous;
  });
  ...
  it("sessions_spawn retires bundle MCP runtime when run-mode cleanup completes", async () => {
    ...
    await getOrCreateSessionMcpRuntime({
      ...
      workspaceDir: "/tmp/alien-subagent-mcp-retire",
