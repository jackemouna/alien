...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ALIEN_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ALIEN_HOME: "/srv/alien-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/alien-home"), ".alien", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".alien", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("alien-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("alien-workspace-");
    await fs.mkdir(path.join(tempDir, ".alien"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-workspace-hardlink-"));
    try {
