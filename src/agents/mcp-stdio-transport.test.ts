...
describe("AlienStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    ...
    const { AlienStdioClientTransport } = await import("./mcp-stdio-transport.js");

    const transport = new AlienStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new AlienStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
