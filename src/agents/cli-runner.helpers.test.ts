...
describe("buildCliArgs", () => {
  ...
  it("passes Codex system prompts via a model instructions file config override", () => {
    expect(
      buildCliArgs({
        ...
        systemPromptFilePath: "/tmp/alien/system-prompt.md",
        useResume: false,
      }),
    ).toEqual(["exec", "--json", "-c", 'model_instructions_file="/tmp/alien/system-prompt.md"']);
  });
  ...
  it("passes Claude system prompts through its file flag", () => {
    ...
    ).toEqual(["-p", "--append-system-prompt-file", "/tmp/alien/system-prompt.md"]);
  });
...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredAlienTmpDir(), "alien-cli-write-images-"),
    );
    ...
    try {
      ...
      expect(first.paths[0]).toContain(`${resolvePreferredAlienTmpDir()}/alien-cli-images/`);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredAlienTmpDir(), "alien-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredAlienTmpDir(), "alien-cli-prompt-image-"),
    );
    ...
    try {
      ...
      expect(argv[imageArgIndex + 1]).toContain("alien-cli-images");
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredAlienTmpDir(), "alien-cli-prompt-image-generic-"),
    );
    ...
    try {
      ...
      expect(promptWithImages).toContain("alien-cli-images");
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredAlienTmpDir(), "alien-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".alien-cli-images"))).toBe(
        true,
      );
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredAlienTmpDir(), "alien-cli-explicit-images-"),
    );
    ...
    try {
      ...
      expect(argv[argv.indexOf("--image") + 1]).toContain("alien-cli-images");
...
describe("writeCliSystemPromptFile", () => {
  it("writes stripped system prompts to a private temp file", async () => {
    ...
    try {
      expect(written.filePath).toContain("alien-cli-system-prompt-");
