...
let configOverride: AlienConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: AlienConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/alien/alien/issues/31298
    previousFastTestEnv = process.env.ALIEN_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.ALIEN_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    resetAnnounceQueuesForTests();
    // ALIEN_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Alien runtime context (internal):");
