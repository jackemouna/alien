...
import { ensureAlienModelsJson } from "./models-config.js";
import {
...
const DIRECT_ENABLED = Boolean(process.env.ALIEN_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.ALIEN_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.ALIEN_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
const LIVE_TEST_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.ALIEN_LIVE_TEST_TIMEOUT_MS, 60 * 60 * 1000),
);
const DEFAULT_LIVE_MODEL_CONCURRENCY = 20;
const LIVE_MODEL_CONCURRENCY = resolveLiveModelConcurrency(
  process.env.ALIEN_LIVE_MODEL_CONCURRENCY,
);
const LIVE_MODELS_JSON_TIMEOUT_MS = resolveLiveModelsJsonTimeoutMs(
  process.env.ALIEN_LIVE_MODELS_JSON_TIMEOUT_MS,
);
  ...
  it(
    ...
    async () => {
      ...
      await withLiveStageTimeout(
        ensureAlienModelsJson(cfg),
        "[live-models] prepare models.json",
        LIVE_MODELS_JSON_TIMEOUT_MS,
      );
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set ALIEN_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const providers = parseProviderFilter(process.env.ALIEN_LIVE_PROVIDERS);
      const providerList = providers ? [...providers] : null;
      logProgress("[live-models] resolving agent dir");
      const agentDir = resolveDefaultAgentDir(cfg);
      const rawModels = process.env.ALIEN_LIVE_MODELS?.trim();
      ...
      const perModelTimeoutMs = toInt(process.env.ALIEN_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via ALIEN_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
