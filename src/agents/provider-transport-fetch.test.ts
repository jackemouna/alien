...
describe("buildGuardedModelFetch", () => {
  beforeEach(() => {
    ...
    delete process.env.ALIEN_DEBUG_PROXY_URL;
    delete process.env.ALIEN_SDK_RETRY_MAX_WAIT_SECONDS;
  });
  ...
  it("does not force explicit debug proxy overrides onto plain HTTP model transports", async () => {
    ...
    process.env.ALIEN_DEBUG_PROXY_URL = "http://127.0.0.1:7799";
  ...
  describe("long retry-after handling", () => {
    ...
    it("respects ALIEN_SDK_RETRY_MAX_WAIT_SECONDS", async () => {
      process.env.ALIEN_SDK_RETRY_MAX_WAIT_SECONDS = "10";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
    ...
    it("can be disabled with ALIEN_SDK_RETRY_MAX_WAIT_SECONDS=0", async () => {
      process.env.ALIEN_SDK_RETRY_MAX_WAIT_SECONDS = "0";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "239" },
