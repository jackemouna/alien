...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "alien-adc-"));
...
vi.mock("./model-auth-env-vars.js", () => {
  ...
  return {
    ...
    resolveProviderEnvAuthEvidence: (params?: { config?: AlienConfig }) => {
      const evidence = {
        "google-vertex": [
          {
...
function buildDemoLocalProviderCfg(apiKey: string): AlienConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withAlienTestState(
      ...
      async (state) => {
        ...
        const store = ensureAuthProfileStore(process.env.ALIEN_AGENT_DIR, {
  ...
  it("uses trusted workspace manifest auth evidence in runtime auth checks", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-workspace-cloud-auth-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode explicit ADC credential paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-google-adc-unicode-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode ADC fallback home paths", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-google-adc-home-"));
  ...
  it("resolveEnvApiKey('google-vertex') rejects GOOGLE_CLOUD_PROJECT_ID-only ADC auth evidence", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-google-adc-project-id-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
  ...
  it("resolveEnvApiKey('google-vertex') accepts Windows APPDATA ADC fallback evidence", async () => {
    const appDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-google-adc-appdata-"));
  ...
  it("resolveEnvApiKey('google-vertex') does not synthesize APPDATA from USERPROFILE", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-google-adc-home-"));
    const userProfileDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "alien-google-adc-userprofile-"),
    );
  ...
  it("resolveEnvApiKey('google-vertex') keeps ADC fallback when manifest env candidates are empty", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-google-adc-candidates-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
