...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("ALIEN_SHELL")) {
        input.onStdout?.(env.ALIEN_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  it("sets ALIEN_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-alien-shell", {
      command: 'printf "%s" "${ALIEN_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/alien-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/alien-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a alien /approve abc123 deny",
    ...
    "sudo ALIEN_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root ALIEN_APPROVE=1 bash -lc '/approve abc123 allow-once'",
