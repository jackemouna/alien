...
} from "./alien-tools.subagents.test-harness.js";
...
describe("alien-tools: subagents scope isolation", () => {
  let storePath = "";
  ...
  beforeEach(() => {
    ...
    storePath = path.join(
      os.tmpdir(),
      `alien-subagents-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
