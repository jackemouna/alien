...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## Alien CLI Quick Reference");
    ...
    expect(prompt).toContain("alien gateway restart");
    expect(prompt).toContain("Do not chain `alien gateway stop`");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.alien-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      '[embed url="/__alien__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
      "Never use local filesystem paths or `file://...` URLs in `[embed ...]`.",
    );
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.alien-dev/canvas`.",
    );
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List Alien agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Alien docs: /tmp/alien/docs");
    expect(prompt).toContain(
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/alien",
      docsPath: "/tmp/alien/docs",
      sourcePath: "/tmp/alien",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Alien docs: /tmp/alien/docs");
    expect(prompt).toContain("Local source: /tmp/alien");
    expect(prompt).toContain(
      "For Alien behavior, commands, config, or architecture: consult local docs first.",
    );
    expect(prompt).toContain(
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Alien docs: https://docs.alien.ai");
    expect(prompt).toContain("Source: https://github.com/alien/alien");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/alien",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/alien/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/alien/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Alien Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/alien. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSupplement", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/alien/BOOTSTRAP.md");
  ...
  it("adds pending bootstrap guidance and BOOTSTRAP.md contents for override prompts", () => {
    ...
    expect(supplement).toContain("## /tmp/alien/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`alien ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Alien subagents");
