...
function resolveAnthropicOpusThinking(cfg: AlienConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: AlienConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<AlienConfig>) {
...
describe("model-selection", () => {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", async () => {
      const warnLogs = createWarnLogCapture("alien-model-selection-test");
      try {
        const cfg: Partial<AlienConfig> = {
          agents: {
            defaults: {
