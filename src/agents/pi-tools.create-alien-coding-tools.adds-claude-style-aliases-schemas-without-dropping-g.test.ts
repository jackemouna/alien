...
import { createAlienReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createAlienCodingTools read behavior", () => {
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "alien-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createAlienReadTool(
      baseRead as unknown as Parameters<typeof createAlienReadTool>[0],
    );
