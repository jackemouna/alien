...
function createCommandsStatusRuntimeModuleMock() {
  return {
    buildStatusText: async (params: {
      ...
      return ["Alien", `🧠 Model: ${primary}`, params.taskLineOverride]
        .filter(Boolean)
        .join("\n");
    },
  };
...
describe("session_status tool", () => {
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("Alien");
  ...
  it("passes spawned workspace to session_status auth labels", async () => {
    resetSessionStore({
      "agent:main:spawned": {
        ...
        updatedAt: 10,
        spawnedWorkspaceDir: "/tmp/alien-spawned-workspace",
    ...
    expect(buildStatusMessageMock).toHaveBeenCalledWith(
      expect.objectContaining({
        workspaceDir: "/tmp/alien-spawned-workspace",
      }),
    );
  });
  ...
  it("treats the TUI client label as the current requester session", async () => {
    ...
    const result = await tool.execute("call-tui-label", { sessionKey: "alien-tui" });
    const details = result.details as { ok?: boolean; sessionKey?: string };
    expect(details.ok).toBe(true);
    expect(details.sessionKey).toBe("agent:main:main");
  });
