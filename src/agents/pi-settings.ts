...
export function resolveCompactionReserveTokensFloor(cfg?: AlienConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: AlienConfig): AgentCompactionMode {
 ...
 * before the provider call) when Alien or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (alien#75799).
