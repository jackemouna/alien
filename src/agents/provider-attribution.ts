import { listAlienPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
import {
...
const ALIEN_ATTRIBUTION_PRODUCT = "Alien";
const ALIEN_ATTRIBUTION_ORIGINATOR = "alien";
...
function formatAlienUserAgent(version: string): string {
  return `${ALIEN_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listAlienPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: ALIEN_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in Alien runtime wrapper.",
    ...identity,
    headers: {
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: ALIEN_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatAlienUserAgent(identity.version),
    },
  };
}
