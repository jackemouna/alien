...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alien-cli-bundle-mcp-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.ALIEN_HOME;
      delete process.env.ALIEN_STATE_DIR;
      process.env.ALIEN_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
      ...
      const pluginRoot = path.join(tempHome, ".alien", "extensions", "bundle-probe");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alien-cli-live-cleanup-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.ALIEN_HOME;
      delete process.env.ALIEN_STATE_DIR;
