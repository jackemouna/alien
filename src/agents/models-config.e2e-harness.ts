...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousAlienAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousAlienAgentDir = process.env.ALIEN_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousAlienAgentDir === undefined) {
      delete process.env.ALIEN_AGENT_DIR;
    } else {
      process.env.ALIEN_AGENT_DIR = previousAlienAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "ALIEN_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: AlienConfig = {
  models: {
    providers: {
      "custom-proxy": {
