...
function isTranscriptOnlyAlienAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = normalizeOptionalString(message.provider) ?? "";
  const model = normalizeOptionalString(message.model) ?? "";
  return provider === "alien" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function handleMessageStart(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyAlienAssistantMessage(msg)) {
    return;
  }
