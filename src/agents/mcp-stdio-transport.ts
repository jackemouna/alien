...
export type AlienStdioServerParameters = {
  command: string;
  args?: string[];
...
export class AlienStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: AlienStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "AlienStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
