...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when ALIEN_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const alienHome = path.join(tmpDir, "alien-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(alienHome, { recursive: true });
    ...
    process.env.ALIEN_HOME = alienHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(alienHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
