...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-trajectory-"));
let tempDirId = 0;
...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", async () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".alien",
        "trajectory-exports",
        "alien-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
  ...
  it("uses the recorded runtime pointer before current environment overrides", async () => {
    ...
    const previous = process.env.ALIEN_TRAJECTORY_DIR;
    process.env.ALIEN_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        delete process.env.ALIEN_TRAJECTORY_DIR;
      } else {
        process.env.ALIEN_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", async () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "alien", version: "0.1.0" },
