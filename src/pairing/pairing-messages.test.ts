...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ALIEN_CONTAINER_HINT", "ALIEN_PROFILE"]);
    delete process.env.ALIEN_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:alien|alien) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
