...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.ALIEN_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.ALIEN_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.ALIEN_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.ALIEN_GATEWAY_PASSWORD)),
  });
