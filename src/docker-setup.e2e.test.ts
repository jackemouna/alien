...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "alien-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ALIEN_GATEWAY_TOKEN: "test-token",
    ALIEN_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ALIEN_WORKSPACE_DIR: join(sandbox.rootDir, "alien"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("alien-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    ALIEN_CONFIG_DIR: configDir,
    ALIEN_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ALIEN_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      ALIEN_EXTRA_MOUNTS: undefined,
      ALIEN_HOME_VOLUME: "alien-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ALIEN_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ALIEN_EXTRA_MOUNTS=");
    expect(envFile).toContain("ALIEN_HOME_VOLUME=alien-home"); // pragma: allowlist secret
    expect(envFile).toContain("ALIEN_DISABLE_BONJOUR=");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("alien-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("alien-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ALIEN_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node alien-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
      'run --rm --no-deps --entrypoint node alien-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]',
    );
    expect(log).not.toContain("run --rm alien-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ALIEN_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ALIEN_DISABLE_BONJOUR=0");
  });

  it("avoids shared-network alien-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(prestartLines.some((line) => /\bcompose\b.*\brun\b.*\balien-cli\b/.test(line))).toBe(
      false,
    );
  });
  ...
  it("writes ALIEN_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ALIEN_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ALIEN_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh alien-gateway -c");
  });

  it("reuses existing config token when ALIEN_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ALIEN_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when ALIEN_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "ALIEN_GATEWAY_TOKEN=dotenv-token-123\nALIEN_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ALIEN_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "ALIEN_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ALIEN_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("ALIEN_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats ALIEN_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ALIEN_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ALIEN_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg ALIEN_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ALIEN_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker alien-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        ALIEN_SANDBOX: "1",
        ALIEN_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps alien-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate alien-gateway"));
  ...
  it("rejects injected multiline ALIEN_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ALIEN_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ALIEN_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ALIEN_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ALIEN_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ALIEN_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ALIEN_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ALIEN_HOME_VOLUME must match");
  });

  it("rejects ALIEN_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ALIEN_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when ALIEN_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ALIEN_SKIP_ONBOARDING: "1",
    });
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ALIEN_SKIP_ONBOARDING=1");
  });

  it("treats ALIEN_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ALIEN_SKIP_ONBOARDING: "0",
    });
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/ALIEN_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/ALIEN_DISABLE_BONJOUR: \$\{ALIEN_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:alien-gateway"');
    expect(compose).toContain("depends_on:\n      - alien-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/ALIEN_GATEWAY_TOKEN: \$\{ALIEN_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{ALIEN_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side workspace and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    // canonical container paths so a host-style ALIEN_WORKSPACE_DIR like
    // `/Users/<you>/.alien/workspace` written to `.env` by docker-setup.sh
    // cannot reach runtime code inside Linux Docker.
    expect(compose.match(/ALIEN_CONFIG_DIR: \/home\/node\/\.alien$/gm)).toHaveLength(2);
    expect(
      compose.match(/ALIEN_WORKSPACE_DIR: \/home\/node\/\.alien\/workspace$/gm),
    ).toHaveLength(2);
  });
});
