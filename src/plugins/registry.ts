...
import type {
  ...
  AlienPluginApi,
  AlienPluginChannelRegistration,
  AlienPluginCliCommandDescriptor,
  AlienPluginCliRegistrar,
  AlienPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  AlienPluginGatewayRuntimeScopeSurface,
  AlienGatewayDiscoveryService,
  AlienPluginHttpRouteParams,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@alien/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@alien/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("alien.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<AlienPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<AlienPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<AlienPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: AlienPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: AlienPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: AlienPluginHookOptions | undefined,
    config: AlienPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: AlienPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: AlienPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const registrationCapabilities = resolvePluginRegistrationCapabilities(mode);
    const normalized =
      typeof (registration as AlienPluginChannelRegistration).plugin === "object"
        ? (registration as AlienPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: AlienPluginCliCommandDescriptor[] },
  ) => {
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is AlienPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: AlienPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: AlienPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: AlienPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: AlienPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: AlienPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: AlienPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: AlienGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: AlienPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<AlienPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<AlienPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as AlienConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
