...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/alien-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "alien.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): AlienConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    ALIEN_HOME: path.join(homeDir, "home"),
    ALIEN_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    ALIEN_DISABLE_PERSISTED_PLUGIN_REGISTRY:
