...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "alien", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".alien", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".alien", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "alien-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".alien", "extensions"),
      },
    });
  });
});
