...
export const PLUGIN_MANIFEST_FILENAME = "alien.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `alien qa`, for example `matrix`. */
  commandName: string;
...
// package.json "alien" metadata (used for setup/catalog)
...
export type AlienPackageStartup = {
  /**
...
export type AlienPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type AlienPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: AlienPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
...
): AlienPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
