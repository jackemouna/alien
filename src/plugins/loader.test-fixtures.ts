...
import { clearPluginLoaderCache, loadAlienPlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadAlienPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadAlienPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "alien-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.ALIEN_DISABLE_BUNDLED_PLUGINS;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "alien.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".alien", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ ALIEN_STATE_DIR: stateDir, ...params.env }, () =>
    loadAlienPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.ALIEN_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ALIEN_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
