...
vi.mock("../infra/alien-root.js", () => ({
  resolveAlienPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("alien-plugin-install");
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    alien?: { install?: Record<string, unknown> };
  };
  manifest.alien = {
    ...manifest.alien,
    install: {
      ...manifest.alien?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "alien.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "alien.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(vi.mocked(runCommandWithTimeout)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: expect.stringContaining(".alien-install-stage-"),
      }),
    );
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@alien/voice-call" });
  ...
  it("rejects native plugin zip archives without alien.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid alien.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@alien/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without alien.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@alien/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("alien.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_ALIEN_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when alien.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@alien/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing alien.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_ALIEN_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without alien.extensions");
  });

  it("rejects package installs when alien.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_ALIEN_EXTENSIONS);
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_ALIEN_EXTENSIONS);
      expect(result.error).toContain("alien.runtimeExtensions[0]");
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        alien: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("still scans declared package entrypoints when they live under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        alien: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/alien points at the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-alien");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"alien"}\n');
      vi.mocked(resolveAlienPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "alien-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(hostRoot, path.join(nodeModulesDir, "alien"), "junction");
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/.bin/alien points inside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-alien");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"alien"}\n');
      const hostBin = path.join(hostRoot, "alien.mjs");
      fs.writeFileSync(hostBin, "#!/usr/bin/env node\n");
      vi.mocked(resolveAlienPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "alien-bin-peer-plugin");

      const binDir = path.join(pluginDir, "node_modules", ".bin");
      fs.mkdirSync(binDir, { recursive: true });
      fs.symlinkSync(hostBin, path.join(binDir, "alien"), "file");
  ...
  it.runIf(process.platform !== "win32")(
    "fails package installs when node_modules/alien points outside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-alien");
      const spoofedRoot = path.join(tmpDir, "spoofed-alien");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.mkdirSync(spoofedRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"alien"}\n');
      fs.writeFileSync(path.join(spoofedRoot, "package.json"), '{"name":"alien"}\n');
      vi.mocked(resolveAlienPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "spoofed-alien-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(spoofedRoot, path.join(nodeModulesDir, "alien"), "junction");
      ...
      if (!result.ok) {
        expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED);
        expect(result.error).toContain("node_modules/alien");
      }
    },
  );

  it.runIf(process.platform !== "win32")(
    "fails package installs for nested or non-exact alien node_modules symlinks",
    async () => {
      const cases = [
        {
          pluginName: "nested-alien-peer-plugin",
          relativePath: path.join("node_modules", "vendor", "node_modules", "alien"),
        },
        {
          pluginName: "uppercase-alien-peer-plugin",
          relativePath: path.join("node_modules", "Alien"),
        },
        {
          pluginName: "trailing-space-alien-peer-plugin",
          relativePath: path.join("node_modules", "alien "),
        },
      ] as const;
  ...
  it("fails package installs when manifest traversal exceeds the directory cap", async () => {
    vi.stubEnv("ALIEN_INSTALL_SCAN_MAX_DIRECTORIES", "4");
  ...
  it("fails package installs when manifest traversal exceeds the depth cap", async () => {
    vi.stubEnv("ALIEN_INSTALL_SCAN_MAX_DEPTH", "2");
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveAlienPackageRootSync).mockReturnValue(process.cwd());
...
describe("installPluginFromDir", () => {
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.alien).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires Alien >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json alien.install.minHostVersion"],
    },
    {
  ...
  it("uses alien.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@alien/cognee-alien"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@alien/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkAlienPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveAlienPackageRootSync);
  ...
  it("creates a node_modules/alien symlink when peerDependencies declares alien", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { alien: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "alien");
  ...
  it("keeps the alien peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { alien: "*" }, { "is-number": "7.0.0" });
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "alien");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "alien");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("warns and skips when resolveAlienPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expect(result.ok).toBe(true);
    expect(warnings.some((w) => w.includes("Could not locate alien package root"))).toBe(true);
  });
});
