...
const openAICodexOAuthOriginator = "alien";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Alien process and then retry `alien models auth login --provider openai-codex`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://docs.alien.ai/start/faq", "OAuth help");
