...
vi.mock("./bundled-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveBundledPluginsDir: (env: NodeJS.ProcessEnv = process.env) =>
      env.ALIEN_BUNDLED_PLUGINS_DIR ?? actual.resolveBundledPluginsDir(env),
  };
});
...
function makeTempDir() {
  return makeTrackedTempDir("alien-plugins", tempDirs);
}

const mkdirSafe = mkdirSafeDir;

function withAlienPackageArgv<T>(packageRoot: string, fn: () => T): T {
  mkdirSafe(path.join(packageRoot, "bin"));
  fs.writeFileSync(path.join(packageRoot, "package.json"), '{"name":"alien"}\n', "utf-8");
  const originalArgv = process.argv;
  process.argv = [originalArgv[0] ?? "node", path.join(packageRoot, "bin", "alien")];
  try {
    return fn();
  } finally {
...
const canCreateDirectorySymlinks = (() => {
  const probeDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-symlink-probe-"));
...
function buildDiscoveryEnvWithOverrides(
  ...
  const enablesBundledOverride =
    Object.prototype.hasOwnProperty.call(overrides, "ALIEN_BUNDLED_PLUGINS_DIR") &&
    overrides.ALIEN_BUNDLED_PLUGINS_DIR !== undefined;
  return {
    ...buildDiscoveryEnv(stateDir),
    ...(enablesBundledOverride ? { ALIEN_DISABLE_BUNDLED_PLUGINS: undefined } : {}),
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverAlienPlugins>[0],
) {
  return discoverAlienPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithEnv(params: Parameters<typeof discoverAlienPlugins>[0]) {
  return discoverAlienPlugins(params);
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "alien.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverAlienPlugins>>,
...
describe("discoverAlienPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".alien", "extensions");
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        ...
        packageName: "@alien/linked-plugin",
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        packageDir: linkedPluginDir,
        packageName: "@alien/workspace-linked-plugin",
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@alien/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@alien/stray-workspace-plugin",
    });

    const result = discoverAlienPlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".alien", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not warn about source checkout deps when bundled plugins are disabled", () => {
    ...
    fs.writeFileSync(path.join(extensionDir, "alien.plugin.json"), '{"id":"twitch"}\n', "utf-8");

    const result = withAlienPackageArgv(packageRoot, () =>
      discoverAlienPlugins({ env: buildDiscoveryEnv(stateDir) }),
    );
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "node_modules", "alien");
    ...
    const { candidates, diagnostics } = withAlienPackageArgv(packageRoot, () =>
    ...
    const { candidates, diagnostics } = withAlienPackageArgv(packageRoot, () =>
      discoverAlienPlugins({
        ...
        env: {
          ...buildDiscoveryEnv(stateDir),
          ALIEN_DISABLE_BUNDLED_PLUGINS: undefined,
          ALIEN_BUNDLED_PLUGINS_DIR: bundledRoot,
        },
      }),
    );
  ...
  it("discovers bind-mounted bundled source overlays before packaged dist bundles", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@alien/synology-chat",
  ...
  it("skips source-only TypeScript entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@alien/source-only-pack",
  ...
  it("reuses one filesystem realpath lookup per package root within a discovery run", () => {
    ...
    const { candidates } = discoverAlienPlugins({
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it("uses explicit runtime extension entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@alien/runtime-pack",
      extensions: ["./src/index.ts"],
  ...
  it("rejects missing explicit runtime setup entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@alien/missing-runtime-setup-pack",
  ...
  it("rejects package runtimeExtensions that do not match extension entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@alien/runtime-mismatch-pack",
      extensions: ["./src/one.ts", "./src/two.ts"],
  ...
  it("rejects blank package runtimeExtensions before falling back to inferred entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@alien/runtime-blank-pack",
      extensions: ["./src/index.ts"],
      runtimeExtensions: [" "],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("alien.runtimeExtensions[0]") &&
  ...
  it("infers built dist entries for installed TypeScript package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@alien/built-peer-pack",
      extensions: ["src/index.ts"],
      setupEntry: "src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
  ...
  it("keeps workspace package TypeScript entries unless runtime entries are explicit", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const pluginDir = path.join(workspaceDir, ".alien", "extensions", "workspace-pack");
    mkdirSafe(path.join(pluginDir, "src"));
    mkdirSafe(path.join(pluginDir, "dist"));

    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@alien/workspace-pack",
      extensions: ["./src/index.ts"],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
  ...
  it("discovers source-checkout-only bundled plugins alongside built bundled plugins", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "alien");
    const bundledDir = path.join(packageRoot, "dist", "extensions");
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-alien");
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@opik/opik-alien",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-alien" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "alien"),
      packageName: "alien",
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-alien"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".alien", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "alien");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@alien/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@alien/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@alien/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("alien");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@alien/missing-entry-pack",
  ...
  it.runIf(process.platform !== "win32")("deduplicates blocked candidate diagnostics", () => {
    ...
    try {
      const result = discoverAlienPlugins({
        env: {
          ...buildDiscoveryEnv(stateDir),
          ALIEN_PLUGINS_PATHS: blockedDir,
        },
      });
  ...
  it("discovers bundled and global plugins for each workspace-specific scan", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@alien/bundled-plugin",
      pluginId: "bundled-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "global-plugin"),
      packageName: "@alien/global-plugin",
      pluginId: "global-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceA, ".alien", "extensions", "workspace-a-plugin"),
      packageName: "@alien/workspace-a-plugin",
      pluginId: "workspace-a-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceB, ".alien", "extensions", "workspace-b-plugin"),
      packageName: "@alien/workspace-b-plugin",
    ...
    const first = withAlienPackageArgv(packageRoot, () =>
    ...
    const second = withAlienPackageArgv(packageRoot, () =>
