...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = makeTrackedTempDir("alien-whatsapp-boundary", tempDirs);
  for (const [relativePath, value] of Object.entries({
    ...
    "dist/connection-controller-registry.js": [
      'const key = Symbol.for("alien.whatsapp.connectionControllerRegistry");',
      "const g = globalThis;",
      "if (!g[key]) {",
...
function createExternalTypeScriptRuntimePackageFixture() {
  const rootDir = makeTrackedTempDir("alien-external-boundary-ts", tempDirs);
  writeRuntimeFixtureText(
    rootDir,
    "package.json",
    JSON.stringify(
      {
        name: "alien-external-ts-runtime",
        type: "module",
      },
      null,
      2,
...
describe("runtime plugin boundary whatsapp seam", () => {
  ...
  it("rejects bundled TypeScript runtime modules instead of using the source loader", () => {
    const rootDir = makeTrackedTempDir("alien-bundled-boundary-ts", tempDirs);
