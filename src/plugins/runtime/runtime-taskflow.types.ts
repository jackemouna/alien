...
import type { AlienPluginToolContext } from "../tool-types.js";
...
export type BoundTaskFlowRuntime = {
  ...
  cancel: (params: { flowId: string; cfg: AlienConfig }) => Promise<BoundTaskFlowCancelResult>;
  runTask: (params: {
    flowId: string;
    runtime: TaskRuntime;
    sourceId?: string;
...
export type PluginRuntimeTaskFlow = {
  ...
  fromToolContext: (
    ctx: Pick<AlienPluginToolContext, "sessionKey" | "deliveryContext">,
  ) => BoundTaskFlowRuntime;
};
