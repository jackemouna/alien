...
const mocks = vi.hoisted(() => ({
  loadAlienPlugins: vi.fn<typeof import("../loader.js").loadAlienPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadAlienPlugins: (...args: Parameters<typeof mocks.loadAlienPlugins>) =>
    mocks.loadAlienPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadAlienPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadAlienPlugins>) => mocks.loadAlienPlugins(...args),
    );
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadAlienPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadAlienPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
