...
function collectConfiguredPluginEntryIds(config: AlienConfig): string[] {
...
function resolveRegister(mod: AlienPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: AlienPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as AlienPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: AlienPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Alien ignored the plugin setup runtime entry. Remove setup-api/alien.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
