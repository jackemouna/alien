...
function getConfigPath(config: AlienConfig | undefined, path: string): unknown {
...
function setConfigPath(target: AlienConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: AlienConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: AlienConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: AlienConfig) =>
