...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".alien", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "ALIEN_HOME", "ALIEN_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("alien-bundle-commands-home-");
      const workspaceDir = await createTempDir("alien-bundle-commands-workspace-");
