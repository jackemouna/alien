...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/alien-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/alien-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-config.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-io.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-network.ts",
  "packages/memory-host-sdk/src/host/alien-runtime-session.ts",
] as const;
...
function collectAlienRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./alien-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@alien/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@alien/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@alien/memory-host-sdk");
    ...
    expect(collectAlienRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
