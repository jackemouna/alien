...
const INDIRECT_RUNTIME_DEPENDENCIES = new Map<string, Set<string>>([
  ...
  [
    ...
    // Packaged memory tools run through generated Alien runtime chunks that parse JSON5 config.
    new Set(["json5"]),
  ],
  [
    "extensions/tlon",
...
function readManifestText(root: string): string {
  const manifestPath = path.join(root, "alien.plugin.json");
