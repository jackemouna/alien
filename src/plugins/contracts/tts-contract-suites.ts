...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "alien/plugin-sdk/speech-core";
...
type TtsRuntimeModule = typeof import("alien/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("alien/plugin-sdk/speech-core");
...
function asLegacyTtsConfig(value: unknown): AlienConfig {
  return value as AlienConfig;
}

function asLegacyAlienConfig(value: Record<string, unknown>): AlienConfig {
  return value as unknown as AlienConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): AlienConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("alien/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: AlienConfig): ResolvedTtsConfig {
  const rawConfig =
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: AlienConfig = asLegacyAlienConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ALIEN_TTS_PREFS;
      process.env.ALIEN_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.ALIEN_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: AlienConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
