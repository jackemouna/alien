...
const fixtureRoot = makeTrackedTempDir("alien-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createPluginSdkAliasFixture(params?: {
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "alien.mjs"), "export {};\n", "utf-8");
  }
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@alien/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".alien", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["alien/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["alien/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["alien/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/alien-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/alien-loader.js",
      argv1: (root: string) => path.join(root, "alien.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ ALIEN_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ ALIEN_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an Alien root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ ALIEN_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["alien/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["alien/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["alien/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["alien/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@alien/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@alien/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@alien/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@alien/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@alien/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["alien/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running alien argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "alien.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the alien root
    // (e.g. single-binary distributions or custom process launchers).
    // Use alien.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real alien repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "alien.mjs")).href;
    ...
    // runner binary, inside the alien repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedAlienRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an Alien root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    const { auto, dist, distAgain } = withEnv({ NODE_ENV: undefined }, () => ({
      auto: resolvePluginLoaderModuleConfig({
        ...
        argv1: path.join(fixture.root, "alien.mjs"),
    ...
    expect(fs.realpathSync(auto.aliasMap["alien/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["alien/plugin-sdk"] ?? "")).toBe(
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "alien/plugin-sdk/outbound-send-deps": copiedChannelRuntimeShim,
        "@alien/plugin-sdk/outbound-send-deps": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["alien/plugin-sdk"]).not.toBe(aliasB["alien/plugin-sdk"]);
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ ALIEN_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ ALIEN_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["alien/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["alien/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["alien/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["alien/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "alien/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "alien/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@alien/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
