...
let loadAlienPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/alien.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/alien.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadAlienPluginsMock).toHaveBeenCalledTimes(count);
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadAlienPluginsMock = vi
      .spyOn(loaderModule, "loadAlienPlugins")
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ ALIEN_HOME: "/tmp/alien-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.ALIEN_HOME = "/tmp/alien-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
  ...
  it("does not reuse snapshot provider loads across host Vitest env changes", () => {
    ...
    expect(loadAlienPluginsMock).toHaveBeenCalledTimes(2);
  });
