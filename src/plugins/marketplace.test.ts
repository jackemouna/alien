...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("alien-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("alien-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("alien-marketplace-test-", async (homeDir) => {
      const openClawHome = path.join(homeDir, "alien-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, ALIEN_HOME: openClawHome },
