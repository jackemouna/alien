...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.test.ts:configSchema: (require("alien/plugin-sdk").emptyPluginConfigSchema)(),',
  'src/plugins/loader.test.ts:const { onDiagnosticEvent } = require("alien/plugin-sdk");',
  // Intentional jiti alias regression test.
  'src/plugins/loader.git-path-regression.test.ts:`import { resolveOutboundSendDep } from "alien/plugin-sdk/outbound-send-deps";',
  'src/plugins/loader.git-path-regression.test.ts:          "alien/plugin-sdk/outbound-send-deps": ${JSON.stringify(copiedChannelRuntimeShim)},',
  // Intentional packaged bundled-plugin SDK alias regression tests.
  'src/plugins/loader.test.ts:`import { normalizeLowercaseStringOrEmpty } from "alien/plugin-sdk/text-runtime";`,',
]);
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("alien/plugin-sdk') ||
        (line.includes("import ") && line.includes('"alien/plugin-sdk'))
      ) {
