...
export const DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV = "ALIEN_DISABLE_PERSISTED_PLUGIN_REGISTRY";

function formatDeprecatedPersistedRegistryDisableWarning(): string {
  return `${DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV} is a deprecated break-glass compatibility switch; use \`alien plugins registry --refresh\` or \`alien doctor --fix\` to repair registry state.`;
}
    ...
    if (persistedReadsEnabled && persistedIndex) {
      if (
        ...
        diagnostics.push({
          ...
          message:
            "Persisted plugin registry policy does not match current config; using derived plugin index. Run `alien plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          message:
            "Persisted plugin registry points at missing plugin files; using derived plugin index. Run `alien plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry points at a different bundled plugin tree; using derived plugin index. Run `alien plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry metadata no longer matches plugin manifest or package files; using derived plugin index. Run `alien plugins registry --refresh` to update the persisted registry.",
        });
      } else if (
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry is missing recoverable managed npm plugins; using derived plugin index. Run `alien plugins registry --refresh` to update the persisted registry.",
        });
      } else {
        return {
