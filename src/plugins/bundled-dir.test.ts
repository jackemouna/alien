...
function createAlienRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "alien" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@alien/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.ALIEN_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ALIEN_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "alien.mjs"),
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "alien-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "alien-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "alien-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "alien-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    const repoRoot = createAlienRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createAlienRoot({
      prefix: "alien-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createAlienRoot({
      prefix: "alien-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createAlienRoot({
      prefix: "alien-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "alien.mjs");
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createAlienRoot({
      prefix: "alien-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by ALIEN_HOME", () => {
    const homeA = makeRepoRoot("alien-bundled-dir-home-a-");
    const homeB = makeRepoRoot("alien-bundled-dir-home-b-");
    seedBundledPluginTree(homeA, "bundled", "memory-core");
    seedBundledPluginTree(homeB, "bundled", "discord");
    const envBase = {
      ALIEN_BUNDLED_PLUGINS_DIR: "~/bundled",
      ALIEN_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
      VITEST: "true",
    } satisfies NodeJS.ProcessEnv;

    const bundledA = resolveBundledPluginsDir({ ...envBase, ALIEN_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, ALIEN_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createAlienRoot({
      prefix: "alien-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "alien.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.ALIEN_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("alien-bundled-dir-vitest-override-reject-");
    ...
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createAlienRoot({
      prefix: "alien-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("alien-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("alien-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.ALIEN_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createAlienRoot({
      prefix: "alien-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("alien-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("alien-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "alien.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createAlienRoot({
      prefix: "alien-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createAlienRoot({
          prefix: "alien-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createAlienRoot({
          prefix: "alien-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "alien.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createAlienRoot({
          prefix: "alien-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
