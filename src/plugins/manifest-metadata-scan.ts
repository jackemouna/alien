...
const ALIEN_PACKAGE_ROOT = fileURLToPath(new URL("../..", import.meta.url));
const PLUGIN_MANIFEST_FILENAME = "alien.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.ALIEN_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.ALIEN_STATE_DIR);
  if (override) {
...
function areBundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const value = normalizeTrimmedString(env.ALIEN_DISABLE_BUNDLED_PLUGINS)?.toLowerCase();
...
function resolveBundledPluginRoot(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = normalizeTrimmedString(env.ALIEN_BUNDLED_PLUGINS_DIR);
  if (override) {
    return resolveUserPath(override, env);
  }

  const sourceRoot = path.join(ALIEN_PACKAGE_ROOT, "extensions");
  const runtimeRoot = path.join(ALIEN_PACKAGE_ROOT, "dist-runtime", "extensions");
  const distRoot = path.join(ALIEN_PACKAGE_ROOT, "dist", "extensions");
...
export function listAlienPluginManifestMetadata(
