...
vi.mock("./loader.js", () => ({
  loadAlienPlugins: (...args: unknown[]) => loadAlienPluginsMock(...args),
}));
...
function setPluginLoadResult(overrides: Partial<ReturnType<typeof createPluginLoadResult>>) {
  ...
  loadAlienPluginsMock.mockReturnValue(result);
      ...
      snapshot: createInstalledPluginIndexSnapshot([
        {
          ...
          manifestPath: "/tmp/provider-env-plugin/alien.plugin.json",
