...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as AlienConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as AlienConfig;
    const load = vi.fn(async (config?: AlienConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: AlienConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
