...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-plugin-loader-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("createPluginModuleLoader", () => {
  it("loads bundled JavaScript without creating a module loader", async () => {
    ...
    const { loadAlienPlugins } = await importFreshModule<typeof import("./loader.js")>(
      import.meta.url,
      "./loader.js?scope=native-module-loader",
    );

    const pluginRoot = writeBundledPluginFixture("demo");
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = pluginRoot;
  ...
  it("loads packaged JavaScript without creating a module loader", async () => {
    ...
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = makeTempDir();
