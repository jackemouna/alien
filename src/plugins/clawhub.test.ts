...
async function createClawHubArchive(entries: Record<string, string>) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-clawhub-archive-"));
  tempDirs.push(dir);
...
describe("installPluginFromClawHub", () => {
  ...
  beforeEach(() => {
    ...
    installPluginFromArchiveMock.mockResolvedValue({
      ok: true,
      pluginId: "demo",
      targetDir: "/tmp/alien/plugins/demo",
      version: "2026.3.22",
    });
  });
  ...
  it("marks official source-linked Alien packages as trusted for install scanning", async () => {
  ...
  it("does not let a wildcard plugin API range hide an invalid runtime version", async () => {
    ...
    expect(result).toMatchObject({
      ...
      error: 'Plugin "demo" requires plugin API *, but this Alien runtime exposes invalid.',
    });
  ...
  it("falls back to strict files[] verification when sha256hash is missing", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'ClawHub package "demo@2026.3.22" is missing sha256hash; falling back to files[] verification. Validated files: dist/index.js, alien.plugin.json. Validated generated metadata files present in archive: _meta.json (JSON parse plus slug/version match only).',
    );
  });
  ...
  it("validates _meta.json against canonical package and resolved version metadata", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'ClawHub package "demo@2026.3.22" is missing sha256hash; falling back to files[] verification. Validated files: alien.plugin.json. Validated generated metadata files present in archive: _meta.json (JSON parse plus slug/version match only).',
    );
  });
  ...
  it("omits the skipped-files suffix when no generated extras are present", async () => {
    ...
    expect(result).toMatchObject({ ok: true, pluginId: "demo" });
    expect(logger.warn).toHaveBeenCalledWith(
      'ClawHub package "demo@2026.3.22" is missing sha256hash; falling back to files[] verification. Validated files: alien.plugin.json.',
    );
  });
  ...
  it("rejects fallback verification when _meta.json exceeds the per-file size limit", async () => {
    ...
    const loadAsyncSpy = vi.spyOn(JSZip, "loadAsync").mockResolvedValueOnce({
      files: {
        ...
        "alien.plugin.json": listedFileEntry,
      },
    } as unknown as JSZip);
    fetchClawHubPackageVersionMock.mockResolvedValueOnce({
      version: {
  ...
  it("rejects fallback verification when a file hash drifts from files[] metadata", async () => {
    ...
    expect(result).toMatchObject({
      ok: false,
      code: CLAWHUB_INSTALL_ERROR_CODE.ARCHIVE_INTEGRITY_MISMATCH,
      error: `ClawHub archive contents do not match files[] metadata for "demo@2026.3.22": expected alien.plugin.json to hash to ${"1".repeat(64)}, got ${sha256Hex('{"id":"demo"}')}.`,
    });
    expect(installPluginFromArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects fallback metadata with leading or trailing path whitespace", async () => {
    ...
    expect(result).toMatchObject({
      ...
      code: CLAWHUB_INSTALL_ERROR_CODE.MISSING_ARCHIVE_INTEGRITY,
      error:
        'ClawHub version metadata for "demo@2026.3.22" has an invalid files[0].path (path "alien.plugin.json " has leading or trailing whitespace).',
    });
    expect(downloadClawHubPackageArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects fallback verification when the archive includes a whitespace-suffixed file path", async () => {
    ...
    expect(result).toMatchObject({
      ...
      code: CLAWHUB_INSTALL_ERROR_CODE.ARCHIVE_INTEGRITY_MISMATCH,
      error:
        'ClawHub archive contents do not match files[] metadata for "demo@2026.3.22": invalid package file path "alien.plugin.json " (path "alien.plugin.json " has leading or trailing whitespace).',
    });
    expect(installPluginFromArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects fallback metadata with duplicate files[] paths", async () => {
    ...
    expect(result).toMatchObject({
      ...
      code: CLAWHUB_INSTALL_ERROR_CODE.MISSING_ARCHIVE_INTEGRITY,
      error:
        'ClawHub version metadata for "demo@2026.3.22" has duplicate files[] path "alien.plugin.json".',
    });
    expect(downloadClawHubPackageArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it.each([
    {
      ...
      expected: {
        ...
        code: CLAWHUB_INSTALL_ERROR_CODE.INCOMPATIBLE_PLUGIN_API,
        error:
          'Plugin "demo" requires plugin API >=2026.3.22, but this Alien runtime exposes 2026.3.21.',
      },
    },
    {
      ...
      expected: {
        ...
        error: '"calendar" is a skill. Use "alien skills install calendar" instead.',
      },
    },
    {
