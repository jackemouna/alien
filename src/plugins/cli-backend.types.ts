...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Alien consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Alien should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Alien's bundle
   ...
   *
   * Use this when the backend should consume a canonical Alien auth profile
   ...
   * `profile-only` treats the selected Alien auth profile as the sole auth
   ...
   * as static config, such as mapping Alien thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Alien's tool
