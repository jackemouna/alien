...
function makeTempDir() {
  return makeTrackedTempDir("alien-installed-plugin-index", tempDirs);
}

function writePluginManifest(rootDir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(rootDir, "alien.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
describe("installed plugin index", () => {
  ...
  it("diffs invalidation reasons for manifest, package, source, host, compat, and migration changes", () => {
    ...
    const previous = loadInstalledPluginIndex({
      ...
      env: hermeticEnv({ ALIEN_VERSION: "2026.4.25" }),
    });
    ...
    const current = {
      ...loadInstalledPluginIndex({
        ...
        env: hermeticEnv({ ALIEN_VERSION: "2026.4.26" }),
      }),
