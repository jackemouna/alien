...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return fs
    ...
    .flatMap((dirName) => {
      ...
      const pkg = readJsonFile<AlienPackageShape>(packagePath);
      ...
      return [
        {
          ...
          packageName,
          manifestId,
          installNpmSpec: normalizeText(pkg.alien?.install?.npmSpec),
          channelId: normalizeText(pkg.alien?.channel?.id),
        },
      ];
    });
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@alien/${pluginId}${suffix}`);
}
  ...
  it.each([
    {
      ...
      message: `Bundled extension package names must stay anchored to the manifest id via @alien/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.`,
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match alien.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled alien.install.npmSpec aligned with the package name",
      message:
        "Bundled alien.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Non-packaged bundled plugins are source-only/private and must not advertise alien.install.npmSpec.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled alien.channel.id values must match alien.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
