...
import { listAlienPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  ALIEN_DISABLE_BUNDLED_PLUGINS: process.env.ALIEN_DISABLE_BUNDLED_PLUGINS,
  ALIEN_BUNDLED_PLUGINS_DIR: process.env.ALIEN_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/alien.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reflects manifest edits and state-dir changes on the next lookup", () => {
    ...
    process.env.ALIEN_STATE_DIR = stateDirA;
    process.env.ALIEN_HOME = undefined;
    process.env.ALIEN_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.ALIEN_STATE_DIR = stateDirB;
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listAlienPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listAlienPluginManifestMetadata(process.env)).toHaveLength(1);
