...
const fixtureRoot = makeTrackedTempDir("alien-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "alien.plugin.json");
...
async function loadPlugins(pluginPaths: string[], warnings?: string[]) {
  const { loadAlienPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("alien plugins list");
  });
});
