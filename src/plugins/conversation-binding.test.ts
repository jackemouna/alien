...
const tempRoot = makeTrackedTempDir("alien-plugin-binding", tempDirs);
...
vi.mock("../infra/home-dir.js", async () => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.alien/plugin-binding-approvals.json") {
...
describe("plugin conversation binding approvals", () => {
  ...
  it("persists plugin-owned binding data on approved plugin bindings", async () => {
    const data = {
      ...
      version: 1,
      sessionFile: "/tmp/alien/session.jsonl",
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "alien-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",
