...
function makeTempDir() {
  return makeTrackedTempDir("alien-plugin-registry", tempDirs);
}
...
function createIndex(
  ...
  return {
    ...
    plugins: [
      {
        pluginId,
        manifestPath: path.join(pluginRoot, "alien.plugin.json"),
...
describe("plugin registry facade", () => {
  ...
  it("keeps absolute and home plugin API paths user-resolved", () => {
    ...
    expect(resolvePluginPath("~/alien/plugin.txt", pluginRoot)).toBe(
      resolvePluginPath("~/alien/plugin.txt", undefined),
    );
  });
  ...
  it("resolves contribution owners from a plugin lookup table without rereading manifests", () => {
    ...
    fs.unlinkSync(path.join(rootDir, "alien.plugin.json"));
      ...
      plugins: [
        {
          ...
          manifestPath: path.join(rootDir, "alien.plugin.json"),
          source: path.join(rootDir, "index.ts"),
          rootDir,
        },
      ],
  ...
  it("reads the persisted registry before deriving from discovered candidates", async () => {
    ...
    fs.writeFileSync(
      path.join(persistedRootDir, "alien.plugin.json"),
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(persistedRootDir, "alien.plugin.json"),
            manifestHash: hashFile(path.join(persistedRootDir, "alien.plugin.json")),
  ...
  it("falls back to the derived registry when persisted bundled roots point at another checkout", async () => {
    ...
    await writePersistedInstalledPluginIndex(
      createIndex("persisted", {
        plugins: [
          {
            ...createIndex("persisted").plugins[0],
            manifestPath: path.join(staleBundledRootDir, "alien.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ ALIEN_BUNDLED_PLUGINS_DIR: rootDir }),
    });
  ...
  it("derives fresh config-scoped registries when the persisted registry is missing", () => {
    ...
    createCandidate(rootDir);
    const env = hermeticEnv({ ALIEN_BUNDLED_PLUGINS_DIR: bundledRoot });
    ...
    const manifestReadsAfterFirst = readFileSyncSpy.mock.calls.filter((call) =>
      String(call[0]).endsWith("alien.plugin.json"),
    ).length;
