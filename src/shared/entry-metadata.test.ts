...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://alien.ai " },
        frontmatter: { emoji: "🙂", homepage: "https://example.com" },
      }),
    ).toEqual({
      emoji: "🦀",
      homepage: "https://alien.ai",
    });
  });
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.alien.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
      homepage: "https://docs.alien.ai",
    });
    expect(
      resolveEmojiAndHomepage({
        metadata: { homepage: "   " },
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://alien.ai/install " },
      }),
    ).toEqual({
      homepage: "https://alien.ai/install",
    });
  });
  ...
  it("does not fall back once frontmatter homepage aliases are present but blank", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: {
          homepage: " ",
          website: "https://docs.alien.ai",
          url: "https://alien.ai/install",
        },
      }),
    ).toEqual({});
  });
