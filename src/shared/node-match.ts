...
function isCurrentAlienClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("alien-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentAlienClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentAlienClient(node.clientId)) {
    score += 10;
