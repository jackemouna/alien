...
describe("shared/node-match", () => {
  ...
  it("prefers a unique current Alien client over a legacy clawdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=alien-macos.*node=third-mac.*client=alien-macos/,
    );
  });
