...
export function resolveAlienManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type AlienManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveAlienManifestRequires(
  metadataObj: Record<string, unknown>,
): AlienManifestRequires | undefined {
  const requiresRaw =
...
export function resolveAlienManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveAlienManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedAlienManifestInstallBase = {
...
export function parseAlienManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedAlienManifestInstallBase | undefined {
  ...
  const spec: ParsedAlienManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyAlienManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedAlienManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
