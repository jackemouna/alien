...
type ConfiguredBinding = NonNullable<AlienConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: AlienConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/alien" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
