...
const createTempDir = () => tempDirs.make("alien-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets ALIEN_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "alien",
    });

    expect(env.ALIEN_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("alien");
  });

  it("overrides pre-existing ALIEN_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      ALIEN_SHELL: "wrong",
    });
    expect(env.ALIEN_SHELL).toBe("acp-client");
  ...
  it("preserves ALIEN_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        ALIEN_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["ALIEN_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Alien bridge", () => {
    ...
    expect(env.ALIEN_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.ALIEN_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("ALIEN_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "alien", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "alien", "dist", "entry.js");
    const shimPath = path.join(dir, "alien.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\alien\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/alien-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/alien-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/alien-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
