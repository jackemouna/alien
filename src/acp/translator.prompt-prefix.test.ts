...
describe("acp prompt cwd prefix", () => {
  ...
  async function runPromptAndCaptureRequest(
    ...
    sessionStore.createSession({
      ...
      cwd: options.cwd ?? path.join(os.homedir(), "alien-test"),
    });
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "alien-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]alien-test\]/),
      }),
      { timeoutMs: null },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\alien-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\alien-test]"),
      }),
      { timeoutMs: null },
    );
  });
  ...
  it("injects a system provenance receipt when requested", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        systemProvenanceReceipt: expect.stringContaining("bridge=alien-acp"),
      }),
      { timeoutMs: null },
    );
    expect(requestSpy).toHaveBeenCalledWith(
  ...
  it("retries without provenance when the gateway rejects admin-only provenance fields", async () => {
    ...
    sessionStore.createSession({
      sessionId: TEST_SESSION_ID,
      sessionKey: TEST_SESSION_KEY,
      cwd: path.join(os.homedir(), "alien-test"),
    });
    const agent = new AcpGatewayAgent(
      createAcpConnection(),
      createAcpGateway(requestSpy as unknown as GatewayClient["request"]),
