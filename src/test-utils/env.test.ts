...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "ALIEN_ENV_TEST_A";
    const keyB = "ALIEN_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "ALIEN_ENV_TEST_ADDED";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "ALIEN_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "ALIEN_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "ALIEN_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "ALIEN_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "ALIEN_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "alien-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousAlienHome = process.env.ALIEN_HOME;
    const previousStateDir = process.env.ALIEN_STATE_DIR;
    const previousBundledDir = process.env.ALIEN_BUNDLED_PLUGINS_DIR;
    process.env.ALIEN_HOME = "/srv/alien-home";
    process.env.ALIEN_STATE_DIR = "/srv/alien-state";
    process.env.ALIEN_BUNDLED_PLUGINS_DIR = "/srv/alien-bundled";
    ...
    try {
      ...
      expect(env.ALIEN_HOME).toBeUndefined();
      expect(env.ALIEN_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.ALIEN_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("ALIEN_HOME", previousAlienHome);
      restoreEnvKey("ALIEN_STATE_DIR", previousStateDir);
      restoreEnvKey("ALIEN_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processAlienHome: process.env.ALIEN_HOME,
          processBundledDir: process.env.ALIEN_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.ALIEN_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processAlienHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.ALIEN_HOME).toBe("/srv/alien-home");
    } finally {
      restoreEnvKey("ALIEN_HOME", previousAlienHome);
    }
  });
