...
function findBundledPluginMetadataFast(
  ...
  const roots = [
    ...
    path.resolve(ALIEN_PACKAGE_ROOT, "extensions"),
    path.resolve(ALIEN_PACKAGE_ROOT, "dist-runtime", "extensions"),
    path.resolve(ALIEN_PACKAGE_ROOT, "dist", "extensions"),
  ].filter(
...
export function resolveRelativeExtensionPublicModuleId(params: {
  ...
  const fromFilePath = fileURLToPath(params.fromModuleUrl);
  const targetPath = resolveVitestSourceModulePath(
    path.resolve(ALIEN_PACKAGE_ROOT, "extensions", params.dirName, params.artifactBasename),
  );
  const relativePath = path
    .relative(path.dirname(fromFilePath), targetPath)
    .replaceAll(path.sep, "/");
