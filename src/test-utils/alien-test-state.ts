...
type AlienTestStateLayout = "home" | "state-only" | "split";

type AlienTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type AlienTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: AlienTestStateLayout;
  scenario?: AlienTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "alien-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "alien.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "alien.json"),
...
function scenarioConfig(options: AlienTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "alien-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: AlienTestStateOptions): Record<string, string | undefined> {
...
export async function createAlienTestState(
  options: AlienTestStateOptions = {},
): Promise<AlienTestState> {
...
export async function withAlienTestState<T>(
  options: AlienTestStateOptions,
  fn: (state: AlienTestState) => Promise<T>,
): Promise<T> {
  const state = await createAlienTestState(options);
  try {
