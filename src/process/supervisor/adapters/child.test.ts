...
describe("createChildAdapter", () => {
  const originalServiceMarker = process.env.ALIEN_SERVICE_MARKER;
  ...
  afterAll(() => {
    if (originalServiceMarker === undefined) {
      ...
      process.env.ALIEN_SERVICE_MARKER = originalServiceMarker;
    }
  });
  ...
  it("uses process-tree kill for default SIGKILL", async () => {
    ...
    const expectedDetached = process.platform !== "win32" && !process.env.ALIEN_SERVICE_MARKER;
  ...
  it("passes detached:false in service-managed mode where useDetached is false from the start (#71662)", async () => {
    process.env.ALIEN_SERVICE_MARKER = "1";
    try {
  ...
  it("disables detached mode in service-managed runtime", async () => {
    process.env.ALIEN_SERVICE_MARKER = "alien";
