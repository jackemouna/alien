...
describe("wrapArgvForChildOomScoreRaise", () => {
  ...
  it("respects the ALIEN_CHILD_OOM_SCORE_ADJ opt-out", () => {
    for (const value of ["0", "false", "FALSE", "no", "off"]) {
      expect(
        wrapArgvForChildOomScoreRaise(argv, {
          ...linux,
          env: { ALIEN_CHILD_OOM_SCORE_ADJ: value },
        }),
      ).toEqual(argv);
    }
  });
...
describe("hardenedEnvForChildOomWrap", () => {
  ...
  it("preserves baseEnv (including undefined) when wrap does not apply", () => {
    ...
    expect(
      hardenedEnvForChildOomWrap(tainted, { ...linux, env: { ALIEN_CHILD_OOM_SCORE_ADJ: "0" } }),
    ).toBe(tainted);
  });
});
