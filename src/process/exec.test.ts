...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.ALIEN_BASE_ENV).toBe("base");
    expect(resolved.ALIEN_TEST_ENV).toBe("ok");
    expect(resolved.ALIEN_TO_REMOVE).toBeUndefined();
    expect(resolved.ALIEN_CLI).toBe(ALIEN_CLI_ENV_VALUE);
  });
