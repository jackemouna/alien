...
function resolveAgentEntry(cfg: AlienConfig, agentId?: string) {
  if (!agentId) {
    return undefined;
  }
...
function resolveScopedExecConfig(cfg: AlienConfig, agentId?: string) {
...
function isYoloHostPosture(cfg: AlienConfig, agentId?: string): boolean {
...
export function resolveCrestodianRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Crestodian rescue is blocked because Alien sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  ...
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Crestodian rescue only accepts commands from an Alien owner.",
    };
  }
