...
const runLive =
  truthy(process.env.ALIEN_LIVE_TEST) &&
  truthy(process.env.ALIEN_LIVE_CRESTODIAN_RESCUE_CHANNEL);
const describeLive = runLive ? describe : describe.skip;

function commandContext(channel = process.env.ALIEN_LIVE_CRESTODIAN_CHANNEL ?? "whatsapp") {
  return {
    surface: channel,
    channel,
    channelId: channel,
...
describeLive("Crestodian live rescue channel smoke", () => {
  ...
  it("handles /crestodian status and a persistent approval roundtrip", async () => {
    ...
    vi.stubEnv("ALIEN_CONFIG_PATH", configPath);
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
