...
const SETUP_RE =
  /^(?:setup(?!\s+agent\b)|set\s+me\s+up|set\s+up\s+alien|onboard|onboard\s+me|bootstrap|first\s+run)(?:\b|$)/i;
...
function formatSetupPlanDescription(
  ...
  return `bootstrap Alien setup for workspace ${workspace}${model}`;
}
