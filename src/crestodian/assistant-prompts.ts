...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, Alien's ring-zero setup helper.",
  "Turn the user's request into exactly one safe Alien Crestodian command.",
  ...
  "Use the provided Alien docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `Alien docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `Alien source: ${
