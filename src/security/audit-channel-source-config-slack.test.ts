...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: AlienConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: AlienConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: AlienConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): AlienConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: AlienConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
