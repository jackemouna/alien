...
function summarizeGroupPolicy(cfg: AlienConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: AlienConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AlienConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AlienConfig): boolean {
...
function isBrowserEnabled(cfg: AlienConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: AlienConfig): SecurityAuditFinding[] {
