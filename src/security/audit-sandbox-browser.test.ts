  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    const findings = await collectSandboxBrowserHashLabelFindings({
      execDockerRawFn: async (args: string[]) => {
        if (args[0] === "ps") {
          return {
            stdout: Buffer.from("alien-sbx-browser-old\nalien-sbx-browser-missing-hash\n"),
            stderr: Buffer.alloc(0),
            code: 0,
          };
        }
        if (args[0] === "inspect" && args.at(-1) === "alien-sbx-browser-old") {
          return {
        ...
        if (args[0] === "inspect" && args.at(-1) === "alien-sbx-browser-missing-hash") {
          return {
    ...
    expect(staleEpoch?.detail).toContain("alien-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    const findings = await collectSandboxBrowserHashLabelFindings({
      execDockerRawFn: async (args: string[]) => {
        if (args[0] === "ps") {
          return {
            stdout: Buffer.from("alien-sbx-browser-exposed\n"),
            stderr: Buffer.alloc(0),
            code: 0,
          };
        }
        if (args[0] === "inspect" && args.at(-1) === "alien-sbx-browser-exposed") {
          return {
        ...
        if (args[0] === "port" && args.at(-1) === "alien-sbx-browser-exposed") {
          return {
