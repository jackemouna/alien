...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("alien status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("alien security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ALIEN_GATEWAY_TOKEN/ALIEN_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("alien security audit --deep")}".`,
    });
  }
  return findings;
}
