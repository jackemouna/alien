...
import type { AlienConfig, ConfigFileSnapshot } from "../config/config.js";
...
async function listSandboxBrowserContainers(
  ...
  try {
    const result = await execDockerRawFn(
      ["ps", "-a", "--filter", "label=alien.sandboxBrowser=1", "--format", "{{.Names}}"],
      { allowFailure: true },
    );
    if (result.code !== 0) {
      return null;
  ...
  try {
    const result = await params.execDockerRawFn(
      [
        "inspect",
        "-f",
        '{{ index .Config.Labels "alien.configHash" }}\t{{ index .Config.Labels "alien.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("alien sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected alien.browserConfigEpoch=${browserHashEpoch}.`,
      remediation: `${formatCliCommand("alien sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("alien sandbox recreate --browser --all")} (add --force to skip prompt), ` +
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `alien security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    try {
      ...
      findings.push({
        ...
        detail:
          ...
          "The extension entrypoint list is unavailable. Deep scan will cover the plugin directory but may miss entries declared via `alien.extensions`.",
        remediation:
          "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your Alien extensions state directory.",
      });
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all alien.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Alien extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (resolveSkillSource(entry.skill) === "alien-bundled") {
        continue;
      }
      ...
      const summary = await getCodeSafetySummary({
        ...
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `alien security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
