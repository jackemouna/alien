...
export async function expectPrivateQaLabRuntimeSurfaceLoad(params: {
  ...
  importRuntime: () => Promise<QaRuntimeModule>;
  loadBundledPluginPublicSurfaceModuleSync: SurfaceLoaderMock;
  resolveAlienPackageRootSync: SurfaceLoaderMock;
}) {
  const sourceRoot = makePrivateQaSourceRoot(params.tempDirs, "alien-qa-runtime-root-");
  params.resolveAlienPackageRootSync.mockReturnValue(sourceRoot);
