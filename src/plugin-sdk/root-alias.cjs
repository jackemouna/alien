...
const pluginSdkPackageNames = ["alien/plugin-sdk", "@alien/plugin-sdk"];
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.ALIEN_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("alien/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.ALIEN_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.ALIEN_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled Alien core graph.
