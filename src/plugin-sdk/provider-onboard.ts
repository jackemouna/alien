...
export type { AlienConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: AlienConfig, ...args: TArgs) => AlienConfig;
  applyConfig: (cfg: AlienConfig, ...args: TArgs) => AlienConfig;
};
...
>(params: {
  ...
  applyPreset: (cfg: AlienConfig, preset: TParams) => AlienConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: AlienConfig): {
  next: AlienConfig;
  changed: boolean;
} {
