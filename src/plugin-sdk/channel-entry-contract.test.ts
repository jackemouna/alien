...
import type { AlienPluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
function createApi(registrationMode: PluginRegistrationMode): AlienPluginApi {
  return {
    registrationMode,
...
describe("defineBundledChannelEntry", () => {
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-entry-tools-"));
    ...
    const registerCliMetadata = vi.fn<(api: AlienPluginApi) => void>();
    const registerFull = vi.fn<(api: AlienPluginApi) => void>((api) => {
      api.registerTool(
        {
          name: "channel_tool",
          label: "Channel Tool",
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-bundled-entry-runtime-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    ...
    const registerCliMetadata = vi.fn<(api: AlienPluginApi) => void>();
    const registerFull = vi.fn<(api: AlienPluginApi) => void>();
    const entry = createBundledChannelEntry({
      importerPath,
      pluginId,
      registerCliMetadata,
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("ALIEN_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-channel-entry-contract-"));
    tempDirs.push(tempRoot);
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openRootFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\alien\\dist\\extensions\\feishu\\helper.ts",
    ...
    try {
      ...
      expect(
        channelEntryContract.loadBundledEntryExportSync<number>(
          "file:///C:/Users/alice/alien/dist/extensions/feishu/index.js",
          {
            specifier: "./helper.ts",
            exportName: "load",
          },
      ...
      expect(jitiLoad).toHaveBeenCalledWith(
        "file:///C:/Users/alice/alien/dist/extensions/feishu/helper.ts",
      );
    } finally {
      platformSpy.mockRestore();
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"alien"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("ALIEN_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
