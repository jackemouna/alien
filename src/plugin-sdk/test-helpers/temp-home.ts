...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ALIEN_HOME.
  delete process.env.ALIEN_HOME;
  process.env.ALIEN_STATE_DIR = path.join(base, ".alien");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "alien-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".alien", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
