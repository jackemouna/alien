...
export const ALIEN_LIVE_TRANSCRIPT_MARKER_RE = /open(?:claw|cl|flaw|clar|core)/;

export function expectAlienLiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(ALIEN_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? ALIEN_LIVE_TRANSCRIPT_MARKER_RE;
