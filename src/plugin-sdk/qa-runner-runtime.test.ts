...
const resolveAlienPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.ALIEN_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveAlienPackageRootSync.mockReset().mockReturnValue(null);
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "alien-qa-test-api-root-");
    resolveAlienPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
      ...
      env: expect.objectContaining({
        ...
        ALIEN_BUNDLED_PLUGINS_DIR: path.join(sourceRoot, "extensions"),
      }),
    });
  });
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "alien-qa-runner-root-");
    resolveAlienPackageRootSync.mockReturnValue(sourceRoot);
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in alien.plugin.json',
    );
  });
});
