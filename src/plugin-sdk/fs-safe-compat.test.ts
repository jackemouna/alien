...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "alien/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "alien/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "alien/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "alien-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "alien-sdk-file-access-compat-" }, async (root) => {
