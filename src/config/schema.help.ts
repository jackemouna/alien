...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by Alien to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when Alien writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for Alien. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "Alien version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping Alien runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "gateway.handshakeTimeoutMs":
    "Pre-auth Gateway WebSocket handshake timeout in milliseconds. Use higher values on loaded or low-powered hosts where local clients can connect during startup warmup. ALIEN_HANDSHAKE_TIMEOUT_MS still takes precedence.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default Alien execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default Alien runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this Alien agent (for example codex, claude, cursor, gemini, alien).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "alien" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for Chrome DevTools MCP attachment on the selected host or browser node.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as alien.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: alien-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /alien).",
  ...
  "gateway.controlUi.allowExternalEmbedUrls":
    "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__alien__/canvas and /__alien__/a2ui documents do not need it.",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `alien security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, Alien enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "audio.transcription.command":
    'Executable + args used to transcribe audio (first token must be a safe binary/path), for example `["whisper-cli", "--model", "small", "{{MediaPath}}"]`. Deprecated `{input}` placeholders are migrated to `{{MediaPath}}` by `alien doctor --fix`.',
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $ALIEN_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether Alien injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "agents.defaults.experimental.localModelLean":
    "Experimental local-model prompt trim. When enabled, Alien drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.alien/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses Alien memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "agents.defaults.agentRuntime":
    "Default agent runtime policy. Omitted id uses built-in Alien Pi. Use id=auto for plugin harness selection, a registered harness id such as codex, or a supported CLI backend alias such as claude-cli.",
  "agents.defaults.agentRuntime.id":
    "Agent runtime id: pi, auto, a registered plugin harness id such as codex, or a supported CLI backend alias such as claude-cli. Omitted id uses built-in Alien Pi.",
  "agents.defaults.embeddedHarness":
    "Legacy input for agents.defaults.agentRuntime. Run alien doctor --fix to rewrite it to agentRuntime.",
  ...
  "agents.list.*.agentRuntime.id":
    "Per-agent agent runtime id: pi, auto, a registered plugin harness id such as codex, or a supported CLI backend alias such as claude-cli. Omitted id inherits the default Alien Pi behavior.",
  "agents.list.*.embeddedHarness":
    "Legacy input for agents.list.*.agentRuntime. Run alien doctor --fix to rewrite it to agentRuntime.",
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional Pi tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, Alien reuses existing precheck recovery to truncate tool results or compact before retrying.",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in Alien sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage Alien MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by Alien. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. Alien stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  ...
  "session.maintenance.rotateBytes":
    'Deprecated and ignored. Do not use for `sessions.json` growth control; Alien no longer creates automatic rotation backups, and "alien doctor --fix" removes this key.',
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in Alien. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
