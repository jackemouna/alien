...
type AlienMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_ALIEN_TRANSPORT: Record<string, AlienMcpHttpTransport | "stdio"> = {
...
export function resolveAlienMcpTransportAlias(
  value: unknown,
): AlienMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_ALIEN_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_ALIEN_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveAlienMcpTransportAlias(next.type);
