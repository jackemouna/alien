...
function collectConfigEnvVarsByTarget(cfg?: AlienConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: AlienConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: AlienConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ALIEN_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
