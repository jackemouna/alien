...
function makeTempDir(): string {
  ...
  const dir = fs.mkdtempSync(path.join(trustedRoot, ".alien-plugin-prefer-over-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin auto-enable preferOver", () => {
  it("tolerates bundled channel id metadata drift during auto-enable", async () => {
    ...
    vi.stubEnv("ALIEN_BUNDLED_PLUGINS_DIR", rootDir);
    ...
    vi.stubEnv("ALIEN_BUNDLED_PLUGINS_DIR", path.join(rootDir, "missing"));
    await setBundledPluginsDirFixture(undefined);
    ...
    const result = materializePluginAutoEnableCandidates({
      ...
      env: {
        ...
        ALIEN_BUNDLED_PLUGINS_DIR: path.join(rootDir, "missing"),
      },
