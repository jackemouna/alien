...
export const ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV =
  "ALIEN_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS";
...
type FutureConfigGuardParams = {
  ...
  config?: Pick<AlienConfig, "meta"> | null;
  currentVersion?: string;
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this Alien binary (${currentVersion}) is older than the config last written by Alien ${touchedVersion}.`,
    hints: [
      "Run the newer alien binary on PATH, or reinstall the intended gateway service from the newer install.",
