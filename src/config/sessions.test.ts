...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ALIEN_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-alien",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-alien#general");
  });
  ...
  it("derives session transcripts dir from ALIEN_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ALIEN_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ALIEN_STATE_DIR differs from stored paths", () => {
