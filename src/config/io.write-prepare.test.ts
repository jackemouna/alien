...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "alien-web-search": {
                source: "npm",
                spec: "@ollama/alien-web-search",
                installPath: "/tmp/alien-web-search",
                resolvedName: "@ollama/alien-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "alien-web-search": {
                source: "npm",
                spec: "@ollama/alien-web-search@0.2.2",
                installPath: "/tmp/alien-web-search",
                resolvedName: "@ollama/alien-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("alien config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('alien config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/alien.json",
        expectedConfigPath: "/tmp/alien.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: AlienConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://alien.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
