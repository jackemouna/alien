...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.alien) || !isRecord(entry.alien.channel)) {
      continue;
    }
    const channel = entry.alien.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: AlienConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: AlienConfig, pluginId: string) => boolean;
