...
import { buildWebSearchProviderConfig, withTempHome, writeAlienConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    if (result.success) {
      expect(result.data.$schema).toBe("https://alien.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AlienSchema.safeParse({ $schema: 123 });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://alien.ai/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@alien"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = AlienSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeAlienConfig(home, {
        gateway: { bind: "${ALIEN_BIND}" },
      });

      const prev = process.env.ALIEN_BIND;
      process.env.ALIEN_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.ALIEN_BIND;
        } else {
          process.env.ALIEN_BIND = prev;
        }
      }
    });
  });
