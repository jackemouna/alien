...
// Keep integration tests deterministic: never read a real alien.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "alien-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.ALIEN_SESSION_CACHE_TTL_MS;
    process.env.ALIEN_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.ALIEN_SESSION_CACHE_TTL_MS;
    } else {
      process.env.ALIEN_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"alien-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"alien-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
