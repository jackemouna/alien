...
function hasConfiguredEmbeddedHarnessRuntime(cfg: AlienConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: AlienConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: AlienConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: AlienConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: AlienConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: AlienConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: AlienConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: AlienConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: AlienConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: AlienConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: AlienConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: AlienConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: AlienConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: AlienConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: AlienConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: AlienConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: AlienConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: AlienConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: AlienConfig): boolean {
  return (
...
function hasPluginEntries(cfg: AlienConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: AlienConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: AlienConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: AlienConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: AlienConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: AlienConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AlienConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: AlienConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: AlienConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
