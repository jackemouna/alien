...
export function makeTempDir(): string {
  return makeTrackedTempDir("alien-plugin-auto-enable", tempDirs);
}
...
export function makeIsolatedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  return {
    ...
    ALIEN_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/alien.plugin.json`,
    })),
    diagnostics: [],
  };
}
