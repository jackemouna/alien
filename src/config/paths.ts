...
import type { AlienConfig } from "./types.js";

/**
 * Nix mode detection: When ALIEN_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
const CONFIG_FILENAME = "alien.json";
...
/** Build a homedir thunk that respects ALIEN_HOME for the given env. */
 ...
 * Can be overridden via ALIEN_STATE_DIR.
 * Default: ~/.alien
 */
 ...
 * outside the config directory. Set via `ALIEN_INCLUDE_ROOTS` as a
 ...
 * directory containing `alien.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.ALIEN_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via ALIEN_CONFIG_PATH.
 * Default: ~/.alien/alien.json (or $ALIEN_STATE_DIR/alien.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.ALIEN_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.ALIEN_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.ALIEN_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const alienStateDir = env.ALIEN_STATE_DIR?.trim();
  if (alienStateDir) {
    const resolved = resolveUserPath(alienStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/alien-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `alien-${uid}` : "alien";
 ...
 *
 * Precedence:
 * - `ALIEN_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.ALIEN_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.ALIEN_GATEWAY_PORT?.trim();
