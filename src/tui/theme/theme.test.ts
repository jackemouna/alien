...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      ALIEN_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when ALIEN_THEME=light", async () => {
    const mod = await importThemeWithEnv({ ALIEN_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when ALIEN_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ ALIEN_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats ALIEN_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ ALIEN_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("ALIEN_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      ALIEN_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ ALIEN_THEME: "light" });
    const darkMod = await importThemeWithEnv({ ALIEN_THEME: "dark" });
