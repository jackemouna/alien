...
import type { GatewayBindMode, GatewayTailscaleMode, AlienConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.ALIEN_GATEWAY_TOKEN)) ||
        randomToken();
