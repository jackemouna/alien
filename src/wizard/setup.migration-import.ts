...
function hasMeaningfulConfig(config: AlienConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.ALIEN_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh Alien setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: AlienConfig) => Promise<AlienConfig>;
}): Promise<void> {
  const [
  ...
  await params.prompter.outro("Migration complete. Run `alien doctor` next.");
}
