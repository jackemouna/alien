...
function makeConfig(): AlienConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveSetupSecretInputString", () => {
  it("resolves env-template SecretInput strings", async () => {
    const resolved = await resolveSetupSecretInputString({
      config: makeConfig(),
      value: "${ALIEN_GATEWAY_PASSWORD}",
      path: "gateway.auth.password",
      env: {
        ALIEN_GATEWAY_PASSWORD: "gateway-secret", // pragma: allowlist secret
      },
    });
  ...
  it("throws with path context when env-template SecretRef cannot resolve", async () => {
    await expect(
      ...
      'gateway.auth.password: failed to resolve SecretRef "env:default:ALIEN_GATEWAY_PASSWORD"',
    );
  });
});
