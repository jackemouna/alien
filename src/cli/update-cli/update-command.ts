...
const POST_CORE_UPDATE_ENV = "ALIEN_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "ALIEN_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "ALIEN_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "ALIEN_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "ALIEN_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_RESULT_POLL_MS = 100;
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "ALIEN_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
...
const POST_UPDATE_PLUGIN_REPAIR_GUIDANCE = "Run alien doctor --fix to attempt automatic repair.";
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run alien plugins inspect ${pluginId} --runtime --json for details.`;
}
...
function formatPostUpdateGatewayRecoveryInstructions(result: UpdateRunResult): string[] {
  const lines = [
    `Recovery: run \`${replaceCliName(formatCliCommand("alien gateway restart"), CLI_NAME)}\`; if macOS reports the LaunchAgent is installed but not loaded, run \`${replaceCliName(formatCliCommand("alien gateway install --force"), CLI_NAME)}\` from the logged-in user session, then rerun \`${replaceCliName(formatCliCommand("alien gateway status --deep"), CLI_NAME)}\`.`,
  ];
  const beforeVersion = normalizeOptionalString(result.before?.version);
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall Alien ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("alien gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `alien update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("alien update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
function isRunningInsideGatewayService(
  ...
  if (env.ALIEN_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.ALIEN_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `Node ${process.versions.node ?? "unknown"} is too old for alien@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    "Upgrade Node to 22.14+ or Node 24, then rerun `alien update`.",
    "Bare `npm i -g alien` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g alien@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.ALIEN_SERVICE_MARKER;
  delete resolvedEnv.ALIEN_SERVICE_KIND;
  return resolvedEnv;
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("alien completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (expectedGatewayVersion: string | undefined) => {
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("alien gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("alien gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (isPackageManagerUpdateMode(params.result.mode)) {
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("alien doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("alien gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("alien gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-update-post-core-"));
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  const devTargetRef =
    channel === "dev" ? process.env.ALIEN_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Alien..."));
    defaultRuntime.log("");
  }
    ...
    if (shouldBlockPackageUpdateFromGatewayServiceEnv({ prePackageServiceStop })) {
      ...
      defaultRuntime.error(
        [
          ...
          "That path replaces the active Alien dist tree while the live gateway may still lazy-load old chunks.",
          `Run \`${replaceCliName(formatCliCommand("alien update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`,
        ].join("\n"),
      );
      defaultRuntime.exit(1);
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `alien update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Alien install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("alien doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("alien gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g alien@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g alien@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
