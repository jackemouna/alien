...
const ALIEN_REPO_URL = "https://github.com/alien/alien.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "alien";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["alien", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.ALIEN_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "alien");
  }
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ALIEN_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ALIEN_GIT_DIR points at a non-git directory: ${params.dir}. Set ALIEN_GIT_DIR to an empty folder or an alien checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ALIEN_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
const COMPLETION_CACHE_MANUAL_REFRESH_HINT =
  "Shell tab-completion may be stale; refresh manually with: alien completion --write-state";

export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "alien.mjs");
