...
const STEP_LABELS: Record<string, string> = {
  ...
  "alien doctor entry": "Checking doctor entrypoint",
  "alien doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g alien@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g alien@latest --omit=optional");
  }
