...
vi.mock("node:child_process", async () => {
  const { mockNodeBuiltinModule } = await import("alien/plugin-sdk/test-node-mocks");
...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-AlienScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-AlienSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "alien restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-AlienListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-AlienSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        ALIEN_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'alien-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses ALIEN_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ALIEN_PROFILE: "default",
        ALIEN_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("alien-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        ALIEN_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        ALIEN_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped alien gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart alien-gateway.service");
      expect(calls).toContain("--user is-active --quiet alien-gateway.service");
      expect(calls).toContain("is-active --quiet alien-gateway.service");
      expect(calls).not.toContain("--user restart alien-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.alien.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.alien.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.alien/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.alien/logs/gateway-restart.log' 2>&1");
    ...
    it("uses ALIEN_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/alien-state/logs' 2>/dev/null && : >>'/tmp/alien-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/alien-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("alien restart attempt source=update target=ai.alien.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.alien.gateway");
      expect(log).toContain("alien restart failed source=update status=42");
      expect(log).not.toContain("alien restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        ALIEN_LAUNCHD_LABEL: "ai.alien.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        ALIEN_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.alien.$(echo injected)");
      expect(log).not.toContain("target=ai.alien.injected");
    });

    it("uses ALIEN_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        ALIEN_PROFILE: "default",
        ALIEN_LAUNCHD_LABEL: "com.custom.alien",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.alien'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Alien Gateway'");
      expect(content).toContain("function Invoke-AlienSchtasksWithTimeout");
      expect(content).toContain("function Get-AlienScheduledTaskState");
      expect(content).toContain("function Invoke-AlienStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("alien restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".alien\\gateway.cmd"',
      );
      expect(content).toContain("alien restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses ALIEN_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        ALIEN_PROFILE: "default",
        ALIEN_WINDOWS_TASK_NAME: "Alien Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'Alien Gateway (custom)'");
      expect(content).toContain("Get-AlienScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-AlienSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-AlienStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ALIEN_PROFILE: "production",
      });
      expect(content).toContain("alien-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        ALIEN_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.alien.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'Alien Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        ALIEN_LAUNCHD_LABEL: "ai.alien.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.alien.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
