...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.alien${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ALIEN_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.ALIEN_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.ALIEN_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.ALIEN_CONFIG_PATH)) {
    env.ALIEN_CONFIG_PATH = path.join(stateDir, "alien.json");
  }

  if (profile === "dev" && !env.ALIEN_GATEWAY_PORT?.trim()) {
    env.ALIEN_GATEWAY_PORT = "19001";
  }
}
