...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "alien", "-V"] },
    { argv: ["node", "alien", "tui"] },
    { argv: ["node", "alien", "terminal"] },
    { argv: ["node", "alien", "chat"] },
    { argv: ["node", "alien", "gateway"] },
    { argv: ["node", "alien", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "alien", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "alien", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "alien", "gateway", "status"] },
    { argv: ["node", "alien", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "alien", "--help"] },
    { argv: ["node", "alien", "gateway"] },
    { argv: ["node", "alien", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "alien.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("alien.internal");
    } finally {
      log.mockRestore();
    }
  });
