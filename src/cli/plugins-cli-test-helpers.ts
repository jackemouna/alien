...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as AlienConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: AlienConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/alien-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: AlienConfig) =>
    invokeMock<
      [AlienConfig],
...
export function resetPluginsCliTestState() {
  ...
  resolveStateDir.mockReturnValue("/tmp/alien-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation(((cfg: AlienConfig, pluginId: string) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })) as (...args: unknown[]) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: AlienConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: AlienConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
