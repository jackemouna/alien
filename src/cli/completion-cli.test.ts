...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_alien_gateway()");
    expect(script).toContain("(status) _alien_gateway_status ;;");
    expect(script).toContain("(restart) _alien_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "alien.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[alien]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_alien_register_completion]}" = "_alien_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _alien_register_completion
            [[ -z "\${precmd_functions[(r)_alien_register_completion]}" ]] || exit 12
            [[ "\${_comps[alien]-}" = "_alien_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'alien gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c alien -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c alien -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c alien -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
