...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+alien\b|^alien\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+alien\b.*(?:^|\s)update(?:\s|$)|^alien\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const rawContainer = env.ALIEN_CONTAINER_HINT?.trim();
  const container = rawContainer && CONTAINER_HINT_RE.test(rawContainer) ? rawContainer : undefined;
  const profile = normalizeProfileName(env.ALIEN_PROFILE);
