...
const CLI_STATE_ROOT = "/tmp/alien";
const ALPHA_INSTALL_PATH = installedPluginRoot(CLI_STATE_ROOT, "alpha");

describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@alien/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@alien/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@alien/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@alien/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @alien/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@alien/plugin-alpha",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @alien/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@alien/plugin-alpha@latest",
      installPath: ALPHA_INSTALL_PATH,
      version: "1.2.3",
