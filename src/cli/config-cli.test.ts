...
import type { ConfigFileSnapshot, AlienConfig } from "../config/types.js";
import { createCliRuntimeCapture, mockRuntimeModule } from "./test-runtime-capture.js";

/**
 * Test for issue #6070:
 * `alien config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: AlienConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (cfg: AlienConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: AlienConfig, config: AlienConfig) {
...
function withRuntimeDefaults(resolved: AlienConfig): AlienConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-alien.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: AlienConfig = {
    ...
    it("rejects plugin install record config updates", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
          'plugins.installs["alien-web-search"].spec',
          '"@ollama/alien-web-search@0.2.2"',
          "--strict-json",
          "--dry-run",
        ]),
      ...
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("alien plugins install <spec>"),
      );
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("alien plugins update <plugin-id>"),
      );
    });
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__ALIEN_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-alien.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: AlienConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("alien config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "alien config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `alien-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("batch-file nested leaf updates preserve agents defaults and list siblings", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-memory-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify([
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
    ...
    it("patches config from one object in one write", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-patch-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("preserves empty object values in config patch", async () => {
      ...
      const pathname = writeTempJson5File("alien-config-patch-empty-object", {
        agents: {
          defaults: {
            models: {
              "openai/gpt-5.5": {},
    ...
    it("treats empty object config patches as recursive merges", async () => {
      ...
      const pathname = writeTempJson5File("alien-config-patch-empty-merge", {
        channels: {
          slack: {},
        },
      });
    ...
    it("dry-runs config patch and resolves changed SecretRefs", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-patch-dry-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("schema-validates SecretRef-only config patch operations", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-patch-ref-schema-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("dry-runs nested SecretRefs inside config patch replacements", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-patch-nested-ref-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("supports replace-path and null deletes in config patch", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-patch-replace-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("rejects unused config patch replace paths", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `alien-config-patch-unused-replace-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/alien.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.alien/alien.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.alien/alien.json");
    });
  });
});
