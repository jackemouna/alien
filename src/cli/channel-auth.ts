...
import { getRuntimeConfig, readConfigFileSnapshot, type AlienConfig } from "../config/config.js";
...
function isConfiguredAuthPlugin(plugin: ChannelPlugin, cfg: AlienConfig): boolean {
  const key = plugin.id;
...
function resolveConfiguredAuthChannelInput(cfg: AlienConfig, mode: ChannelAuthMode): string {
