...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [AlienConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@alien/codex",
        installPath: "/tmp/alien/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/alien/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@alien/codex",
        installPath: "/tmp/alien/npm/node_modules/@alien/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/alien/extensions/codex",
    );
  });
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@alien/discord",
        installPath: "/tmp/alien/npm/node_modules/@alien/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/alien-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/alien/npm/node_modules/@alien/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("alien plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/alien/npm/node_modules/@alien/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@alien/discord",
        installPath: "/tmp/alien/npm/node_modules/@alien/discord",
      },
    });
