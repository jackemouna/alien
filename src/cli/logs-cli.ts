...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `alien logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
async function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("alien doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.alien.ai/cli/logs")}\n`,
    );
