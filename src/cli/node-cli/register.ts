    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "alien node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["alien node status", "Check node host service status."],
          ["alien node install", "Install the node host service."],
          ["alien node start", "Start the installed node host service."],
          ["alien node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.alien.ai/cli/node")}\n`,
    );
