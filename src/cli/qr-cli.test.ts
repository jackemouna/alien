...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("ALIEN_GATEWAY_TOKEN", "");
    vi.stubEnv("ALIEN_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("alien devices approve <requestId>");
  });
  ...
  it("uses ALIEN_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("ALIEN_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
