...
describe("buildDaemonHintItems", () => {
  it("classifies common daemon hint kinds", () => {
    expect(
      ...
      { kind: "install", text: "alien gateway install" },
      {
        kind: "container-restart",
        text: "Restart the container or the service that manages it for alien-demo-container.",
      },
      {
      ...
      },
      {
        ...
        text: "If you're in a container, run the gateway in the foreground instead of `alien gateway`.",
      },
      {
        kind: "wsl-systemd",
