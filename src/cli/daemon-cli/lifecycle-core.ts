    ...
    if (preflight) {
      fail(
        preflight.hints
          ...
          : `${params.serviceNoun} aborted: config is invalid.\n${preflight.message}\nFix the config and retry, or run "alien doctor" to repair.`,
        preflight.hints,
      );
      return;
    }
    ...
    if (startResult.outcome === "repair-required") {
      ...
      fail(
        `${params.serviceNoun} service needs repair before it can start: ${startResult.issues
          ...
          .join("; ")}`,
        [formatCliCommand("alien gateway install --force")],
      );
      return;
    }
    emit({
...
export async function runServiceRestart(params: {
  ...
  if (loaded && params.checkTokenDrift) {
    ...
    try {
      ...
      const serviceToken = command?.environment?.ALIEN_GATEWAY_TOKEN;
