...
async function assertUnmanagedGatewayRestartEnabled(port: number): Promise<void> {
  ...
  const probe = await probeGateway({
    ...
    auth: {
      token: normalizeOptionalString(process.env.ALIEN_GATEWAY_TOKEN),
      password: normalizeOptionalString(process.env.ALIEN_GATEWAY_PASSWORD),
    },
    timeoutMs: 1_000,
  }).catch(() => null);
...
async function restartGatewayWithoutServiceManager(
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "alien gateway status --deep" before retrying restart`,
    );
  }
