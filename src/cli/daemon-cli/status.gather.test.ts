...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ALIEN_STATE_DIR: "/tmp/alien-daemon",
    ALIEN_CONFIG_PATH: "/tmp/alien-daemon/alien.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ALIEN_STATE_DIR ?? "/tmp/alien-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ALIEN_CONFIG_PATH ?? `${stateDir}/alien.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/alien-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ALIEN_STATE_DIR = "/tmp/alien-cli";
    process.env.ALIEN_CONFIG_PATH = "/tmp/alien-cli/alien.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/alien-daemon/alien.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/alien-cli/alien.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ALIEN_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.ALIEN_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alien-status-config-"));
    const configPath = path.join(tmp, "alien.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.ALIEN_STATE_DIR = tmp;
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.ALIEN_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "alien-gateway" }],
        hints: [],
      },
      healthy: false,
