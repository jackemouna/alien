...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: { wrapperPath?: string; env?: Record<string, string | undefined> }) => {
    ...
    if (params?.wrapperPath || params?.env?.ALIEN_WRAPPER) {
      environment.ALIEN_WRAPPER = params.wrapperPath ?? params.env?.ALIEN_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/alien.json" },
  })),
}));
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveAlienWrapperPath: async (value: string | undefined) => value?.trim() || undefined,
}));
...
function mockResolvedGatewayTokenSecretRef() {
  resolveSecretInputRefMock.mockReturnValue({
    ref: { source: "env", provider: "default", id: "ALIEN_GATEWAY_TOKEN" },
  });
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:ALIEN_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${ALIEN_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds ALIEN_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["alien", "gateway", "run"],
      environment: {
        ALIEN_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(actionState.warnings).toContain(
      "Gateway service ALIEN_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["alien", "gateway", "run"],
      environment: {
        ALIEN_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/alien-doppler", "gateway", "run"],
      environment: {
    ...
    expect(buildGatewayInstallPlanMock).toHaveBeenCalledWith(
      expect.objectContaining({
        wrapperPath: "/usr/local/bin/alien-doppler",
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/alien-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service ALIEN_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when ALIEN_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["alien", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["alien", "gateway", "run"],
      environment: {
        ALIEN_STATE_DIR: "/tmp/alien-doctor-manual",
        ALIEN_CONFIG_PATH: "/tmp/alien-doctor-manual/alien.json",
        ALIEN_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.ALIEN_STATE_DIR).toBeUndefined();
      expect(env.ALIEN_CONFIG_PATH).toBeUndefined();
