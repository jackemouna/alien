...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: AlienConfig) => void }) => {
      ...
      return {
        path: "/tmp/alien.json",
        previousHash: "hash-1",
        snapshot: { path: "/tmp/alien.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: AlienConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: AlienConfig }>
    >(async () => ({
      path: "/tmp/alien.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: AlienConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/alien.json");
    ...
    expect(output).not.toContain("/tmp/alien.json\nforged");
