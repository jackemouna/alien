...
function rejectAgentScopedModelWrite(command: Command, commandName: "set" | "set-image"): void {
  ...
  throw new Error(
    `\`alien models ${commandName}\` does not support \`--agent\`; it only updates global model defaults. Remove \`--agent\` or use agent config to set a per-agent model override.`,
  );
}
...
export function registerModelsCli(program: Command) {
  const models = program
    ...
    .option(
      "--agent <id>",
      "Agent id to inspect (overrides ALIEN_AGENT_DIR/PI_CODING_AGENT_DIR)",
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/models", "docs.alien.ai/cli/models")}\n`,
    );
