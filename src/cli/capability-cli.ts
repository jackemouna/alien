...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside Alien.";
...
function getAuthProfileIdsForProvider(cfg: AlienConfig, providerId: string): string[] {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.alien.ai/cli/infer")}\n`,
    );
