...
function appendClawHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`alien skills search\`, \`alien skills install\`, and \`alien skills update\` for ClawHub-backed skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("alien skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendClawHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("alien skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`alien config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$ALIEN_CONFIG_PATH")} ${theme.muted("(default: ~/.alien/alien.json)")}`,
    );
  }
