...
describe("acp cli option collisions", () => {
  ...
  it.each([
    ...
    await withTempSecretFiles("alien-acp-cli-", files, async ({ tokenFile, passwordFile }) => {
  ...
  it("trims token file path before reading", async () => {
    await withTempSecretFiles("alien-acp-cli-", { token: "tok_file\n" }, async (files) => {
  ...
  it("reports missing token-file read errors", async () => {
    await parseAcp(["--token-file", "/tmp/alien-acp-missing-token.txt"]);
