...
describe("gateway-cli coverage", () => {
  ...
  it("prints the latest stability bundle without calling Gateway", async () => {
    callGateway.mockClear();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-gateway-cli-bundle-"));
    try {
      const bundleDir = path.join(tempDir, "logs", "stability");
      const bundlePath = path.join(
        bundleDir,
        "alien-stability-2026-04-22T12-00-00-000Z-123-test.json",
      );
      const bundle = {
        version: 1,
      ...
      await withEnvOverride({ ALIEN_STATE_DIR: tempDir }, async () => {
  ...
  it("writes gateway diagnostics export with a best-effort health snapshot", async () => {
    callGateway.mockClear();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-gateway-cli-support-"));
    try {
      const outputPath = path.join(tempDir, "diagnostics.zip");
      await withEnvOverride(
        { ALIEN_STATE_DIR: tempDir, ALIEN_TEST_FILE_LOG: undefined },
        async () => {
          await runGatewayCommand([
            "gateway",
            "diagnostics",
  ...
  it("registers gateway discover and prints json output", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (Alien)",
        displayName: "Studio",
        domain: "alien.internal.",
        host: "studio.alien.internal",
        port: 18789,
    ...
    await withEnvOverride(
      {
        ...
        ALIEN_LAUNCHD_LABEL: undefined,
        ALIEN_SYSTEMD_UNIT: undefined,
        INVOCATION_ID: undefined,
        SYSTEMD_EXEC_PID: undefined,
        JOURNAL_STREAM: undefined,
        ALIEN_WINDOWS_TASK_NAME: undefined,
        ALIEN_SERVICE_MARKER: undefined,
        ALIEN_SERVICE_KIND: undefined,
      },
      async () => {
  ...
  it("keeps exit 1 for gateway lock acquisition failures", async () => {
    ...
    serviceIsLoaded.mockResolvedValue(true);
    startGatewayServer.mockRejectedValueOnce(
      new GatewayLockError("failed to acquire gateway lock at /tmp/alien/gateway.lock"),
    );
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ ALIEN_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
