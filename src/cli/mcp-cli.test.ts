...
const serveAlienChannelMcp = mocks.serveAlienChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveAlienChannelMcp: mocks.serveAlienChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("alien-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("alien-cli-mcp-home-", async () => {
      ...
      expect(serveAlienChannelMcp).toHaveBeenCalledWith({
