...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.ALIEN_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or ALIEN_GATEWAY_PASSWORD.",
  );
}
...
async function maybeLogPendingControlUiBuild(cfg: AlienConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("alien setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("alien onboard --mode local")}\` or \`${formatCliCommand("alien setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.ALIEN_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.ALIEN_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.ALIEN_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.ALIEN_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (futureStartupBlock && process.env.ALIEN_SERVICE_MARKER?.trim()) {
  ...
  if (process.env.ALIEN_SERVICE_MARKER?.trim()) {
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or ALIEN_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set ALIEN_GATEWAY_TOKEN or ALIEN_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or ALIEN_GATEWAY_TOKEN/ALIEN_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("alien gateway stop")}`,
      );
      try {
