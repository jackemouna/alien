...
async function installBundledPluginSource(params: {
  ...
  const configWarning = shouldEnable
    ? ""
    : `Installed bundled plugin "${params.bundledSource.pluginId}" without enabling it because it requires configuration first. Configure it, then run \`alien plugins enable ${params.bundledSource.pluginId}\`.`;
...
async function loadConfigFromSnapshotForInstall(
  ...
  if (
    ...
    throw buildInvalidPluginInstallConfigError(
      `Config invalid outside the plugin recovery path for ${pluginLabel}; run \`alien doctor --fix\` before reinstalling it.`,
    );
  }
