...
${formatHelpExamples([
  ['alien agent --to +15555550123 --message "status update"', "Start a new session."],
  ['alien agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['alien agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.alien.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.alien.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['alien agents set-identity --agent main --name "Alien" --emoji "👾"', "Set name + emoji."],
  ["alien agents set-identity --agent main --avatar avatars/alien.png", "Set avatar path."],
  [
    "alien agents set-identity --workspace ~/.alien/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "alien agents set-identity --identity-file ~/.alien/workspace/IDENTITY.md --agent main",
