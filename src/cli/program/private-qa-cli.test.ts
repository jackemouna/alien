...
describe("private-qa-cli", () => {
  ...
  afterEach(() => {
    ...
    if (originalPrivateQaCli === undefined) {
      ...
      process.env.ALIEN_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
    }
  });

  it("loads the private QA CLI from a source checkout path", async () => {
    process.env.ALIEN_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-private-qa-source-"));
  ...
  it("rejects non-source package roots even when private QA is enabled", async () => {
    process.env.ALIEN_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "alien" }), "utf8");
    ...
    ).toThrow("Private QA CLI is only available from an Alien source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", async () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an Alien source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
