...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alien status", "Show channel health + session summary."],
          ["alien status --all", "Full diagnosis (read-only)."],
          ["alien status --json", "Machine-readable output."],
          ["alien status --usage", "Show model provider usage/quota snapshots."],
          [
            "alien status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["alien status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.alien.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.alien.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alien sessions", "List all sessions."],
          ["alien sessions --agent work", "List sessions for one agent."],
          ["alien sessions --all-agents", "Aggregate sessions across agents."],
          ["alien sessions --active 120", "Only last 2 hours."],
          ["alien sessions --limit 25", "Show the newest 25 sessions."],
          ["alien sessions --json", "Machine-readable output."],
          ["alien sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.alien.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alien sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "alien sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["alien sessions cleanup --enforce", "Apply maintenance now."],
          ["alien sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["alien sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "alien sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .alien/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alien commitments", "List pending inferred follow-ups."],
          ["alien commitments --all", "List all inferred follow-ups."],
          ["alien commitments --agent work", "List one agent's inferred follow-ups."],
          ["alien commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
