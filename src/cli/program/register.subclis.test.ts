...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.ALIEN_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.ALIEN_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.ALIEN_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.ALIEN_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.ALIEN_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.ALIEN_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.ALIEN_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "alien", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "alien"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "alien", "nodes", "list"], "alien");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "alien", "infer"], "alien");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "alien", "acp", "--help"], "alien");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "alien", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "alien", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("alien");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "alien", "completion", "--write-state"];
    const program = new Command().name("alien");
  ...
  it("keeps plugin CLI registrations available for the plugins command root", async () => {
    process.argv = ["node", "alien", "plugins"];
    const program = new Command().name("alien");
