...
describe("program routes", () => {
  ...
  it("passes parsed agents list flags through", async () => {
    await expect(expectRoute(["agents"])?.run(["node", "alien", "agents"])).resolves.toBe(true);
  ...
  it("passes parsed channel read-only route flags through", async () => {
    ...
    await expect(
      listRoute?.run(["node", "alien", "channels", "list", "--json", "--no-usage"]),
    ).resolves.toBe(true);
  ...
  it("returns false for gateway status route when option values are missing", async () => {
    await expectRunFalse(["gateway", "status"], ["node", "alien", "gateway", "status", "--url"]);
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "alien", "gateway", "status", "--token"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "alien", "gateway", "status", "--password"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "alien", "gateway", "status", "--timeout"],
    );
  });
  ...
  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "alien", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "alien", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "alien", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("passes --no-probe through to daemon status", async () => {
    const route = expectRoute(["gateway", "status"]);
    await expect(route?.run(["node", "alien", "gateway", "status", "--no-probe"])).resolves.toBe(
      true,
    );
  ...
  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "alien", "status", "--timeout"]);
  });
  ...
  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "alien", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "alien", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "alien", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "alien", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "alien", "config", "unset"]);
  });
  ...
  it("passes config unset path correctly when root option values precede command", async () => {
    ...
    await expect(
      route?.run(["node", "alien", "--profile", "work", "config", "unset", "update.channel"]),
    ).resolves.toBe(true);
  ...
  it("passes config unset path when root value options appear after subcommand", async () => {
    const route = expectRoute(["config", "unset"]);
    await expect(
      route?.run(["node", "alien", "config", "unset", "--profile", "work", "update.channel"]),
    ).resolves.toBe(true);
    expect(runConfigUnsetMock).toHaveBeenCalledWith({ path: "update.channel" });
  });
  ...
  it("returns false for models list route when --provider value is missing", async () => {
    await expectRunFalse(["models", "list"], ["node", "alien", "models", "list", "--provider"]);
  });

  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "alien", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "alien", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "alien", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "alien", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "alien", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "alien", "models", "status", "--probe-profile"],
    );
  });
  ...
  it("routes tasks list JSON through the lean task JSON command", async () => {
    ...
    expect(rootRoute?.canRun?.(["node", "alien", "tasks"])).toBe(false);
    await expect(
      rootRoute?.run([
        "node",
        "alien",
    ...
    await expect(
      listRoute?.run(["node", "alien", "tasks", "list", "--json", "--runtime=cron"]),
    ).resolves.toBe(true);
  ...
  it("routes tasks audit JSON through the lean task JSON command", async () => {
    ...
    expect(route?.loadPlugins).toBeUndefined();
    expect(route?.canRun?.(["node", "alien", "tasks", "audit"])).toBe(false);
    await expect(
      route?.run([
        "node",
        "alien",
  ...
  it("returns false for task JSON routes when option values are missing or unknown", async () => {
    await expectRunFalse(["tasks"], ["node", "alien", "tasks", "--json", "--runtime"]);
    await expectRunFalse(["tasks", "list"], ["node", "alien", "tasks", "list"]);
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "alien", "tasks", "audit", "--json", "--limit"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "alien", "tasks", "audit", "--json", "--unknown"],
    );
    expect(
      findRoutedCommand(["tasks", "cli"], ["node", "alien", "tasks", "--runtime", "cli"]),
    ).toBeNull();
  });
});
