...
export function isPrivateQaCliEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.ALIEN_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function resolvePrivateQaSourceModuleSpecifier(params?: {
  ...
  argv1?: string;
  moduleUrl?: string;
  resolvePackageRootSync?: typeof resolveAlienPackageRootSync;
  ...
  const resolvePackageRootSync = params?.resolvePackageRootSync ?? resolveAlienPackageRootSync;
...
export function loadPrivateQaCliModule(params?: {
  ...
  if (!specifier) {
    throw new Error("Private QA CLI is only available from an Alien source checkout.");
  }
