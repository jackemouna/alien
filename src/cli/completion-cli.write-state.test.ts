...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["alien.bash", "alien.fish", "alien.ps1", "alien.zsh"]),
    );
