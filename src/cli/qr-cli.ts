...
function readDevicePairPublicUrlFromConfig(cfg: AlienConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.ALIEN_GATEWAY_PASSWORD)) {
    return false;
  }
  ...
  const envToken = trimToUndefined(env.ALIEN_GATEWAY_TOKEN);
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: AlienConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.alien.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the Alien mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("alien devices list")}`,
          `  ${theme.command("alien devices approve <requestId>")}`,
        );
