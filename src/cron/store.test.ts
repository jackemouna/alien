...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ALIEN_HOME for tilde expansion", () => {
    vi.stubEnv("ALIEN_HOME", "/srv/alien-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/alien-home", "cron", "jobs.json"));
  });
});
