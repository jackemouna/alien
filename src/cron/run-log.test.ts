...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    await withRunLogDir("alien-cron-log-", async (dir) => {
  ...
  it("reads run-log entries synchronously for task reconciliation", async () => {
    await withRunLogDir("alien-cron-log-sync-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("alien-cron-log-perms-", async (dir) => {
        const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("alien-cron-log-dir-perms-", async (dir) => {
  ...
  it("reads newest entries and filters by jobId", async () => {
    await withRunLogDir("alien-cron-log-read-", async (dir) => {
  ...
  it("ignores invalid and non-finished lines while preserving delivery fields", async () => {
    await withRunLogDir("alien-cron-log-filter-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("does not include raw delivery targets in run-log search", async () => {
    await withRunLogDir("alien-cron-log-target-query-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
      await fs.mkdir(path.dirname(logPath), { recursive: true });
      await fs.writeFile(
        logPath,
  ...
  it("reads and searches run diagnostics", async () => {
    await withRunLogDir("alien-cron-log-diagnostics-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("reads telemetry fields", async () => {
    await withRunLogDir("alien-cron-log-telemetry-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("cleans up pending-write bookkeeping after appends complete", async () => {
    await withRunLogDir("alien-cron-log-pending-", async (dir) => {
  ...
  it("read drains pending fire-and-forget writes", async () => {
    await withRunLogDir("alien-cron-log-drain-", async (dir) => {
