...
function pruneCompletedDirectCronDeliveries(now: number) {
  const ttlMs = process.env.ALIEN_TEST_FAST === "1" ? 60_000 : 24 * 60 * 60 * 1000;
...
function resolveDirectCronRetryDelaysMs(): readonly number[] {
  return process.env.NODE_ENV === "test" && process.env.ALIEN_TEST_FAST === "1"
    ? [0, 0, 0]
    : [5_000, 10_000, 20_000];
}
...
export async function dispatchCronDelivery(
  ...
  const formatDeliveryTargetError = (error: string) =>
    params.unverifiedMessagingToolDelivery === true
      ? `${error}; the agent used the message tool, but Alien could not verify that message matched the cron delivery target`
      : error;
    ...
    try {
      ...
      const runDelivery = async () =>
        await deliverOutboundPayloads({
          ...
          // See: https://github.com/alien/alien/issues/40545
          skipQueue: true,
        });
