...
type HomeEnvSnapshot = {
  ...
  ALIEN_STATE_DIR: string | undefined;
  ALIEN_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    ALIEN_AGENT_DIR: process.env.ALIEN_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".alien", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ALIEN_STATE_DIR = path.join(home, ".alien");
    process.env.ALIEN_AGENT_DIR = path.join(home, ".alien", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".alien", "agent");
