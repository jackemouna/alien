...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "alien"),
    sessionKey,
    remoteCacheDir: join(home, ".alien", "media", "remote-cache", slugifySessionKey(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("alien-triggers-", async (home) => {
