...
function withAcpConfig(acp: AlienConfig["acp"]): AlienConfig {
  return { acp } as AlienConfig;
}
...
describe("ACP install hints", () => {
  it("prefers explicit runtime install command", () => {
    const cfg = withAcpConfig({
      runtime: { installCommand: "pnpm alien plugins install acpx" },
    });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("pnpm alien plugins install acpx");
  });
  ...
  it("uses local acpx extension path when present", () => {
    ...
    expect(hint).toBe(`alien plugins install ${path.join(repoRoot, "extensions", "acpx")}`);
  });
  ...
  it("falls back to scoped install hint for acpx when local extension is absent", () => {
    ...
    const cfg = withAcpConfig({ backend: "acpx" });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("alien plugins install acpx");
  });
