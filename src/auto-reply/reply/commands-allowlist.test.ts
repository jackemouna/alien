...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-allowlist-config-"));
  const configPath = path.join(dir, "alien.json");
  const previous = process.env.ALIEN_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ALIEN_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<AlienConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: AlienConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
