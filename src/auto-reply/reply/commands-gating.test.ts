...
function buildParams(commandBody: string, cfg: AlienConfig): HandleCommandsParams {
  const ctx = {
    Body: commandBody,
...
describe("command gating", () => {
  ...
  it("blocks disabled bash", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: false } } as AlienConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: { enabled: true, allowed: true, failures: [] },
    });
  ...
  it("blocks bash when elevated access is unavailable", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: true } } as AlienConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: {
        enabled: true,
  ...
  it("ignores inherited command flags", () => {
    ...
    const cfg = { commands: inheritedCommands as never } as AlienConfig;
  ...
  it("enforces gateway client permissions for /config commands", async () => {
    const baseCfg = { commands: { config: true, text: true } } as AlienConfig;
