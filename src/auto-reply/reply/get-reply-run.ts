...
type AgentDefaults = NonNullable<AlienConfig["agents"]>["defaults"];
...
type RunPreparedReplyParams = {
  ...
  sessionCfg: AlienConfig["session"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.ALIEN_TEST_FAST === "1",
  });
  ...
  const transcriptBodyBase = isHeartbeat
    ...
    : isBareSessionReset
      ? softResetTail || `[Alien session ${startupAction}]`
      : hasUserBody
        ? baseBodyFinal
