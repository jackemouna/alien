...
function baseConfig(): AlienConfig {
  return {
    commands: { text: true },
    agents: { defaults: {} },
...
function resolveModelSelectionForCommand(params: {
  ...
  return resolveModelSelectionFromDirective({
    ...
    cfg: { commands: { text: true } } as unknown as AlienConfig,
    agentDir: TEST_AGENT_DIR,
...
describe("/model chat UX", () => {
  ...
  it("uses workspace-scoped auth evidence in /model list provider visibility", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-model-list-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".alien", "extensions", "workspace-model-list");
  ...
  it("uses workspace-scoped auth evidence in /model status labels", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "alien-model-status-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".alien", "extensions", "workspace-model-auth");
    const bundledDir = path.join(tempRoot, "bundled");
    const stateDir = path.join(tempRoot, "state");
    const credentialPath = path.join(tempRoot, "credentials.json");
    fs.mkdirSync(pluginDir, { recursive: true });
  ...
  it("auto-applies closest match for typos", () => {
    ...
    const cfg = { commands: { text: true } } as unknown as AlienConfig;
  ...
  it("includes additive allowlist repair when a runtime switch targets a blocked model", () => {
    ...
    expect(resolved.errorText).toContain(
      `alien config set agents.defaults.models '{"openai/gpt-5.5":{}}' --strict-json --merge`,
    );
    expect(resolved.errorText).toContain("Then retry: /model openai/gpt-5.5 --runtime codex");
    expect(resolved.errorText).toContain("alien plugins enable codex");
  });
