...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: alien doctor --non-interactive"),
  ...
  triggerAlienRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerAlienRestart: mocks.triggerAlienRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerAlienRestart.mockReset();
    mocks.triggerAlienRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerAlienRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerAlienRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerAlienRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
