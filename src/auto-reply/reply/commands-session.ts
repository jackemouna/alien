...
import { scheduleGatewaySigusr1Restart, triggerAlienRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Alien in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  let sentinelPath: string | null = null;
  ...
  const restartMethod = triggerAlienRestart();
  if (!restartMethod.ok) {
    await removeRestartSentinelFile(sentinelPath);
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Alien via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
