...
describe("runPreparedReply media-only handling", () => {
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[Alien heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[Alien heartbeat poll]");
  });
  ...
  it("uses a non-empty transcript marker while keeping bare reset startup instructions out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[Alien session new]");
    expect(call?.followupRun.transcriptPrompt).toBe("[Alien session new]");
  });
