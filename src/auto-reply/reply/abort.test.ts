...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "alien-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop alien!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@alien_bot", { botUsername: "alien_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@alien_bot", { botUsername: "alien_bot" })).toBe(true);
