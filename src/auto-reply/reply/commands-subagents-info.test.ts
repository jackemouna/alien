...
const TEST_SESSION_STORE_PATH = path.join(
  os.tmpdir(),
  `alien-commands-subagents-info-${process.pid}.json`,
);

function buildCommandTestConfig(): AlienConfig {
  return {
    ...baseCommandTestConfig,
    session: {
...
function buildInfoContext(params: { cfg: AlienConfig; runs: object[]; restTokens: string[] }) {
  return {
    params: {
      cfg: params.cfg,
...
describe("subagents info", () => {
  ...
  it("sanitizes leaked task details in /subagents info", () => {
    ...
    expect(text).not.toContain("Alien runtime context (internal):");
