...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("alien.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("alien.reply.full-runtime");

type ReplyConfigWithMarker = AlienConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.ALIEN_ALLOW_SLOW_REPLY_TESTS === "1" || env.ALIEN_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends AlienConfig>(
  config: T,
...
export function withFastReplyConfig<T extends AlienConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends AlienConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is AlienConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set ALIEN_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as AlienConfig;
}
