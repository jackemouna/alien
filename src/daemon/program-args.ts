...
export const ALIEN_WRAPPER_ENV_KEY = "ALIEN_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/alien -> .pnpm/alien@X.Y.Z/...
...
export async function resolveAlienWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${ALIEN_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveAlienWrapperPath(params.wrapperPath);
  if (wrapperPath) {
