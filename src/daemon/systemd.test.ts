...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessExecFile } = await import("alien/plugin-sdk/test-node-mocks");
...
const TEST_MANAGED_HOME = "/tmp/alien-test-home";
const GATEWAY_SERVICE = "alien-gateway.service";
const NODE_SERVICE = "alien-node.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/alien gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/alien", "gateway", "run"]);
...
describe("systemd availability", () => {
  ...
  it("does not let stale SUDO_USER override a sudo-u target user scope", async () => {
    ...
    await expect(
      isSystemdUserServiceAvailable({ USER: "alien", SUDO_USER: "admin" }),
    ).resolves.toBe(true);
    expect(execFileMock).toHaveBeenCalledTimes(1);
  });
});
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/alien-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "alien-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "alien-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/alien-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled alien-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled alien-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when ALIEN_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/alien-gateway.service",
    },
    {
      name: "uses profile-specific service name when ALIEN_PROFILE is set to a custom value",
      env: { HOME: "/home/test", ALIEN_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/alien-gateway-jbphoenix.service",
    },
    {
      name: "prefers ALIEN_SYSTEMD_UNIT over ALIEN_PROFILE",
      env: {
        HOME: "/home/test",
        ALIEN_PROFILE: "jbphoenix",
        ALIEN_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles ALIEN_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        ALIEN_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from ALIEN_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        ALIEN_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/alien gateway start --name "My Bot"')).toEqual([
      "/usr/bin/alien",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('alien --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["alien", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('alien --path "C:\\\\Program Files\\\\Alien"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["alien", "--path", "C:\\\\Program Files\\\\Alien"]);

    expect(
      splitArgsPreservingQuotes('alien --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["alien", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/alien gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads ALIEN_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/alien gateway run", "EnvironmentFile=%h/.alien/.env"],
      { [`${TEST_SERVICE_HOME}/.alien/.env`]: "ALIEN_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.ALIEN_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/alien gateway run",
        "EnvironmentFile=%h/.alien/.env",
        'Environment="ALIEN_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.alien/.env`]: "ALIEN_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.ALIEN_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.ALIEN_GATEWAY_TOKEN).toBe("inline-and-file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.alien/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/alien-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/alien gateway run",
          'EnvironmentFile=%h/.alien/first.env "%h/.alien/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.alien/first.env") {
        return "ALIEN_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.alien/second env.env") {
        return 'ALIEN_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ALIEN_GATEWAY_TOKEN: "first-token",
      ALIEN_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "ALIEN_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "ALIEN_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "ALIEN_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      ALIEN_GATEWAY_TOKEN: "override-token",
      ALIEN_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/alien-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/alien gateway run",
          "EnvironmentFile=%h/.alien/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.alien/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'ALIEN_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "ALIEN_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      ALIEN_GATEWAY_TOKEN: "quoted token",
      ALIEN_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      ALIEN_GATEWAY_TOKEN: "file",
      ALIEN_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  async function withStageFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".alien");
    const env = {
      HOME: home,
      ALIEN_STATE_DIR: stateDir,
      ALIEN_SYSTEMD_UNIT: "alien-gateway-stage-test",
    };
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        ...
        ["ALIEN_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/alien", "gateway", "run"],
        workingDirectory: "/tmp",
        environment: {
          ALIEN_GATEWAY_TOKEN: "dotenv-token",
          LLM_API_KEY: "dotenv-key",
          ALIEN_GATEWAY_PORT: "18789",
        },
      });
      ...
      expect(unit).toContain("Environment=ALIEN_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=ALIEN_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("ALIEN_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        path.join(stateDir, ".env"),
        ["ALIEN_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          ALIEN_GATEWAY_TOKEN: "fresh-token",
          LLM_API_KEY: "dotenv-key",
        },
      });
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=ALIEN_GATEWAY_TOKEN=fresh-token");
  ...
  it("clears stale inline-managed keys from env file on re-stage (#76860)", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      // Existing env file carries a stale ALIEN_GATEWAY_TOKEN that the
      // operator previously wrote there but staging now supplies inline.
      await fs.writeFile(
        envFilePath,
        ["ALIEN_GATEWAY_TOKEN=stale-gateway-token", "OPENROUTER_API_KEY=or-operator-key"].join(
          "\n",
        ) + "\n",
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/alien", "gateway", "run"],
        workingDirectory: "/tmp",
        // Staging manages ALIEN_GATEWAY_TOKEN inline; ALIEN_SERVICE_MANAGED_ENV_KEYS
        // marks it as an Alien-managed key so the stale env-file copy is cleared.
        environment: {
          ALIEN_GATEWAY_TOKEN: "fresh-gateway-token",
          LLM_API_KEY: "dotenv-key",
          OPENROUTER_API_KEY: "or-operator-key",
          ALIEN_SERVICE_MANAGED_ENV_KEYS: "ALIEN_GATEWAY_TOKEN",
        },
        environmentValueSources: {
          ALIEN_GATEWAY_TOKEN: "inline-and-file",
          LLM_API_KEY: "inline",
          OPENROUTER_API_KEY: "file",
          ALIEN_SERVICE_MANAGED_ENV_KEYS: "inline",
        },
      });
      ...
      expect(envFile).not.toContain("ALIEN_GATEWAY_TOKEN");
      ...
      expect(unit).toContain("Environment=ALIEN_GATEWAY_TOKEN=fresh-gateway-token");
  ...
  it("preserves operator secrets when incoming .env is empty (#76860)", async () => {
    await withStageFixture(async ({ env, envFilePath }) => {
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/alien", "gateway", "run"],
        workingDirectory: "/tmp",
        environment: { ALIEN_GATEWAY_PORT: "18789" },
      });
...
describe("systemd service install and uninstall", () => {
  async function withNodeSystemdFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alien-node-systemd-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".alien");
    const env = {
      HOME: home,
      ALIEN_STATE_DIR: stateDir,
      ALIEN_SYSTEMD_UNIT: "alien-node",
    };
    const unitPath = resolveSystemdUserUnitPath(env);
  ...
  it("activates the ALIEN_SYSTEMD_UNIT override during install", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath }) => {
      ...
      await installSystemdService({
        env,
        stdout: { write: vi.fn() } as unknown as NodeJS.WritableStream,
        programArguments: ["/usr/bin/alien", "node", "run"],
        workingDirectory: "/tmp",
        environment: {
          ALIEN_SYSTEMD_UNIT: "alien-node",
        },
      ...
      expect(unitPath).toMatch(/alien-node\.service$/);
      expect(unit).toContain("alien node run");
      ...
      execFileMock
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          ...
          cb(
            ...
            "",
            "Unit file alien-node.service does not exist.",
          );
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "daemon-reload");
  ...
  it("uses the sudo-u target user for install activation machine-scope retry", async () => {
    await withNodeSystemdFixture(async ({ env }) => {
      const installEnv = { ...env, USER: "alien", SUDO_USER: "admin" };
      execFileMock
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "status");
          cb(null, "", "");
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertMachineUserSystemctlArgs(args, "alien", "enable", NODE_SERVICE);
          cb(null, "", "");
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "restart", NODE_SERVICE);
  ...
  it("disables the ALIEN_SYSTEMD_UNIT override during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath }) => {
      await fs.mkdir(path.dirname(unitPath), { recursive: true });
      await fs.writeFile(unitPath, "[Unit]\nDescription=Alien Node\n", "utf8");
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "alien-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ ALIEN_PROFILE: "work" });
  });
