...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.ALIEN_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.ALIEN_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.ALIEN_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.ALIEN_GATEWAY_PORT) ??
...
async function activateScheduledTask(params: {
  ...
  const taskDescription = params.description ?? "Alien Gateway";
