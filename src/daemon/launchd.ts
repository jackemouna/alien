...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.ALIEN_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.ALIEN_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by Alien. Do not edit while the gateway service is installed.",
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.ALIEN_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.alien.ai/gateway",
    ].join("\n"),
  );
}
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.ALIEN_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "alien gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "alien gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `alien gateway restart` is an explicit operator request to bring the
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "alien gateway restart",
  });
  writeLaunchAgentActionLine(stdout, "Restarted LaunchAgent", serviceTarget);
  return { outcome: "completed" };
}
