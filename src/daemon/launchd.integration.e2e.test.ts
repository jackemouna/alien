...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `alien-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ALIEN_LAUNCHD_LABEL: `ai.alien.launchd-int-${testId}`,
      ALIEN_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
