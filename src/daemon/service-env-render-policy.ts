...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.ALIEN_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.ALIEN_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.ALIEN_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  if (!isLaunchAgentServiceEnvironment(params)) {
    return;
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    ALIEN_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
