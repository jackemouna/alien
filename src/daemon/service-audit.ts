...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ALIEN_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `alien gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ALIEN_GATEWAY_TOKEN does not match gateway.auth.token in alien.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.ALIEN_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ALIEN_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `alien gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
