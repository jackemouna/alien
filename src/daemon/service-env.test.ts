...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ALIEN_GATEWAY_PORT).toBe("18789");
    expect(env.ALIEN_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ALIEN_SERVICE_MARKER).toBe("alien");
    expect(env.ALIEN_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ALIEN_SERVICE_VERSION).toBe("string");
    expect(env.ALIEN_SYSTEMD_UNIT).toBe("alien-gateway.service");
    expect(env.ALIEN_WINDOWS_TASK_NAME).toBe("Alien Gateway");
    if (process.platform === "darwin") {
      expect(env.ALIEN_LAUNCHD_LABEL).toBe("ai.alien.gateway");
    }
  });

  it("passes through ALIEN_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ALIEN_WRAPPER: " /usr/local/bin/alien-doppler ",
      },
      port: 18789,
    });

    expect(env.ALIEN_WRAPPER).toBe("/usr/local/bin/alien-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".alien", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ALIEN_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ALIEN_SYSTEMD_UNIT).toBe("alien-gateway-work.service");
    expect(env.ALIEN_WINDOWS_TASK_NAME).toBe("Alien Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ALIEN_LAUNCHD_LABEL).toBe("ai.alien.work");
    }
  });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ALIEN_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.ALIEN_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through ALIEN_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ALIEN_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ALIEN_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through ALIEN_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ALIEN_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.ALIEN_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits ALIEN_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        ALIEN_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".alien"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ALIEN_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".alien-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ALIEN_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".alien"));
  });

  it("uses ALIEN_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ALIEN_STATE_DIR: "/var/lib/alien" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/alien"));
  });

  it("expands ~ in ALIEN_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ALIEN_STATE_DIR: "~/alien-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/alien-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ALIEN_STATE_DIR: "C:\\State\\alien" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\alien");
  });
});
