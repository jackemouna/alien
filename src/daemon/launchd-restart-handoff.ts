...
function collectRestartLogEnv(env?: Record<string, string | undefined>): LaunchdRestartLogEnv {
  ...
  return {
    ...
    ALIEN_STATE_DIR: source.ALIEN_STATE_DIR,
    ALIEN_PROFILE: source.ALIEN_PROFILE,
  };
}

function resolveLaunchAgentLabel(env?: Record<string, string | undefined>): string {
  const envLabel = normalizeOptionalString(env?.ALIEN_LAUNCHD_LABEL);
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(env?.ALIEN_PROFILE));
}
...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.ALIEN_LAUNCHD_LABEL);
...
printf '[%s] alien restart attempt source=launchd-handoff mode=${mode} target=%s waitPid=%s\\n' "$(date -u +%FT%TZ)" "$service_target" "$wait_pid" >&2
...
if [ "$status" -eq 0 ]; then
  printf '[%s] alien restart done source=launchd-handoff mode=${mode}\\n' "$(date -u +%FT%TZ)" >&2
else
  printf '[%s] alien restart failed source=launchd-handoff mode=${mode} status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
fi
exit "$status"
`;
  }
...
while [ "$print_retry_count" -gt 0 ]; do
  if launchctl print "$service_target" >/dev/null 2>&1; then
    printf '[%s] alien restart done source=launchd-handoff mode=${mode} reason=launchd-auto-reload\\n' "$(date -u +%FT%TZ)" >&2
    exit 0
  fi
