...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ALIEN_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".alien", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ALIEN_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ALIEN_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".alien-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ALIEN_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ALIEN_PROFILE: "rescue",
        ALIEN_STATE_DIR: "C:\\State\\alien",
      },
      expected: path.join("C:\\State\\alien", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", ALIEN_PROFILE: "default" },
      expected: path.join("/home/test", ".alien", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alien-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Alien Gateway",
          "cd /d C:\\Projects\\alien",
          "set NODE_ENV=production",
          "set ALIEN_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\alien",
          environment: {
            NODE_ENV: "production",
            ALIEN_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\alien\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\alien\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Alien Share\\node.exe" "\\\\fileserver\\Alien Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Alien Share\\node.exe",
            "\\\\fileserver\\Alien Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from ALIEN_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ALIEN_STATE_DIR: path.join(tmpDir, "custom-state") }),
