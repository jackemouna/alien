...
export class AlienChannelBridge {
  ...
  constructor(
    private readonly cfg: AlienConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`alien mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `alien mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
