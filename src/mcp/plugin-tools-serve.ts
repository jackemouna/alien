/**
 * Standalone MCP server that exposes Alien plugin-registered tools
...
function resolvePluginToolPolicy(config: AlienConfig): {
...
function resolveTools(config: AlienConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "alien-plugin-tools", tools });
}
