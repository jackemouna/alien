...
import { createAlienChannelMcpServer, AlienChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createAlienChannelMcpServer({
...
describe("alien channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new AlienChannelBridge({} as never, {
