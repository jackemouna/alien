...
import { loadAlienProviderIndex, normalizeAlienProviderIndex } from "./index.js";

describe("Alien provider index", () => {
  it("normalizes provider preview catalog rows through model catalog validation", () => {
    const index = normalizeAlienProviderIndex({
      version: 1,
      providers: {
        Moonshot: {
          id: "moonshot",
  ...
  it("loads the bundled provider index without runtime plugin loading", () => {
    const index = loadAlienProviderIndex();
