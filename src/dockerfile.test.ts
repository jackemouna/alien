...
import { BUNDLED_PLUGIN_ROOT_DIR } from "alien/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain("FROM ${ALIEN_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${ALIEN_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${ALIEN_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("ALIEN_VARIANT");
  });
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG ALIEN_INSTALL_BROWSER");
  ...
  it("prunes runtime dependencies after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG ALIEN_EXTENSIONS");
    expect(dockerfile).toContain("ARG ALIEN_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG ALIEN_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bALIEN_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for ALIEN_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
  ...
  it("pre-creates the Alien home before switching to the node user", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.alien");
    expect(dockerfile).toContain(
