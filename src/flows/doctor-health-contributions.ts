...
function resolveDoctorMode(cfg: AlienConfig): DoctorFlowMode {
...
export function shouldSkipLegacyUpdateDoctorConfigWrite(params: {
  ...
  if (!isTruthyEnvValue(params.env.ALIEN_UPDATE_IN_PROGRESS)) {
    return false;
  }
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("alien configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("alien config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("alien setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("alien config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("alien config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("alien doctor --fix")}" to apply changes.`);
  }
}
