---
name: alien-pr-maintainer
description: Use immediately for any pasted Alien GitHub issue or PR URL/number, and for Alien issue/PR review, triage, duplicate search, opener identity/who wrote it, author account age/activity, comments, labels, close, land, or maintainer evidence checks.
---
...
- Use `$gitcrawl` first anytime you inspect Alien issues or PRs.
...
gitcrawl neighbors alien/alien --number <issue-or-pr-number> --limit 12 --json
gitcrawl search alien/alien --query "<scope or title keywords>" --mode hybrid --json
...
- Report opener identity as one compact line:
  `By: Jane Doe (@jane, acct 2021-04-03) | Alien: 4 PRs, 2 issues, 11 commits/12mo | GitHub: 9 repos, 86 commits, 9 PRs, 3 issues, 12 reviews`
- Always show recent activity in two lanes: Alien-local PRs, issues, and commits in the last 12 months; and general public GitHub activity over the same window. For linked issue-fixing PRs, include both the PR author and issue opener when they differ.
- Prefer the bundled helper for activity lookups:

```bash
.agents/skills/alien-pr-maintainer/scripts/github-activity.sh <login> [other-login...]
.agents/skills/alien-pr-maintainer/scripts/github-activity.sh --global <login>
```
...
- If the contribution graph is misleading or zero but public events/repos show activity, keep it one line, for example:
  `By: pickaxe (@ProspectOre, acct 2019-08-24) | Alien: 5 PRs, 0 issues, 5 commits/12mo | GitHub: 5 repos, 29 recent events, 100 public own-repo commits; graph=0`
...
- Good manual-close categories:
  - blank or mostly untouched PR template with no concrete Alien problem/fix
...
- Use `--repo alien/alien` with `--match title,body` first when using `gh search`.
...
```bash
gh search prs --repo alien/alien --match title,body --limit 50 -- "auto-update"
gh search issues --repo alien/alien --match title,body --limit 50 -- "auto-update"
gh search issues --repo alien/alien --match title,body --limit 50 \
