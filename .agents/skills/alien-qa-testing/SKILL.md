---
name: alien-qa-testing
description: Run, watch, debug, extend, or explain Alien qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
ALIEN_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
pnpm alien qa suite \
...
5. If the user wants to watch the live UI, find the current `alien-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm alien qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
ALIEN_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `ALIEN_QA_PROVIDER_MODE`
  - `ALIEN_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Alien`
  - item: `ALIEN_QA_CONVEX_SITE_URL`
  - item: `ALIEN_QA_CONVEX_SECRET_MAINTAINER`
  - item: `ALIEN_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `ALIEN QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `ALIEN_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `ALIEN_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Alien/ALIEN_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
ALIEN_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
ALIEN_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
ALIEN_QA_PROVIDER_MODE="mock-openai" \
ALIEN_NPM_TELEGRAM_PACKAGE_SPEC="alien@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `ALIEN_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `ALIEN_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `ALIEN_QA_PROVIDER_MODE`, map it explicitly to `ALIEN_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    - `ALIEN_QA_CREDENTIAL_SOURCE=convex`
    - `ALIEN_QA_CREDENTIAL_ROLE=maintainer`
    - `ALIEN_QA_CONVEX_SITE_URL`
    - `ALIEN_QA_CONVEX_SECRET_MAINTAINER`
    - `ALIEN_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo alien/alien --ref main \
  -f package_spec=alien@YYYY.M.D-beta.N \
  -f package_label=alien@YYYY.M.D-beta.N \
...
```bash
gh api repos/alien/alien/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal Alien WhatsApp accounts in the shared pool.
...
- Validate with `pnpm alien qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Alien identity combines with the character.
...
```bash
pnpm alien qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `ALIEN_HOME` sandboxed.
