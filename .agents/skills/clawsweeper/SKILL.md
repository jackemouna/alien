---
name: clawsweeper
description: "Use for all ClawSweeper work: Alien issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one Alien
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `alien/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo alien/alien --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo alien/clawsweeper \
  -f target_repo=alien/alien \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/alien-alien/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/alien/inbox/clawsweeper-alien-alien-123.md
pnpm run repair:dispatch -- jobs/alien/inbox/clawsweeper-alien-alien-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo alien/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo alien/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo alien/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo alien/clawsweeper --body 1
```
...
@alien-clawsweeper fix ci
@alien-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo alien/alien --lookback-minutes 180
pnpm run repair:comment-router -- --repo alien/alien --execute --wait-for-capacity
```
...
- merges automerge PRs only when ClawSweeper passed the exact current head,
  ...
  the PR is not draft, required user-facing Alien changelog entries are
...
leave his local Alien checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central Alien security handling.
...
```bash
gh run list --repo alien/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo alien/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo alien/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo alien/alien --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/alien/alien/commits/<sha>/check-runs?per_page=100" \
