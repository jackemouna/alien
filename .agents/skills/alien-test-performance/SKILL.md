---
name: alien-test-performance
description: Benchmark, diagnose, and optimize Alien test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# Alien Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `ALIEN_VITEST_IMPORT_DURATIONS=1 ALIEN_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Blacksmith Testbox by default on
   ...
   - `blacksmith testbox run --id <ID> "ALIEN_TESTBOX=1 pnpm test:extensions:batch <ids>"`
   - stop the box when done.
5. If plugin performance is package-artifact sensitive, switch to
   `alien-pre-release-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `ALIEN_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap snapshots  | real leak vs retained module graph | `alien-test-heap-leaks` workflow                                         |
...
```bash
ALIEN_TEST_MEMORY_TRACE=1 \
ALIEN_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
ALIEN_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
ALIEN_TEST_WORKERS=2 \
ALIEN_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `alien-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `ALIEN_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  ALIEN_VITEST_IMPORT_DURATIONS=1 \
  ALIEN_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
