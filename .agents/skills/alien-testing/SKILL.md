---
name: alien-testing
description: Choose, run, rerun, or debug Alien tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
- For Blacksmith Testbox proof, reuse only an id warmed and claimed in this
  ...
  `pnpm testbox:run --id <id> -- "<command>"` for Alien gates so stale
  org-visible ids fail fast before syncing. Claims older than 12 hours are
  stale unless `ALIEN_TESTBOX_CLAIM_TTL_MINUTES` is explicitly set for long
  work.
...
ALIEN_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
ALIEN_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `ALIEN_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `ALIEN_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `Alien Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the private `alien/releases-private` evidence ledger.
Use the manual `Alien Release Evidence`
(`alien-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation alien/alien <run-id> blocking
package-acceptance alien/alien <run-id> blocking
release-checks alien/alien <run-id> blocking
```
...
`ALIEN_RELEASES_PRIVATE_DISPATCH_TOKEN` is configured in the public repo, it
requests the private `Alien Release Evidence From Full Validation` workflow.
...
`Alien Release Checks` (`alien-release-checks.yml`) is the release child
...
```bash
gh workflow run alien-release-checks.yml \
  --repo alien/alien \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`Alien Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/alien/alien-live-media-runner:ubuntu-24.04` container so
...
`pnpm alien qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm alien qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `ALIEN_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `Alien Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`Alien Live And E2E Checks (Reusable)`
(`alien-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run alien-live-and-e2e-checks-reusable.yml \
  --repo alien/alien \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `ALIEN_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`ALIEN_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
ALIEN_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
ALIEN_DOCKER_ALL_DRY_RUN=1 ALIEN_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
ALIEN_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
ALIEN_DOCKER_ALL_LANES=<lane> \
ALIEN_DOCKER_ALL_BUILD=0 \
ALIEN_DOCKER_ALL_PREFLIGHT=0 \
ALIEN_SKIP_DOCKER_BUILD=1 \
ALIEN_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
ALIEN_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view alien@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/alien-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=alien@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=alien@YYYY.M.D-beta.N` or
  `package_spec=alien@YYYY.M.D`.
- For stable package proof, use `package_spec=alien@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `alien@beta`,
  `alien@latest`, or exact Alien release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `alien update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `alien@beta`, `alien@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Alien branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-alien-for-docker.mjs` is the
