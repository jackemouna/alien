---
name: gitcrawl
description: Use gitcrawl for Alien issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  alien:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging Alien issues or PRs.
...
gitcrawl search alien/alien --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:alien/gitcrawl-store.git --json
```
