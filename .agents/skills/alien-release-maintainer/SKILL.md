---
name: alien-release-maintainer
description: Prepare or verify Alien stable/beta releases, changelogs, release notes, publish commands, and artifacts.
---

# Alien Release Maintainer
...
- Core `alien` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Alien/Resources/Info.plist`
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/alien/alien/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `alien` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `alien YYYY.M.D-beta.N`
...
Use the Alien account's existing release-post style:

- Format: `Alien YYYY.M.D 👾` or `👾 Alien YYYY.M.D is live`, blank line,
  then 3-4 emoji-led bullets, blank line, one short punchline, then the release
  link.
- For beta: say `Alien YYYY.M.D-beta.N 👾` or `Alien YYYY.M.D beta N is
...
```bash
  ALIEN_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/alien-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `ALIEN_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `ALIEN_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Alien npm release path before creating the tag.
...
- Full pre-npm beta test roster:
  ...
  - all Parallels install/update tests:
    ...
    rerun lanes from `alien-parallels-smoke`
...
- Post-published beta verification roster:
  - `node --import tsx scripts/alien-npm-postpublish-verify.ts <beta-version>`
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=alien@<beta-version>` and
    ...
    `ALIEN_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `ALIEN_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `alien-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `alien update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `alien/alien` and the real mac preflight in
  `alien/releases-private` for every release.
...
- Alien publish uses GitHub trusted publishing.
...
- Prefer fixing the private workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `alien` package with read/write
...
- If the private dist-tag workflow cannot promote because `NPM_TOKEN` is absent
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add alien@YYYY.M.D latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view alien dist-tags --json --prefer-online --cache /tmp/alien-npm-cache-verify-$$`
    and `npm view alien@latest version dist.tarball --json --prefer-online`.
...
- `.github/workflows/macos-release.yml` in `alien/alien` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `alien/releases-private/.github/workflows/alien-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `alien/releases-private/.github/workflows/alien-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@alien/alien-release-managers` before publish continues.
- Mac publish uses
  `alien/releases-private/.github/workflows/alien-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `alien/alien`
  automatically when `ALIEN_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Alien package
...
- `@alien/*` plugin publishes use a separate maintainer-only flow.
...
14. Dispatch Actions > `QA-Lab - All Lanes` against the release tag and wait
    ...
    lanes to pass.
15. Start `.github/workflows/alien-npm-release.yml` from the release branch
...
16. For stable releases, start `.github/workflows/macos-release.yml` in
    `alien/alien` and wait for the public validation-only run to pass.
...
20. Start `.github/workflows/alien-npm-release.yml` from the same branch with
    the same tag for the real publish, choose `npm_dist_tag` (`beta` default,
    `latest` only when you intentionally want direct stable publish), keep it
    the same as the preflight run, and pass the successful npm
    `preflight_run_id`.
21. Wait for `npm-release` approval from `@alien/alien-release-managers`.
...
- Use `alien-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
