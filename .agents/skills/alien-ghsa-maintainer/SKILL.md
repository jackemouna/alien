---
name: alien-ghsa-maintainer
description: Inspect, patch, validate, publish, or confirm Alien GHSA security advisories and private-fork state.
---

# Alien GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `alien-release-maintainer`.
...
```bash
fork=$(gh api /repos/alien/alien/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/alien/alien/security-advisories/<GHSA> \
