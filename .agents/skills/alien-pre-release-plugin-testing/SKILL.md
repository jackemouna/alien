---
name: alien-pre-release-plugin-testing
description: Plan and run pre-release Alien plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# Alien Pre-Release Plugin Testing
...
release?" It complements `alien-testing`; use that skill too when choosing
...
the main Alien checkout. Do not run `pnpm install` there. For broad or
...
```bash
ALIEN_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
ALIEN_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
ALIEN_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
ALIEN_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=alien@beta` for published beta proof. Keep
...
Start packaged Alien in Docker with clean state:
...
- `alien status --json` includes plugin diagnostics
- `alien plugins inspect --all --json` is parseable
...
`alien/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,
