...
description: Triage Alien security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---

# Security Triage

Use when reviewing Alien security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/alien/alien/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view alien version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/alien/alien/security-advisories/<GHSA>
gh api /repos/alien/alien/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view alien version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo alien/alien --match title,body,comments -- "<terms>"
gh search prs --repo alien/alien --match title,body,comments -- "<terms>"
```
