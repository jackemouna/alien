...
```bash
export ALIEN_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.alien/alien.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env ALIEN_PARALLELS_DISCORD_TOKEN \
...
- Use the `alien` wrapper for guest `message send/read`; `node alien.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/alien-parallels-smoke.*`
...
- host inbound nonce appears in `alien message read` output
