...
description: Use gitcrawl to search duplicate Alien PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local Alien history and clustering source.
...
```bash
gitcrawl threads alien/alien --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors alien/alien --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search alien/alien --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search alien/alien --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail alien/alien --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo alien/alien --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo alien/alien --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo alien/alien --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R alien/alien "<problem phrase>" --types group --limit 10
prtags search similar -R alien/alien "<problem summary>" --types group --limit 10
prtags group list -R alien/alien
```
...
```bash
prtags group create -R alien/alien \
  --kind mixed \
...
```bash
prtags field ensure -R alien/alien --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R alien/alien --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R alien/alien --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R alien/alien --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R alien/alien --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R alien/alien --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R alien/alien --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R alien/alien --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R alien/alien --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R alien/alien <pr-number> \
...
```bash
prtags annotation issue set -R alien/alien <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R alien/alien
```
