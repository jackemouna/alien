...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "alien/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ ALIEN_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ ALIEN_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ ALIEN_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from ALIEN_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the alien runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the alien runtime setup", () => {
  ...
  it("keeps the ui lane off both the alien runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from ALIEN_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "alien-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
