...
import { BUNDLED_PLUGIN_PATH_PREFIX } from "alien/plugin-sdk/test-fixtures";
...
describe("non-extension test boundaries", () => {
  ...
  it("keeps bundled extension sources off deprecated channel config schema aliases", () => {
    ...
    const offenders = files.filter((file) => {
      const source = fs.readFileSync(path.join(repoRoot, file), "utf8");
      return source.includes("alien/plugin-sdk/channel-config-schema-legacy");
    });
