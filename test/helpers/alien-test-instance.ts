...
import {
  ...
  type AlienTestState,
  type AlienTestStateOptions,
} from "../../src/test-utils/alien-test-state.js";
import { sleep } from "../../src/utils.js";

export type AlienTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<AlienTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type AlienTestInstanceCommandResult = {
  code: number | null;
...
export type AlienTestInstance = {
  ...
  ) => Promise<AlienTestInstanceCommandResult>;
...
export async function createAlienTestInstance(
  options: AlienTestInstanceOptions,
): Promise<AlienTestInstance> {
  ...
  const instance: AlienTestInstance = {
    name: options.name,
    port,
