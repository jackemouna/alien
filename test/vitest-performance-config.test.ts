...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("enables the filesystem module cache explicitly", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          ALIEN_VITEST_FS_MODULE_CACHE: "1",
        },
        "linux",
      ),
    ).toEqual({
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          ALIEN_VITEST_FS_MODULE_CACHE_PATH: "/tmp/alien-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/alien-vitest-cache",
      },
    });
  });
  ...
  it("allows disabling the filesystem module cache explicitly", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          ALIEN_VITEST_FS_MODULE_CACHE: "0",
        },
        "linux",
      ),
    ).toEqual({});
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          ALIEN_VITEST_IMPORT_DURATIONS: "true",
