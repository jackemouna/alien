...
describe("package Telegram live Docker E2E", () => {
  it("supports npm-specific Convex credential aliases", () => {
    ...
    expect(script).toContain("ALIEN_NPM_TELEGRAM_CREDENTIAL_SOURCE");
    expect(script).toContain("ALIEN_NPM_TELEGRAM_CREDENTIAL_ROLE");
    expect(script).toContain('docker_env+=(-e ALIEN_QA_CREDENTIAL_SOURCE="$credential_source")');
    expect(script).toContain('docker_env+=(-e ALIEN_QA_CREDENTIAL_ROLE="$credential_role")');
  });
  ...
  it("defaults CI runs to Convex when broker credentials are present", () => {
    ...
    expect(script).toContain("ALIEN_QA_CONVEX_SECRET_CI");
    expect(script).toContain("ALIEN_QA_CONVEX_SECRET_MAINTAINER");
  ...
  it("can install a resolved package tarball instead of a registry spec", () => {
    ...
    expect(script).toContain("ALIEN_NPM_TELEGRAM_PACKAGE_TGZ");
    expect(script).toContain("ALIEN_CURRENT_PACKAGE_TGZ");
    expect(script).toContain(
      'package_mount_args=(-v "$resolved_package_tgz:$package_install_source:ro")',
    );
    expect(script).toContain('validate_alien_package_spec "$PACKAGE_SPEC"');
    expect(script.indexOf('if [ -n "$resolved_package_tgz" ]; then')).toBeLessThan(
      script.indexOf('validate_alien_package_spec "$PACKAGE_SPEC"'),
    );
  });
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(script).toContain('ln -sfnT "$alien_package_dir/dist" /app/dist');
    expect(script).toContain('cp "$alien_package_dir/package.json" /app/package.json');
    expect(script).toContain('ln -sfnT /app/extensions "$alien_package_dir/extensions"');
    expect(script).toContain("node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs");
    expect(script).toContain("/app/node_modules/alien/package.json");
    ...
    expect(gatewayRpcClient).toContain('from "alien/plugin-sdk/gateway-runtime"');
    expect(qaRuntimeApi).toContain('from "alien/plugin-sdk/gateway-runtime"');
  });
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      __testing.resolveCredentialSource({
        ALIEN_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        ALIEN_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      __testing.resolveCredentialRole({
        ALIEN_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",
