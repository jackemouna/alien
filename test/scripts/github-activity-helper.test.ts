...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/alien-pr-maintainer/scripts/github-activity.sh",
);
...
describe("alien-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("alien/alien last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/alien\/alien\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/alien\/alien\/commits/g)).toHaveLength(1);
