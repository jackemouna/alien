...
async function loadRawVitestConfig(configPath: string): Promise<VitestConfig> {
  ...
  const previousIncludeFile = process.env.ALIEN_VITEST_INCLUDE_FILE;
  process.argv = [previousArgv[0] ?? "node", previousArgv[1] ?? "vitest"];
  delete process.env.ALIEN_VITEST_INCLUDE_FILE;
  try {
    ...
    if (previousIncludeFile === undefined) {
      ...
      process.env.ALIEN_VITEST_INCLUDE_FILE = previousIncludeFile;
    }
  }
}
...
describe("scripts/test-projects changed-target routing", () => {
  ...
  it("keeps the broad changed run available for Vitest wiring edits", () => {
    expect(
      resolveChangedTargetArgs(
        ...
        { env: { ALIEN_TEST_CHANGED_BROAD: "1" } },
      ),
    ).toBeNull();
  });
  ...
  it("skips app-only changes because app tests are separate from Vitest lanes", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "apps/macos/Alien/AppDelegate.swift",
      ]),
    ).toEqual([]);
  });
...
describe("scripts/test-projects local heavy-check lock", () => {
  const localCheckEnv = () => ({
    ...process.env,
    ALIEN_TEST_HEAVY_CHECK_LOCK_HELD: undefined,
    ALIEN_TEST_PROJECTS_FORCE_LOCK: undefined,
  });
  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          ALIEN_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  it("keeps explicit parallel overrides ahead of the host-aware profile", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        61,
        {
          ALIEN_TEST_PROJECTS_PARALLEL: "3",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
  ...
  it("keeps serial untargeted runs on aggregate shards", () => {
    const previousParallel = process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.ALIEN_TEST_PROJECTS_SERIAL;
    delete process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    delete process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    process.env.ALIEN_TEST_PROJECTS_SERIAL = "1";
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.ALIEN_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.ALIEN_TEST_PROJECTS_SERIAL;
      } else {
        process.env.ALIEN_TEST_PROJECTS_SERIAL = previousSerial;
      }
    }
  });

  it("expands untargeted local runs to leaf project configs by default", () => {
    const previousLeafShards = process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    const previousParallel = process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.ALIEN_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    const previousVitestMaxWorkers = process.env.ALIEN_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.ALIEN_TEST_WORKERS;
    delete process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    delete process.env.ALIEN_TEST_PROJECTS_SERIAL;
    delete process.env.CI;
    delete process.env.GITHUB_ACTIONS;
    delete process.env.ALIEN_VITEST_MAX_WORKERS;
    delete process.env.ALIEN_TEST_WORKERS;
    try {
      ...
      if (previousLeafShards === undefined) {
        ...
        process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
      }
      if (previousParallel === undefined) {
        delete process.env.ALIEN_TEST_PROJECTS_PARALLEL;
      } else {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.ALIEN_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.ALIEN_TEST_WORKERS;
      } else {
        process.env.ALIEN_TEST_WORKERS = previousTestWorkers;
      }
    }
  });

  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.ALIEN_TEST_PROJECTS_SERIAL;
    delete process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    process.env.ALIEN_TEST_PROJECTS_SERIAL = "1";
    process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.ALIEN_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("can expand full-suite shards to project configs for perf experiments", () => {
    const previous = process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    const gatewayServerConfig = "test/vitest/vitest.gateway-server.config.ts";
    process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS = "1";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.ALIEN_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.ALIEN_TEST_PROJECTS_PARALLEL;
    delete process.env.ALIEN_TEST_PROJECTS_LEAF_SHARDS;
    process.env.ALIEN_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
...
describe("scripts/test-projects parallel cache paths", () => {
  it("assigns isolated Vitest fs-module cache paths per parallel shard", () => {
    ...
    expect(specs.map((spec) => spec.env)).toEqual([
      {
        ALIEN_VITEST_FS_MODULE_CACHE_PATH: path.join(
          "/repo",
          "node_modules",
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { ALIEN_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.ALIEN_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
...
describe("scripts/test-projects Vitest stall watchdog", () => {
  it("adds a default no-output timeout to non-watch specs", () => {
    ...
    expect(spec?.env.ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
  });
  ...
  it("keeps explicit watchdog settings and watch mode untouched", () => {
    const specs = applyDefaultVitestNoOutputTimeout(
      [
        ...
        },
        {
          ...
          env: { ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" },
          includeFilePath: null,
          includePatterns: null,
          pnpmArgs: [],
          watchMode: false,
    ...
    expect(specs[0]?.env.ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS).toBeUndefined();
    expect(specs[1]?.env.ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("0");
  });

  it("allows changed checks to disable automatic silent-run retries", () => {
    expect(shouldRetryVitestNoOutputTimeout({})).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ ALIEN_VITEST_NO_OUTPUT_RETRY: "1" })).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ ALIEN_VITEST_NO_OUTPUT_RETRY: "0" })).toBe(false);
    expect(shouldRetryVitestNoOutputTimeout({ ALIEN_VITEST_NO_OUTPUT_RETRY: "false" })).toBe(
      false,
    );
  });
});
...
describe("scripts/test-projects Vitest cache isolation", () => {
  it("assigns isolated fs-module caches to multi-spec non-watch runs", () => {
    ...
    expect(specs.map((spec) => spec.env.ALIEN_VITEST_FS_MODULE_CACHE_PATH)).toEqual([
      path.join(
        "/repo",
        "node_modules",
        ".experimental-vitest-cache",
