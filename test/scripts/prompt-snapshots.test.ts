...
const promptSnapshotsEnabled = process.env.ALIEN_RUN_PROMPT_SNAPSHOTS !== "false";
...
describe("happy path prompt snapshots", () => {
  ...
  promptSnapshotIt("deletes stale generated snapshot artifacts", async () => {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-prompt-snapshot-stale-"));
    try {
  ...
  promptSnapshotIt("keeps heartbeat guidance in heartbeat collaboration mode only", async () => {
    ...
    expect(heartbeat).toContain('"developer_instructions": "This is an Alien heartbeat turn.');
  ...
  it("finds the first available default Codex model catalog source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-codex-catalog-"));
    try {
  ...
  it("skips Codex model prompt fixture sync when no default catalog exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "alien-codex-catalog-missing-"));
    const chunks: string[] = [];
    try {
