...
describe("scripts/test-live-cli-backend-docker.sh", () => {
  it("forwards both fresh and resume CLI arg overrides into the Docker container", () => {
    ...
    expect(forwardedVars).toContain("ALIEN_LIVE_CLI_BACKEND_ARGS");
    expect(forwardedVars).toContain("ALIEN_LIVE_CLI_BACKEND_RESUME_ARGS");
    expect(forwardedVars).toContain("ALIEN_TEST_CONSOLE");
  });
});
