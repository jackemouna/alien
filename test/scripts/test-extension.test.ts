...
describe("scripts/test-extension.mjs", () => {
  ...
  it("runs extension batch config groups concurrently when requested", async () => {
    ...
    const runPromise = runExtensionBatchPlan(
      ...
      {
        env: { ALIEN_EXTENSION_BATCH_PARALLEL: "2" },
        runGroup,
    ...
    expect(runGroup.mock.calls[0]?.[0].env.ALIEN_VITEST_FS_MODULE_CACHE_PATH).toContain(
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { ALIEN_EXTENSION_BATCH_PARALLEL: "2" })).toBe(2);
    expect(resolveExtensionBatchParallelism(1, { ALIEN_EXTENSION_BATCH_PARALLEL: "4" })).toBe(1);
    expect(resolveExtensionBatchParallelism(3, { ALIEN_EXTENSION_BATCH_PARALLEL: "nope" })).toBe(
      1,
    );
  });
