...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/alien-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/alien-live-and-e2e-checks-reusable.yml";
...
describe("test-install-sh-docker", () => {
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(workflow).toContain(
      "ALIEN_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${ALIEN_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an Alien branch or release tag");
  });
  ...
  it("writes the package dist inventory before packing ignore-scripts tarballs", () => {
    ...
    expect(script).toContain("node scripts/check-alien-package-tarball.mjs");
  });
});
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain("alien update --tag");
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${ALIEN_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${ALIEN_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("ALIEN_INSTALL_SMOKE_MODE=npm-global");
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("ALIEN_BUN_GLOBAL_SMOKE_DIST_IMAGE");
  });
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "ALIEN_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("ALIEN_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain('ALIEN_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
