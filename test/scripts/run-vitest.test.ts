...
describe("scripts/run-vitest", () => {
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ ALIEN_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
  });
