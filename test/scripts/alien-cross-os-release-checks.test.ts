...
} from "../../scripts/alien-cross-os-release-checks.ts";

describe("scripts/alien-cross-os-release-checks", () => {
  ...
  it("gives the Windows packaged updater wrapper enough headroom for Alien timeout output", () => {
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "alien-cross-os-agent-output-"));
    try {
  ...
  it("skips optional live agent turns only for model availability failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "alien-cross-os-agent-skip-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        ALIEN_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        ALIEN_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.4", () => {
    const workflow = readFileSync(
      ".github/workflows/alien-cross-os-release-checks-reusable.yml",
      "utf8",
    );
    const releaseChecks = readFileSync(".github/workflows/alien-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "ALIEN_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.ALIEN_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.4' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("alien@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "alien@2026.5.2",
      "--omit=dev",
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/alien-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/alien-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    const source = readFileSync("scripts/alien-cross-os-release-checks.ts", "utf8");
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'alien.cmd')");
    expect(script).toContain("$cmd = Get-Command alien -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        ALIEN_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        ALIEN_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        ALIEN_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("alien-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://alien.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://alien.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://alien.ai/install.ps1");
  });
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "alien/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "ALIEN_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/alien/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/alien/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/alien-cross-os-release-checks.ts", "utf8")).toContain(
      "ALIEN_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/alien", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "alien-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "alien");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "alien" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "alien"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "alien"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "alien"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-alien-cache",
        ALIEN_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
      }),
    ).toEqual({
      FOO: "bar",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[alien] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/alien/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\alien-upgrade-q9DsA7\\prefix\\node_modules\\alien\\alien.mjs update --tag http://127.0.0.1:49951/alien-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\alien\\alien.mjs update --tag http://127.0.0.1:49951/alien-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node alien.mjs update --tag alien@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.alien-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("alien@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("alien@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "alien-cross-os-installed-version-"));
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "alien")
          : join(prefixDir, "lib", "node_modules", "alien");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "alien-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "alien-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Alien-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Alien-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "alien-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "alien-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
