...
async function createExtensionsDir() {
  const root = await createTempDirAsync("alien-postinstall-");
...
describe("bundled plugin postinstall", () => {
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/alien/scripts/postinstall-bundled-plugins.mjs",
        modulePath: "/private/var/folders/tmp/alien/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    const configuredBase = path.join("/tmp", "alien-cache");
    ...
    expect(removed).not.toContain(path.join(configuredBase, "alien"));
  ...
  it("keeps pruning sibling compile cache dirs after one removal fails", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] could not prune Alien compile cache: Error: locked",
    );
  });

  it("does not warn when compile-cache pruning hits EACCES or EPERM (shared caches)", () => {
    const base = path.join("/tmp", "alien-shared-compile-cache");
  ...
  it("does not warn when the compile-cache base directory cannot be listed (EACCES)", () => {
    const base = path.join("/tmp", "alien-compile-cache-no-list");
    const warn = vi.fn();
    const rmSync = vi.fn();
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("alien-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("alien-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("alien-source-checkout-state-skip-");
    const home = await createTempDirAsync("alien-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".alien", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("alien-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { ALIEN_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("alien-postinstall-registry-");
  ...
  it("surfaces deprecated plugin registry migration break-glass warnings", async () => {
    ...
    const migratePluginRegistryForInstall = vi.fn(async () => ({
      ...
      migrated: false,
      preflight: {
        deprecationWarnings: ["ALIEN_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated"],
      },
    }));
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] ALIEN_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated",
    );
  });
  ...
  it("honors plugin registry postinstall migration disable env", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { ALIEN_DISABLE_PLUGIN_REGISTRY_MIGRATION: "1" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("does not disable plugin registry migration for falsey env flag strings", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { ALIEN_DISABLE_PLUGIN_REGISTRY_MIGRATION: "0" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-");
  ...
  it("prunes legacy plugin runtime deps state during packaged postinstall", async () => {
    const prefix = await createTempDirAsync("alien-packaged-prefix-");
    const packageRoot = path.join(prefix, "lib", "node_modules", "alien");
    const nodeModulesRoot = path.dirname(packageRoot);
    const home = await createTempDirAsync("alien-packaged-home-");
    const stateOverride = path.join(home, "custom-state");
    const systemState = path.join(home, "system-state");
    const defaultLegacyRoot = path.join(home, ".alien", "plugin-runtime-deps");
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "[postinstall] could not prune legacy plugin runtime deps /home/alice/.alien/plugin-runtime-deps: Error: locked",
      ),
    );
  });

  it("resolves legacy plugin runtime deps roots from Alien state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          ALIEN_HOME: "/srv/alien-home",
          ALIEN_CONFIG_PATH: "~/profile/alien.json",
          ALIEN_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/alien",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      "/srv/alien-home/.clawdbot/plugin-runtime-deps",
      "/srv/alien-home/.alien/plugin-runtime-deps",
      "/srv/alien-home/profile/plugin-runtime-deps",
      "/srv/alien-home/state/plugin-runtime-deps",
      "/var/lib/alien/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-import-");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expect(
      fs.stat(path.join(packageRoot, "dist", "extensions", "qa-channel", "alien.plugin.json")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("alien-packaged-install-dist-prune-");
