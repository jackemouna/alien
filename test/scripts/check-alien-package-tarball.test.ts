...
const CHECK_SCRIPT = "scripts/check-alien-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "alien-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "alien", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><alien-app></alien-app>",
    ...
    const tarball = join(root, "alien.tgz");
...
describe("check-alien-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Alien package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
