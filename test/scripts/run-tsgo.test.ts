...
describe("run-tsgo sparse guard", () => {
  it("ignores non-core projects", () => {
    const cwd = createTempDir("alien-run-tsgo-");
  ...
  it("returns a helpful message for sparse core worktrees missing transitive project files", () => {
    ...
    expect(
      ...
      - apps/shared/AlienKit/Sources/AlienKit/Resources/tool-display.json
      Expand this worktree's sparse checkout to include those paths, or rerun in a full worktree."
    `);
  });
  ...
  it("recognizes the check:changed sparse-skip env", () => {
    expect(shouldSkipSparseTsgoGuardError({ ALIEN_TSGO_SPARSE_SKIP: "1" })).toBe(true);
    expect(shouldSkipSparseTsgoGuardError({ ALIEN_TSGO_SPARSE_SKIP: "true" })).toBe(true);
    expect(shouldSkipSparseTsgoGuardError({ ALIEN_TSGO_SPARSE_SKIP: "0" })).toBe(false);
