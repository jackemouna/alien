...
function makeEnv(overrides: Record<string, string | undefined> = {}) {
  ...
  if (!Object.hasOwn(overrides, "ALIEN_LOCAL_CHECK_MODE")) {
    delete env.ALIEN_LOCAL_CHECK_MODE;
  }
  return env;
}
...
describe("local-heavy-check-runtime", () => {
  it("reenables local heavy-check policy for local wrapper entrypoints", () => {
    expect(resolveLocalHeavyCheckEnv({ ALIEN_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      ALIEN_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalHeavyCheckEnv({ ALIEN_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      ALIEN_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ ALIEN_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        ALIEN_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ ALIEN_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        ALIEN_LOCAL_CHECK_MODE: "full",
        ALIEN_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ ALIEN_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the tsgo lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLockForTsgo(
        ["--help"],
        makeEnv({ ALIEN_TSGO_FORCE_LOCK: "1" }),
      ),
    ).toBe(true);
  });
  ...
  it("skips the heavy-check lock for explicit oxlint file targets", () => {
    const cwd = createTempDir("alien-oxlint-lock-skip-");
  ...
  it("keeps the heavy-check lock for directory targets and broad oxlint runs", () => {
    const cwd = createTempDir("alien-oxlint-lock-keep-");
  ...
  it("allows forcing the oxlint lock back on", () => {
    const cwd = createTempDir("alien-oxlint-lock-force-");
    ...
    expect(
      shouldAcquireLocalHeavyCheckLockForOxlint(["--type-aware", "--", "sample.ts"], {
        cwd,
        env: makeEnv({ ALIEN_OXLINT_FORCE_LOCK: "1" }),
      }),
    ).toBe(true);
  });

  it("reclaims stale local heavy-check locks from dead pids", () => {
    const cwd = createTempDir("alien-local-heavy-check-");
    const commonDir = path.join(cwd, ".git");
    const lockDir = path.join(commonDir, "alien-local-checks", "heavy-check.lock");
  ...
  it("cleans up stale legacy test locks when acquiring the shared heavy-check lock", () => {
    const cwd = createTempDir("alien-local-heavy-check-legacy-");
    const commonDir = path.join(cwd, ".git");
    const locksDir = path.join(commonDir, "alien-local-checks");
