...
describe("scripts/test-live-codex-harness-docker.sh", () => {
  it("mounts cache and npm tool dirs outside the bind-mounted Docker home", () => {
    ...
    expect(script).toContain('DOCKER_CACHE_CONTAINER_DIR="/tmp/alien-cache"');
    expect(script).toContain('DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/alien-npm-global"');
    expect(script).toContain("alien_live_codex_harness_is_ci()");
  ...
  it("fails before Docker build when codex-auth has no host auth file", () => {
    ...
    expect(script).toContain(
      "ALIEN_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image",
    );
    expect(script).toContain(
      "If this is a Testbox/API-key run, set ALIEN_LIVE_CODEX_HARNESS_AUTH=api-key and run through alien-testbox-env.",
    );
    expect(script.indexOf("requires ~/.codex/auth.json before building")).toBeLessThan(
      script.indexOf('ALIEN_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR"'),
    );
  });
});
