...
describe("scripts/lib/vitest-shard-timings.mjs", () => {
  ...
  it("uses the CI shard name for include-pattern timing keys", () => {
    expect(
      resolveShardTimingKey({
        ...
        env: { ALIEN_VITEST_SHARD_NAME: "auto-reply/reply agent dispatch" },
  ...
  it("persists include-pattern timing metadata", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alien-shard-timings-"));
    tempDirs.push(tempDir);
    const env = {
      ALIEN_TEST_PROJECTS_TIMINGS_PATH: path.join(tempDir, "timings.json"),
      ALIEN_VITEST_SHARD_NAME: "auto-reply-reply-agent-runner",
    };
    ...
    expect(
      JSON.parse(fs.readFileSync(env.ALIEN_TEST_PROJECTS_TIMINGS_PATH, "utf8")).configs[
