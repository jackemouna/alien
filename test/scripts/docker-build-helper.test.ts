...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("ALIEN_DOCKER_BUILD_RETRIES");
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("ALIEN_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("ALIEN_TESTBOX");
    ...
    expect(liveCliBackend).toContain("direct Codex CLI probe failed before Alien gateway smoke");
    expect(liveCliBackend).toContain("==> Direct Codex CLI probe ok");
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (ALIEN_SKIP_DOCKER_BUILD=1)"',
    );
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("ALIEN_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(runner).toContain(
      'AGENT_TURNS_PARALLEL="${ALIEN_INSTALL_E2E_AGENT_TURNS_PARALLEL:-1}"',
    );
    ...
    expect(wrapper).toContain("ALIEN_INSTALL_E2E_AGENT_TURNS_PARALLEL");
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("ALIEN_PLUGINS_E2E_CLAWHUB=0");
  });
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(scripts.join("\n")).toContain("ALIEN_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("ALIEN_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("ALIEN_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("ALIEN_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runner).toContain("scripts/e2e/lib/bundled-plugin-install-uninstall/sweep.sh");
    expect(probe).toContain('"alien.plugin.json"');
    expect(runtimeSmoke).toContain("process.env.ALIEN_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runtimeSmoke).toContain("900000");
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$ALIEN_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$ALIEN_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ALIEN_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ALIEN_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('ALIEN_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'ALIEN_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("keeps ClawHub plugin Docker smoke hermetic by default", () => {
    ...
    expect(runner).toContain("ALIEN_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(sweep).toContain("scripts/e2e/lib/plugins/clawhub.sh");
    expect(clawhub).toContain("start_clawhub_fixture_server()");
    expect(clawhub).toContain('ALIEN_CLAWHUB_URL="http://127.0.0.1:');
    expect(clawhub).toContain("ALIEN_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(clawhub).toContain("ALIEN_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(clawhub).toContain("live ClawHub can rate-limit CI");
    expect(clawhub).toContain('[[ -n "${ALIEN_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]');
    expect(clawhub).toContain("Ignoring ambient ClawHub URL for fixture-mode plugin E2E");
    expect(clawhub).toContain("unset ALIEN_CLAWHUB_URL CLAWHUB_URL");
  });
  ...
  it("covers plugin install/update sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('plugins install "npm:@alien/demo-plugin-npm@0.0.1"');
    ...
    expect(clawhub).toContain("clawhub:@alien/kitchen-sink");
