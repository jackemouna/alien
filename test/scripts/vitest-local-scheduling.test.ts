...
describe("vitest local full-suite profile", () => {
  it("forces local Vitest runs back onto local-check policy", () => {
    expect(resolveLocalVitestEnv({ ALIEN_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      ALIEN_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalVitestEnv({ ALIEN_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      ALIEN_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("lets explicit system throttle opt-out ignore memory pressure", () => {
    const env = { ALIEN_VITEST_DISABLE_SYSTEM_THROTTLE: "1" };
    const hostInfo = {
      cpuCount: 10,
      loadAverage1m: 0,
      totalMemoryBytes: 24 * 1024 ** 3,
