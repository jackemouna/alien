...
import { loadConfig, type AlienConfig } from "../src/config/config.js";
import {
...
const REQUIRE_PROFILE_KEYS =
  isLiveProfileKeyModeEnabled() || isTruthyEnvValue(process.env.ALIEN_LIVE_REQUIRE_PROFILE_KEYS);
const describeLive = LIVE ? describe : describe.skip;
const providerFilter = parseCsvFilter(process.env.ALIEN_LIVE_IMAGE_GENERATION_PROVIDERS);
const caseFilter = parseCaseFilter(process.env.ALIEN_LIVE_IMAGE_GENERATION_CASES);
const envModelMap = parseProviderModelMap(process.env.ALIEN_LIVE_IMAGE_GENERATION_MODELS);
const DEFAULT_LIVE_IMAGE_GENERATION_TIMEOUT_MS = 120_000;
const LIVE_IMAGE_GENERATION_TIMEOUT_MS = resolvePositiveIntegerEnv(
  process.env.ALIEN_LIVE_IMAGE_GENERATION_TIMEOUT_MS,
...
function withPluginsEnabled(cfg: AlienConfig): AlienConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
