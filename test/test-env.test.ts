...
function createTempHome(): string {
  return makeTempDir(tempDirs, "alien-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-alien.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.alien/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.alien/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".alien", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".alien", "external-plugins", "glueclaw", "alien.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".alien", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ALIEN_LIVE_TEST_QUIET = "1";
    process.env.ALIEN_CONFIG_PATH = "~/custom-alien.json5";
    process.env.ALIEN_TEST_HOME = priorIsolatedHome;
    process.env.ALIEN_STATE_DIR = path.join(priorIsolatedHome, ".alien");
    ...
    expect(process.env.ALIEN_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".alien", "alien.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".alien", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".alien", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ALIEN_LIVE_TEST = "1";
    process.env.ALIEN_LIVE_USE_REAL_HOME = "1";
    process.env.ALIEN_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ALIEN_LIVE_TEST;
    delete process.env.ALIEN_LIVE_GATEWAY;
    delete process.env.ALIEN_LIVE_USE_REAL_HOME;
    delete process.env.ALIEN_LIVE_TEST_QUIET;
