...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "alien-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "alien.mjs"),
    path.join(fixtureRoot, "alien.mjs"),
  );
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/alien\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.ALIEN_SOURCE_COMPILE_CACHE_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
describe("alien launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-alien-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "alien.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-alien-launcher-dep");
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:alien/alien#<ref>");
  });
  ...
  it("keeps compile cache off for source-checkout launchers", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "alien.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "alien-launcher-sigterm-test-child";',
          "setInterval(() => {}, 1000);",
          "",
        ].join("\n"),
        "utf8",
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "alien.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          'process.title = "alien-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          "setInterval(() => {}, 1000);",
          "",
        ].join("\n"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "alien-launcher-link-");
      const linkedRoot = path.join(linkParent, "alien-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "alien.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "alien", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "alien-launcher-cwd-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "alien", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "alien"));
  });
