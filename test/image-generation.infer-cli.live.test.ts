...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.ALIEN_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through alien infer", () => {
    const outputBase = path.join(os.tmpdir(), `alien-infer-image-${process.pid}.png`);
