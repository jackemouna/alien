...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ALIEN_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ALIEN_TEST_FAST", value: process.env.ALIEN_TEST_FAST },
    {
      key: "ALIEN_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ALIEN_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ALIEN_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ALIEN_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ALIEN_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ALIEN_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ALIEN_STATE_DIR", value: process.env.ALIEN_STATE_DIR },
    { key: "ALIEN_CONFIG_PATH", value: process.env.ALIEN_CONFIG_PATH },
    { key: "ALIEN_GATEWAY_PORT", value: process.env.ALIEN_GATEWAY_PORT },
    { key: "ALIEN_BRIDGE_ENABLED", value: process.env.ALIEN_BRIDGE_ENABLED },
    { key: "ALIEN_BRIDGE_HOST", value: process.env.ALIEN_BRIDGE_HOST },
    { key: "ALIEN_BRIDGE_PORT", value: process.env.ALIEN_BRIDGE_PORT },
    { key: "ALIEN_CANVAS_HOST_PORT", value: process.env.ALIEN_CANVAS_HOST_PORT },
    { key: "ALIEN_TEST_HOME", value: process.env.ALIEN_TEST_HOME },
    { key: "ALIEN_AGENT_DIR", value: process.env.ALIEN_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "alien-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ALIEN_TEST_HOME = tempHome;
  process.env.ALIEN_TEST_FAST = "1";
  process.env.ALIEN_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ALIEN_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ALIEN_BRIDGE_ENABLED;
  delete process.env.ALIEN_BRIDGE_HOST;
  delete process.env.ALIEN_BRIDGE_PORT;
  delete process.env.ALIEN_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.ALIEN_STATE_DIR = path.join(tempHome, ".alien");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ALIEN_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.ALIEN_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".alien");
  const priorIsolatedHome = params.env.ALIEN_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".alien")
  ) {
    realStateDir = path.join(params.realHome, ".alien");
  }
  const tempStateDir = path.join(params.tempHome, ".alien");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.ALIEN_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.ALIEN_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "alien.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "alien.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.ALIEN_LIVE_TEST === "1" ||
    process.env.ALIEN_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ALIEN_LIVE_USE_REAL_HOME);
