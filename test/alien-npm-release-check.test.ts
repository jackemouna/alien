...
} from "../scripts/alien-npm-release-check.ts";
import {
...
describe("shouldSkipPackedTarballValidation", () => {
  ...
  it("accepts truthy values for metadata-only validation", () => {
    expect(
      shouldSkipPackedTarballValidation({
        ALIEN_NPM_RELEASE_SKIP_PACK_CHECK: "1",
      }),
    ).toBe(true);
  });
  ...
  it("treats false-like values as disabled", () => {
    expect(
      shouldSkipPackedTarballValidation({
        ALIEN_NPM_RELEASE_SKIP_PACK_CHECK: "false",
      }),
    ).toBe(false);
  });
});
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"alien.tgz","files":[]}]')).toEqual([
      { filename: "alien.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> alien@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"alien.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> alien@2026.3.23 prepack")).toBeNull();
  });
});
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "alien-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "alien-pack-inventory-"));
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/alien/alien.git" },
        bin: { alien: "alien.mjs" },
      }),
    ).toEqual([]);
  });
  ...
  it("rejects local fs-safe dependency specs for npm release", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        repository: { url: "git+https://github.com/alien/alien.git" },
        bin: { alien: "alien.mjs" },
        dependencies: { "@alien/fs-safe": "link:../fs-safe" },
      }),
    ).toContain(
      'package.json dependencies["@alien/fs-safe"] must use a published semver range before npm release; found "link:../fs-safe".',
    );
  });
