...
import { bundledPluginFile, bundledPluginRoot } from "alien/plugin-sdk/test-fixtures";
...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @alien/zalo, @alien/feishu  @alien/zalo "),
    ).toEqual(["@alien/feishu", "@alien/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@alien/"; found "broken".',
      "package.json private must not be true.",
      `package.json repository.url must be "${ALIEN_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.D, YYYY.M.D-N, YYYY.M.D-alpha.N, or YYYY.M.D-beta.N; found "latest".',
      "alien.extensions must contain only non-empty strings.",
      "alien.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["alien.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  ...
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "alien-plugin-npm-release-");
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@alien/demo-plugin",
      },
    ]);
  });
  ...
  it("does not validate unselected publishable plugin manifests", () => {
    ...
    writeJsonFile(join(repoDir, "extensions", "private-plugin", "package.json"), {
      name: "@alien/private-plugin",
      version: "2026.4.10-beta.1",
      private: true,
      alien: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@alien/private-plugin",
        },
        release: {
          publishToNpm: true,
        },
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@alien/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @alien/missing.");
  });
});
