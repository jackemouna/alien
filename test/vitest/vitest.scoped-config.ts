...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeAlienRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
  ...
  const includeFromEnv = loadPatternListFromEnv("ALIEN_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeAlienRuntimeSetup === false ? [] : ["test/setup-alien-runtime.ts"]),
    ]),
